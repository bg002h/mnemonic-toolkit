...
- [ ] **Step 3: Implement.**
  - `parse.rs::strip_whitespace` → body becomes `crate::format::strip_display_separators(s)` (delegate; keeps whitespace, adds `-`/`,`; NO dedup). REMOVE `strip_whitespace_dedupes_doubled_content` + the doubling clause of `strip_whitespace_handles_all_three_workflows` (keep the engraver-typed-back + paste cases, which still hold). Update the doc comment (drop the doubling rationale). Optionally rename `strip_whitespace`→keep name (callers unchanged) OR alias; keep the name `strip_whitespace` to avoid churn, but its body now strips `-`/`,` too — update its doc.
