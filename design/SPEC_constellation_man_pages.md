...
> - **C-2 (Critical) — CORRECTED post-implementation (round-2 claim was itself empirically false).** The round-2 fold asserted `--no-auto-repair` renders in ZERO generated pages regardless of `.build()`. That is FALSE: clap_mangen 0.3.0 surfaces the root command's `global = true` args on EVERY generated page, so `--no-auto-repair` renders in all 38 pages under `.SH OPTIONS` (verified empirically; roff-escaped `\-\-no\-auto\-repair`). The pre-`.build()` is still correctly deleted (C-1, the help-shadow-tree poisoning is real and independent), but its *original* stated rationale (build needed to propagate the global flag) is moot — the naive call already renders the flag everywhere. → §2 (verified global-flag-renders-on-every-page behavior stated), §8 P1 (a POSITIVE assertion that the root page renders `--no-auto-repair` lives in `cli_gen_man.rs::gen_man_root_page_renders_global_no_auto_repair_flag`).
...
**Global-flag behavior (C-2, verified post-implementation — the round-2 "renders in zero pages" claim was empirically false).** The toolkit's sole `global = true` flag, `--no-auto-repair` (`main.rs:91`), renders in the `.SH OPTIONS` / `.TP` section of **EVERY** generated page — the root `mnemonic.1`, `mnemonic-bundle.1`, nested `mnemonic-seed-xor-split.1`, all 38 pages — under clap_mangen 0.3.0: its `Man` renderer surfaces the root command's global args on each (sub)command page (verified empirically; the roff-escaped `\-\-no\-auto\-repair` appears in all pages). There is therefore no `.build()`-buys-global-propagation question to resolve — the naive call (no pre-build, C-1) already renders the flag everywhere. The flag is also discoverable via `mnemonic --help`. P1 carries a **POSITIVE** assertion (`cli_gen_man.rs::gen_man_root_page_renders_global_no_auto_repair_flag`) pinning that the root page renders `--no-auto-repair` under `.SH OPTIONS`.
...
- **POSITIVE assertion that the root page renders `--no-auto-repair` (C-2, corrected):** the `global=true` flag DOES render in every generated page's `.SH OPTIONS` section under clap_mangen 0.3.0 (the round-2 "renders in zero pages" claim was empirically false); pin it via `cli_gen_man.rs::gen_man_root_page_renders_global_no_auto_repair_flag` (asserts the roff-escaped `\-\-no\-auto\-repair` under `.SH OPTIONS` in `mnemonic.1`).
...
5. **Global-flag in man output (C-2): RESOLVED — no question remains.** `--no-auto-repair` (`global=true`) renders in EVERY generated page under clap_mangen 0.3.0 + clap 4.6.1 (all 38 pages, under `.SH OPTIONS`; verified empirically). The round-2 "renders in ZERO pages" premise was false, so the (a)-accept / (b)-append-stanza fork is moot: clap_mangen's `Man` renderer already surfaces the root's global args on every page. No upstream FOLLOWUP and no manual GLOBAL OPTIONS stanza are needed; the positive behavior is pinned by `cli_gen_man.rs::gen_man_root_page_renders_global_no_auto_repair_flag`.