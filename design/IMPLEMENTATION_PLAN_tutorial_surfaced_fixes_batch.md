...
**The ONE gallery re-pin (R-A):** `UPDATE_SNAPSHOTS=1 cargo test --test gui_form_snapshots` — census **exactly 28** moved PNGs (the `<masked>`-on-load forms); export-wallet expected UNMOVED (`[ slot editor: 0 rows ]` on load). Any 29th mover or any unmoved expected-mover = STOP-5.
...
**Batch-inertness assertion:** re-run `gui_form_snapshots` WITHOUT update — zero movement (restore PNG already at its post-reveal baseline); `gui_render_emit` untouched (restore has no pin — grep-confirmed zero hits, restore-R0 item 5); conditional-drift floor `("restore",1)` unchanged.
