...
- **Status:** ✅ R0-GREEN (0C/0I) @ round 3 (Fable) — CLEARED for the IMPLEMENTATION_PLAN + plan-R0. Reviews cycleG-spec-r0-round-{1,2,3}.md. [rev-3 folded SPEC-R0-round-1 (0C/1I/6M): I1 (compare-cost existing test is `wpkh`=UnsupportedWrapper regardless of multipath → UPDATE it to assert the new wrapper error, ADD `wsh` acceptance tests); M1 full migration surface (2nd wire struct AutoFireRepairJson + verify_mk1_set + `&*`); M2 verify-bundle no-`Default` compare; M3 stale comments; M4 malformed-multipath fixture; M5 split-first-mirror-prior-art; M6 slice-serialize unit. Compare-cost SemVer R0-ruled MINOR. + round-2 (0C/1I/2M: I1(r2) §0-scope-line harmonized to UPDATE-not-invert; M1(r2) count 8-not-~11; M2(r2) producer-local note). Reviews `cycleG-spec-r0-round-{1,2}.md`.]
