...
**Emission rule (binding → `ms1[i]`).** For every slot `@i` (i ∈ 0..N), `ms1[i]` is populated independently from `cosigners[i]`. If `cosigners[i]` carries BIP-39 entropy (supplied via `--slot @i.phrase=` or `--slot @i.entropy=`; or derived from `--slot @i.wif=` / `--slot @i.xprv=` per SPEC §4), `ms1[i]` is the bech32-encoded `ms_codec::Tag::ENTR` payload over that entropy. If `cosigners[i]` is watch-only (supplied via `--slot @i.xpub=`, `--slot @i.master_xpub=`, or fingerprint+path), `ms1[i] == ""`. **This rule is uniform across all bundle modes** (single-sig, template multisig, descriptor multisig, multi-source, hybrid). Implementations MUST NOT restrict ms1 emission to slot `@0` in any mode. The legacy "descriptor-mode @0-only contract" — present in `synthesize_descriptor` in pre-v0.21.0 toolkit releases — is **repudiated by §5.8**; descriptor-mode multi-cosigner bundles emit one `ms1` string per supplied phrase-bearing slot.

**mk1 emission contract.** `Bundle.mk1` is a discriminated `MkField::Single(Vec<String>)` (n=1) or `MkField::Multi(Vec<Vec<String>>)` (n≥2) — outer per cosigner, inner per bech32 chunk. Each slot's inner chunk vec is non-empty by invariant; synthesis fails at `bind_descriptor_keys` (`parse_descriptor.rs`) if any slot lacks a key source (phrase / xpub / cosigner triple). mk1 emission is independent of ms1: a watch-only slot still produces a mk1 chunk-set carrying that slot's xpub. There is no mk1 "empty-string sentinel" — the type encodes the "always present per slot" invariant structurally.

**CLI input forms (verify-bundle).** Two equivalent forms express watch-only cosigners on the verify-bundle CLI surface, both grounded in this §5.8 invariant:
1. **Empty-string sentinel — `--ms1 ""`** (positional). Each `""` value marks the positionally-aligned cosigner as watch-only. Required to express "middle-cosigner watch-only" (e.g., `--ms1 <s0> --ms1 "" --ms1 <s2>` for a 3-cosigner bundle where you have seeds 0+2 but not seed 1). Verify-bundle emits a one-line stderr NOTICE per skipped cosigner: `notice: cosigner[{idx}] marked watch-only via empty --ms1 sentinel (SPEC §5.8); no seed will be derived for this slot`. This restores the pre-v0.24.0 convention (the v0.24.0 §2.C.1 strict-HRP gate accidentally hard-failed empty strings; v0.25.1 patches `validate_flag_hrp` to special-case `""` per this SPEC).
2. **Trailing flag omission** (shorthand for trailing skips only). Supply `--ms1` only for the full-path cosigners; the positional vec naturally stops at the last full-path index. Equivalent to form 1 for trailing cosigners; cannot express middle-cosigner skips. Useful as a shorthand when the watch-only cosigners are all at the tail.

Both forms produce the same wire-level result (`ms1[i] == ""` for skipped slots). Form 1 is canonical; form 2 is a CLI convenience.

**v0.21.0 wire-format-shape change for descriptor-mode multi-cosigner bundles.** Toolkit releases v0.20.x and earlier ran `synthesize_descriptor` with a "v0.3 descriptor mode contract" (codified at `crates/mnemonic-toolkit/src/synthesize.rs:254-257` in those releases) that emitted `ms1[0]` only, leaving `ms1[1..]` as `""` even when phrases for `@1+` were supplied. v0.21.0 removes this restriction and conforms to the §5.8 emission rule above. Consequence: replaying a pre-v0.21.0 descriptor-mode multi-cosigner bundle (with phrases supplied for `@1+`) through v0.21.0 `verify-bundle --bundle-json` reports `ms1_decode[i]: passed=false, decode_error="error: ms1[{i}] expected (full-mode bundle) but not supplied"` for each `i ≥ 1` where the pre-v0.21.0 bundle emitted `""`. Regenerate the bundle from the same phrases under v0.21.0 to fix. Single-sig (N=1) and pure-watch-only bundles are byte-identical across the v0.20.x ↔ v0.21.0 boundary.
...
- **Closed in v3 cycle (cycle status reads `ENCODED v3` or `ENCODED v3 (GUI-internal)` or `ENCODED v3 (GUI-internal warning)`):** §6.6 rows 8, 9, 10, 11. **All four close GUI-side** via Option A patterns — no toolkit wire-format emissions for any of them:
  - **Row 8**: `slot_editor.rs::detect_slot_index_gaps` + inline warning banner adjacent to slot grid.
  - **Row 9**: `NumberMax::FromSlotCount` FlagKind extension binds `--threshold`'s resolved max to `state.slot_count()` at render time.
