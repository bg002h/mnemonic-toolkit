...
- **Impl review round 1 (GREEN 0C/0I, 2026-06-09; persisted at `design/agent-reports/descriptor-builder-allow-impl-r1-review.md`):** faithful realization verified line-by-line + probed live in every output mode; byte-stability goldens untouched. M1 hint assertion added to the emit-spec replay leg; M2 coverage-gap note added to the gate `--allow` test section (malleable/resource-limit = mapping-only per the SPEC's best-effort posture); M3 (provenance suffix trails the hint in human refusals) deferred as a future-touch cosmetic — no test pins the concatenation; M4 = the release items, shipped with the v0.52.0 release commit.
