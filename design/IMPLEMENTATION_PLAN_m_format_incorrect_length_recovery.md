...
7. **Trigger:** engage indel search only when normal `repair_card` returns `Err(RepairError::{TooManyErrors | PostCorrectionDecodeFailed | UnparseableInput | ReservedInvalidLength})` AND `--max-indel ≥ 1`. (`HrpMismatch`/`EmptyInput`/`UnsupportedCodeVariant` pass through unchanged — see §2.4.)
...
- `run()` (`:89-123`) — **multi-group-safe control flow (R0 I1).** A single invocation may carry multiple groups (`--ms1 X --mk1 Y`; D35). The current `repair_card(*kind, chunks)?` at `:103` short-circuits on the first failure; we replace the `?` with a `match` and aggregate, **without early-returning on the non-fatal (Ambiguous/recovered) outcomes**, so every group still emits. New orchestrator:
  ...
  ```
  - `is_indel_trigger(&RepairError) -> bool` — true for `TooManyErrors | PostCorrectionDecodeFailed | UnparseableInput | ReservedInvalidLength` (§1.7); false otherwise.
