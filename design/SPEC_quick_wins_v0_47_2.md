...
- **(R0 I4) ALSO fix the SYNOPSIS brace-pipe** at `:2744` (repair) + `:3016` (inspect): both render `{--ms1 <MS1> | --mk1 <MK1> | --md1 <MD1>}` — the brace-pipe wrongly implies "pick exactly one," the same false mutex, AND violates CLAUDE.md's "manual mirrors clap `--help`" invariant (clap renders these as independently-optional `[--ms1 …] [--mk1 …] [--md1 …]` given `ArgGroup….required(false).multiple(true)` + 3 independent `#[arg(long)]`). Reword the synopses to independently-optional form. **The implementer MUST run `mnemonic repair --help` / `mnemonic inspect --help` and mirror the EXACT USAGE line** (architect-must-RUN discipline) — don't hand-guess the bracket form.
...
- **What to fire:** if ANY `args.ms1` entry is inline-secret-bearing — non-empty AND not `@env:`-prefixed (the `""` watch-only sentinel + `@env:VAR` values are NOT argv leaks) — fire once `secret_in_argv_warning(stderr, "--ms1", "@env:VAR")`. Separately, if ANY `--slot @N.phrase` value is inline-secret-bearing (same non-empty + non-`@env:` test) → fire once `secret_in_argv_warning(stderr, "--slot @N.phrase=", "@env:VAR")`. Each label fires at most once.
