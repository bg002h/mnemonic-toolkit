...
**Seed stretch** (`mnemonic.py::mnemonic_to_seed`): `seed64 = PBKDF2-HMAC-SHA512(normalize_text(phrase), b"electrum" + normalize_text(passphrase), 2048)` → 64 bytes. **`normalize_text` is already in-tree** as `electrum.rs::normalize_phrase_for_hmac` (NFKD → strip combining/accents → lower → collapse whitespace → strip CJK-internal whitespace) — equivalent to Electrum's `normalize_text` (`mnemonic.py:80-91`) **for all shipped-wordlist scripts** (R0 M2: the in-tree order is NFKD→strip→lower vs Electrum's NFKD→lower→strip — equivalent since combining marks are caseless; and `is_cjk` covers the 9 intervals of Electrum's 28 `CJK_INTERVALS` that correspond to shipped wordlists — adequate, not literally all 28). The SAME normalization applies to BOTH the seed and the passphrase. **(R0 M1) The NFKD/lower order + the CJK-strip are exercised ONLY by §5 vector 3 (`UNICODE_HORROR`)** — that cell is the load-bearing normalization discharge (vectors 1/2 are pure ASCII). `normalize_phrase_for_hmac` is private in `electrum.rs`; the new `electrum_seed_to_bip32_seed` lives in `electrum.rs` too → calls it in-module, **no `pub(crate)` widening needed**.
