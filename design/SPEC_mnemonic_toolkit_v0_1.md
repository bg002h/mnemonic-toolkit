...
|---|---|---|
| `InvalidHrp(s)` | exit 2 | "mk1 wrong HRP: got `{s:?}`, expected \"mk\"" |
...
| `InvalidChar { ch, position }` | exit 1 | "mk1 invalid character `ch` at position `position` (not in bech32 alphabet)" |
