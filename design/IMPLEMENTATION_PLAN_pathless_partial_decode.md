...
**P1.2 — `md encode` advisory.** `cmd/encode.rs`: when the encoded descriptor is `canonical_origin==None` AND no `--path` supplied, print a loud stderr advisory (mirror the F-A4 footgun tone) nudging `--path` for a fully-decodable backup. Still emits, exit 0. TDD RED: advisory fires on pathless shape, absent on canonical/`--path` shapes; card bytes unchanged.
