...
### §2.5 Refusals (25 classes; expanded 2026-05-14 from `python-shamir-mnemonic/vectors.json` audit covering negative-vector categories the original SPEC enumeration elided; further expanded at v0.13.0 P1c-E.1 GREEN with 5 combine-driver + parse-layer refusal rows for empty share lists, per-share + cross-share value-length consistency, extendable-bit consistency, and the python-parser `group_count >= group_threshold` mirror; row 24 `MemberThresholdMismatch` added at v0.13.0 P2.2 GREEN per Q3 fold; row 25 `--group N,1 with N>1` toolkit-policy refusal added at v0.13.0 P3 R1 fold per python `split_ems` rule at `slip39/mod.rs:135` — the algorithm replicates the group share to all N members so T=1+N>1 is degenerate; the previously-undocumented refusal class was surfaced when chapter examples 1+2 attempted `--group 1,1` AND `--group 2,1` with both refused, leaving `--group 2,2` as the smallest legal split)
...
| 7 | `combine` shares: identifier mismatch across shares (vectors.json #6, #25) | 1 | `slip39 combine: shares disagree on identifier; shares must come from the same secret` |
...
| 25 | Any `--group N,T` with `T == 1` AND `N > 1` (toolkit policy; python `split_ems` rule mirror per `python-shamir-mnemonic/cryptography.py` @ 17fcce14: the algorithm replicates the group share to all N members, so a T=1+N>1 spec is degenerate replication and offers no recovery benefit beyond `--group 1,1` which row 5 separately refuses; the lib emits the row 4 catch-all stem `1 <= T <= N <= 16` because the BadGroupSpec variant is shared, but the conceptual refusal class is distinct — added at v0.13.0 P3 R1 fold) | 1 | `slip39 split: --group N,T requires 1 <= T <= N <= 16; got group <idx>=N,T` |

(Refusal 18 covers the N+1 pairwise candidates explicitly. Refusals 5 and 25 jointly enforce that the smallest legal `--group N,T` is `2,2` — both shares required for any single-group split.)
...
| G5 — Refusal coverage | All 25 refusal classes (§2.5; row 24 added at v0.13.0 P2.2 GREEN per Q3 fold; row 25 `--group N,1 with N>1` toolkit-policy added at v0.13.0 P3 R1 fold per python `split_ems` rule) have CLI tests asserting exit code 1 + pinned stderr stem. The 30 negative vectors from G1 are exercised at the lib layer; CLI-level tests verify each stem surfaces byte-faithfully. (Note: row 25 is doc-only at SPEC layer — the lib enforces it but emits the row 4 catch-all stem `1 <= T <= N <= 16`; the cli_slip39_refusals.rs `stderr.contains` substring match for row 4 covers row 25 too. A future tightening would be a row-25-specific lib stem like `--group N,T with T==1 requires N==1; got group <idx>=N,1` — out of P3 scope.) |
