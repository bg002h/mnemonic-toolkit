...
- **Phase 1 — registry + producers + goldens.** `archetype.rs` (registry, `ArchetypeParams`, `validate_params`, 5 `lower` fns); **`DiagnosticKind::Param` + its `as_str` arm** (R0-r1 I2 — Phase 1's producer negatives construct it); `CliArchetype` + the 10 param flags + preset dispatch (before spec intake, §1) in `cmd/build_descriptor.rs`; tests: layer 1 + layer 2 + stdin-contract cells + producer negatives + key-order + non-vacuity + drift self-tests (a)/(b). RED first.
