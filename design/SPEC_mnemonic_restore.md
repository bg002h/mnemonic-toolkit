...
- **ms1 (I-C fold):** `slot_ms1::resolve_ms1_slot(value, flag_language=--language, idx) -> Ms1SlotResolution { entropy, derive_language, emit_language }` (`slot_ms1.rs:37`). ms1 `Mnem` payloads carry the wordlist language ON THE WIRE (drives BIP-39 PBKDF2), so a bare `ms_codec::decode` is wrong for non-English `mnem` cards. `resolve_ms1_slot` applies wire-wins / refuse-on-`--language`-conflict (`SlotInputViolation` exit 2). Use `res.derive_language` downstream. Pass `slot_index = 0` (M-idx: `slot_index` is used only in the conflict message string, `slot_ms1.rs:63`, not derivation — a restore-time conflict would read "slot @0.ms1="; cosmetic, the I-C test should confirm the phrasing is acceptable or the implementer adjusts the message). (Builds on `project_ms_mnem_v0_2_shipped` / `project_ms1_slot_v0_41_0_shipped`.)
...
2. First receive address(es): derive `m/0/0..0/n-1` children of `account_xpub` (verification-only secp + `Xpub::derive_pub` — watch-only by construction, `addresses.rs:232,242`); `address_render::render_address_from_xpub(secp, &child, script_type, network)` (`address_render.rs:18`). **M-c (R2-corrected):** `script_type` via the EXISTING `convert::script_type_from_template(T) -> Option<ScriptType>` (`convert.rs:393`, bump to `pub(crate)`) — bip44→P2pkh / bip49→P2shP2wpkh / bip84→P2wpkh / bip86→P2tr (multisig → `None`, unreachable after the I-B rejection / internal-invariant guard). Do NOT hand-write a 4th copy. `--count` controls n.
...
**Round 1** (RED 0C/3I — 0 leakage, single-sig design proven) → **I-A** `--format` + `--template None` → `ModeViolation` **exit 2** (NOT BadInput); `--template` becomes `Option`. **I-B** multisig `--template` → `BadInput` exit 1 via `is_multisig()`. **I-C** ms1 via `slot_ms1::resolve_ms1_slot` (preserve `mnem` wire-language; use `derive_language`). **M-a** error anchors `471/529/588/775`. **M-b** `DerivedAccount` `derive.rs:23-39`. **M-c** `CliTemplate→ScriptType` map. **M-d** `main.rs` enum/dispatch feature-clustered — don't re-sort.
**Round 2** (RED 0C/1I — full single-sig chain implementable, 0 leakage) → **I-Mc** the round-1 M-c claim "no `CliTemplate→ScriptType` helper" was WRONG: reuse the existing `convert::script_type_from_template` (`convert.rs:393`, bump `pub(crate)`) instead of hand-writing a 4th copy (fix-the-class). **M-idx** ms1 `slot_index=0` is cosmetic-only (conflict-message wording). **M-b** `DerivedAccount` `derive.rs:23-36`. Re-grep at plan-write; re-dispatch R0 round 3.
