...
- [ ] **Step 4 — evidence per flag route:** each route carries an EXPLICIT `source_file` (R2 M-1: NOT derived from the subcommand name — the harness `fs::read_to_string`s one file + panics if missing; gui-schema FLATTENS nested subcommands as hyphenated names `gui_schema.rs:1002`, and some stdin routes live in SHARED library modules). Set `source_file` to the file that actually CONTAINS the anchor (grep-verified at impl). **Non-obvious mappings (R2 I-1/I-2, all verified):**
  - `xpub-search-path-of-xpub` / `-account-of-descriptor` / `-passphrase-of-xpub`: `--ms1` → `src/cmd/xpub_search/seed_intake.rs` (`ms1-stdin` / `secret_in_argv_warning`); `--passphrase` → the per-mode file `src/cmd/xpub_search/<mode>.rs` (or wherever `passphrase-stdin`/`@env:` for that mode lives — verify per route).
  - `seedqr-decode`: `src/cmd/seedqr.rs` (NOT `seedqr-decode.rs`).
  - `inspect` / `repair` `--ms1`: `src/repair.rs` (the shared `value == "-"` route via `resolve_groups`/`expand_dashes` — NOT `cmd/inspect.rs`/`cmd/repair.rs`, which carry only the doc-phrase).
  - `seed-xor-split`/`-combine`: `src/cmd/seed_xor.rs`; `slip39-split`/`-combine`: `src/cmd/slip39.rs`; export-wallet: `src/cmd/export_wallet.rs` (`validate_watch_only` @256); the rest (nostr/silent-payment/electrum-decrypt/import-wallet/convert/derive-child/final-word/bundle/verify-bundle) → their own `src/cmd/<name>.rs`.
  For each route assert the `source_file` contains a NON-ARGV-channel anchor — any of: `"--X-stdin"` / `"X_stdin"` / (`secret_in_argv_warning` naming `"--X"`) / (`"--X -"` / `value == "-"` carve-out) / **(R0 I3) `@env:`-class: `"@env:"` / `"resolve_env_sentinels"` / `"needs_env_sentinel_resolution"`**. Encode as a per-route `evidence: &[&str]` keyed by `(subcommand, flag)` — the route SET is now closure-checked (Step 1), so a new flag can't be added without BOTH a route entry AND its evidence. **R0 I3:** `import-wallet --ms1` + `verify-bundle --ms1` anchor on `@env:` (they have no `*-stdin`/`-`). **R0 M2:** `--share` appears on BOTH slip39-combine (`"--share -"`) and seed-xor-combine (`"--share phrase=-"`/`secret_in_argv_warning`) — distinct anchors, correctly handled by the route-keyed map (do NOT share one anchor). **If any route has NO non-argv anchor → real leak (escalate; recon+R0 found none).**
...
- [ ] **Step 3** — populate the declared sets to match; assert each subcommand's source has its non-argv anchor:
  - `--from` → `"=-"` / `value == "-"` (all 7). source_file = `src/cmd/<name>.rs` EXCEPT the flattened `seedqr-decode` + `seedqr-encode` → `src/cmd/seedqr.rs` (R2 M-1: no `seedqr-decode.rs`).
