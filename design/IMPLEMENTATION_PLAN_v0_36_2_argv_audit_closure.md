...
3. **`--slot` axis** — subcommands whose gui-schema flags include `--slot`. Each must wire `slot-stdin` OR (export-wallet) REFUSE secret subkeys (`export_wallet.rs:92-94` — the refusal IS the safety anchor). Membership = the `--slot`-bearing subcommand set.
...
- [ ] **Step 2** — run; FAIL listing the live members (`--from`: convert, derive-child, final-word, seed-xor-split, slip39-split, seedqr-decode, seedqr-encode, …; `--slot`: bundle, verify-bundle, export-wallet, import-wallet — verify against the dump).
- [ ] **Step 3** — populate the declared sets to match; assert each subcommand's source has its non-argv anchor: `--from` → `"=-"`/`value == "-"`; `--slot` → `"slot-stdin"`/`"slot_stdin"` EXCEPT export-wallet → the secret-subkey REFUSAL anchor (`export_wallet.rs:92-94`, e.g. `"secret subkeys"` + `"REFUSED"`). (R0 I1: the `=-`/slot-stdin route is uniform per-subcommand — it covers every node/subkey accepted, so no per-node enumeration is needed.)
- [ ] **Step 4** — GREEN; commit.
...
- **R0 dispositions folded (all RESOLVED):** I1+I2 (axes 2/3 collapsed to per-`(subcommand, --from|--slot)` name-match — no per-node cartesian; export-wallet refusal is its anchor); I3 (`@env:` added to recognized anchors; import-wallet/verify-bundle `--ms1` anchor on it; advisory-gap FOLLOWUP filed); M1 (seed = 9 flag-NAME routes); M2 (`--share` route-keyed collision noted); M3 (closure-vs-evidence orthogonality stated). Confirmed: gui-schema subprocess is the flag-axis source (Cli is binary-private); `--from`/`--slot` membership = name-match (no `NodeValueComposite` kind); set-equality (bidirectional) preserves removal-detection; no new lib export needed (`secret_taxonomy` consts already pub).
