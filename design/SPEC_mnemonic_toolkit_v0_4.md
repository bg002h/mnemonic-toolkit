# mnemonic-toolkit v0.4 SPEC

**Version:** 0.4.0
**Date:** 2026-05-05 (brainstorm convergence)
**Status:** DRAFT (architect r6 0C/0I/2L → APPROVE; user-approved cycle plan; pending in-toolkit-repo r1+ architect review on this transcribed file)
**Predecessor:** [SPEC_mnemonic_toolkit_v0_3.md](SPEC_mnemonic_toolkit_v0_3.md) (v0.3.0 + v0.3.1)

## Carry-forward from v0.3

Unless explicitly noted as DELTA below, all sections of v0.3 SPEC carry forward unchanged. This SPEC writes only the delta sections.

**Carry forward unchanged:** §1 (Metadata), §2 (Versioning), §3 (Wire format — md1/mk1/ms1 codecs). See [SPEC_mnemonic_toolkit_v0_3.md](SPEC_mnemonic_toolkit_v0_3.md) for the full text of §1–§3.

**Delta sections in v0.4 (rewritten / added / removed below):** §4.9.a, §4.11 (NEW), §5.5 (NEW), §5.6, §5.7, §5.8 (NEW), §6.6, §6.7 (NEW), §6.8 (REMOVED — multi-leaf rejection rule supersed by §4.9.a multi-leaf-supported promotion), §6.9, §8, §9, §10, §11.
...
**Normalization domain:** `derivation_path` for collision purposes is the raw path string as supplied by the user (e.g., `"48'/0'/0'/2'"`). If a slot has no `path` subkey supplied (watch-only slot with degenerate origin metadata, or single-key `wif` slot), `derivation_path` is treated as the empty string `""` for collision comparison. Two slots with identical xpubs and both lacking `path` subkeys ARE considered colliding. Two slots with identical xpubs but different non-empty paths are NOT colliding (BIP-388 letter; different key-vector entries). Collision is purely on the `(xpub, derivation_path_string)` tuple under raw-string equality (no path canonicalization beyond what the input source preserves, no xpub network normalization).

**v0.4.1 raw-string equality clarification:** the literal byte-for-byte path string supplied via `--slot @N.path=<value>` is preserved end-to-end through `CosignerKeyInfo.path_raw`. Under raw-string equality, `48h/0h/0h/2h` and `48'/0'/0'/2'` compare UNEQUAL — they are different raw byte sequences even though they encode the same BIP-32 path. Legacy descriptor-placeholder paths (parsed from `@N[fp/path]xpub` annotations) flow through the parser's canonicalized form (`'`-notation), so legacy paths supplied as `48h/0h` collapse to `48'/0'` during parse and then compare as canonicalized. Users who care about preserving non-canonical notation should use the `--slot` path; users who don't see no behavior change vs v0.4.0.
...
**Card cardinality:** 1 master card per bundle (locked 2026-05-05 brainstorm). JSON: `engraving_card: Option<String>`.
...
- **Multisig (N>1):** **3 + 6N checks** total = 3 shared (`md1_decode` + `md1_wallet_policy` + `md1_xpub_match`) + 6 per cosigner (`ms1_decode[i]` + `ms1_entropy_match[i]` + `mk1_decode[i]` + `mk1_xpub_match[i]` + `mk1_fingerprint_match[i]` + `mk1_path_match[i]`). The "6N" already includes per-slot ms1 checks; schema 4's per-slot ms1 array is what drives the 2 ms1 checks per slot in the 6N count.
- **Hybrid mode** (some slots secret-bearing, some watch-only): same 3+6N schema; for watch-only slots (where `ms1[i] == ""`), `ms1_decode[i]` and `ms1_entropy_match[i]` checks pass-vacuously (set `passed: true`, all forensic fields null, decode_error = "skipped: watch-only slot").
- **Pure watch-only multisig** (all slots xpub-only, ms1 all empty): same 3+6N schema; all `ms1_decode[i]` and `ms1_entropy_match[i]` pass-vacuously.
- **`wif` slot handling:** verify-bundle treats `wif` slots as watch-only for ms1/check purposes (`ms1[i] == ""`, `ms1_decode[i]`/`ms1_entropy_match[i]` pass-vacuously). The mk1 card for a wif slot is still a real engraved card; `mk1_decode[i]` and `mk1_xpub_match[i]` checks run normally against the supplied wif's derived public point.
...
## §6.6 Mode-violation pre-check ladder (REWRITTEN for unified `bundle`)

The v0.2 / v0.3 mode-violation row table is REPLACED. v0.4 ladder runs before any synthesis:

| Row | Condition | Exit | Stderr (byte-exact) |
|---|---|---|---|
| 1 | `bundle multisig-full` or `bundle multisig-watch-only` subcommand invoked | 2 | `error: 'bundle multisig-full' / 'bundle multisig-watch-only' subcommands removed in v0.4. Use 'bundle' (mode auto-detected from --slot @N.<subkey>=<value> inputs).` |
...
| 5 | `--cosigner-count K` supplied AND K ≠ max(slot indices) + 1 | 2 | `error: --cosigner-count deprecated and inconsistent with slot indices (declared N={K}, derived N={max+1})` |
| 6 | `--phrase X` supplied AND `--slot @0.phrase=` also supplied | 2 | `error: --phrase deprecated; cannot combine with --slot @0.phrase=` |
| 7 | `--cosigner @N=[fp/path]xpub` supplied AND conflicting `--slot @N.<subkey>=` for the same N also supplied | 2 | `error: --cosigner annotation conflicts with --slot @N.<subkey>=` |
...
| 13 | BIP-388 violation: any two slots resolve to the same (xpub, path) tuple per §4.11.b normalization | 2 | `error: BIP-388 distinct-key violation: slot @{i} and slot @{j} resolve to identical (xpub, path)` |
...
Rows 1-12 fire pre-synthesis; rows 13-14 fire post-binding (after derivation but before bundle emission).

### §6.6.a v0.2-flag deprecation alias mapping table

Source of truth for the alias-handler text.

| v0.2 form | v0.4 alias expansion | Conflict-resolution rule |
|---|---|---|
| `--phrase X` (standalone, no `--slot` flags present) | `--slot @0.phrase=X` | If `--slot @0.phrase=` ALSO supplied, exit 2 row 6 |
| `--cosigner @N=xpub` (alone) | `--slot @N.xpub=xpub` | Companion flags `--fingerprint @N=fp` + `--path @N=path` (if v0.2 had them at this granularity) expand to `--slot @N.fingerprint=fp` + `--slot @N.path=path`; missing companions for an xpub-only slot remains valid |
| `--cosigner @N=[fp/path]xpub` (annotated form) | `--slot @N.xpub=xpub` + `--slot @N.fingerprint=fp` + `--slot @N.path=path` (three subkey expansions from one annotated value) | If conflicting `--slot @N.<subkey>=` ALSO present, exit 2 row 7 |
| `--cosigner-file <path>` | Each line expands via the `--cosigner` rules above | (no conflict-resolution; file is just a bulk source) |
| `--cosigner-count N` | DEPRECATED; v0.4 derives N from `max(slot indices) + 1` | If `--cosigner-count K` is supplied AND `K != max(slot_indices) + 1`, exit 2 row 5 |
| `bundle multisig-full <flags>` (subcommand) | NO ALIAS — exit 2 row 1 | (subcommand removed entirely; no expansion) |
| `bundle multisig-watch-only <flags>` (subcommand) | NO ALIAS — exit 2 row 1 | (subcommand removed entirely; no expansion) |
