...
**Files:**
- Create: `crates/mnemonic-toolkit/src/slot_ms1.rs` (+ **`mod slot_ms1;` in `crates/mnemonic-toolkit/src/main.rs`** alongside `mod slot_input;` at `:27` — the BINARY crate, NOT lib.rs: the helper uses `crate::error`/`crate::language` binary-internal paths (both private `main.rs` mods, `:14`/`:18`) and is consumed as `crate::slot_ms1::resolve_ms1_slot` from `cmd/*` — R0-I2)
...
```
Register the module: add `mod slot_ms1;` to `crates/mnemonic-toolkit/src/main.rs` (next to `mod slot_input;` at `:27`) — the BINARY crate, NOT `lib.rs` (R0-I2; `crate::error` `:14` / `crate::language` `:18` are binary-internal mods). ms_codec error already maps via `ToolkitError::from` (`convert.rs:1464`).
