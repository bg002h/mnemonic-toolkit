...
- **Status:** `open` (defense-in-depth; no NEW residue vs pre-cycle-14).
- **Tier:** `polish` / `next-cycle`.
...
- **Status:** `open` (defense-in-depth; the owners are all scrubbed as of v0.67.0).
- **Tier:** `polish` / `next-cycle`.
- **Companion:** none (toolkit-internal). Continuation of the SHIPPED `secret-memory-hygiene-cycle-b` Site-1 scrub leg (cycle-14, toolkit v0.67.0).
...
- **Status:** `open` — **NARROWED (toolkit v0.70.0, 2026-06-22).** The minimal additive helper SHIPPED in v0.70.0 (P0 of the own-account subset-search cycle): `struct ScrubbedXpriv(Xpriv)` (move-only, no `Copy`/`Clone`, no escape hatch, Drop-time `private_key.non_secure_erase()` + volatile `chain_code` zero) + `derive_account_xpub_only(...) -> (Xpub, Fingerprint)` + the fan-out `derive_accounts_xpub_only` (`crates/mnemonic-toolkit/src/derive_slot.rs:195,251`), with the public-only TDD/byte-identical/scrub tests. The over-supply own pool derives K_own keys through it (public-only — never owning an `Xpriv`). The **REMAINING open scope is the broader 7-site lift:** replace `DerivedAccount.account_xpriv: Xpriv` (`derive.rs:27`) with `ScrubbedXpriv` across all ~7 `derive_slot.rs` consumers + `DerivedAccount::into_parts`. The bare field still drops un-scrubbed for those legacy callers.
