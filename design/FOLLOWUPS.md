...
### `restore-multisig-taproot-reconstruction` — `mnemonic restore --md1` refuses a taproot multisig md1; md1→`tr(sortedmulti_a)` reconstruction is blocked at the rust-miniscript layer
...
- **What:** A wallet-policy md1 whose tree is `Tr{…, SortedMultiA{k, indices}}` cannot be reconstructed to a concrete `tr(sortedmulti_a(…))` descriptor via `to_miniscript_descriptor` — runtime-verified this cycle: the call ERRORS on a real `tr-sortedmulti-a` md1. The toolkit refuses taproot reconstruction everywhere (from-import-json refuses it; `template_from_descriptor` refuses `Tr`; the descriptor emitter only passes through a user-supplied tr string). So restore refuses a taproot multisig md1 (exit 2) rather than mis-emit. Extending it requires EITHER (a) an md-codec/rust-miniscript fix giving a `Terminal::SortedMultiA` (or a multipath-aware `to_miniscript` for tr), OR (b) a bespoke toolkit-side tr-descriptor string emitter that walks `d.tree` (internal key / `is_nums` / `key_index`, `k`, indices) + `expand_per_at_n` keys and computes the BIP-386 checksum by hand. Single-sig restore ships bip86 (taproot single-sig), so this closes a parity asymmetry for multisig.
- **Why deferred:** Material new sub-feature (no reuse; manual descriptor + checksum + key-sort correctness) — descoped to keep the v0.44.0 cycle tight on the proven wsh/sh-wsh path (user decision, recon'd at cycle time). md-codec pinned `0.35.0`.
- **Status:** `open`
- **Tier:** `v0.5`
- **Tags:** `restore` `wallet` `multisig` `taproot`
