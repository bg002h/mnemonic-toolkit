...
- **Status:** `open`. #28 phase 2 P3a–P5 SHIPPED at toolkit **v0.60.0** *around* this gate — the `--own-account-max` flag is wired and **refuses** with an actionable message; the exact-`--account`-list completion path (`pool.len() == n`, `realized_s = n!`) is fully live. Only the over-supplied own-account subset-search remains deferred.
