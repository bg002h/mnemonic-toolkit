...
- **Status:** `open` — **RE-SCOPED 2026-06-08 (the prerequisite `toolkit-trmultia-nums-internal-key` shipped in v0.48.0, but it does NOT unblock this — recon-disproven).** True blockers: (1) `tr-sortedmulti-a` is blocked UPSTREAM — rust-miniscript v13.0.0 has no `Terminal::SortedMultiA` fragment (`descriptor-mnemonic/crates/md-codec/src/to_miniscript.rs:406-410` unconditionally errors, independent of the internal-key shape) → tracked as the md-codec companion `md-codec-sortedmulti-a-to-miniscript-rendering-gap`; (2) `tr-multi-a` already renders `tr(NUMS, multi_a(…))` (v0.48.0) but `restore.rs:777` (`d.tree.tag == md_codec::Tag::Tr`) refuses ALL Tr md1 at the pre-gate — a targeted pre-gate lift for the renderable `MultiA` leaf would deliver tr-multi-a restore. The earlier "blocked on trmultia-nums" framing was incomplete.
- **Tier:** `v0.5`
