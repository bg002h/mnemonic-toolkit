...
- **Surfaced:** 2026-06-06, mnemonic-gui v0.28.0 R0 (the architect's "GUI repo has no `docs/`" observation traced the stale prose to THIS toolkit repo's `docs/manual-gui/`, correcting the v0.47.3 R0 M2's mis-cited `:422`).
- **Where:** `docs/manual-gui/src/40-mnemonic/45-export-wallet.md:30` ("Bitcoin Core `timestamp` field (default `now`)") + `:340-343` ("`now` (the default; emits the literal string `"now"` in the JSON …)"). The `docs/manual-gui/tests/expected_gui_schema_inventory.json` `--timestamp` entry has NO `default_value` field → NOT stale, NOT gated.
- **What:** toolkit v0.47.3 (`export-wallet-timestamp-default-zero`) flipped `export-wallet --timestamp`'s default `now → 0` (genesis rescan). The GUI user manual (`docs/manual-gui/`, a separate pinned cadence, ~v0.3.0, OUT of routine pin catch-ups) still documents the old `now` default. Reword `:30` + `:340-343` to lead with the `0` default (mirror the toolkit manual's `41-mnemonic.md:707` + `37-wallet-export.md` wording from v0.47.3). Prose-only; the manual-gui lint (`make -C docs/manual-gui lint`) is the gate.
- **Why deferred:** `docs/manual-gui/` rides its own pinned cadence (not bumped in routine toolkit/GUI cycles); folding a prose fix into the v0.47.3 toolkit cycle or the GUI v0.28.0 cycle (different repo) was out of scope. Natural fold: the next manual-gui cadence bump.
- **Status:** `open`
- **Tier:** `docs`
- **Tags:** `gui`, `manual-gui`
