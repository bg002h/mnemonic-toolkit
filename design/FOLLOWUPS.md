...
### `toolkit-check-pkk-non-tap-non-canonical` — toolkit keeps `Check(PkK/PkH)` in wsh/sh → non-conformant md1 vs md-cli for `wsh(pk)` shapes
...
- **Fix (deferred, R0-gated, separate cycle):** the TOOLKIT adopts SPEC §5.1 — drop the `tap_context` gate so `Check(PkK|PkH)` collapses to bare in non-tap too, and INVERT `walk_check_kept_in_non_tap_context` (verify first the toolkit has no countervailing SPEC reason). When that ships, the Cycle-D harness's `wsh-pk`/`wsh-pkh`/`wsh-and_v`/`wsh-or_d` entries flip `Verdict::Diverge → Match` (update the corpus table). NOT "fix md-cli", NOT "define canonicity" (already defined).
- **Companion:** descriptor-mnemonic `design/FOLLOWUPS.md` (this slug, the conformant side) + a cross-reference on the existing related `v2-design-questions` item 12 ("Walker context-dependent `Check(PkK)` mangling") at :562.
- **Tier:** deferred (wire-canonicity / interop hazard; surfaced + gated by Cycle-D, fix is a separate R0-gated cycle).
