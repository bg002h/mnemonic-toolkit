...
- **Status:** `open` (re-scoped from `v1+` to `v0.13.0-feature` 2026-05-14 as the second cycle in the share-splitting pair planned at `~/.claude/plans/radiant-seeking-teacup.md`; companion to resolved [[seed-xor-coldcard-compat]]).
- **In flight:** v0.13.0 P0–PE. P0 SPEC `design/SPEC_slip39_v0_13_0.md` authored 2026-05-14; R0 reviewer-loop pending. Hand-roll path locked (no maintained Rust SLIP-39 crate exists as of 2026-05-14 audit — `sssmc39` abandoned 2020, `slip39` GPL-3.0 CLI-only, trezor-firmware SLIP-39 is C+Python under GPL-3.0). Vendoring `python-shamir-mnemonic/vectors.json` as G1 fixture (45 vectors = 15 positive + 30 negative).
