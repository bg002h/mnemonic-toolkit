...
- **Surfaced:** 2026-06-10, the 23-agent read-only architecture audit (find → adversarial-verify → synthesize). 48 verified findings constellation-wide (0 critical); this repo's share below. **Full report + per-finding detail (claim/evidence/fix/disposition):** `design/agent-reports/constellation-architecture-audit-2026-06-10.md` (committed in the toolkit repo). Promote any line to its own `### <id>` entry when worked; resolve here as fixed.
- **This repo's verified findings (23):**
  - **[IMPORTANT] ✓ RESOLVED (audit I10, v0.53.0, 2026-06-10 — backlog index was not updated at the time)** `mk1-csi-collision-multisig-grouping` — the finding's premise (csi derived from the cosigner xpub fingerprint, path-independent) is now FALSE in source. **Emit** (`synthesize.rs:294-297`): `csi = derive_mk1_chunk_set_id_for_slot(&stub, i)` = `stub ^ slot`, distinct per slot (n=1 branch likewise at `:277`), with a comment citing audit I10 for exactly this same-xpub-different-path collision. **Verify** (`verify_bundle.rs:1568-1576`): groups supplied mk1 by the STORED csi (`chunk_set_id_extract`), never recomputed from the fingerprint — so the slot-unique emit csi yields slot-unique groups; a cross-slot substitution surfaces as a missing/duplicate group. **Test:** audit M1 (v0.53.2) added a `cross-slot-swap` synthesize-then-mutate self-check RED cell (multisig fixture uses DISTINCT per-slot seeds) + slot-exact `desc.tlv.pubkeys` binding. The collision the finding describes cannot occur. (Verified 2026-06-11 against `origin/master` @ 00a76ed.)
  - **[IMPORTANT/I3] ✓ RESOLVED (v0.53.1, 2026-06-10)** `vacuous-secret-flag-gate` — promoted below; three non-circular §7b gate cells + `--phrase`/`--phrase-stdin`/`--ms1-stdin` secret-classified.
  - **[minor] ✓ RESOLVED (2026-06-11, NO-BUMP)** `g6-invariant-sibling-master-not-pin` — the g6-invariant job now resolves the canonical ms-cli pin from `scripts/install.sh` (a new fail-loud `Resolve pinned ms-cli tag` step mirroring `sibling-pin-check.yml`'s parser) and checks out `mnemonic-secret` at `ref: ${{ steps.pin.outputs.tag }}` (= `ms-cli-v0.7.0`) instead of `master` — so the byte-equality runs against the pinned version, with NO second pin to drift (verified mlock.rs is byte-identical master vs the tag). SPEC `design/SPEC_ci_test_hygiene_g6_pin_and_synthesize_correctness.md`.
  - **[minor/M11] ✓ RESOLVED (v0.53.3, 2026-06-10)** `hrp-classifier-rejects-valid-uppercase-cards` — promoted below; The toolkit's card-type prefix probes match lowercase ms1/mk1/md1 only, but the codecs decode all-uppercase strings per BIP-173 (mk-codec rejects only Mixed case then to_lowercase-normalizes). So a va (`crates/mnemonic-toolkit/src/repair.rs:106-119 (classify_hrp_prefix); cmd/restore.rs:1026 (values.iter().all(|v| v.starts_with("mk1")))`)
  - **[minor] ✓ RESOLVED (v0.53.6, 2026-06-11)** `import-json-schema-version-unchecked` — `parse_import_json_envelopes` (the shared chokepoint for `export-wallet --from-import-json` + `bundle --import-json`) now `validate_schema_versions`-gates the SELECTED envelope's outer (`"1"`) + inner bundle (`"4"`) `schema_version` strict-equal, rejecting unsupported/future versions with `BadInput` (fail-closed; zero regression for valid envelopes). Tests: accept + 2 reject cells (RED-proven) in `json_envelope.rs`. SPEC `design/SPEC_import_json_schema_gate_and_secret_zeroize.md`.
  - **[minor/M3] ✓ RESOLVED (v0.53.2, 2026-06-10)** `inspect-repair-no-argv-advisory` — promoted below (audit-minors entry); Both `inspect --ms1 ms1abc...` and `repair --ms1 ms1abc...` place a secret-bearing ms1 card (BIP-39 entropy; OutputClass::PrivateKeyMaterial) on argv without ever calling crate::secret_advisory::secre (`crates/mnemonic-toolkit/src/cmd/inspect.rs:88-156; crates/mnemonic-toolkit/src/cmd/repair.rs:101-218`)
  - **[minor/M2] ✓ RESOLVED (v0.53.2, 2026-06-10)** `localize-broad-error-collapse` — promoted below (audit-minors entry); localize() maps every Err(_) from per-subtree from_str_ext(insane) to None, relying on the M3 invariant that after step-2 type-check the only subtree failure can be NonTopLevel; localize_parse_failure (`crates/mnemonic-toolkit/src/descriptor_builder/gate.rs:438-453 (localize) vs gate.rs:405-418 (localize_parse_failure)`)
  - **[minor/M13] ✓ RESOLVED (v0.53.2, 2026-06-10)** `orphaned-v0_2-multisig-goldens` — resolved by reference to `orphaned-v0_2-md1-vectors-no-harness` (25 goldens DELETED); The v0_2 vector directory holds 26 golden bundle fixtures; only bip84-mainnet-0-false-true.txt is read by any test. The 25 multisig fixtures are dead goldens — a regression changing the bundle multisi (`crates/mnemonic-toolkit/tests/vectors/v0_2/{wsh-multi,wsh-sortedmulti,sh-wsh-multi,sh-wsh-sortedmulti}-*.txt; cli_self_check.rs:13`)
  - **[minor] ✓ RESOLVED (2026-06-11, NO-BUMP)** `policynode-grammar-coverage-vacuous-on-joint-omission` — a new `declare_policy_node_kinds!` macro (precedent `declare_node_type_variants!`) single-sources `NODE_KINDS` + `PolicyNode::kind()` from ONE `(Variant => "kind")` list. The generated `kind()` `match self` is exhaustive → a new variant missing from the macro is a COMPILE error → extending the list auto-grows NODE_KINDS → `node_kinds_cover_enum` (samples==NODE_KINDS) then FAILS until the sample is added (non-vacuous; RED-proven by dropping a sample). The REMINDER match in `all_variant_samples` is kept (belt-and-suspenders). `grammar_matches_node_kinds_hand_list` likewise de-vacuified. SPEC `design/SPEC_ci_test_hygiene_enumerator_completeness.md`.
  - **[minor/M1] ✓ RESOLVED (v0.53.2, 2026-06-10)** `self-check-no-mk1-xpub-binding` — promoted below (audit-minors entry); self_check_bundle validates md1 reassembly, the shared 4-byte policy_id_stub linkage, mk1 fingerprint presence/absence, and per-slot ms1 entropy round-trip — but never checks that each decoded mk1 car (`crates/mnemonic-toolkit/src/cmd/bundle.rs:2060-2139 (Multi branch 2112-2138, Single branch 2086-2110)`)
  - **[minor] ✓ RESOLVED (v0.53.6, 2026-06-11)** `silentpayment-nostr-priv-not-zeroizing` — new serialize-transparent `SecretString(Zeroizing<String>)` (`crate::secret_string`) wraps the derived private-key strings (silent-payment `scan_priv`/`spend_priv`, nostr `wif` + the `electrum` import string that embeds it), so every heap copy zeroizes on drop. `--json` wire + text output BYTE-IDENTICAL (Serialize=serialize_str; Display/Deref transparent); length-only Debug. +4 `lint_zeroize_discipline` rows + range bump. Accepted limit: the secret is intentionally emitted to stdout; secp source keys out of scope. SPEC `design/SPEC_import_json_schema_gate_and_secret_zeroize.md`.
  - **[obs] ✗ WONTFIX (deep recon 2026-06-11)** `addresses-env-sentinel-overapplied` — In addresses::run the --from value is passed through resolve_env_var_sentinel unconditionally (line 162) regardless of from.node, so `addresses --from xpub=@env:VAR` (a non-secret watch-only source) also env-resolves (`crates/mnemonic-toolkit/src/cmd/addresses.rs:159-163`). **Recon verdict: BENIGN-QUIRK, not worth a code change.** The `@env:` prefix is unreachable from the base58 xpub value space (an xpub literal never begins with `@`) → the sentinel never corrupts a real xpub; the only delta is `@env:VAR` resolves instead of failing `Xpub::from_str`. No security (xpub watch-only; env-sourcing keeps it out of shell history) / no correctness. Gating it would DELETE a working convenience and replace a clean success with convert's "invalid base58" UX. addresses.rs:162 is the sole unconditional-AND-non-vacuous sentinel site; the real (also-minor) inconsistency is the OPPOSITE direction — convert/derive-child do NOT accept `xpub=@env:`. Optional doc-only touch: amend manual `41-mnemonic.md:2552` to note addresses resolves `@env:`/stdin for all `--from` nodes.
  - **[obs] ✓ RESOLVED (2026-06-11, NO-BUMP)** `anti-collision-16bit-invariant-false` — GROUND TRUTH (R0-verified): the invariant is TRUE for the ENGRAVED identifiers and the manual conflated terminology, NOT a false invariant. `bundle.rs::build_unified_card` engraves md1 = `compute_wallet_policy_id(d)[0..2]` and mk1/ms1 = `derive_mk1_chunk_set_id_for_slot(policy_id[0..4], slot)` — BOTH policy-id-derived → engraved leading-16 genuinely AGREE. The md1 *wire* `chunk_set_id` (`Md1EncodingId`/bytecode-derived) is a SEPARATE value (chunk-grouping, not binding). Fixed the terminology across `42-anti-collision-invariants.md:19,21`, `41-bundle-anatomy.md:138,193`, `61-glossary.md:57,89`, `CHANGELOG.md:2009` to distinguish engraved-display ids from the md1 wire csi. (FOLLOWUP's `:1899` cite was wrong → real line `:2009`.) technical-manual lint OK. SPEC `design/SPEC_doc_test_hygiene_cell27_and_anti_collision.md`.
  - **[obs] ✓ RESOLVED (v0.53.8, 2026-06-11)** `bip388-policy-name-lossy-roundtrip` — the policy `name` is now lifted (new `wallet_import::pipeline::bip388_policy_name`) into `EmitInputs.wallet_name` (whose --descriptor-path default is already "imported-descriptor", so unnamed inputs unchanged) and the emit reads it (`descriptor_to_bip388_wallet_policy(desc, name)`). A named policy round-trips its name (precedence flag > policy-name > default) and, being a general lift, unblocks `--format specter` (policy name → Specter `label`). Tests T1-T5 (one-step round-trip + Specter both RED-proven). SPEC `design/SPEC_bip388_policy_name_roundtrip.md`. Template-path gap carved out → `bip388-template-path-wallet-name`.

### `manual-output-blocks-non-gateable-residual` — catalog of the ~38 manual output/example fences NOT wired into the doc-fidelity gate after Cycle-C P0b/P0c

- **Surfaced:** 2026-06-23, Cycle-C P0c (manual-residual) — the closing inventory after P0b converted the gateable CLI-output fences to `{.text/.json include="<stem>.out"}` goldens (now 56 gated transcripts, +2 from P0c: `37-wallet-export-sparrow.out`, `35-recovery-paths-ms1-length-reject.out`). This entry records WHY the remaining ~38 fenced blocks across `docs/manual/src/` are NOT golden-gated and which are future-conversion candidates vs genuinely non-gateable. Counts are approximate (a re-grep at fix time is required — manual prose churns every cadence).
- **Mechanism recap:** a gateable fence is one whose body is *deterministic CLI stdout/stderr* reproducible from the pinned absolute bins; it becomes `{.text include="<stem>.out"}` (optionally `lines="N-M"`), captured by `docs/manual/tests/verify-examples.sh` and rendered by `pandoc/filters/include-transcript.lua` (fail-closed: a missing `.out` breaks the build). A block is non-gateable when its body is *not* such output.
- **Categories (the ~38 residual blocks):**
  - **(a) synopsis / usage templates with `<PLACEHOLDER>` tokens** (e.g. `--slot @0.xpub=<xpub-0>`, `mk inspect [OPTIONS] <MK1-STRING>...`). These are *command syntax*, not program output — derivable from `--help` usage but they are inputs the reader fills in, not captured stdout. **GENUINELY NON-GATEABLE** (no canonical output to diff; flag-NAME parity is already covered by the manual `flag-coverage` lint running live `--help`). ~11 files carry these.
  - **(b) schema-shape illustrative `--json` fences with `...` / elided fields** (e.g. the `mk encode --json` example with `"mk1_strings": ["mk1...", "mk1..."]` whose chunk-set id randomizes run-to-run; xpub-elided JSON skeletons). The `...` is deliberate — the field is non-deterministic or shortened for readability. **GENUINELY NON-GATEABLE** as written (a literal diff would fail on the elision); a *subset* could be converted if rewritten to a deterministic fixture, but the schema-skeleton intent is pedagogical, not a wire pin.
  - **(c) non-self-contained `xpub-search` recipes** (undefined `$ZPUB`, elided `xpub6..` operands; `41-mnemonic.md`). These COULD be made runnable by substituting a concrete never-fund ZPUB/xpub and capturing the real result. **FUTURE-CONVERSION CANDIDATE** — the highest-value remaining conversion target; flagged for a later phase (P1b/P4-adjacent).
  - **(d) external-tool-input recipes** (`bitcoin-cli listdescriptors`, BSMS round-1/round-2 exchange artifacts; `37`, `39`, `45`, `57`, `67`). The fenced body is the *other tool's* output or a hand-exchanged artifact, not `mnemonic`/`md`/`ms`/`mk` stdout. **GENUINELY NON-GATEABLE** (would require a bitcoind/peer-wallet harness; out of scope for the bin-only fidelity gate).
  - **(e) secret-bearing `nsec` STDOUT fences** (the 2 nostr `nsec1…` examples in `41-mnemonic.md`). Capturing these as goldens would commit private-key material to the repo. **CORRECTLY NEVER GATED** (secret-memory-hygiene bar; the example seed is the never-fund seed but the principle stands — no secret golden corpus).
- **Disposition:** **future-conversion candidate = (c) only**; **genuinely non-gateable = (a), (b), (d), (e)**. No action required to ship Cycle-C; this is the audit-trail record so a later phase can pick up (c) without re-discovering the taxonomy.
- **Severity:** INFO (documentation completeness / future-work catalog; no correctness or funds-safety exposure).
- **Status:** `open` (catalog only). **Tier:** `docs` / `next-cycle`. **Companion:** the doc-fidelity mechanism (`design/agent-reports/` Cycle-C P0a/P0b) + `cli-help-golden-broad-staleness-not-gated` (the retired stale-snapshot precedent) + `manual-prose-command-execution-gate` (the prose-recipe execution gate).

### `technical-manual-output-blocks-non-gateable-residual` — catalog of the 47 technical-manual output/example fences NOT wired into the doc-fidelity gate after Cycle-C P1b

- **Surfaced:** 2026-06-23, Cycle-C P1b (technical-manual fold). After P1b wired `pandoc/filters/include-transcript.lua` into `docs/technical-manual/Makefile` (symlinked to the single-source copy under `docs/manual/pandoc/filters/`; `TRANSCRIPTS_DIR` exported; both `MD_FILTER_ARGS`/`PDF_FILTER_ARGS` prepend it; `.md`/`.tex` build-prereqs updated) and converted the gateable CLI-output fences to `{.text/.json include="<stem>.out"}` goldens, **4 fences are now gated** (`mnemonic-bundle-bip388-collision.out` lines 3-3, `ms1-encode-12word-abandon-unbroken.out` lines 1-1, `54-bundle-json-bip84-singlesig.out`, `54-verify-bundle-json-bip84.out`; verify-examples = 18 transcripts pass). This entry records WHY the **47 remaining `text`/`json` output-class fences** across `docs/technical-manual/src/` (plus the 32 `rust` / 9 `mermaid` / 3 `toml` non-output fences) are NOT golden-gated and which are future-conversion candidates vs genuinely non-gateable. Counts are a snapshot — a re-grep at fix time is required (tech-manual prose churns).
- **Mechanism recap:** a gateable fence is one whose body is *deterministic CLI stdout/stderr* reproducible from the pinned absolute bins; it becomes `{.text include="<stem>.out"}` (optionally `lines="N-M"`), captured by `docs/technical-manual/tests/verify-examples.sh` and rendered by the include-transcript filter (fail-closed: a missing `.out`, an unset `TRANSCRIPTS_DIR`, or an out-of-range `lines=` each breaks the build with FATAL exit 1 — verified). A block is non-gateable when its body is *not* such output.
- **Categories — the 47 residual `text`/`json` output-class fences (`text` = 35, `json` = 12):**
  - **(a) hand-drawn wire / byte-layout DIAGRAMS** — bitfield tables (`| 4:version | 1:chunked | … |`), byte-offset layouts (`[xpub_compact : 73 B]`), header-bit maps, the conventions-and-notation key. ~20 fences, mostly `src/20-wire-formats/{21-md1,22-mk1,23-ms1}-wire-format.md` + `src/10-foundations/14-conventions-and-notation.md` + a few in `src/30-address-derivation/`. **GENUINELY NON-GATEABLE** — these are authored ASCII figures, not program output; no canonical stdout to diff. (The structure they depict IS gated indirectly via the `.out` worked examples that exercise the same encoder.)
  - **(b) constant listings** — the NUMS H-point `50929b74…803ac0` (`21-md1-wire-format.md:203`), the `MK_REGULAR_CONST`/`MK_LONG_CONST`/domain-string + byte-size constants (`22-mk1-wire-format.md:34,70`), and the target-residue / `POLYMOD_INIT` discussion in `src/10-foundations/13-codex32-and-bch.md`. ~4-6 fences. **GENUINELY NON-GATEABLE** as fenced literals — they are derived constants quoted for reference, not CLI output; their correctness is pinned in the codec source (`bch.rs` consts) and the cross-format BCH-separation tests, not in a doc transcript.
  - **(c) schema-shape struct-fragment `json` fences with elided fields** — `VerifyCheck` / `MultisigInfo` / `CosignerEntry` / `MkField` (×2, with `"mk1qprsqhpqqsq3c..."` / `"mk1qa9xxxx..."` elisions) in `src/50-rust-api/54-mnemonic-toolkit-api.md` §V.4.5.3-6. ~5 fences. **GENUINELY NON-GATEABLE** as written (the `...` ellipses and `xxxx` placeholders are deliberate pedagogical skeletons of the serde wire shape; a literal diff would fail). The wire shape itself is *not* schema-mirror-gated either (see `schema-mirror-flag-name-vs-wire-shape-conceptual-clarification`).
  - **(d) external-tool reference illustrations** — `export-wallet` output shapes §V.4.5.9.* for `bitcoin-core` / `bip388` / `coldcard` / `sparrow` / `specter` / `electrum` (×2) / `green`, several carrying `...` ellipses or `<checksum>` placeholders. ~9 fences. **MIXED** — see disposition: the `mnemonic export-wallet` *own* stdout (sparrow/specter/electrum/bitcoin-core/bip388 JSON) is in-principle capturable as a golden (future-conversion), whereas the coldcard `.txt` and `green` text artifacts and any fence interleaving a *consuming* tool's view are genuinely illustrative.
  - **(e) Rust source snippets** — 32 `rust` fences across `src/50-rust-api/{51,52,53,54}-*.md` + `src/20-wire-formats/23-*` + `src/40-bundle-formation/42-*` (pub-use re-export lists, signatures, worked `decode()?` bodies) and 3 `toml` Cargo-dep fences. **GENUINELY NON-GATEABLE** by the output-fidelity mechanism (they are source, not CLI output; the `*-api-roundtrip` cargo-example transcripts already gate that the *documented API compiles and runs*).
  - **(f) CSPRNG-randomized mk1 corpus** — the hand-encoded 84-byte bytecode stream + the resulting `mk1qpzg69…` / `mk1qpg4nc…` chunked strings in `22-mk1-wire-format.md` (worked encode/decode for vectors `V1_bip48_mainnet_1_stub_with_fp` / `V4_bip84_mainnet_1_stub_no_fp`). **GENUINELY NON-GATEABLE via live capture** — the `chunk_set_id` (`0x12345` / `0x45678` in the prose) is CSPRNG-randomized per encode in real runs, so a live `mk encode` would not reproduce these exact strings; the fences are pinned to *fixed corpus vectors* instead. (The corpus vectors themselves are gated in `mk-codec`'s test suite.)
  - **(g) mermaid figures** — 9 `mermaid` fences (relationship/flow diagrams). Gated for *render* by `figures-cache-verify`, not output-diffable; out of scope for this mechanism.
- **Systemic illustrative-fingerprint drift (the ~11 `5436d724` spots):** all 11 occurrences of the **non-canonical** `master_fingerprint` `5436d724` live in `src/50-rust-api/54-mnemonic-toolkit-api.md` (lines 355, 448, 455, 487, 585, 617, 649, 697, 746, 759, + the transcript-prose recap at 809). The gated `.out` goldens use the **canonical** never-fund `73c5da0a` (the P0a-pinned fixture fingerprint). The split:
  - **export-wallet shapes §V.4.5.9.* (CosignerEntry 355; bitcoin-core 448/455; bip388 487; sparrow 585; specter 617; electrum 649; green 697)** — **FUTURE-CONVERSION CANDIDATES.** These illustrate `mnemonic export-wallet --json/--format …` *own stdout*; rewriting them to a gated `{.json include="…export-…bip84.out"}` captured from the real `73c5da0a` fixture would both gate the wire shape AND eliminate the fingerprint inconsistency. Highest-value remaining tech-manual conversion target (P4-adjacent).
  - **engraving-card examples §V.4.6.* (single-sig BIP-86 layout 746/759) + the §V.4.5.9.3 coldcard external-tool view** — **GENUINELY ILLUSTRATIVE.** These are hand-drawn card-face mockups / a third-party tool's file shape, not `mnemonic` stdout; no clean golden source. **RECOMMENDATION:** add an explicit `(illustrative)` label (and ideally migrate the fingerprint to the canonical `73c5da0a` for visual consistency even though they stay un-gated) so a reader does not mistake the mockup fingerprint for a reproducible transcript value.
- **Disposition:** **future-conversion candidates = (d)-subset (export-wallet own-stdout JSON) + the §V.4.5.9.* fingerprint spots**; **genuinely non-gateable = (a), (b), (c), (e), (f), (g) + the §V.4.6.* engraving / coldcard illustrative fingerprint spots** (label these `(illustrative)`). No action required to ship Cycle-C P1b; this is the audit-trail record so a later phase can pick up the export-wallet conversions without re-discovering the taxonomy.
- **Severity:** INFO (documentation completeness / future-work catalog; no correctness or funds-safety exposure).
- **Status:** `open` (catalog only). **Tier:** `docs` / `next-cycle`. **Companion:** the manual sibling catalog `manual-output-blocks-non-gateable-residual` + the doc-fidelity mechanism (`design/agent-reports/` Cycle-C P0a/P1b) + `schema-mirror-flag-name-vs-wire-shape-conceptual-clarification` (the un-gated `--json` wire-shape note that category (c)/(d) lean on).

### `quickstart-output-blocks-non-gateable-residual` — catalog of the 4 QuickStart output fences NOT wired into the doc-fidelity gate after Cycle-C P2 + the prose-drift the conversions surfaced

- **Surfaced:** 2026-06-23, Cycle-C P2 (quickstart). After P2 wired `pandoc/filters/include-transcript.lua` into `docs/quickstart/Makefile` (the `pandoc/filters` + `transcripts` dirs are SYMLINKS to the reference manual's single-source copies; `TRANSCRIPTS_DIR` exported; both `MD_FILTER_ARGS`/`PDF_FILTER_ARGS` prepend the filter; `.md`/`.tex` build-prereqs updated) and added real-bin CI gating to `.github/workflows/quickstart.yml` (install md-cli v0.9.2 `--features cli-compiler` / ms-cli v0.11.0 / mk-cli v0.10.2 + `cargo build --bin mnemonic` + a `make verify-examples` step mirroring `manual.yml`), **6 fences are now gated** (`qs-23-bundle.out` lines 2-, `qs-24-verify.out` lines 2-, `qs-26-recover-phrase.out` lines 2-, `qs-26-recover-mk1.out` whole, `qs-26-recover-md1.out` whole, `qs-41-watch-only-xpub.out` lines 2-; quickstart verify-examples = 62 transcripts pass = 56 shared-manual + 6 new). This entry records WHY the **4 remaining `text` output-class fences** in `docs/quickstart/src/` are NOT golden-gated.
- **Mechanism recap:** a gateable fence is one whose body is *deterministic CLI stdout/stderr* reproducible from the pinned absolute bins; it becomes `{.text include="<stem>.out"}` (optionally `lines="N-M"` to drop a leading argv-secret warning the prose intentionally omits), captured by `docs/quickstart/tests/verify-examples.sh` (a symlink to the manual runner; P2 added a `readlink -f "$TRANSCRIPTS"` so GNU `find` descends the symlinked start dir) and rendered fail-closed by the include-transcript filter (missing `.out` / unset `TRANSCRIPTS_DIR` / out-of-range `lines=` each FATAL exit 1 — verified). A block is non-gateable when its body is *not* such output.
- **The 4 residual `text` fences (all in `src/`):**
  - **(a) illustrative descriptor schema** — `src/10-foundations/12-bitcoin-in-30-seconds.md:36` `wpkh([fingerprint/84h/0h/0h]xpub6Cat.../<0;1>/*)`. A hand-authored descriptor *shape* with `fingerprint`/`xpub6Cat...` placeholders + `...` ellipsis; not CLI output. **GENUINELY NON-GATEABLE** (no canonical stdout to diff).
  - **(b) illustrative `--threshold` error** — `src/50-next-steps/52-troubleshooting.md:9` `error: a value is required for '--threshold <THRESHOLD>'`. **DRIFT:** this is the *truncated* clap message from the contrived `mnemonic bundle --threshold` (bare-flag, no value) path; the real message is `error: a value is required for '--threshold <THRESHOLD>' but none was supplied` + a blank line + `For more information, try '--help'.` (exit 64). The chapter's *heading* ("I forgot `--threshold`") actually describes the *omitted-flag* path, whose real output is the DIFFERENT `error: --threshold required for multisig template 'wsh-sortedmulti'` (exit 2). Kept as a hand-authored illustrative error inline-style fence (the reference manual treats the identical strings as table cells, not transcripts — `docs/manual/src/60-appendices/67-troubleshooting.md:10-11`). **NON-GATEABLE as written** (the body is neither path's verbatim output).
  - **(c) fictional ms1 decode-position error** — `src/50-next-steps/52-troubleshooting.md:23` `ms1_decode: error at position 27`. **DRIFT / does-not-exist:** a corrupt ms1 through `verify-bundle` yields `ms1_decode: fail Error("invalid short checksum (input withheld)")` (the codec WITHHOLDS input + emits NO character position — by design, to avoid leaking secret-bearing card content). The surrounding prose about a "position N" BCH locator for ms1 is aspirational. **GENUINELY NON-GATEABLE** (the documented behavior does not exist). Tracked as the existing slug `ms1-bch-per-character-diagnostic-not-emitted` (the manual leg of the same fiction was corrected in P0c).
  - **(d) external-tool (Bitcoin Core) error** — `src/50-next-steps/52-troubleshooting.md:45` `error: Descriptor not in canonical form`. This is *Bitcoin Core's* import-rejection message, not `mnemonic`/`md`/`ms`/`mk` stdout. **GENUINELY NON-GATEABLE** (would require a bitcoind harness; out of scope for the bin-only fidelity gate).
- **Prose-drift the 6 conversions surfaced (`.out` is canonical — prose now renders FROM the golden):** the hand-pasted `mk decode` fence in `src/20-singlesig/26-recover.md` (step 2) had THREE stale values vs the real `mk decode` output — `origin_path: m/84'/0'/0'` (real: `84'/0'/0'`, no `m/` prefix), `policy_id_stubs: deadbeef` (real: `1c0170fe` — `deadbeef` was a never-real placeholder), `chunks: 2 (regular)` (real: `2 (long)`) — plus a missing trailing `note: stdout is watch-only — public keys only, cannot spend`. The `md decode` fence (step 3) was missing the trailing `note: stdout is a keyless descriptor template (no keys)`. The `convert --to xpub` fence in `41-singlesig-watch-only.md` was missing the trailing watch-only `note:`. All now sourced from the captured goldens; no surrounding prose referenced the specific drifted *values* (`deadbeef`/`regular` appear nowhere else), so no prose edits beyond the fence bodies were needed.
- **Disposition:** **genuinely non-gateable = (a), (c), (d); (b) is a hand-authored illustrative error that does not match either real path** (a future phase could either gate the omitted-flag path against `error: --threshold required for…` or relabel it illustrative). No action required to ship Cycle-C P2; this is the audit-trail record.
- **Severity:** INFO (documentation completeness / future-work catalog; no correctness or funds-safety exposure — the drift was display-only fixture metadata, never an engraved or funds-bearing value).
- **Status:** `open` (catalog only). **Tier:** `docs` / `next-cycle`. **Companion:** the manual + technical-manual sibling catalogs (`manual-output-blocks-non-gateable-residual`, `technical-manual-output-blocks-non-gateable-residual`) + `ms1-bch-per-character-diagnostic-not-emitted` (the fiction behind residual (c)) + the doc-fidelity mechanism (`design/agent-reports/` Cycle-C P0a).

### `ms1-bch-per-character-diagnostic-not-emitted` — `convert --from ms1` rejects a corrupt card without naming the offending character position (the manual claimed a per-position diagnostic that does not exist)

- **Surfaced:** 2026-06-23, Cycle-C P0c. `docs/manual/src/30-workflows/35-recovery-paths.md` (pre-P0c §"Multisig wallet — partial card damage") claimed `mnemonic convert --from ms1=…` emits `position 11: invalid character 'Q' (expected 'q')` — a per-character BCH diagnostic. **The binary emits no such thing.** Verified against `/scratch/code/shibboleth/.docbins/current/bin/mnemonic` (P0a-pinned bins): the cited 49-char string is rejected on LENGTH first (`error: ms1 string length 49 not in v0.1 set [50, 56, 62, 69, 75]`, exit 1), never reaching any checksum diagnostic. Even a *valid-length* (50-char) corrupt card yields only a coarse error — an out-of-charset/wrong-case symbol → `ms1 codex32: InvalidCase(Lower, 'Q')`; an in-charset substitution → `ms1 codex32: invalid short checksum (50 chars; input withheld)`. No "position N: invalid character" output at any path. **P0c corrected the prose** to show the true length-rejection (now a gated golden, `35-recovery-paths-ms1-length-reject.out`) and to describe the real coarse checksum errors; this slug tracks the *aspirational feature* the old prose implied.
- **What (the gap / aspirational feature):** the codex32/BCH layer *can* in principle identify the error positions it corrects (the `repair` surfaces already report corrected positions for `mk`/`md`), but `convert --from ms1` on a checksum-failing card does not surface a candidate offending-position set to guide manual re-reading of an engraved plate. A per-position diagnostic ("the codec narrows the candidate set to 1-2 characters at position N") would be a genuine recovery-UX improvement.
- **Fix (direction, if pursued):** decide whether the ms1 decode/repair path can expose the BCH error-locator output as a position list (mirror the `mk repair` / `md repair` corrected-position reporting), and surface it on `convert --from ms1` decode-failure (or route users to a future `ms repair` / `mnemonic repair --ms1` that already reports positions). Likely a sibling-codec (ms-codec) change → cross-repo companion. **Do NOT** re-introduce the faked output into the manual until a real diagnostic ships.
- **Severity:** LOW (recovery-UX enhancement; the card is still correctly REJECTED — no funds-safety or wrong-decode exposure, the failure is fail-closed). The doc-bug it replaced was the actual defect and is now fixed.
- **Status:** `open` (feature/UX; doc-bug leg RESOLVED in Cycle-C P0c). **Tier:** `recovery-ux` / `next-cycle`. **Companion:** ms-codec (the BCH error-locator surface lives there; mirror an entry if pursued) + `mk repair` / `md repair` corrected-position reporting (the existing precedent for per-position diagnostics).

### `self-check-ms1-decode-not-zeroizing` — `self_check_bundle` + `inspect` drop a decoded ms1 `Payload` (master-seed entropy) un-scrubbed

- **Surfaced:** 2026-06-20, the SeedHammer fork **Rust-primary cross-check** (new project rule: when a defect is found+fixed in a downstream Go port, ALWAYS check whether the SAME defect exists in the primary Rust implementation, and if so fix Rust first). The fork's Go own-code bug-hunt finding **L1** (scrub the ms1 decode-probe) was cross-checked against this toolkit → the same defect class is present here (the other 7 of 8 findings were Go-only). **[handed off from another Claude instance working the SeedHammer fork — flagged here for pickup, NOT fixed there.]** **Broadened 2026-06-21** by the secret-keymat sweep (`design/agent-reports/sweep-keymat-toolkit.md` finding 4) with a SECOND site of the identical class — `cmd/inspect.rs` — folded in here per the sweep's own recommendation (one shared fix idiom).
- **Defect (LOW):** an `ms_codec::Payload` carrying master-seed entropy is decoded and used ONLY for inspection, then dropped **un-scrubbed**, at TWO sites:
  - **Site A — `self_check_bundle`** (`crates/mnemonic-toolkit/src/cmd/bundle.rs:2529`) decodes ms1 via `ms_codec::decode(ms)` into a bare `Payload`, uses it ONLY as an equality oracle (`:2533` `payload.as_bytes() != expected_bytes`), then drops the husk un-scrubbed.
  - **Site B — `inspect`** (`crates/mnemonic-toolkit/src/cmd/inspect.rs:160-162` `InspectPayload::Ms1 { tag, payload: ms_codec::Payload }`, built at `:171-172` via `ms_codec::decode(chunks[0])`) holds the bare `Payload` for the whole inspect-handler scope and drops it un-scrubbed. The `entropy_hex` print IS `--reveal-secret`-gated (`:206`), so this is NOT an output leak — only the in-memory husk. Sweep finding 4 (NEW SITE, KNOWN class — folded here, not filed separately).

  `ms_codec::Payload` is `#[non_exhaustive]` and intentionally NOT zeroize-wrapped — its own documented caller-wrap contract (`ms-codec payload.rs:17-44`; widening the public type is a deferred breaking change, OOS-2) puts the scrub on the caller. The **other 4** toolkit ms1-decode sites correctly move bytes into `Zeroizing` (`slot_ms1.rs:50,74`; `silent_payment.rs:150-151`; `overlay.rs:133,148`; `bundle.rs:2029,2038`); these two sites are the misses.
- **Fix:** mirror the in-file sibling idiom (`bundle.rs:2028-2039`) at BOTH sites — destructure/move the entropy OUT of `Payload` into `Zeroizing<Vec<u8>>` (`Entr(bytes)`→`Zeroizing::new(bytes)`, `Mnem{entropy,..}`→`Zeroizing::new(entropy)`, non-exhaustive `_`→`Zeroizing::new(p.as_bytes().to_vec())`) and inspect/compare that. Moving (not copying) avoids leaving the husk's `Vec` un-scrubbed. Existing tests `bundle.rs:2687/2694` guard Site A; add a row to the existing `lint_zeroize_discipline` test for both. **NO-BUMP candidate** (internal secret-hygiene; no wire/API change).
- **Cross-ref:** verbatim cross-check report `mnemonic-engrave/design/agent-reports/seedhammer-own-code-rust-crosscheck-scrub.md`; the Rust-primary rule (`mnemonic-engrave/CLAUDE.md` Conventions, 2026-06-20); convergence with the already-shipped fork Go L1 fix (`seedhammer` merge `39cb5cf`); sweep report `design/agent-reports/sweep-keymat-toolkit.md` (finding 4). **Tier:** toolkit-local (Rust). Once both sites are fixed, the constellation-wide convergence for fork-L1 is closed.
- **Status:** ✓ **RESOLVED (toolkit v0.71.0, wave2 T2 — 2026-06-22).** BOTH sites move the decoded entropy out of the bare `Payload` into a fn-local `Zeroizing<Vec<u8>>`: Site A `self_check_bundle` (`zeroize::Zeroizing::new` in the equality-oracle path); Site B `inspect` reshapes `InspectPayload::Ms1` to carry `Zeroizing<Vec<u8>>` + `PayloadKind` + `Option<u8>` (the husk drops at decode time). `#[non_exhaustive]` `_` arm covered. Output byte-identical (inspect text/JSON, `--reveal-secret`-gated entropy_hex unchanged). Lint rows added for both; `src/cmd/inspect.rs` joins the secret-bearing partition (floor 37→39). NO-BUMP leg folded into the v0.71.0 MINOR. Fork-L1 constellation convergence closed.

### `bundle-md1-template-only-option` — `bundle` flag to emit a TEMPLATE-ONLY md1 (no keys) instead of the full wallet policy, to cut engraving plates

- **Surfaced:** 2026-06-19, during the SeedHammer flagship (on-device "seed → ms1+mk1+md1 → engrave") design. The user wants an option for `bundle` to emit the md1 as a **wallet-policy TEMPLATE** (script + origin + use-site, `pubkeys:null`) instead of the full wallet policy (keys embedded). **Motivation: fewer costly engraving plates** — a single-sig template ≈ 1 chunk/1 plate vs the full policy's ~2-3 chunks (md1 single-string cap = `SINGLE_STRING_PAYLOAD_BIT_LIMIT` 320 bits; a 65-byte xpub + 4-byte fp TLV pushes a single-sig policy to ~81 bytes → multi-chunk). The watch-only wallet stays recomposable from the template + the mk1 key card (public) or template + ms1 (secret) + `--template`.
- **The constraint that makes this a real cycle, not a flag:** today `bundle` emits ONLY full-policy md1 and the cards BIND via a **key-dependent** id — `bundle.rs::build_unified_card` engraves md1's binding as `compute_wallet_policy_id(d)[0..2]` and the per-slot mk1/ms1 chunk-set-ids via `derive_mk1_chunk_set_id_for_slot(policy_id[0..4], slot)`, all derived from `WalletPolicyId`, which is **key-presence-significant** (`md-codec identity.rs` — nulling pubkeys+fp changes the id; test `walletpolicyid_template_only_differs_from_full_cell_7`). So a template md1 and the same wallet's full-policy md1 hash to DIFFERENT ids → a template card would NOT bind to its mk1/ms1 under the current formula. Making template-engraving honest requires moving the binding to a **template-stable** id (e.g. `WalletDescriptorTemplateId`, `identity.rs:71-104`) OR carrying both — a coordinated change across **md-codec + mk-codec + toolkit + SeedHammer**, plus `restore`/`verify-bundle` learning to recompose a template + key/seed (single-sig `restore` already derives from seed+`--template`; multisig `restore` currently REQUIRES a full-policy md1, `cmd/restore.rs:1232-1238`).
- **Scope sketch:** `bundle --md1-form=policy|template` (default `policy` = today's behavior); when `template`, emit the keyless md1 + bind cards by the template-stable id + ensure `restore --md1 <template> --from <key|seed>` and `verify-bundle` recompose. Multisig template needs all cosigner origins but no keys.
- **Status:** **toolkit + codec layers COMPLETE** (phase 1 single-sig v0.59.0; phase 2 multisig / general policies + `tr(NUMS,multi_a)` v0.60.0 — emit + `restore`-completion + `verify-bundle`-completion + the parallel permutation-search engine, all R0-GREEN; see `design/agent-reports/template-multisig-*`). md-codec / mk-codec needed **no change** — the keyless multisig wire form + `WalletDescriptorTemplateId` + the form-aware mk-cli stub all pre-existed. **The SeedHammer device adoption is the only remaining leg** — SeedHammer's T6 flagship is still **full-policy-only** (user-confirmed); SH revisits on-device template engraving only after the constellation adopts this. NOT fully resolved until the SH leg lands. Cross-ref: SeedHammer design `seedhammer-T6-recon-bundle-composition-stub.md` + `mnemonic-engrave/design/FOLLOWUPS.md` `constellation-template-only-engraving`. **Tier:** `cross-repo` (constellation-level: md-codec id-choice + mk-codec stub formula + toolkit bundle/restore/verify + SH device).

### `template-multisig-own-account-range-subset-search` — `restore --own-account-max K` (own-account RANGE) needs a subset-aware permutation engine

- **Surfaced:** 2026-06-20, #28 phase 2 (multisig-template RESTORE completion) P3a R0 EXECUTION review (`design/agent-reports/template-multisig-p3a-completion-exec-review.md`, IMPORTANT I-1). Funds-recovery class (a documented recovery feature was non-functional).
- **Where:** `crates/mnemonic-toolkit/src/cmd/restore.rs::run_multisig_template_completion` (the `--own-account-max` gate + the every-slot `pool.len() == n` gate) and the engine `crates/mnemonic-toolkit/src/permutation_search.rs::search` / `unrank_permutation`.
- **What:** the `--own-account-max K` feature (D10, SPEC §2/§6.1) over-supplies the own candidate pool (derive the own seed at every account in `0..K` → more own candidates than slots, `pool.len() > n`). But the P1 permutation engine enumerates only `n!` FULL permutations of the FIRST `n` pool entries (`unrank_permutation(rank, n)` builds `elems = (0..n)`); any pool index `≥ n` is NEVER evaluated, so the over-supplied own accounts AND the cosigners beyond pool index `n-1` are unreachable → a legitimate wallet silently returns NO MATCH. Concurrently the realized-`S` the prefix/cap were sized to (`P(pool,n)`) was larger than the space actually scanned (`n!`) — a SPEC §6.2/§7-floor-5 sizing-fidelity break. **P3a disposition (the gentle, safe gate):** `--own-account-max` now REFUSES at input with an actionable message (point at `--account <N[,N,…]>`); the supported invariant is `pool.len() == n` (own keys from the `--account` LIST + cosigners exactly fill the N slots), and `realized_s = n!` is restored. Tests: `own_account_max_flag_refuses_with_actionable_message`, `pool_larger_than_slots_refuses_with_actionable_message` (`tests/cli_restore_md1_template_multisig.rs`).
- **Fix (deferred):** build the genuine subset-search — a subset-aware engine that enumerates the `P(pool, n)` injective k-permutations (own-subset selection across an account range + which-slots-are-mine), AND a design for the **unknown own-slot-count** (how many of the N slots are the operator's own seed at unknown accounts). Re-enable `--own-account-max` to route through it; size `realized_s = P(pool, n)` to the truly-enumerated space; ADD a working `--own-account-max` completion vector. The engine already carries the `P(pool,n)`/`perm_count` math (`permutation_search.rs` `perm_count` test + `prefix_ladder_own_account_max_subset_space`) — only the enumeration generator (full-permutation → k-permutation) + the unknown-own-slot-count UX are missing.
- **Design (2026-06-20) — COMPLETE + R0-GREEN; supersedes the "Fix (deferred)" framing above.** Burned down as a selected post-#28-p2 follow-up: brainstorm + SPEC + plan-doc all R0-GREEN (0C/0I). Branch `feature/own-account-subset-search` (off v0.60.0, unmerged). Artifacts: `design/{BRAINSTORM,SPEC,IMPLEMENTATION_PLAN}_own_account_subset_search_2026-06-20.md` + `design/agent-reports/own-account-subset-search-{brainstorm,spec,plan}-r0-*-review.md`. **Decided model:** own-only subset-search by DEFAULT (re-enable `--own-account-max` / `--account` list; cosigners EXACT; own-slot-count `j = N−M` determined) + opt-in BOUNDED unowned search via a NEW `--search-cosigner-subset` flag. **The "deferred Fix" sizing above (`realized_s = P(pool,n)`) was REFINED by the design:** own-only enumerates the tighter own-anchored `S_own = C(K_own, j)·N!` (j = N−M; collapses to N! at no over-supply), NOT the looser `P(pool,N)` — so the pinned `prefix_ladder_own_account_max_subset_space` test must update to `S_own` at ship. R0-verified bijective; address-search early-exit gated to the over-supply path (exact path byte-identical).
- **Status:** ✓ **RESOLVED (toolkit v0.70.0, 2026-06-22).** The own-anchored k-permutation subset-search SHIPPED on `restore` + `verify-bundle`: `--own-account-max K` (re-enabled — over-supplies own accounts `0..K-1` and resolves the unique multisig-template assignment) own-only by DEFAULT, plus the opt-in `--search-cosigner-subset` for over-supplied cosigner cards (bounded: `K_own ≤ 256`, `S_opt ≤ 1e15` hard ceiling + adaptive time-cap). Realized space `S_own = C(K_own,j)·N!` (own-only; collapses to N! at no over-supply) sizes the strong-prefix requirement; the own pool is derived public-only and refusals exit ≠ 0. md/mk NO-BUMP. P0–P5 all R0-GREEN — see `design/agent-reports/own-account-subset-search-*` + `design/{BRAINSTORM,SPEC,IMPLEMENTATION_PLAN}_own_account_subset_search_2026-06-20.md`. (Shipped at 0.70.0, not the originally-projected 0.61.0 — the cycle landed after intervening releases; flipped in the shipping commit.)
- **Tier:** `feature` / `next-cycle` (deferred from #28 phase 2 P3a; cross-cite the P3a R0 review above).
- **Companion:** none (toolkit-local).

### `fuzz-build-broken-unrestorable-advisory-references-bin-only-cmd` — ✓ RESOLVED (2026-06-20, NO-BUMP) — `cargo fuzz build` fixed (was E0433): the `cfg(fuzzing)` lib mount of `unrestorable_advisory` referenced the bin-only `cmd::restore`

- **Status:** ✓ **RESOLVED 2026-06-20 (NO-BUMP, post-v0.60.0 burn-down).** Fixed via option **(a)**: the two predicates moved VERBATIM to a new `md_codec`-only leaf module `crates/mnemonic-toolkit/src/taproot_override_classify.rs` (mounted bin-private in `main.rs` + under `#[cfg(fuzzing)]` in `lib.rs`); `cmd::restore` re-exports them (`pub(crate) use crate::taproot_override_classify::{…}`) so every caller + the P2.1 truth-table tests + the classify-reroute resolve the bare names unchanged; `unrestorable_advisory.rs:116-117` now references `crate::taproot_override_classify::…` (resolves in the lib under `cfg(fuzzing)`). **Pure relocation, byte-identical predicate logic — no behavior change.** Gates: `cargo +nightly fuzz build` GREEN (was E0433); `RUSTFLAGS="--cfg fuzzing" cargo build --lib` GREEN; full `cargo test -p` GREEN incl. the truth-table parity tests; clippy `-D warnings` clean. Single-source parity (advisory fires IFF restore refuses) preserved by the re-export. Audit: `design/agent-reports/fuzz-build-fix-*`.
...
- **Where:** the binding originates in mk-codec (`mnemonic-key crates/mk-codec/src/key_card.rs:24-58` mandatory `policy_id_stubs`; `bytecode/encode.rs:24` empty → `InvalidPolicyIdStubCount`). Toolkit-side surface: `bundle`/`restore`/`verify-bundle` all assume an mk1 already bound to a policy/template; there is no toolkit door to emit or ingest an unbound public-key card.
- **What:** An mk1 always binds to ≥1 md1 policy/template, so the constellation has no policy-independent public-key (xpub) backup; the only pre-wallet backup is the SEED (`ms1`/words/SeedQR, secret), not a shareable/pre-engravable public card. The key-first ("generate + back up before use") workflow can't pre-mint an mk1 — back up the seed, or wait for an agreed template shape (bind to the key-independent `WalletDescriptorTemplateId`, which records shape but not the `@N` assignment). See the mk-codec entry for the full statement.
- **Decision needed:** (a) support an unbound/template-agnostic key card + define its later policy/template binding and `verify-bundle`/`self_check_bundle` handling (a card with no binding breaks the bundle integrity link), or (b) seed-only canonical (wont-fix). Re-evaluate with the template-engraving work (`bundle-md1-template-only-option` + its multisig phase 2).
- **Analysis (2026-06-20) — leaning (a); full version in the canonical mk-codec entry:** an unbound mk1 = xpub + optional origin, no stub, **no coherence claim**. Key facts: a standard k-of-n is watch-only-recoverable from **{xpubs, k, script-type, use-site} alone** — `sortedmulti` is order-invariant (BIP-67, sorts derived pubkeys) and origins don't affect address derivation — so the toolkit recovery path is **enumerate-script-type + match-address / `--expect-wallet-id`** (no permutation search except for unsorted `multi` / distinct-origin/override slots). The BIP-48 origin pins the script type, so origin is the field worth keeping. Toolkit impact when picked up: a `verify-bundle`/`self_check_bundle` "unbound card" disposition + an enumerate-and-confirm recovery entry; primary value is durably storing the *other* cosigners' xpubs (own key is covered by the seed). Does NOT enable otherwise-impossible recovery — it's a durable medium for the inputs. Still `open`, no code.
- **Why deferred:** open design question; held pending the m* multisig-template upgrade.
- **Status:** open. **Tier:** `cross-repo`.
- **Companion (LOCKSTEP — settling one MUST settle the other):** `mnemonic-key/design/FOLLOWUPS.md` `mstar-prepolicy-key-backup` (canonical). BOUND PAIR — neither entry may be resolved/closed without the same change updating the other. Related: `bundle-md1-template-only-option` (this file) + `mnemonic-engrave/design/FOLLOWUPS.md` `seedhammer-template-engrave-key-search-time-estimate` + `constellation-template-only-engraving`.

### `import-wallet-format-descriptor` — ✓ RESOLVED (v0.58.0, 2026-06-17) — generic commented-descriptor import door

- **Surfaced:** 2026-06-16, the remaining-open-items recon (Tier-2 C5), re-scoped by the user from the original "import `--format green`" framing to a GENERIC commented-descriptor format. Precondition check: `import-wallet --format bitcoin-core` expects JSON, so it could NOT read the toolkit's own `--format green`/`--format descriptor` text export — there was no `import-wallet` door for a bare/commented descriptor text file (the gap from `cycle-prep-recon-theme-a-wallet-interop.md`: "here is a raw `wsh(sortedmulti(...))`, make cards" had no door). NOT doc-only.
...
- **Status (cycle-11b L24):** the standalone gate landed in `verify_bundle.rs` (v0.65.1 @03017917) — a deliberate duplicate of `bundle.rs`'s, carrying an in-code S-VERIFY fold comment citing THIS slug.
- **Resolution (Wave-4 L1, toolkit NO-BUMP):** folded the exact-coverage gate + H12 default-script-type inference + `new_paths` default-path build + per-slot `--slot @N.path=` override loop + F4 `Shared`-collapse into ONE `pub(crate) fn bind_descriptor_mode_paths(mode, slots, path_decl, n, is_non_canonical, root_tag, network, account)` in `cmd/bundle.rs` (near `compute_default_origin_path`), consumed by BOTH `bundle_run_unified_descriptor` (`DescriptorBindMode::Emit`) and `descriptor_mode_verify_run` (`DescriptorBindMode::Verify`). The emit-only §6.6 row-19 path-mismatch refusal is mode-gated to `Emit`; the emit-only §4.12.g `--account` refusal, §6.6-row-4 `[Phrase,Path]` refusal, and the stderr default-inference notice stay at the bundle CALL SITE. `verify-bundle` discards the returned `defaulted_indices` (path_decl.paths byte-preserved). The deleted S-VERIFY fold-comment is gone. Guard-drift (the L24 class) is now structurally impossible — one gate, both callers. Regression oracle: the bundle↔verify path-derivation **parity matrix** in `tests/cli_non_canonical_descriptor.rs` (`parity_harness_self_test_wsh_n2_all_elided` + `parity_wsh_all_shapes_round_trip` / `parity_sh_wsh_round_trips` / `parity_tr_nums_round_trips`) plus D1/D2/D3 characterization cells; the 2 cycle-11b L24 cells stay GREEN. NO-BUMP (pure internal hygiene; sole observable delta = error-precedence reshuffle on doubly-malformed emit input, every path still exit-≠0).
- **Tier:** RESOLVED (Wave-4 L1).
- **Companion:** none (toolkit-local).
...
- **Status:** ✓ RESOLVED (2026-06-23, Wave-3-deferrals L1, NO-BUMP, no tag) — all 4 sibling legs done. Prior Wave-3 (NO-BUMP) de-staled ms-cli `v0.7.0→v0.11.0`, mk-cli `v0.8.0→v0.10.1`, mnemonic-gui `v0.40.0→v0.48.1` in `scripts/install.sh` + lockstep workflow `--tag` pins (`manual.yml` mk/ms, `quickstart.yml` mk) + the `44-mk-cli.md:12` prose pin; g6 confirmed byte-equal at ms-cli v0.11.0 (no mlock re-baseline). **The md-cli leg is now RESOLVED (2026-06-23, NO-BUMP):** bumped `descriptor-mnemonic-md-cli-v0.7.1 → v0.9.2` at all 3 md sites (install.sh:35, manual.yml:86, cross-tool-differential.yml:50) in one atomic commit. The deferred concern (the bump turns `cross-tool-differential` RED) was REAL and is reconciled in the SAME commit: md-codec 0.38.0's 80-data-symbol single-string-encode cap (shipped md-cli v0.8.1, commit f9c1e57) required adding `--force-chunked` to `cli_cross_tool_differential.rs::md_cli_ids()`. The `#[ignore]`-gated differential re-ran GREEN at v0.9.2 (all 17 corpus entries Match; walker ids byte-identical — e.g. `wpkh` `1c0170fe…`/`45775d4d…`, `wsh-pk` `58d18033…`/`9208f590…`). Manual flag-coverage stays GREEN (md help surface unchanged v0.7.1↔v0.9.2). With ms/mk/gui already de-staled (Wave-3) and md-cli now bumped, **all 4 sibling legs of this slug are RESOLVED.** **Tier:** `cross-repo`.
...
- **Where (toolkit P4):** `src/display_grouping.rs` (`render_grouped`/`strip_display_separators`/`is_display_separator`/`parse_separator` — added unconditionally to lib.rs P0 + main.rs P4, dual-homed); `cmd/bundle.rs` (print-once + `--group-size`/`--separator`); `cmd/convert.rs` + `cmd/ms_shares.rs` (node-gated grouping + flags); intake strip at `slot_ms1.rs`, `cmd/verify_bundle.rs`, `repair.rs`, `cmd/convert.rs --from`. `format.rs::chunk_5char`/`chunk_mk1`/`chunk_md1` DELETED. Goldens `tests/vectors/v0_1`+`v0_2` regenerated unbroken. Both manuals updated; `docs/Examples.pdf` carries a new degrade2 bundle→restore round-trip example.
- **What (SHIPPED across the cycle):** uniform `--group-size <u16>` (default 5, `0`=unbroken) + `--separator <space|hyphen|comma>` on every emit subcommand; default **space/5 print-once**; `--json` + `verify-bundle` forensics + `repair` output stay **unbroken**; separator-stripping intake everywhere (strips whitespace + `-` + `,`). Drift control = copy-with-checksum conformance vectors (canonical `design/display-grouping-vectors.tsv` authored here; byte-identical copies + `.sha256` in each sibling; CI `sha256sum -c` + a driver test). **SemVer MINOR per crate.**
- **Released:** **P1** md-codec 0.36.0 + md-cli 0.7.0; **P2** ms-cli 0.8.0 (ms-codec untouched); **P3** mk-cli 0.9.0 (mk-codec untouched) — all on crates.io, CI green. **P4** = this toolkit MINOR (in progress). **P5** = `mnemonic-gui` `schema_mirror` (flags + separator keyword dropdown) — paired PR, PENDING.
- **Status:** ✓ RESOLVED (reconciled 2026-06-22) — ALL phases shipped: P1 md-codec 0.36.0 / md-cli 0.7.0, P2 ms-cli 0.8.0, P3 mk-cli 0.9.0, **P4 toolkit v0.56.0**, **P5 mnemonic-gui v0.41.0** (`schema_mirror` flags + `SEPARATORS` keyword dropdown — GUI CHANGELOG "final phase, P5"). Ground-truth verified at reconcile: all 4 CLIs carry `--group-size`/`--separator` (mnemonic bundle/convert, md encode, ms encode/split, mk encode), the GUI schema carries them, and `display-grouping-vectors.tsv`(+`.sha256`) are present in all repos. The "P4 shipping; P5 open" status had lagged since the cycle shipped (status never flipped in the shipping commits). Sibling companions (descriptor-mnemonic / mnemonic-secret / mnemonic-key) flipped in lockstep.
- **Tier:** `cross-repo`.

### `toolkit-check-pkk-non-tap-non-canonical` — ✓ RESOLVED (v0.55.0, 2026-06-12) — toolkit keeps `Check(PkK/PkH)` in wsh/sh → non-conformant md1 vs md-cli for `wsh(pk)` shapes

- **Resolution (v0.55.0, MINOR wire-content change):** the toolkit walker now collapses `Terminal::Check(PkK|PkH) → bare Tag::PkK|PkH` UNCONDITIONALLY (the `if tap_context` gate at `parse_descriptor.rs:602` dropped; the now-dead `tap_context` param removed from `walk_miniscript_node`/`walk_one_child`/`walk_two_children` + all call sites). The deviant unit test `walk_check_kept_in_non_tap_context` was renamed to `walk_check_collapsed_in_non_tap` and inverted; the 3 other AST tests (`walk_wsh_pk_root`/`walk_sh_ms_pk_root`/`walk_pk_h_via_wsh_andor`) flip to assert the bare leaf. The Cycle-D differential's 4 `wsh-pk`/`wsh-pkh`/`wsh-and_v`/`wsh-or_d` entries flipped `Diverge → Match` (vacuity guards restructured to a verdict-agnostic check: ≥1 Match + `n_bothError==0 && n_toolError==0`; now a cross-tool re-divergence regression gate). NEW always-on in-suite golden `tests/cli_check_pkk_canonical_golden.rs` pins the post-fix `wallet_policy_id`/`template_id` for the 4 flagship shapes (decoded IN-CRATE via `md_codec::chunk::reassemble → compute_wallet_policy_id`, no MD_BIN) + a `bundle → restore --md1` round-trip on `wsh(pk)`/`wsh(pkh)`. `restore` reads BOTH the old `Check(PkK)` and new bare-`PkK` md1 (md-codec 0.35.1 idempotence arm) — no card-reading regression. Differential confirmed locally (8/8 Match). SPEC `design/BRAINSTORM_check_pkk_non_tap_canonical_fix.md` + `design/agent-reports/check-pkk-fix-r0-round{1,2}-review.md` (GREEN 0C/0I).
- **Surfaced:** 2026-06-12, stress Cycle D — the cross-tool md1 DIFFERENTIAL harness (`tests/cli_cross_tool_differential.rs` + `.github/workflows/cross-tool-differential.yml`). Design + 2 R0 rounds (GREEN 0C/0I): `design/BRAINSTORM_stress_cycle_d_cross_tool_differential.md`; `design/agent-reports/cycle-d-differential-r0-round{1,2}-review.md`.
- **What:** the toolkit walker collapses `Terminal::Check(Terminal::PkK|PkH)` → bare `Tag::PkK`/`PkH` ONLY inside tap leaves; in `wsh`/`sh` (non-tap) it KEEPS `Tag::Check(Tag::PkK)` on the wire. md-cli collapses to bare UNCONDITIONALLY (per descriptor-mnemonic SPEC v0.30 §5.1: "Walker emits bare `Tag::PkK`/`Tag::PkH` … regardless of context; `Tag::Check` is never emitted wrapping a key leaf on the wire"). ⇒ for `wsh(pk)`-shaped descriptors the two tools emit DIFFERENT md1 wire bytes AND a DIFFERENT `wallet_policy_id` / `wallet_descriptor_template_id` — an interop hazard: a card engraved by `mnemonic` won't match one engraved by `md` for the SAME wallet, and a third party reconstructing may get a different policy-id. **Wire-canonicity, NOT funds-loss** — both md1 forms `md decode` to the SAME descriptor (md-codec 0.35.1's `to_miniscript` Check-idempotence arm renders `Check(PkK)` and bare `PkK` identically).
  - **Canonicity authority:** md-codec defines NO canonical wrapper for `c:pk_k` in wsh (`canonical_origin` treats `wsh(pk)` as non-canonical in EITHER form; encode + `compute_wallet_policy_id` faithfully hash whatever tree the walker emits, NOT normalizing `Check(PkK)`↔bare`PkK`). The ONLY authority is **descriptor-mnemonic SPEC v0.30 §5.1** (`SPEC_v0_30_wire_format.md:246`) = bare PkK. So **md-cli is conformant; the TOOLKIT is the deviant.** The toolkit's deviation is DELIBERATE + tested.
- **Proven example (Cycle-D corpus):** `wsh(pk(@0))` @ a fully-origin-matched depth-4 key (mfp `73c5da0a`, m/48'/0'/0'/2') → toolkit policy-id `9ad78e4f…` / template-id `ef980fcc…` vs md-cli `58d18033…` / `9208f590…`. `wsh(pkh)`, `wsh(and_v(v:pk,pk))`, `wsh(or_d(pk,pk))` likewise diverge. Controls (`wpkh`, `pkh`, `wsh(multi)`, tap `tr(…,pk-leaf)` — tap collapses Check too) MATCH.
- **Where (toolkit, the two deviant sites — line numbers vs `origin/master` @ 9d3da6c):**
  - `crates/mnemonic-toolkit/src/parse_descriptor.rs:601-602` — the `Terminal::Check(inner)` arm collapses to bare `Tag::PkK` only `if tap_context` (fn `walk_miniscript_node` takes `tap_context: bool` at :558; wsh/sh callers pass `tap=false`, tap-leaf callers pass `tap=true`).
  - `crates/mnemonic-toolkit/src/parse_descriptor.rs:2551` — the deliberate test `walk_check_kept_in_non_tap_context` PINS the current (non-conformant) behavior.
  - (md-cli conformant site: descriptor-mnemonic `crates/md-cli/src/parse/template.rs:607` — same `Terminal::Check` arm, NO `tap_context` gate.)
...
- **Resolution:** ROOT CAUSE was a shared-temp-file race, not PTY-specific: `tests/cli_auto_repair.rs::write_temp_json` named the temp file by `std::process::id()` ONLY, so all 5 callers (cell_27/28/29/30/…) — threads in ONE process — wrote the SAME path; under parallel execution a sibling read a mid-write/empty file → "EOF line 1 col 0". Fixed with a process-global `AtomicU64` counter in the filename (`mnemonic_v0_22_1_bundle_{pid}_{seq}.json`) → per-call-unique. SPEC `design/SPEC_doc_test_hygiene_cell27_and_anti_collision.md`.
- **Surfaced:** 2026-06-11, the Cycle-A push (aa3e46d) — `test (macos-latest)` red on `cell_27_verify_bundle_auto_fire_happy_path_tty`; PASSED on rerun (confirmed transient). The Cycle-A change (g6 CI + a synthesize test) is unrelated.
- **Where:** `tests/cli_auto_repair.rs::cell_27_verify_bundle_auto_fire_happy_path_tty` — "Unexpected return code, failed var == 5" + `--bundle-json <tmp>/mnemonic_v0_22_1_bundle_3335.json parse: EOF while parsing a value at line 1 column 0` (the temp bundle-json was EMPTY when read).
...
- **Surfaced:** 2026-06-11, stress Cycle A (`tests/prop_backup_restore_roundtrip.rs`, found on the FIRST proptest run). `build-descriptor`, `bundle --descriptor`, and `export-wallet --descriptor` all ACCEPT a descriptor with `sortedmulti` inside a combinator (e.g. `wsh(or_d(sortedmulti(2,A,B),and_v(v:pk(C),older(144))))` — rust-miniscript parses it, checksum `#qy7ka0ay`), but `restore --md1` REFUSES to reconstruct it: "`Tag::SortedMulti` must be the sole child of wsh/sh; cannot appear as a miniscript leaf." `sortedmulti` is a BIP-380 DESCRIPTOR-level wrapper (not a miniscript Terminal).
- **ROOT-CAUSE CORRECTION (2026-06-12, GAP-3 recon + R0, verified):** the earlier "the md1 wire only represents it as the sole wsh/sh child" was STRUCTURALLY WRONG — the wire is NOT the limit. md-codec's `Body::MultiKeys` encodes `Tag::SortedMulti` generically at ANY tree position, and md-codec's P7 (`self_test_bad_sortedmulti_under_combinator`, `descriptor-mnemonic .../proptest_to_miniscript.rs:663-676` @ a3abdc8) proves a nested-sortedmulti card wire-round-trips BYTE-EXACT. The refusal lives in the RENDERER (`to_miniscript.rs:419`): md-codec pins crates.io miniscript 13.0.0 (zero `Terminal::SortedMulti`), while the toolkit pins git rev `95fdd1c` (HAS it, `decode.rs:157`) — the two-miniscripts split (v0.49.1 precedent). So option (b) below ("extend md-codec to ENCODE") is mis-framed: encoding already works; what's missing is a md-codec RENDERER (or a toolkit string-parse fallback on the patched miniscript).
- **What:** a user can bundle + engrave a card the toolkit cannot mechanically restore. NOT silent funds-loss (restore refuses LOUDLY — pinned by `prop_backup_restore_roundtrip.rs::sortedmulti_in_combinator_bundles_but_restore_refuses_loudly`, the GAP-3 contract cell — and the engraved md1 is a faithful backup), but a backup→restore gap (cf. the pk-keyed flagship before PART 2).
- **Fix (design call):** either (a) a non-blocking bundle-time ADVISORY (the `bundle-unrestorable-shape-advisory` umbrella — recommended over creation-time REJECT, which would forbid provably-wire-faithful cards that rust-miniscript itself parses), or (b) faithful RECONSTRUCTION — a md-codec textual renderer for nested sortedmulti, or a toolkit string-parse fallback on the patched miniscript (the encoding already works), or wait for an upstream miniscript release with `Terminal::SortedMulti`. `sortedmulti` as the sole wsh/sh child is unaffected (reconstructs fine).
- **Tier:** deferred (loud-refuse, not funds-loss; GAP-3 cycle 2026-06-12 pinned the contract; advisory + faithful-reconstruction remain).
...
- **Status:** **PARTIALLY RESOLVED — 2026-06-19.** The **non-taproot per-cosigner use-site override** half SHIPPED: md-codec **0.37.0** (`to_miniscript_descriptor_multipath` + per-`@N` faithful reconstruction + `has_hardened_use_site`) → md-cli **0.7.1** (closes the live `md address` silent-wrong-address bug) → toolkit **0.58.2** (`restore --md1` reconstructs `wsh(multi/sortedmulti)`, `sh(wsh(multi))`, bare `sh(multi)`, `Some`/`None` mix faithfully; guard narrowed + advisory parity off one shared predicate). All R0-GREEN (4 reviews in `design/agent-reports/restore-use-site-override-*`), funds-safety gate verified vs an independent golden + bitcoind differential. The **hardened-wildcard `/*h`** half stays **WONTFIX** (a hardened child is underivable from an xpub for watch-only) — loud refuse via `has_hardened_use_site`. The **taproot** override half remains OPEN — see `restore-md1-taproot-use-site-override-arm` below (brainstorm+SPEC R0-GREEN; impl gated on the next rust-miniscript release for the `sortedmulti_a` leg).

### `restore-md1-taproot-use-site-override-arm` — taproot override-card restore stays GUARDED (the tr() reconstruction arm is use-site-override-blind)

- **Surfaced:** 2026-06-19, the per-key use-site override fix cycle (architect pre-spec confirmation, Point C). The **wsh/sh-wsh** leg of `restore-md1-per-key-use-site-and-hardened-wildcard` is being fixed this cycle (md-codec `to_miniscript_descriptor` consumes the already-resolved `ExpandedKey.use_site_path`; toolkit `ReconstructTranslator` made per-`@N` aware). The **taproot** leg is deferred and stays GUARDED because two reconstruction paths route AROUND the fix:
  - **Toolkit taproot Template arm** (`cmd/restore.rs::classify_taproot_restore` → `TrMultiA`/`TrSortedMultiA`, ~:728/:735): builds the descriptor via `build_descriptor_string(slots, k, …)` with the plain `<0;1>` template — never consults `d.tlv.use_site_path_overrides`, so it would silently render the baseline suffix on every cosigner (the same funds-safety class the wsh fix closes).
  - **md-codec `sortedmulti_a` root gap** (`descriptor-mnemonic/crates/md-codec/src/to_miniscript.rs:423-428`): `tr(NUMS, sortedmulti_a(…))` cannot render via `to_miniscript_descriptor` at all (pinned rust-miniscript has no `Terminal::SortedMultiA`), so the general/faithful path can't reach it either.
- **Where:** `mnemonic-toolkit/crates/mnemonic-toolkit/src/cmd/restore.rs` (taproot Template arm + the override guard that KEEPS refusing taproot override cards this cycle); `descriptor-mnemonic/crates/md-codec/src/to_miniscript.rs:423-428` (the `sortedmulti_a` render gap).
- **Fix:** teach the taproot reconstruction arm to honor per-`@N` use-site overrides — either (a) route taproot multisig override cards through a per-key-aware path instead of the plain-template `build_descriptor_string`, or (b) thread the resolved per-key `UseSitePath` into the Template emitters; paired with closing the `sortedmulti_a` derive gap so `tr(…, sortedmulti_a)` divergent cards render. The shared md-codec hardened-anywhere predicate (this cycle) is reusable for the taproot guard. NOTE: md-cli `address`/`derive_address` of taproot override cards IS fixed this cycle (it goes through `to_miniscript_descriptor` directly); only the TOOLKIT restore taproot arm is deferred.
- **Why deferred:** keeps the current cycle tight + funds-safe — taproot override cards stay LOUDLY GUARDED at restore (no silent infidelity) while the non-taproot wsh/sh-wsh + general-policy leg ships. Exotic shape; no regression (restore refuses today).
- **Status:** **PARTIALLY RESOLVED — 2026-06-20, toolkit v0.59.1 (#26).** The **`tr(NUMS, multi_a)`** leg SHIPPED: a single `restorable_taproot_override_card` predicate (`Tag::Tr` ∧ NUMS internal ∧ plain `multi_a` leaf ∧ ¬hardened) re-routes override-carrying cards from the override-blind `Template` string-builder to the faithful arm → md-codec 0.37.0's per-`@N` `to_miniscript_descriptor_multipath` (taproot-agnostic; NO md-codec bump). The SAME predicate drives the guard, the classify re-route, and the `TaprootUseSiteOverride` advisory (refuse ⟺ advisory — structurally-closed parity, R0-verified). Funds-safety gate: independent rust-miniscript golden (divergent ≠ baseline-collapse) + a bitcoind differential row (ran vs Core v27.0). Brainstorm+SPEC+plan+impl all R0-GREEN (reviews `design/agent-reports/taproot-override-*`). The **`tr(sortedmulti_a)`** leg remains OPEN — designed (SPEC §5/§8) but gated on the `taproot-coverage-cycle-on-miniscript-gt-13-1-0` umbrella (crates.io rust-miniscript > 13.1.0 for `Terminal::SortedMultiA`); interim loud-refuse + advise. Non-NUMS-internal + hardened taproot overrides also stay refused.
- **Tier:** `tr(multi_a)` DONE; `tr(sortedmulti_a)` rides the miniscript>13.1.0 umbrella.
- **Companion:** `descriptor-mnemonic/design/FOLLOWUPS.md::sortedmulti-a-derive-gap-fenced` (the md-codec `sortedmulti_a` render-gap leg — the remaining open piece).
...
- **Status:** the pk-keyed concern is **RESOLVED** 2026-06-12 — PART 2 shipped (`to-miniscript-check-pkh-double-wrap` closed via md-codec 0.35.1 + toolkit v0.54.1); `bundle`→`restore` of a `pk(@N)`/`pkh(@N)`-keyed policy now round-trips exit-0 with the correct descriptor (verified at `ca7d7bc`). The non-blocking bundle-time **advisory idea survives but re-targets the REMAINING unrestorable shapes** — sortedmulti-in-combinator (`bundle-accepts-sortedmulti-in-combinator-restore-cannot`), per-key use-site overrides + hardened wildcard (`restore-md1-per-key-use-site-and-hardened-wildcard`). RE-SCOPED into the `bundle-unrestorable-shape-advisory` umbrella — **STILL OPEN.** The GAP-3 cycle (2026-06-12) pinned the sortedmulti-in-combinator refusal CONTRACT only (`prop_backup_restore_roundtrip.rs::sortedmulti_in_combinator_bundles_but_restore_refuses_loudly` — proves it is loud-safe-refuse, not silent-corrupt). The advisory ITSELF — bundle-time shape-detection + a non-blocking warning across all 3 shapes (sortedmulti-in-combinator + use-site overrides + hardened wildcard) — is a deferred PATCH UX feature needing its own R0 (where to detect; the 3-shape set). This entry is the historical pk-keyed record + the advisory umbrella tracker.
- **Tier:** resolved (pk-keyed); `bundle-unrestorable-shape-advisory` = **RESOLVED 2026-06-16, toolkit v0.57.1 (Tier-2 C1).**
- **`bundle-unrestorable-shape-advisory` resolution:** new `src/unrestorable_advisory.rs` (mirrors `timelock_advisory`) emits a non-blocking stderr advisory at engrave time on the TWO md1-engraving surfaces — `bundle` (3 sites) AND `import-wallet` (`import_wallet.rs:1291`, the second engrave surface found by the C1 R0; export-wallet excluded — only holds `miniscript::Descriptor`, no md1) — for all three shapes: sortedmulti-in-combinator (a NEW tree-walk mirroring md-codec's three accepted sole-child positions `wsh(sortedmulti)`/`sh(wsh(sortedmulti))`/`sh(sortedmulti)`; R0 caught the bare-P2SH false-positive), `tlv.use_site_path_overrides`, `use_site_path.wildcard_hardened`. Fires IFF restore refuses (parity tests bundle→restore each shape). Card still emitted (faithful backup), exit 0 (GAP-3 `.success()` guard). PATCH, no schema_mirror/no ToolkitError, manual `### Unrestorable descriptor shapes` + import-wallet cross-ref. Plan + R0 GREEN×2: `design/PLAN_C1_unrestorable_shape_advisory_2026-06-16.md`, `design/agent-reports/c1-unrestorable-advisory-plan-r0-round{1,2}-review.md`. The two RECONSTRUCTION halves (`bundle-accepts-sortedmulti-in-combinator-restore-cannot`, `restore-md1-per-key-use-site-and-hardened-wildcard`) stay open — the advisory warns; it does not reconstruct.
...
- **Where:** `wallet_export/bip388.rs::emit` template branch → `format_bip388_wallet_policy` hardcodes `"name": template.human_name()` (`bip388.rs:133`); does NOT read `inputs.wallet_name`.
- **What:** for a TEMPLATE-based export (`export-wallet --template <T> --format bip388`), the emitted policy `name` is always `<template>.human_name()` and `--wallet-name` is silently ignored. (The descriptor-passthrough path was fixed in v0.53.8; this template path is a separate pre-existing gap.)
- **Fix:** thread `inputs.wallet_name` into `format_bip388_wallet_policy`'s `"name"` field (fallback to `template.human_name()` when default), mirroring the passthrough fix.
- **Tier:** deferred (low-risk; cosmetic label on a template-derived policy).
  - **[obs] ✓ RESOLVED (v0.53.1, 2026-06-10)** `flag-is-secret-completeness-unguarded-by-design` — folded into `vacuous-secret-flag-gate` below; the design acceptance was overturned by the §7b gate (residual: novel-vocabulary names outside the net).
  - **[FUNDS-SAFETY] ✓ RESOLVED (v0.53.9, 2026-06-11)** `gate-after-older-upper-bound-deferred-to-step2` — deep recon FALSIFIED the entry's "fail-closed" premise for `older()`. `after()` IS fail-closed (step-2 `from_str` rejects N>0x7FFFFFFF cleanly), but `older(N)` was NOT: BIP-68 consensus masks the CSV operand to `0x0040FFFF`, so step-1's `n==0 || n>=2^31` check let masked-garbage values build exit-0 engraving-ready descriptors with a silently weakened/zero timelock — `older(105120)`→~275 days, `older(65536)`→**spendable immediately**, `older(0x400000)`→zero-delay. The gate now rejects `(n & !0x0040FFFF) != 0 || (n & 0xFFFF) == 0` with a bit-31-aware node-localized diagnostic; `after(N)` gains a step-1 `>0x7FFFFFFF` field diag (behavior-equiv). Valid block (1..=65535) + 512s-unit (0x400001..=0x40FFFF) encodings unaffected; all presets/fixtures safe (one test fixture `older(0x400000)`→`older(0x400001)`). SPEC `design/SPEC_older_timelock_mask_gate.md`; R0 ×3 GREEN (`older-timelock-mask-gate-r0-round{1,2,3}-review.md`). Carved out: `archetype-older-blocks-flag-accepts-time-units` + `intake-surfaces-accept-masked-older-no-advisory` (below). (`crates/mnemonic-toolkit/src/descriptor_builder/gate.rs::validate_fields` Older/After arms)
  - **[obs] ✓ RESOLVED (2026-06-11, NO-BUMP)** `hand-frozen-lint-canons-no-completeness` — the 19-file zeroize audit ran (`design/agent-reports/zeroize-lint-19-file-audit.md`): **14 CANONICAL** files own real secrets → PROMOTED to 16 new `ZEROIZE_ROWS` (incl. `verify_bundle.rs` + `ms_shares.rs`, the R0 I3 concern — rows, NOT allowlisted), **5** crypto-internal/pass-through/primitive → explicit `NON_ROW_SECRET_FILES` allowlist. Added the source→declared scan `every_secret_bearing_src_file_is_declared_or_allowlisted` (enumerates every `src/**/*.rs` matching `SECRET_PATTERNS`, asserts each is a row-source OR allowlisted) + a persistent glob-cardinality floor (`SECRET_FILE_FLOOR = 35`, catches a vacuous-pass from a broken glob) + a stale-allowlist guard (`non_row_secret_allowlist_is_non_empty_and_each_entry_still_bears_a_secret`). Count-range bumped `18..=42`→`18..=60` (36+16=52). RED-proven via real-loop allowlist-entry removal (`secret_string.rs` → scan REDs naming the file → restored). SPEC `design/SPEC_zeroize_lint_completeness.md`; R0 ×2 GREEN (`zeroize-lint-completeness-r0-round{1,2}-review.md`). Source SHA `438de94`. Bonus gap filed below: `addresses-restore-passphrase-not-zeroizing`.
  - **[IMPORTANT/I10] ✓ RESOLVED (v0.53.0, 2026-06-10)** `mk1-chunk-set-id-fingerprint-grouping-assumes-distinct-fps` — promoted below; multisig csi is now slot-XOR (distinct per cosigner, immune to same-xpub fingerprint collision).
  - **[obs] ✓ RESOLVED (v0.53.0, 2026-06-10)** `n1-vs-nge2-csi-derivation-inconsistency` — promoted below; unified under the single `derive_mk1_chunk_set_id_for_slot` scheme + the display helper now matches.
  - **[obs] ✓ RESOLVED (v0.53.2, 2026-06-10)** `self-check-header-claims-unread-vector` — cli_self_check.rs module doc corrected to the bip84-only truth alongside the goldens deletion; The module doc states fixtures live at tests/vectors/v0_2/{bip84,wsh-sortedmulti}-mainnet-0-false-true.txt, implying both are exercised. The code only reads the bip84 one. wsh-sortedmulti-mainnet-0-fa (`crates/mnemonic-toolkit/tests/cli_self_check.rs:5 vs :13`)
  - **[obs] ✓ RESOLVED (v0.53.7, 2026-06-11)** `silentpayment-phrase-english-only` — the raw-phrase branch (`silent_payment.rs:170-172`) is now English-first then auto-detect: `parse_in(English).or_else(|_| bip39::Mnemonic::parse(s))` (NFKD + `language_of` over `all-languages`). Non-English phrases derive correctly (funds-relevant: seed = phrase WORDS); English unchanged; ambiguous non-English → `AmbiguousLanguages`. Tests T1 (JP resolves, RED-proven) + T2 (JP≠EN same-entropy) + T3 (English pinned to the published BIP-39 root xprv — external oracle). SPEC `design/SPEC_silentpayment_phrase_language_autodetect.md`.

### `silentpayment-japanese-bip39-seed-vector-cross-check` — cross-check the Japanese silent-payment derivation against an external BIP-39 seed vector

- **Surfaced:** 2026-06-11, v0.53.7 (`silentpayment-phrase-english-only` cycle, R0-r1 M2 / SPEC T4 deferral).
- **What:** v0.53.7 proves the non-English path RESOLVES (T1) and is words-based / distinct from English (T2), and pins the ENGLISH derivation to a published external oracle (T3). It does NOT cross-check the exact JAPANESE master xpriv against an external reference (the standard JP BIP-39 vectors use a non-empty passphrase; an empty-passphrase JP vector wasn't sourced in-window).
- **Fix:** add a frozen-literal assertion that the Japanese all-zeros-entropy phrase + empty passphrase derives the master xpriv from an independent reference (a known tool / BIP-39 JP vector recomputed with empty passphrase).
- **Tier:** deferred (low-risk; T1+T2+T3 already give strong coverage — the JP path works, is words-based, and the shared derivation machinery is externally validated on the English vector).

### `addresses-restore-passphrase-not-zeroizing` — ✓ RESOLVED (cycle-14 / toolkit v0.67.0, 2026-06-21) — BIP-39 passphrase held as plain `String` (entropy already Zeroizing)

- **Surfaced:** 2026-06-11, the 19-file zeroize audit (`hand-frozen-lint-canons-no-completeness` cycle, bonus finding).
- **Where:** `cmd/addresses.rs` (`let passphrase: String` @ `:149`) + `cmd/restore.rs` (`let passphrase: String` @ `:258`). Both resolve the effective BIP-39 passphrase (stdin / `@env:` / inline) into a plain `String`; only the derived seed/entropy is wrapped in `Zeroizing<Vec<u8>>`.
- **What:** the passphrase is a real (if small) owned secret that lingers un-scrubbed on the heap until the `String` drops. Every other seed-source command wraps it (`bundle.rs` wraps `args.passphrase` in `Zeroizing<String>`); these two are the outliers.
- **Fix:** wrap the resolved passphrase in `zeroize::Zeroizing<String>` at the `:149`/`:258` binding (it derefs to `&str` at the existing `&passphrase` call sites, so the change is local). Then PROMOTE both files' passphrase to a `ZEROIZE_ROWS` evidence anchor (they're already rows for the entropy site).
- **Status:** ✓ **RESOLVED by cycle-14 / L22 (toolkit v0.67.0, 2026-06-21).** Re-verified vs `origin/master` `ddabf5e3` by the 2026-06-21 secret-keymat sweep (`design/agent-reports/sweep-keymat-toolkit.md` § "Confirmed-still-open but ALREADY FILED"): both sites now wrap the effective passphrase in `Zeroizing<String>` — `addresses.rs:151` + `restore.rs:401/832/1296/3050` (the binding moved during cycle-14). Stale-open entry flipped to resolved rather than re-filed.
- **Tier:** ✓ resolved (was: deferred — low-risk; the passphrase is short-lived and the high-value secret — the entropy — is already wrapped).

### `archetype-older-blocks-flag-accepts-time-units` — preset `--older` (documented "blocks") still accepts the valid 512-second-unit window

- **Surfaced:** 2026-06-11, v0.53.9 (`older()` mask-gate cycle, R0-r1 M1 / R0-r2 I-A carve-out).
- **Where:** `cmd/build_descriptor.rs` `--older`/`--recovery-older` (`ParamKind::Blocks`, `descriptor_builder/archetype.rs`); manual `41-mnemonic.md:3993` documents `--older` as "relative timelock (blocks)".
- **What:** after the v0.53.9 mask gate, the preset `--older` flag still accepts the narrow `0x400001..=0x40FFFF` window (valid BIP-68 512-second-unit encodings), silently reinterpreting an ~80-year-implausible "block count" (e.g. `--older 4200000` ≈ 33.8 days) as a time-lock. Exposure is near-nil (the window is implausible as an intended block count), but it is the same silent-unit class as the gated bug, one notch narrower, on a flag whose docs say "blocks".
- **Fix:** bound `ParamKind::Blocks` params to `1..=65535` at the preset layer (`archetype::validate_params`). **Caveat — needs a design call, NOT a mechanical fold:** `validate_params` has a deliberate **"does-not-duplicate-gate-rules"** boundary (`archetype.rs:711` test); the gate legitimately accepts 512s-unit encodings, so the "blocks-only" constraint is a *preset-semantic*, not a gate rule — decide where it belongs before adding it.
- **Tier:** deferred (near-nil exposure; design call on the preset/gate boundary).

### `intake-surfaces-accept-masked-older-no-advisory` — import/round-trip surfaces accept consensus-masked `older()` with no warning (RESOLVED this cycle)

- **Surfaced:** 2026-06-11, v0.53.9 (`older()` mask-gate cycle, R0-r1 M5).
- **Where:** all SEVEN intake/round-trip surfaces — `bundle --descriptor`, `export-wallet --descriptor`, `import-wallet`, `xpub-search`, `verify-bundle --descriptor`, `restore --md1`, `compare-cost` — parse descriptors via miniscript `from_str` (`wallet_import/*`, `parse_descriptor.rs`, `descriptor_intake.rs`), which accepts the masked `older()` values the v0.53.9 *authoring* gate now rejects. (The originally-filed Where list named only 4 surfaces — `xpub-search`, `import-wallet`, `export-wallet --descriptor`, `compare-cost`; the implementing cycle extended coverage to all 7.)
- **What:** this is CORRECT for intake (the toolkit must not refuse to import an already-deployed wallet whose descriptor carries a consensus-weakened timelock — that would block recovery of real funds). But the user gets NO advisory that the imported/round-tripped descriptor's `older()` is weaker than its literal suggests.
- **Fix:** a NON-blocking advisory (warning, not refusal) on all 7 intake surfaces when a descriptor's `older()` carries garbage bits or a zero 16-bit value — "this descriptor's relative timelock is consensus-masked to <effective>; the literal <N> overstates it." The command still succeeds (exit 0); the advisory prints to stderr only.
- **Tier:** **RESOLVED (this cycle).** SPEC `design/SPEC_older_timelock_advisory.md`; plan `design/PLAN_older_timelock_advisory.md`; R0 reports `design/agent-reports/older-timelock-advisory-plan-r0-round{1,2}-review.md` (plan) + `design/agent-reports/older-timelock-advisory-r0-round{1,2,3}-review.md` (impl) + advisor `design/agent-reports/older-timelock-advisory-advisor-opus.md`. PATCH (advisory-only, zero clap delta → no GUI `schema_mirror` impact); manual prose at `docs/manual/src/40-cli-reference/41-mnemonic.md` (shared "Consensus-masked relative timelocks" section, cross-referenced from all 7 subcommands). Spawned `older-advisory-blindness-suppression` (below). The funds-safety-critical *authoring* path was already fixed in v0.53.9; this cycle adds the advise-on-intake half.
  - **[obs] ✓ RESOLVED (2026-06-11, NO-BUMP)** `synthesize-incrate-presence-not-correctness` — `full_bundle_emits_three_cards` now decodes the emitted ms1 (`ms_codec::decode`) and asserts the `Payload::Entr` bytes == the INPUT entropy (round-trip CORRECTNESS, not just `starts_with("ms1")`); RED-proven via expected-side perturbation. mk1→xpub + md1→policy correctness were ALREADY covered by `cross_binding_holds_round_trip` (`synthesize.rs:1046-1071`), so only ms1→entropy was the genuine gap. SPEC `design/SPEC_ci_test_hygiene_g6_pin_and_synthesize_correctness.md`.
  - **[obs] ✗ WONTFIX in toolkit — fold into md-codec entry (deep recon 2026-06-11)** `two-miniscripts-patch-load-bearing-stale-error` — Toolkit patches miniscript to git rev 95fdd1c (has Terminal::SortedMultiA) for its entire dep graph, so md-codec is recompiled against the patched miniscript inside the toolkit binary; md-codec's to_miniscript SortedMultiA refusal (`toolkit Cargo.toml:12-17 ([patch.crates-io] miniscript git rev 95fdd1c — cited :9-16, DRIFTED-by-3); descriptor-mnemonic crates/md-codec/src/to_miniscript.rs:406-411 (refusal arm, message :408)`). **Recon verdict: the audit's "dead/unreachable" framing is MECHANICALLY WRONG.** The refusal is a `match` arm on md-codec's OWN `Tag::SortedMultiA` (hand-rolled AST enum in `tree.rs`), NOT on any miniscript `Terminal` — so the `[patch]` cannot change its reachability, only render the message STRING ("rust-miniscript v13 has no Terminal::SortedMultiA fragment") technically false inside the toolkit build. No user-reachable toolkit path is wrong: the legit tr-sortedmulti-a route goes AROUND md-codec by design (`derive_address.rs::derive_receive_addresses`, `build_descriptor_string`, `restore.rs:830-893`), the degenerate wsh-leaf case correctly refuses, and `template.rs:546` characterization-tests it `is_err()`-only (message-insensitive → zero toolkit lockstep on an md-codec reword). Standalone md-codec (crates.io miniscript 13.0.0, no patch) the message is fully correct. **CODE-HYGIENE only, lives in md-codec, already tracked** by `md-codec-sortedmulti-a-to-miniscript-rendering-gap` (descriptor-mnemonic FOLLOWUPS, tier next-cycle). Fix = one build-config-neutral string reword at md-codec's next touch, or dissolved entirely by the real rendering-gap fix.
...
- **Surfaced:** 2026-06-13 (user question "if a user installs the constellation, do they get the latest code?" during the v0.55.2 release).
- **Where:** `scripts/install.sh` `component_info` table. The TOOLKIT self-pin (`mnemonic-toolkit-vX.Y.Z`) IS gated — `.github/workflows/install-pin-check` fails the tag build unless it equals the tag. But the **GUI pin (`mnemonic-gui-vX.Y.Z`, installed via `cargo install --git --tag`) and the sibling-CLI git-fallback tags have NO equivalent gate**, so they drift unnoticed.
- **What:** at v0.55.2 the GUI pin was **8 versions stale** (`v0.32.0` vs released `v0.40.0`) — a constellation install silently shipped a GUI MISSING the v0.33–v0.40 secret-exposure fixes (slot masking, on-screen command masking, paste-warn). `mk-cli` git-fallback tag was also stale (`v0.7.0` vs `v0.8.0`). Bumped both this session, but the SYSTEMIC gap remains: nothing catches the next drift. (Codec CLIs `md/ms/mk` install crates.io-LATEST via `cratesio=yes`, so their default path self-currents; only the git-fallback tags + the GUI git-tag pin drift.)
- **Fix (options):** (a) a CI check that compares `install.sh`'s `mnemonic-gui` pin to the latest `mnemonic-gui-v*` tag (cross-repo `gh api`) and warns/fails on drift — mirrors `install-pin-check` but cross-repo; (b) add the GUI install-pin bump to the GUI's release lockstep (when GUI tags vX, bump toolkit `install.sh`); (c) make the GUI also `cratesio=yes` if/when it's published. Lean (a)+(b).
- **rustc-MSRV tension (verified 2026-06-13, user kept v0.40.0):** GUI **v0.40.0** does NOT build on **rustc 1.85** (the toolkit's MSRV) — its `--locked` deps (`icu_*@2.2.0`, `image@0.25.10`, `idna_adapter`) require **rustc ≥1.86–1.88**. `cargo install --locked --git --tag mnemonic-gui-v0.40.0` exits 101 on 1.85 (cargo won't downgrade locked deps). So a constellation install on rustc 1.85–1.87 gets the toolkit + codecs but FAILS the GUI step; rustc ≥1.88 installs all. Decision: keep v0.40.0 (latest + security fixes) — the higher GUI rustc requirement is accepted; documented as an install prerequisite in `README.md`. NOT a GUI defect (the GUI's own CI builds it on its newer toolchain). Future GUI bumps may raise this further; a drift-check (option a) should ideally also surface the GUI's MSRV.
- **Status:** `partially-resolved` (Wave-3 pin de-stale: `mnemonic-gui-v0.40.0→v0.49.0` in `scripts/install.sh:44`; README rustc ≥1.88 prerequisite already documented). **MSRV-surfacing piece RESOLVED** (2026-06-23, Wave-3-deferrals L5, toolkit leg, NO-BUMP): (i) added a `scripts/install.sh` rustc ≥1.88 skip-with-warning guard (`GUI_MIN_RUSTC_MINOR=88` constant + a guard placed after token-validation/before the install loop that parses `rustc --version`, strips nightly/pre-release suffixes, and on rustc<1.88 de-selects the GUI on BOTH axes — appends to `$EXCLUDE` for the default/`--exclude` set, token-exact rebuilds `$ONLY` for an `--only mnemonic-gui[,…]` run with a never-match sentinel when it empties — then warns to stderr + exits 0 with the 4 CLIs installed; parse-failure falls through, `--dry-run` exempt) + the REQUIREMENTS help-block line; (ii) the three stale/silent doc prereqs corrected (manual `21-install.md` Pre-requisites `1.77+`→CLIs ≥1.85 / GUI ≥1.88 + Path D GUI ≥1.88 note; quickstart `21-install.md` Pre-requisites `1.77`→≥1.85/≥1.88 + GUI ≥1.88 note); (iii) cspell dict additions (`MSRV` both, `rustc` quickstart) keep markdownlint+cspell GREEN. Smoke-verified all 5 axes (rustc 1.85 skip+warn / 1.88 attempt / 1.97-nightly attempt / rustc-absent fall-through / `--only mnemonic-gui`→0 installed / `--dry-run` 5-component plan unchanged). **OPEN remainder:** ~~(a) the systemic cross-repo `gh api` drift-check~~ **RESOLVED (2026-06-23, NO-BUMP):** the systemic cross-repo leg (LANE-PIN-B) shipped as `.github/workflows/gui-pin-drift-check.yml` — a warn-only `gh release list -R bg002h/mnemonic-gui --limit 1` check comparing the `scripts/install.sh:44` GUI pin to the latest released tag (version-aware via `sort -V`; daily `schedule:` cron; `::warning::` + exit 0 on drift; `permissions: contents: read` + `GH_TOKEN: secrets.GITHUB_TOKEN`). No live drift at ship (pin == `mnemonic-gui-v0.49.0`). **Remaining OPEN:** (b) the GUI `Cargo.toml` `rust-version "1.85"→"1.88"` correction + GUI README prereq line — a SEPARATE `mnemonic-gui` paired-PR (not this toolkit lane). **Tier:** deferred (install-hygiene; not funds-safety, but it silently ships stale security fixes to GUI installers).
...
- **Surfaced:** 2026-06-10 audit (report alias: `secret-flag-completeness-gate-non-circular` — never filed under that name). **Resolved:** 2026-06-10, toolkit v0.53.1 (PATCH). Plan + reviews: `design/PLAN_secret_flag_gate_non_circular.md`, `design/agent-reports/secret-flag-gate-plan-r0-round{1,2}-review.md`; recon `cycle-prep-recon-vacuous-secret-flag-gate.md`.
- **What:** the §7 cell `secret_flag_enumeration_matches_authoritative_predicate` compared the schema `secret` bit against the very predicate the emitter derives it from (`gui_schema.rs` `emit_flag` ← `secrets::flag_is_secret`) — a tautology; and `flag_is_secret` itself was a hand `matches!` allowlist with a design-accepted "no completeness gate" boundary (`lint_argv_secret_flags.rs` module doc). Live consequence: `xpub-search --phrase` (raw master BIP-39 phrase) + `--phrase-stdin` + `--ms1-stdin` were `secret:false`.
...
- **Surfaced:** 2026-06-10 audit. **Resolved:** 2026-06-10, toolkit v0.53.0 (MINOR). Plan + reviews: `design/PLAN_mk1_csi_slot_unique.md`, `design/agent-reports/mk1-csi-slot-unique-plan-r0-round{1,2}-review.md`.
- **What (I10):** multisig mk1 `chunk_set_id` was `derive_mk1_chunk_set_id(xpub.fingerprint())` per cosigner. Two cosigners reusing one xpub at different paths (admitted by the BIP-388 distinctness gate, which rejects only when BOTH xpub AND path are equal) shared a fingerprint → shared csi → `verify-bundle` merged their chunks into one reassembly group → `ChunkedHeaderMalformed` → spurious `result: mismatch` (mk1_decode[0] DecodeFailed, [1] NotSupplied) on two individually-valid cards. Spurious-failure, not funds-loss / not wrong-bytes. **(n1-vs-nge2):** n=1 derived csi from the policy stub while n≥2 derived from the fingerprint, and the engraving-display helper showed the stub-derived id for all slots → displayed mk1_card_id ≠ actual emitted csi for multisig.
- **Fix:** unified `derive_mk1_chunk_set_id_for_slot(stub, slot) = derive_mk1_chunk_set_id(stub) ^ slot`, applied at all 6 emission sites (live `synthesize_descriptor` n≥2 + n=1; the `synthesize_full`/`watch_only` single-card; the dead-code `synthesize_multisig_full`/`_watch_only`) and the display helper (`bundle.rs`). XOR is injective in slot ⇒ distinct per cosigner; **immune to fingerprint collision**. Cosigner count ≤16 ⇒ slot ≤15 = 4 bits ⇒ XOR touches only the low nibble (5th hex char) ⇒ the **leading 16 bits (= policy_id[0..2], the bundle-binding prefix) are preserved** across cosigners (a strength vs the old fingerprint scheme, which broke that agreement). n=1 = `^0` ⇒ single-sig wire byte-identical (pins the pre-fix fixture). Display now matches actual emission for both mk1 and ms1 card-ids (resolves n1-vs-nge2). **csi is never recomputed at verify time** (verify reads+groups by it, maps by xpub) ⇒ old cards keep verifying; no compat break.
- **Wire/SemVer:** multisig mk1 csi bytes change for all multisig bundles ⇒ **MINOR** (precedent v0.48.0 trmultia-NUMS); single-sig unchanged. No flag/surface change ⇒ no GUI `schema_mirror` impact; the `bundle --json` mk1 values change (same shape) — downstream `--json` consumers that compare csi self-update per the paired-PR wire-shape rule (no automated gate). Tests: end-to-end I10 regression (same-xpub-two-paths 2-of-2 round-trip via verify-bundle, RED pre-fix) + inverted self-multisig characterization + re-captured byte-exact goldens (synthesize_unified + envelope_v0_27_0 fixture). Docs: technical-manual Invariant-1 + end-user manual updated.
- **Still open:** `anti-collision-16bit-invariant-false` (md1 uses `compute_md1_encoding_id` while mk1/ms1 use `compute_wallet_policy_id` — md1↔mk1 cross-card 16-bit agreement; NOT touched here — this fix only made the multisig mk1 *cosigners* agree with each other + with the WalletPolicyId-based stub).
- **Tier:** resolved.
...
- **Surfaced:** 2026-06-10 audit (MINOR section M1/M2/M3/M13). **Resolved:** 2026-06-10, toolkit v0.53.2 (PATCH). Plan + reviews: `design/PLAN_minors_m1_m2_m3_m13.md`, `design/agent-reports/minors-m1-m2-m3-m13-plan-r0-round{1,2}-review.md` (+ impl review); recon `cycle-prep-recon-minors-m1-m2-m3.md`.
- **M1 (`self-check-no-mk1-xpub-binding`):** `self_check_bundle` now binds each decoded mk1 card SLOT-EXACTLY to the descriptor — `desc.tlv.pubkeys` must contain `(i as u8, xpub_to_65(&card.xpub))` in BOTH Single and Multi branches (slot-exact catches cross-slot card swaps that order-agnostic membership + the stub check would pass; sound because pubkeys and cards are built from the same slots vector in the same order at every call site). `pubkeys: None` = distinct defensive failure. 3 synthesize-then-mutate RED cells (Single/Multi/cross-slot-swap; multisig fixture uses DISTINCT per-slot seeds — self-multisig would make swaps undetectable).
- **M3 (`inspect-repair-no-argv-advisory`):** the shared `repair.rs::resolve_groups` chokepoint now fires `secret_in_argv_warning` per-occurrence on RAW pre-expansion argv values — unconditionally for non-`-` `--ms1` (covers the indel-relaxed corrupted-prefix case the HRP probe would miss) and HRP-probe-gated for positionals (label `positional ms1`); `alternative` = `--ms1 -` both. Covers BOTH subcommands + the positional intake the audit missed. Manual: advisory rows added to the inspect + repair sections. Lint Route rows' evidence gained the `secret_in_argv_warning` anchor.
- **M2 (`localize-broad-error-collapse`):** `localize` narrowed to `Err(NonTopLevel) => None`; any other variant `debug_assert!`s (naming it) then returns `None` — release behavior unchanged (the audit's literal fix was a behavioral no-op: the sole caller already root-falls-back); dev/test builds now catch an invariant break the moment step-2 is relaxed.
...
- **Surfaced:** 2026-06-10 audit. **Resolved:** 2026-06-10, toolkit v0.53.3 (PATCH). Plan + 4 R0 rounds + impl review: `design/PLAN_hrp_case_insensitive_probes.md`, `design/agent-reports/hrp-case-insensitive-*.md`; recon `cycle-prep-recon-hrp-case-insensitive.md`.
- **What:** 7 probe sites lowercased before the HRP check (original string always passed to the codec — never normalize-at-intake, which would case-launder the codecs' deliberate mixed-case rejections); the v0.24.0 I5 typed-flag case-mismatch rejection RELAXED (uniform rule: the codecs are the case authority; true HRP mismatches still reject); `UnknownHrp` display truncates to 12 chars + `…` (an uppercase ms1 positional previously echoed the FULL master secret on stderr); the v0.53.2 positional secret-argv advisory now fires for uppercase ms1.
- **RECON OVERTURNED the audit's codec claim:** ms-codec 0.4.0's envelope layer (`envelope.rs:100/:112`) is case-sensitive PAST codex32 — uppercase ms1 cannot decode toolkit-side; mk-codec (Mixed-only reject) and md-codec (accepts uppercase AND mixed — its BIP-173 leniency, characterization-tested here, not "fixed") verified tolerant at the pinned crates.io sources.
...
- **Why deferred:** ms-codec 0.4.1 is tagged but not yet published to crates.io (the toolkit consumes ms-codec from the registry); the pin bump can't resolve until publish. Bundle with the next toolkit release that re-pins siblings. **UPDATE (2026-06-10): bundle 0.4.2 too** (uppercase-ms1 acceptance + combine-guard security fix, `ms-codec-v0.4.2`) — one pin bump 0.4.0 → 0.4.2 covers both, and MUST also invert the v0.53.3 staged uppercase-ms1 characterization cells (see `hrp-classifier-rejects-valid-uppercase-cards`). Toolkit `mnemonic ms-shares combine` Phrase arm uses `.map_err(ToolkitError::Bip39)?` (not `.expect`), so the toolkit surfaces a mapped error rather than aborting — but it still wrongly accepts a non-standard-length recovery pre-bump; the bump makes it reject cleanly at the codec boundary.
- **Status:** **resolved** `mnemonic-toolkit-v0.53.5` (2026-06-10) — ms-codec pin 0.4.0 → 0.4.2 (published). Inherits the 0.4.1 combine non-standard-length validation AND the 0.4.2 combine secret-guard fix (`ms-shares combine --share <UPPER secret-at-S>` now refuses cleanly — red-first cell added) AND makes uppercase ms1 decode end-to-end (audit M11 ms1 leg closed — 4 staged characterization cells inverted). Plan: `design/PLAN_ms_codec_pin_bump_0_4_2.md`.
- **Tier:** `cross-repo`
...
- **§11 I4 template arm (2026-06-20, toolkit v0.60.0):** the §11 I4 "wallet-policy-vs-template-only `tlv.pubkeys` auto-detect branch" now has its TEMPLATE-ONLY arm — #28 phase 2 added keyless multisig/general TEMPLATE completion: `restore --md1 <keyless template> --from <seed> --cosigner <mk1>…` routes (`!is_wallet_policy() && n>=2`, canonical OR general) to a permutation-search completion that resolves the unique `@N`→key assignment (id-search / address-search / explicit), distinct from this entry's wallet-policy (keyed-md1) reconstruction. The same intake lands on `verify-bundle`. See `bundle-md1-template-only-option` (umbrella) + `design/agent-reports/template-multisig-*` (all R0-GREEN + whole-diff ship review).
- **Tier:** `v0.5`
- **Tags:** `restore` `wallet` `multisig`
- **Spawned FOLLOWUPs:** `restore-multisig-taproot-reconstruction`, `restore-multisig-format-payloads`, `gui-restore-multisig-flags-pending-pin-bump`, `template-multisig-own-account-range-subset-search` (the keyless-template completion arm's deferred over-supply subset search).
...
- **Status:** `resolved` toolkit-**v0.49.1**. **NOT upstream-blocked** (the "rust-miniscript v13 has no SortedMultiA" framing was STALE — the toolkit pins miniscript git rev `95fdd1c` which HAS `Terminal::SortedMultiA`; only md-codec's crates.io-`0.35.0` pin lacks it). Shipped per `design/SPEC_restore_multisig_taproot.md` v2 (supersedes the RED v1 — both its Criticals dissolved by v0.48.0's `is_nums:true`/NUMS flip): `restore --md1` reconstructs `tr-multi-a` AND `tr-sortedmulti-a` NUMS multisig md1 by reading template + NUMS internal-key off `d.tree` directly and building via `build_descriptor_string(…, Some(Nums))` — **routing around md-codec** for BOTH the descriptor build AND the first-address derivation (a new `derive_address::derive_receive_addresses` on the descriptor STRING; `d.derive_address` re-enters md-codec's `to_miniscript` which errors on SortedMultiA — R0 v2 C1). `is_nums:false` (cosigner-internal) deferred. PATCH, watch-only, no schema_mirror. R0 v2: r1 RED (1 Critical) → r2 GREEN; impl review 0C/2I folded. Golden-address tests + `--format` taproot cell. Audit: SPEC + `design/agent-reports/restore-multisig-taproot-v2-r0-round{1,2}-review.md` + `restore-multisig-taproot-impl-review.md`.
- **Tier:** `v0.5`
...
- **Status:** `resolved` (Cycle A, 2026-06-08, **toolkit v0.48.0** + `mnemonic-toolkit-v0.48.0` tag — MINOR **wire-content change**). `template.rs::wrapper_node` `TrMultiA|TrSortedMultiA` arm now emits **`is_nums: true`** (BIP-341 NUMS H-point `50929b74…803ac0`) instead of the non-standard `is_nums:false`/cosigner-@0. **Empirically:** `tr(@0/<0;1>/*, multi_a(2,@0,@1))` → `tr(50929b74…803ac0, multi_a(2,@0,@1))` (leaf unchanged; propagates via `wallet_policy_id` → the stub seeding BOTH md1 AND mk1, so both cards shift). Pinned by 3 `template::tests::tr_*` tests (source `is_nums:true`, wire round-trip preserves it, `to_miniscript` renders `tr(NUMS, multi_a)`); descriptor-vs-template consistency test updated to `tr(NUMS,…)`; 8 stale orphaned `tr-*-0-false-false.txt` goldens deleted (→ `orphaned-v0_2-md1-vectors-no-harness`). **CORRECTION (recon-disproven):** the "this unblocks restore" claim is FALSE — `tr-sortedmulti-a` restore is blocked UPSTREAM (rust-miniscript v13.0.0 has no `Terminal::SortedMultiA` fragment, `md-codec to_miniscript.rs:406-410`, independent of `is_nums`); `tr-multi-a` needs a separate `restore.rs:777` pre-gate lift. Audit: `design/SPEC_trmultia_nums_internal_key.md` + `design/agent-reports/trmultia-nums-r0-round{1,2}-review.md`.
- **Tier:** `v0.5`
- **Tags:** `bundle` `taproot` `multisig` `wire-format` `blocks-restore-taproot`

### `md-codec-sortedmulti-a-to-miniscript-rendering-gap` — md-codec `to_miniscript` cannot render `SortedMultiA` (rust-miniscript v13 gap)
- **Surfaced:** 2026-06-08, Cycle A (`toolkit-trmultia-nums-internal-key`) recon — the true blocker of `restore-multisig-taproot-reconstruction` for `tr-sortedmulti-a`.
- **Where:** `descriptor-mnemonic/crates/md-codec/src/to_miniscript.rs:406-410` — `(Tag::SortedMultiA, Body::MultiKeys {..})` unconditionally `return Err("Tag::SortedMultiA must be a tap-leaf root child; rust-miniscript v13 has no Terminal::SortedMultiA fragment")`. (`Tag::MultiA` at `:394-398` DOES render.) md-codec pins `miniscript = 13.0.0`.
- **What:** a wallet-policy md1 whose tree is `Tr{…, SortedMultiA{k, indices}}` cannot be lowered to a `miniscript::Descriptor` — so any toolkit consumer that classifies/reconstructs via `to_miniscript_descriptor` (notably `restore`) cannot handle `tr-sortedmulti-a`, independent of the internal-key (`is_nums`) shape. Options: (a) wait for / upstream a `Terminal::SortedMultiA` fragment in rust-miniscript; (b) md-codec lowers `sortedmulti_a` to a *sorted* `multi_a` at descriptor-build time (semantically equivalent for taproot script-path); (c) document as a permanent limit.
- **Status:** `open`
- **Tier:** `v0.5`
- **Companion:** mirror in `bg002h/descriptor-mnemonic` `design/FOLLOWUPS.md` (md-codec is the implementing repo); cross-cites `toolkit-trmultia-nums-internal-key` + `restore-multisig-taproot-reconstruction`.

### `orphaned-v0_2-md1-vectors-no-harness` — the `tests/vectors/v0_2/*-false-false.txt` md1 goldens are read by no test
- **Surfaced:** 2026-06-08, Cycle A R0-r1 I1 (discovered the tr `-false-false` goldens were orphaned → a code change false-greened).
- **Where:** `crates/mnemonic-toolkit/tests/vectors/v0_2/` — the whole `{wsh-multi,wsh-sortedmulti,sh-wsh-multi,sh-wsh-sortedmulti}-*-0-false-false.txt` + `-5-false-false` family. NO test reads them (no `read_dir`/`insta::glob`/`WalkDir`; `cli_self_check.rs` reads only `bip84-…-0-false-true.txt`, `cli_bundle_full.rs` only `v0_1/`). The 8 `tr-*-0-false-false.txt` were DELETED in v0.48.0 (stale after the NUMS wire change); the rest remain dead-but-correct.
- **What:** either wire a harness (a `read_dir`-driven test that decode-asserts each vector — turning them into live wire-regression pins) or delete them as dead goldens (mirroring the `cli-help-golden-cleanup` + the v0.48.0 tr-vector deletion). Decide one; do not leave a large unread golden corpus that silently rots.
- **Status:** **resolved** `mnemonic-toolkit-v0.53.2` (2026-06-10) — **DELETED the 25** unread fixtures (kept the read `bip84-mainnet-0-false-true.txt`). Decision input that settled delete-vs-harness: v0.48.0 (NUMS internal key) and v0.53.0 (slot-unique multisig csi) both changed multisig wire bytes AFTER these v0_2 goldens were captured — they were no longer dead-but-correct; a harness wired today would FAIL against them or pin outdated wire. `cli_self_check.rs:5` module doc corrected in the same commit (resolves the audit `[obs] self-check-header-claims-unread-vector` line). Plan: `design/PLAN_minors_m1_m2_m3_m13.md` Part 4.
...
- **Status:** `resolved` toolkit-**v0.47.2** (quick-wins PATCH). The cited lines had drifted (repair rows now `:2751-2753`, inspect row `:3023`, synopses `:2744`/`:3016`); recon `cycle-prep-recon-quick-wins-batch.md` re-grepped them at SHA `f87de2e`. Ground truth re-confirmed at R0: `repair.rs:30-53` + `inspect.rs:23-36` both define `ArgGroup::new("kind").args(["ms1","mk1","md1"]).required(false).multiple(true)` with NO `conflicts_with_all`; `inspect.rs:24-26` comments "mixed-HRP invocations are valid". Reworded the 4 rows to "may be combined with … (one HRP per card; per D35)" + rewrote both synopses from the false brace-pipe to the curated independently-optional form. The `inspect`-allows-combining check (R0 I3) confirmed inspect's table has only ONE in-scope mutex row (`--ms1` at `:3023`), not three. Audit trail: `design/SPEC_quick_wins_v0_47_2.md` + `design/agent-reports/quick-wins-v0_47_2-*`.
- **Tier:** `docs`
...
### `mlock-g4-a-page-count-assert-flake` — `mlock::tests::g4_a_pin_and_zeroize_compose_without_panic` flakes on the same page-alignment luck as g1_1, but lives in the SYNCED file

- **Surfaced:** 2026-06-17, observed flaking during the D1-B (actions-major) master CI re-run. Same root cause as the resolved `mlock-g1-1-test-page-alignment-luck` above: `g4_a` does `let mut v: Vec<u8> = vec![0xAAu8; 64];` then `assert_eq!(pin.page_count, 1, "64-byte buf pins exactly one page")` — the plain (non-page-aligned) 64-byte `Vec` can straddle a page boundary under the parallel runner → `page_count == 2` → rare failure. **Production is correct** (pinning 2 pages is fine; `round_to_pages` at `mlock.rs:129-138` is right); only the test's `== 1` assertion is over-strict, and it is incidental to the test's actual purpose (its name: pin + `zeroize()` + drop compose *without panic*).
- **Where:** `crates/mnemonic-toolkit/src/mlock.rs:424-433` (`g4_a`; the assert is `:429`) — and byte-identically at `mnemonic-secret` `crates/ms-cli/src/mlock.rs:429-434`.
- **Why this is NOT the cheap fix g1_1 was:** `g4_a` lives INSIDE the SYNCED `mlock.rs` library file (the `#[cfg(test)] mod tests`), which the **g6 invariant** (`tests/mlock_g6_invariant.rs::normalize()`) byte-compares in full against the FROZEN pinned ms-cli tag (`ms-cli-v0.7.0`, resolved from `scripts/install.sh:38` by the `rust.yml` `g6-invariant` job). g1_1 was toolkit-only-fixable because it lives in the unsynced `tests/mlock_unit.rs`; g4_a does not. Any non-comment byte change to g4_a (the comparator strips only `//`/blank lines — `#[ignore]` attrs included in the diff) forces: edit `mlock.rs` in **both** repos identically → **publish a new public ms-cli tag** (v0.7.0 is immutable) → re-pin `install.sh:38` + `manual.yml:90` (`sibling-pin-check` gate) + the v0.7.0 comment refs in `rust.yml`/this file. That publishes an outward-facing artifact and **moves the deliberately-frozen g6 anchor** (the "asymmetric pin"; see `mlock-rs-fmt-exempt` + `secret-memory-hygiene-cycle-b`).
- **What (the minimal fix, when it IS done):** **delete the single `assert_eq!(pin.page_count, 1, …)` line** (−1 synced line). NOT relax-to-`1..=2` (adds a trivially-true low-value assertion) and NOT page-align-the-buffer (largest synced delta + duplicates coverage). Coverage is **not** lost: the resolved `g1_1` (page-aligned `alloc_zeroed`) already pins the deterministic `page_count == 1` contract; g4_a's unique value is the zeroize-compose, which the deletion preserves.
- **Decision (architect consult, opus, 2026-06-17 — VERDICT (c) leave tracked):** do NOT cut a standalone tag for this. Cost ≫ benefit — an irreversible public ms-cli tag + moving a documented-deliberate frozen anchor, to scrub a rare cosmetic flake on a correct path. A standalone v0.7.1 backport would ALSO fail to discharge `mlock-rs-fmt-exempt` (the backport wouldn't be 1.95.0-formatted), so it pays the full cross-repo cost AND keeps the fmt carve-out — worst of both worlds. Note: `mlock.rs` is the same git blob across `ms-cli-v0.7.0`/`v0.8.0`/master, so there is zero file-drift to reconcile if/when the pin moves.
- **Trigger to fix (runner-up (a)):** fold the one-line deletion into BOTH repos the **next time the ms-cli g6 pin is bumped for an independent reason** — ideally the 1.95.0-formatted tag that also discharges `mlock-rs-fmt-exempt` — so the marginal cost is ~zero and multiple FOLLOWUPS close together. Until then: leave it tracked; the rare flake costs only an occasional CI re-run.
- **Status:** open
- **Tier:** `cross-repo`
- **Companion:** `bg002h/mnemonic-secret` — mirror this entry (g4_a is byte-shared via g6; any fix is paired across both repos + a new ms-cli tag).
...
- **PROTOCOL-TRAP NOTE (added 2026-06-22, v0.70.1 cycle, open-followups maturity program — DEFER ruling):** the `bitcoin::bech32::decode` approach in the original "What" is **WRONG for md1** and would REGRESS funds-feature routing. md1 is a **custom BCH(93,80,8) code** (13-symbol checksum, target residue `md_codec::bch::MD_REGULAR_CONST` = `0x0815c07747a3392e7`, NOT bech32's residue `1` / bech32m's `0x2bc830a3`). `bitcoin::bech32::decode("md1…")` fails the checksum on EVERY valid md1 string → a `bech32::decode(t).is_ok()` tie-break would misroute valid md1 cards to literal-xpub. The CORRECT future primitive is the public per-chunk checksum `md_codec::bch::bch_verify_regular(hrp, data_with_checksum) -> bool` (`descriptor-mnemonic/crates/md-codec/src/bch.rs:89`; the toolkit already imports `md_codec::bch::MD_REGULAR_CONST` at `repair.rs`), NOT a sibling change and NOT bech32. Note md1 is multi-chunk (whole-descriptor `decode_md1_string` ≠ multi-chunk `reassemble`), so any per-token check must be chunk-level like `bch_verify_regular`. **DEFER ruled:** there is NO live defect — the current routing (`descriptor_intake.rs:156` tie-break → `parse_md1` :210 → `md_codec::chunk::reassemble` :224 → typed `ToolkitError::MdCodec` exit 1 on checksum failure) is already loud-and-safe; the value of tightening which error you get for `md1garbage` is negligible over the existing typed error.
- **Status:** open (deferred — DEFER ruled under the open-followups maturity program, v0.70.1 cycle; no live defect, the bech32-validate draft is a protocol trap, correct primitive recorded above).
...
- **ATTEMPT 2026-06-22 (Wave-2 G1-B) — REVERTED (`a3ff1c3f`); corrected understanding:** This is NOT a standalone prose fix. The prose fix is only honest if `pinned-upstream.toml [mnemonic-gui]` is bumped past v0.3.0 (the v0.3.0 GUI genuinely had NO redaction — so the honest-broken prose is CORRECT for the pinned version). BUT the `gui-schema-coverage` lint (`manual-gui.yml`) clones the GUI at the pinned tag and requires the manual to document its **full schema** — bumping to v0.48.1 surfaced **~497 missing schema anchors** (md-encode/mk-address/mk-derive/… — the GUI grew enormously since v0.3.0) + 67 missing outlines. So closing this is **GATED on the v1.1 manual-gui modernization** (re-pin the GUI tag AND document the grown surface — a large dedicated cycle, NOT a redaction-prose patch). The pinned-upstream comment confirms bumping the GUI tag is "a v1.1 cycle." Do not re-attempt the prose+pin fix without the full manual modernization. (Local gotcha: `cspell`/`markdownlint` pass locally but `gui-schema-coverage` needs the GUI clone — only CI catches the anchor gap.)
- **Status:** `open` (blocked-on v1.1 manual-gui modernization)
- **Tier:** `v1.1+`
...
- **2026-06-06 cycle-prep deep dive — the blocker is TYPE entanglement, not import paths (the `pub use` shim does NOT work).** `error.rs`'s `ToolkitError` embeds **bin-only types in 3 variant fields**: `ExportWalletMissingFields { script_type: wallet_export::WalletScriptType, missing: Vec<wallet_export::MissingField> }` (`:65/:147`), `CompareCost(cost::CompareCostError)` (`:75`), `Repair(repair::RepairError)` (`:266` + `From` impl `:353`); its `Display` also calls `wallet_export::{build_missing_fields_refusal, script_type_short_name, format_stub_message, taproot_multisig_unsupported_message, REFUSAL_SECRET_INPUT}`. `wallet_export`/`cost`/`repair` are all main.rs-private. A `pub use mnemonic_toolkit::error;` re-export shim fixes the 80 files' `crate::error::` import paths but CANNOT compile `error.rs` as a lib module while it references bin-only types. Three real approaches: **(a) cascade-move** `wallet_export`+`cost`+`repair`(+their deps: repair→`indel`+`secret_advisory`+circular `error`) to lib — `cost.rs` is self-contained (clean) but `wallet_export` is the whole emitter tree → ≈ promoting most of the bin; **(b) de-entangle** — restructure the 3 entangled variants to carry pre-rendered `String`s (or move `CompareCostError`/`RepairError`/`MissingField`/`WalletScriptType` to lib), bounded (~3-4 variants + their `Display` arms + every construction site) but **error-message-byte-sensitive** (must preserve exact `Display` output across the constellation); **(c) status quo** — keep `error` in the bin + the `FinalWordError`/`FinalWordLanguage` mirror (the v0.11.0 decision; the mirror cost is bounded + stable). Deleting the `final_word` mirror REQUIRES error+language in lib → same blocker.
- **Status:** `open` — **NOT a clean shim cycle (recon-confirmed).** Recommend (c) status-quo unless the lib-consumer need becomes concrete; if pursued, (b) de-entangle is the bounded path but needs its own SPEC + byte-exact `Display` characterization (the same golden-capture discipline as the synthesize/emit_payload dedups). Do NOT attempt the `pub use` shim — it does not address the type entanglement.
...
- **Where:** New module `crates/mnemonic-toolkit/src/mlock.rs` (533 LOC, Fix-B slice-fn-only design). Cross-repo: toolkit handles Sites 1-4; ms-cli handles Site 5 via inline copy. Sites realized:
  1. **Site 1** (4 per-handler clap-binding pins): `cmd/bundle.rs:129+133` (passphrase + per-slot), `cmd/verify_bundle.rs:143-150` (same), `cmd/convert.rs:668+` (effective_passphrase / effective_bip38_passphrase / primary_value), `cmd/derive_child.rs:122+` (from_value + stdin_passphrase). **SCRUB LEG RESOLVED at toolkit v0.67.0 (cycle-14, L22):** the mlock *pin* leg shipped here at v0.10.0, but the pinned-yet-un-scrubbed `String` secrets (mlock ≠ zeroize) were the open residue. v0.67.0 wraps them: the persistent `SlotInput.value` field is now `SecretString` (closing the stdin `=-` AND `@env:` write-back paths in `slot_input.rs` / `bundle.rs` / `import_wallet.rs` / `verify_bundle.rs`), and the `convert` / `restore` / `addresses` handler locals (+ `restore`'s `TemplateSeed.passphrase`) are `Zeroizing<String>`. The toolkit's canonical 5-site list is now fully closed (Sites 2-4 at v0.10.x, Site 5 in ms-cli, Site 1 scrub at v0.67.0). Precedents this mirrors (cited, NOT re-opened): [[resolved-slot-derived-account-zeroizing-field]] (v0.10.1), `import-wallet-blob-zeroizing` (v0.33.3), `bsms-decrypt-record-string-zeroizing` (v0.34.1). Deferred deeper wrap: [[phrase-overlay-secretstring]].
...
- **Status:** `resolved` — 2026-06-22, Wave-4 L4 "batch-doc" (branch `chore/wave4-batch-doc-md1`). The v0.17-era `md1zsxdspq…` set is GONE from all live documentation. The content had migrated since this slug was filed (the cited `docs/manual/src/…` paths no longer carry it); the actual stale sites at HEAD were in **three** books — `docs/quickstart/`, `docs/technical-manual/`, `docs/manual-gui/`. The canonical `docs/manual/` book was already fresh (`md1fgdxlp…`, kept current by `make audit` CI). Refresh details:
  - **Current valid bip84/abandon md1 set (regenerated from `target/debug/mnemonic bundle --network mainnet --template bip84 --slot @0.phrase=<abandon×11 about>`):** `md1fgdxlpqpqpm6jzzqqvqpdqw0za5zs4gyy55aq4vsmnhy4s6wyaypu34c7raqu8np` / `md1fgdxlpqf2zcgefcpupmel75q5435j7seugaj5jr7qyur6vt76es5cdeyrq7zdy0d` / `md1fgdxlpq3xa2dk8vwpj7gx74hwqxqdp083jehp5tdrfa0n5zdfkqcdlrvnh5r62jn`. Decodes to `wpkh(@0/<0;1>/*)`. (ms1/mk1 unchanged — only md1 broke under the v0.18 wire-format change.)
  - **technical-manual:** all 11 drifted transcript `.out` files **regenerated** via the binary (`make verify-examples` mechanism, not hand-edited), incl. the genuinely-wrong `mnemonic-verify-bundle-bip84-abandon` pair — its `.cmd` `--md1` args were updated to the current set, eliminating the `md1_decode: fail WireVersionMismatch { got: 1 }` / `result: mismatch` that replayed before (now `result: ok`). The D9 output-class advisory re-word (`note: stdout is watch-only…` / `note: stdout is a keyless descriptor template…` / `warning: stdout carries private key material…` / argv-exposure `warning:`) was folded in the same regen pass per the 2026-05-31 note below. Also refreshed the abbreviated illustration md1 in `src/50-rust-api/54-mnemonic-toolkit-api.md` + `examples/examples/mnemonic-toolkit-api-roundtrip.rs` (`md1zsxdspq…`→`md1fgdxlpq…`).
  - **quickstart:** `src/20-singlesig/23-bundle.md` (output block re-rendered to the current chunked-only display + prose), `24-verify.md` (`--md1` args), `26-recover.md` (`md decode` md1 args).
  - **manual-gui:** `src/40-mnemonic/42-bundle.md` (JSON illustration + output panel), `43-verify-bundle.md` (`--md1` args), `src/50-md/51-overview.md` (canonical md1 block), `54-decode.md` (worked example now pastes all 3 strings so it actually reassembles to `wpkh(@0/<0;1>/*)`).
  - **Verified locally (neither quickstart nor manual-gui runs verify-examples in CI):** technical-manual `make verify-examples` = OK 15/15; `docs/manual` (where quickstart's transcripts symlink) = OK 20/20; manual-gui = vacuous pass (GUI-prose, no transcripts); technical-manual `symbol-ref-check.py` OK (726 anchors, 0 drift); cspell clean on all 8 changed `.md`. (`make lint` end-to-end not runnable locally — `markdownlint` absent — but the gating Python/cspell pieces ran clean.)
  - **NOT regenerated:** `docs/manual/agent-reports/phase-3-review-1.md` still holds the old strings — intentionally frozen historical review transcript, not live docs.
- **Tier:** `v0.2` (next minor; non-blocking for descriptor-mnemonic v0.18 release).

- **2026-05-31 (cycle B):** `docs/technical-manual` is NOT CI-gated; cycle B's output-class advisory re-word was intentionally NOT applied there (P5 reverted it) because the stale v0.17 md1 makes a clean advisory-only re-capture impossible and a full re-capture would bake a `md1_decode: fail WireVersionMismatch`. When this FOLLOWUP's stale-md1 is fixed, ALSO re-word the D9→output-class advisory in the technical-manual transcripts (`ms1-{encode,decode}`, `mnemonic-bundle-bip84-abandon`, etc.) in the same pass. *(Done in the Wave-4 L4 resolution above — the regen folded the advisory re-word.)*
...
- **Status:** `resolved` — verified real-binary-bound at `.github/workflows/manual.yml` `make audit` (`MNEMONIC_BIN` built via `cargo build --bin mnemonic`, `MD_BIN=md` / `MS_BIN=ms` / `MK_BIN=mk` all `cargo install`'d), 2026-06-22, v0.70.1 cycle (open-followups maturity program). The `MD_BIN=true`/`MS_BIN=true`/`MNEMONIC_BIN=true` vacuity this entry describes is GONE — all four binaries execute real flag-coverage in CI. (Stale-open: the promotion landed since this slug was filed.)
...
- **What:** v0.3 toolkit ships an expanded `walk_miniscript_node` covering the v0.3-NEW `Terminal` arms (hash terminals, timelocks, wrappers, AND/OR/Thresh). The v0.3-era entry over-scoped the md-cli gap as "md-cli rejects all 24 of these" — the reality (verified 2026-06-23 against md-cli v0.9.2 `crates/md-cli/src/parse/template.rs::walk_miniscript_node`) is that md-cli has SINCE backported essentially the whole set: it walks PkK, PkH, Multi, MultiA, Check, Verify, AndV, Older, After, AndB, AndOr, OrB, OrC, OrD, OrI, Sha256, Hash256, Ripemd160, Hash160, Swap, Alt, DupIf, NonZero, ZeroNotEqual, True, False, Thresh. **`RawPkH` is the SOLE remaining arm the toolkit walks (`parse_descriptor.rs:739`) that md-cli does not** (md-cli routes it to its `_ => Err("unsupported miniscript fragment")` catch-all). And `RawPkH` is descriptor-unreachable in rust-miniscript v13 (appears only via raw-script decode — v0.4+ scope; the toolkit's own arm is `#[ignore]`-fenced at `parse_descriptor.rs:2777`). So the practical walker divergence is closed: the cross-tool differential corpus runs byte-identical toolkit↔md-cli ids across all 17 descriptor shapes at md-cli v0.9.2 (Wave-3-deferrals L1).
- **Why deferred:** scope of v0.3 was toolkit-only by user direction; the cross-repo backport landed in md-cli over the v0.4→v0.9 cycles.
- **Status:** `resolved` (2026-06-23, Wave-3-deferrals; the toolkit-side companion to LANE-2's md-cli backport). md-cli's walker now covers every descriptor-reachable arm the toolkit walks; the single descriptor-unreachable remainder (`RawPkH`, raw-script-only) is v0.4+ scope and does not affect any descriptor-mode differential.
...
- **RE-OPENED 2026-06-20 (cycle-1 H1):** the prior `resolved by v0.5.0 Phase B.3 (commit 9f1a4e7) — sort-then-compare multiset equality` status was WRONG — sorted-multiset equality is PROVABLY INSUFFICIENT as the sole `md1_xpub_match` determinant. It binds only the cosigner pubkey SET; it is blind to Tag/k/wrapper/nesting drift (e.g. `sh(multi)` vs `sh(wsh(multi))`, `sortedmulti(2,…)` vs `sortedmulti(1,…)`), to slot-order permutation in unsorted `multi`, and — the funds-safety hole — to the change-chain / multipath `use_site_path` that fixes the WATCHED ADDRESS SET (`<0;1>` vs `<2;3>` derive different addresses). A `verify-bundle` reporting `md1_xpub_match: passed` on a different-watched-address wallet is the worst failure class (false assurance). Empirically proven by the cycle-1 constellation bug-hunt as CRITICAL H1.
- **Status:** ✓ **RESOLVED by cycle-1 (toolkit v0.61.0, this commit) — H1 structural decoded-policy compare.** `emit_multisig_checks`'s `md1_xpub_match` now requires `expected_md_decoded.tree == desc.tree && expected_md_decoded.use_site_path == desc.use_site_path` (the derived structural compare: `tree` binds Tag/k/wrapper/index-aware slot-binding/nesting; `use_site_path` binds the change-chain/multipath + per-`@N` `tlv.use_site_path_overrides` that fix the watched-address set), with the sorted-pubkey-multiset check retained SUBORDINATE. Origins (`path_decl` + origin/fingerprint `tlv`) stay deliberately EXCLUDED (L14 elision brittleness — binding them would false-FAIL legitimately origin-elided descriptor-mode bundles, which is exactly the hole B.3's multiset change existed to avoid). Check NAME `md1_xpub_match` unchanged (no `--json` wire-shape churn). See `h1-verify-bundle-structural-policy-compare` (cycle-1) for the full record.
- **Tier:** `v0.4.5-nice-to-have`
...
- **Status:** `resolved` — verified real-binary-bound at `.github/workflows/manual.yml` `make audit` (`MNEMONIC_BIN` built + `MD_BIN=md` / `MS_BIN=ms` / `MK_BIN=mk` all `cargo install`'d), 2026-06-22, v0.70.1 cycle (open-followups maturity program). Promotes the prior `resolved-partial 52f33f7` (mnemonic-side only) to fully resolved: the MD/MS sibling-bin promotions tracked by the successor slugs `manual-md-bin-real-binary-promote` / `manual-ms-bin-real-binary-promote` (also flipped this cycle) have landed, so all four flag-coverage gates are real-binary-bound.
...
- **Status:** `resolved` — `mnemonic-gui` master `bdecfff` (test-only, no version bump / no tag). The floor (cited `:138` had drifted to `:131-136`) is replaced by a per-fixture `enum Expect { Canonical, NonCanonical, ParseFails }` table (`const FIXTURES: &[(&str, Expect)]`, 11 Canonical / 4 NonCanonical / 3 ParseFails = 18) + four accumulators: `newly_parsed` (a ParseFails fixture that now parses → "promote it"), `regressed` (an expected-classify fixture that now parse-fails → the broad regression the floor tolerated), `wrong_verdict` (toolkit verdict ≠ pinned `want` → pins the *absolute* canonical↔non-canonical verdict, stronger than agreement-only), `disagreements` (GUI verdict ≠ live toolkit verdict — the original drift check). Strictly dominates the old floor: every one of the 18 fixtures now has an exact expectation. Verified against the CI-pinned `mnemonic-toolkit-v0.47.3` (= `8502723`); negative checks proved all three new accumulators bite. Audit trail GUI-side: `mnemonic-gui/design/SPEC_canonicity_drift_per_fixture_table.md` + `design/agent-reports/canonicity-drift-per-fixture-table-r0-round{1,2}-review.md`.
- **Tier:** `v0.20+-test-hygiene`
- **Companion:** `bg002h/mnemonic-gui` `FOLLOWUPS.md::canonicity-drift-gate-floor-too-lenient` (filed resolved in lockstep, `bdecfff`).
...
- **Status:** `resolved` toolkit-**v0.47.1** (`1ea2317`). The shared back-half is `synthesize_descriptor` itself — and `type CosignerKeyInfo = ResolvedSlot` (`synthesize.rs:219`) means `synthesize_unified`'s `slots: &[ResolvedSlot]` IS `&[CosignerKeyInfo]`, so NO `emit_unified_cards`/`BundleMode`-dispatch helper was needed: `synthesize_unified` keeps its front-half (validation + descriptor construction) and delegates emission via `synthesize_descriptor(&descriptor, slots, privacy_preserving, run_language)`. Net −69 LOC. Recon corrected the cited line numbers (`:229`/`:745`, not `:200-275`/`:709-774`) + call-site count (~9, not 2). Guarded by a new byte-exact multisig characterization cell (R0 I1) — the n>1 Multi branch previously had no byte-exact golden. Audit trail: `design/SPEC_synthesize_descriptor_dedup.md` + `design/agent-reports/synthesize-descriptor-dedup-*`.
- **Tier:** `v0.22+-refactor`
- **Companion:** none.
...
- **Status:** `wontfix` (2026-06-06, product-owner decision). The pre-v0.21.0 "broken descriptor-mode" bundle shape (`ms1[0]` populated, `ms1[1+]` empty) was **never used in the wild to encode an ms1 string** — no real engraved/persisted bundles carry it — so there is nothing for the advisory to detect. Detecting a shape that no human ever produced is dead code. The existing (technically-accurate) SPEC §5.7 case-4 error suffices for the theoretical case. Closed without implementation.
- **Tier:** `v0.22+-ux`
- **Companion:** none.
...
- **Status:** `resolved` toolkit-**v0.47.4** (cited `bundle.rs:1478-1504` had drifted to `:2034`). `self_check_bundle` now validates `ms1` (added after the md1+mk1 checks): per-slot **emptiness parity** + **decode** + **entropy round-trip**, keyed off the source-entropy oracle `resolved_slots[i].entropy.is_some()` (NOT `args.slot` — R0 C1 caught that the supplied-subkey oracle would false-reject import-json envelopes + wif slots). Added `entropy_bearing`/`expected_entropy: &[Option<&[u8]>]` param threaded from all 4 call sites. Catches the @0-only emission reversion + corrupted/wrong-entropy ms1. R0 GREEN over 2 rounds (C1 oracle pivot + the discriminating G-A/G-B guards). Audit trail: `design/SPEC_self_check_ms1_iteration.md` + `design/agent-reports/self-check-ms1-iteration-*`.
...
- **Status:** `resolved` (technical-manual symbol-pin + lint cycle, 2026-06-07; docs-only, no version bump/tag — binary byte-identical). **Replaced ALL ~826 `file.rs:N` line-number citations across the 14 rendered `src/` chapters with drift-resistant symbol anchors `` `file.rs::symbol` `` (methods `T::method`, test fns `tests::fn`, re-export/line-content refs bare `file.rs`), and added a BLOCKING `make lint` gate** (`docs/technical-manual/tests/symbol-ref-check.py`, step 7/7) enforcing G1 (line-ref ban, incl. mermaid/fences, `<!-- lint-allow-lineref -->` hatch) + G2 (multi-segment symbol existence) + a narrowed colliding-basename rule (repo-qualify in non-authoritative chapters). The three named residual classes all fixed by intent (chromium IS available → mermaid node regenerated; `chunk_set_id` fmt → `bundle.rs::build_unified_card`; `verify_bundle` helpers → `emit_multisig_checks`/`MappingFailure`/`emit_md1_checks`). R0 4 rounds → GREEN; Phase-2 review 0C/0I. Method: 26-agent evidence-grounded audit (locate-in-source, NOT a mechanical line-swap — a blind sweep would cement drifted refs as wrong-but-existing symbols). Audit trail: `design/SPEC_technical_manual_symbol_pin_lint.md` + `design/agent-reports/technical-manual-symbol-pin-lint-r0-round{1,2,3,4}-review.md` + `…-phase2-review.md`. Deferred residuals → `technical-manual-ci-workflow-source-checkout` + `technical-manual-transcript-lineref-staleness`.
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `technical-manual-ci-workflow-source-checkout` — technical-manual has NO CI workflow; `make lint` (incl. the new symbol-ref gate) runs only locally
- **Surfaced:** 2026-06-07, technical-manual symbol-pin + lint cycle (R0 §4.6; deferred).
- **Where:** `.github/workflows/` has no `technical-manual.yml` (unlike `manual.yml`/`manual-gui.yml`/`quickstart.yml`). The new `symbol-ref-check` gate + the other 6 lint checks + `figures-cache-verify` are enforced only when a human runs `make -C docs/technical-manual lint`.
- **What:** add a `technical-manual.yml` CI workflow that runs `make lint`. Wrinkle: `symbol-ref-check` resolves `file.rs::symbol` against the **sibling codec source trees** (md/mk/ms-codec + clis), so CI must check out all four repos at coherent SHAs (heavier than the existing manuals' binary-`cargo install` CI). The gate degrades gracefully (skips, does not fail) when siblings are absent, so a naive single-repo CI would under-enforce SILENTLY — design the multi-repo checkout deliberately. Until then the gate is `make lint`-gated only (acceptable per the resolved parent's "CI-gated OR make lint-gated" wording).
- **Status:** `resolved` (technical-manual CI + transcript-cleanup cycle, 2026-06-07; docs+CI-only, no version bump/tag — binary byte-identical). Added `.github/workflows/technical-manual.yml` running `make lint` on push/PR under `docs/technical-manual/**` + `docs/tools/render-mermaid-cache.py` + the workflow file (no tag trigger — the technical manual ships no release asset; `manual.yml` owns `manual-v*`). **Deliberately chose option (A) single-repo, lint-only over the "multi-repo checkout" wrinkle:** the codecs are independently crates.io-versioned, so a 4-repo source checkout would couple this CI to live codec drift; and `make lint` is binary-independent (markdownlint/cspell/lychee scan source text, api-surface-coverage is warning-only + reads `lib.rs`/`format.rs`, figures-cache-verify is render-free) so CI needs only node+markdownlint-cli2+cspell+lychee+python3 — no cargo, no chromium. The regression-prevention prize is CI-gated **on changes under `docs/technical-manual/**`** (+ the cache tool + the workflow file): **G1 (line-ref ban) on all 14 chapters + G2 (symbol existence) for every toolkit ref.** That covers the leading case — a chapter edit that reintroduces a `file.rs:N` or a bad anchor fires the docs path → G1/G2 catch it. Two CI-coverage caveats remain, both filed below as `technical-manual-g2-uncovered-in-bare-ci`: (1) **skip-logic** — siblings-absent bare CI skips codec G2 + renamed-toolkit-by-bare-basename refs in non-auth chapters; (2) **trigger-path** — the workflow does NOT fire on `crates/**`, so a toolkit-source rename that breaks a citation WITHOUT a docs change won't fire CI until the next docs edit (caught meanwhile by local `make lint`). Adding `crates/**` to the trigger is deliberately declined (it would run the docs lint on every code PR; `manual.yml` declines it too). The "naive single-repo under-enforces SILENTLY" concern is addressed by (i) fixing the graceful-skip hole that previously FALSE-FAILED bare CI on codec refs in non-authoritative chapters (`symbol-ref-check.py` `resolve()` now returns `skip:absent-sibling` for `not auth and not qualified and ABSENT`; warning reworded to name the gap) and (ii) the gap FOLLOWUP below making both residuals explicit. Proven on a faithful siblings-absent `make lint` (298 checked / 427 skipped, all 7 steps green) + planted-failure G1/prize checks. Audit trail: `design/SPEC_technical_manual_ci_and_transcript_cleanup.md` + `design/agent-reports/technical-manual-ci-and-transcript-cleanup-r0-round{1,2,3}-review.md`.
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `technical-manual-g2-uncovered-in-bare-ci` — single-repo CI under-enforces symbol-ref G2 in two distinct ways (skip-logic + trigger-path)
- **Surfaced:** 2026-06-07, technical-manual CI + transcript-cleanup cycle (the deliberate option-A CI scope decision; R0-r1 M1 / R0-r2 I1 broadened the skip-logic framing; the trigger-path mechanism surfaced at the task-complete advisor checkpoint).
- **Where:** `docs/technical-manual/tests/symbol-ref-check.py` `resolve()` (skip-logic) + `.github/workflows/technical-manual.yml` `on.*.paths` (trigger-path). The CI is single-repo (toolkit only, no sibling checkout).
- **What — two distinct CI-coverage gaps, both caught meanwhile by local `make lint`:**
  - **(1) Skip-logic (siblings absent).** With siblings absent, **any** bare-basename ref in a non-authoritative chapter (10/14/30s/60s) whose file is unresolvable in the present (toolkit) repo is CI-skipped — the `skip:absent-sibling` arm (`not auth and not qualified and ABSENT`) + the qualified-`crates/<codec>/` `skip:<repo>` arm. That set is two-part: (i) codec-file G2 (the common case — md/mk/ms-codec + cli files cited from non-auth chapters), AND (ii) a **renamed toolkit file** cited by bare basename from a non-authoritative chapter (its file is unresolvable so it skips, even though it is a toolkit-intended ref). Locally all repos are present → `ABSENT == []` → the skip branch never fires → these ARE checked.
  - **(2) Trigger-path (toolkit-source rename without a docs edit).** The workflow fires on `docs/technical-manual/**` + `docs/tools/render-mermaid-cache.py` + the workflow file — NOT on `crates/**`. So a PR that renames/moves a toolkit symbol the manual cites, without touching any doc, won't run this workflow; the now-broken `file.rs::symbol` citation ships uncaught until the next change under the docs path (or a local `make lint`). This is a *different mechanism* from (1): the gate logic is fine; the workflow just isn't triggered. Adding `crates/**` to the trigger is deliberately declined here (it would run the full docs lint on every unrelated code PR; `manual.yml` declines the symmetric coupling).
- G1 (line-ref ban) on all 14 chapters + toolkit-ref G2 in authoritative chapters remain CI-gated *for changes under the docs trigger path*, so the leading regression (a chapter edit reintroducing a `file.rs:N`) is covered. To close gap (1): either (a) a deliberate multi-repo source checkout (the original `technical-manual-ci-workflow-source-checkout` wrinkle — heavy, couples to codec drift) or (b) commit a generated codec-basename manifest the gate consults so an absent-sibling skip can be distinguished from a genuine typo in bare CI. To close gap (2): add `crates/**` (or a curated subset) to the trigger, accepting docs-lint runs on code PRs. Lean: leave both as-is (local `make lint` is the full gate; CI is the line-ref + docs-edit backstop) unless drift recurs.
- **Status:** `resolved` for the **toolkit** half (technical-manual G2-bare-CI cycle, 2026-06-07; docs+CI-only, no version bump/tag — binary byte-identical). **Toolkit-G2 is now airtight in bare CI:** (gap 2) the workflow trigger gained `crates/mnemonic-toolkit/{src,tests}/**` so a toolkit-symbol rename re-runs `symbol-ref-check`; (gap 1 part ii) the 12 non-auth (all `61-glossary.md`) toolkit refs were **fully `crates/mnemonic-toolkit/src/...`-qualified** (only that form is rename-safe — a subpath/bare ref resolves today but `skip`s after a rename in bare CI) AND a hard-FAIL gate guard `not auth and repos_with == ["toolkit"] → "unqualified-toolkit"` prevents reintroduction. **Decision key:** bare CI can fully check toolkit-G2 (toolkit source IS the repo) but never codec-G2 (needs absent siblings). **Gap 1 part i (codec-G2) is the explicitly-ACCEPTED residual** — a basename manifest proves file-existence not symbol-existence (no true G2 + regen tax), a crates.io download couples to *published* codec versions that lag the cited dev symbols, and the multi-repo checkout was deliberately rejected (codec-drift coupling); full codec-G2 stays local-`make lint`-only. Proven: pre-qualify the guard REDs exactly the 12 (siblings absent); post-qualify toolkit-only GREEN (298/427, addr-derivation codec refs still skip); siblings-present `make lint` GREEN (725/0); bare-CI `make lint` GREEN; **planted-rename of `cmd/bundle.rs` → the qualified glossary ref FAILs (not skips) in bare CI**; actionlint OK. R0 2 rounds → GREEN. Audit trail: `design/SPEC_technical_manual_g2_bare_ci_fix.md` + `design/agent-reports/technical-manual-g2-bare-ci-fix-r0-round{1,2}-review.md` + `…-architect-blueprint.md`.
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `technical-manual-codec-g2-not-enforceable-in-single-repo-ci` — bare-CI `make lint` cannot enforce codec-symbol G2 (accepted wontfix)
- **Surfaced:** 2026-06-07, technical-manual G2-bare-CI cycle (the accepted residual of the resolved `technical-manual-g2-uncovered-in-bare-ci`; lifted here so it stays discoverable rather than buried in a resolved entry).
- **Where:** `.github/workflows/technical-manual.yml` runs single-repo (toolkit-only checkout); `symbol-ref-check.py` G2 resolves codec `file.rs::symbol` against the sibling codec source trees, which are absent in CI → those refs `skip:absent-sibling` (~59 non-auth + the codec-authoritative chapters).
- **What / why wontfix:** symbol-level codec-G2 fundamentally needs the codec *source*. A multi-repo source checkout was deliberately rejected (couples docs CI to independently-versioned codec drift). A committed basename manifest proves file-existence, not symbol-existence (no true G2) and drifts on every codec rename. A crates.io source-download couples to *published* codec versions that lag the dev symbols the manual cites vs `origin/master`. All net-negative. **Full codec-G2 gate = local `make lint` with all sibling repos present** (every contributor runs it). Revisit only if codec-citation drift actually recurs in shipped docs.
- **Status:** `open` (accepted wontfix tracker)
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `technical-manual-glossary-timestamparg-default-prose-stale` — `61-glossary.md` `TimestampArg` entry describes the pre-v0.47.3 default `now`
- **Surfaced:** 2026-06-07, technical-manual G2-bare-CI cycle (R0-r1 M2; the path-qualification cycle edited the citation on this line but, per scope, did NOT re-validate the prose).
- **Where:** `docs/technical-manual/src/60-back-matter/61-glossary.md:383-385` — the `TimestampArg` glossary entry describes `Now (renders to "now")` / `Unix(i64)` with `default now`. Source is now `pub timestamp: TimestampArgValue` (`crates/mnemonic-toolkit/src/cmd/export_wallet.rs::ExportWalletArgs::timestamp`) with **default `0`/genesis-rescan** since v0.47.3 (the timestamp-default-zero cycle); the enum is `TimestampArgValue` (a `crates/mnemonic-toolkit/src/wallet_export/mod.rs` type), not `TimestampArg`. **G2 is unaffected** (both `ExportWalletArgs` and `timestamp` segments exist; the qualified citation passes) — this is a prose-claim staleness, not a citation drift.
- **What:** reword the entry to the current default (`0`/genesis-rescan). Prose-only; the symbol-pin/qualification cycles pin *locations*, not *claims* (claim re-validation is the api-harvest class). Trivial.
- **Status:** `resolved` (CI node-version + glossary-prose cycle, 2026-06-07; docs+CI-only, no version bump/tag). `61-glossary.md:385` `default now` → `default 0 — rescan from genesis; now and unix seconds also accepted`. **CYCLE-PREP CORRECTION (the "What/Where" above was itself a self-mis-cite, caught by cycle-prep before applying):** only the default was stale — the `crates/mnemonic-toolkit/src/wallet_export/mod.rs::TimestampArg` citation is ACCURATE and was KEPT (NOT renamed). `TimestampArg` IS still the `pub(crate) enum` (`Now`/`Unix(i64)`) at `wallet_export/mod.rs:144`; `TimestampArgValue` is a SEPARATE newtype struct in a DIFFERENT file (`cmd/export_wallet.rs:297`), so the FOLLOWUP's "the enum is TimestampArgValue, not TimestampArg" claim was wrong — renaming would have pointed at the wrong file and self-RED'd G2. Audit trail: `design/SPEC_ci_node_version_and_glossary_prose.md` + `design/agent-reports/ci-node-version-glossary-prose-r0-round{1,2}-review.md`.
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `ci-actions-node20-runtime-major-bump-deferred` — bump JS-action majors `@v4→@v5` (Node-24 runtime) across all workflows
- **Surfaced:** 2026-06-07, CI node-version + glossary-prose cycle (the technical-manual CI run emitted the GitHub Node-20-deprecation annotation; the user deliberately DEFERRED the action-major migration this cycle, doing only the `node-version` input bump).
- **Where:** all 7 workflows under `.github/workflows/` (`rust`, `manual`, `manual-gui`, `quickstart`, `technical-manual`, `install-pin-check`, `sibling-pin-check`) pin JavaScript actions at `@v4`: `actions/checkout@v4`, `actions/setup-node@v4`, `actions/upload-artifact@v4`, and `actions/download-artifact@v4` (`manual-gui.yml:192,:198`). These declare the Node-20 runtime.
- **What:** bump the action majors `@v4→@v5` (checkout/setup-node/upload-artifact/download-artifact — all have v5 releases targeting Node 24). **This is the ACTION RUNTIME, distinct from the `node-version` input (already bumped 20→22 this cycle for the lint tools).** Verification cost: each workflow must be re-run green (`upload-artifact`/`download-artifact` v4→v5 + `setup-node` v5 have had behavior changes — read the release notes, do NOT blind-sed). Pin to the then-current major at implementation time.
- **Why deferred / NEAR-TERM deadline:** GitHub forces `@v4` actions onto Node 24 on **2026-06-16** ("may not work as expected") and removes Node 20 from runners **2026-09-16**. **Revisit before 2026-06-16** (9 days from filing).
- **Status:** `resolved` (CI action-major `@v5` bump cycle, 2026-06-07; CI-only, no version bump/tag). Bumped **25 sites** (`checkout`×15, `setup-node`×4, `upload-artifact`×4, `download-artifact`×2) `@v4→@v5` across all 7 workflows (`dtolnay/rust-toolchain` out of scope — composite/shell, not Node-JS). **DECISION: pinned `@v5` (the minimal Node-24 major), NOT latest** — live majors are checkout v6.0.3 / setup-node v6.4.0 / upload-artifact v7.0.1 / download-artifact v8.0.1, and that multi-major jump carries un-PR-testable risk (download-artifact v8 stops auto-decompressing + hash-errors; checkout v6 moves git creds to `$RUNNER_TEMP`) concentrated in `manual-gui`'s tag-gated release job. Every v5 delta verified inapplicable here (no `package.json` → no setup-node auto-cache; manual-gui downloads by-name → download-artifact by-ID change N/A; checkout v5 keeps creds in workspace → gh-pages `git push` untouched). Shipped on a **PR** (pre-merge validation: checkout+setup-node+upload-artifact@v5 went green on rust/manual-gui/technical-manual/sibling-pin-check before merge); `git revert`-ready. The v6/v7/v8 catch-up → `ci-actions-catch-up-to-latest-majors`. Audit trail: `design/SPEC_ci_actions_v5_bump.md` + `design/agent-reports/ci-actions-v5-bump-r0-round{1,2}-review.md`.
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `ci-actions-catch-up-to-latest-majors` — bump JS-actions `@v5 → v6/v6/v7/v8` (catch up to latest), AFTER v5 is confirmed stable
- **Surfaced:** 2026-06-07, CI action-major `@v5` bump cycle (the `@v5` bump cleared the Node-20 deprecation; latest majors deferred per user direction — "investigate v6/v7/v8 if v5 goes ok").
- **Where:** all 7 workflows (now at `@v5`). Live latest: `checkout v6.0.3`, `setup-node v6.4.0`, `upload-artifact v7.0.1`, `download-artifact v8.0.1`.
- **What / must-vet (per-major, NOT a blind sed):**
  - **download-artifact v8** — **breaking: stops auto-decompressing downloaded files + hash-mismatch now ERRORS (was warn).** `manual-gui`'s release job downloads the PDF/HTML **by name** and attaches them to the release + publishes HTML to gh-pages — a decompression-behavior change could ship a `.zip` instead of the file. **This job is TAG-gated → can't be PR-tested → REHEARSE via a throwaway `manual-gui-vTEST` tag (or fork) before trusting it.**
...
- **Status:** `resolved` — 2026-06-17, PR #57 (D1-B; CI-only, NO-BUMP, merged `8ed21d5`). Precondition fully met first: this cycle's `mnemonic-toolkit-v0.58.0`/`v0.58.1` ran the tag-gated v5 jobs (`install-pin-check`/`changelog-check`/`bitcoind-differential`/`fuzz-smoke`/`sibling-pin-check`) GREEN, closing the last `install-pin-check@v5` residual. Bumped all majors: **21 `checkout@v5→v6`, 4 `setup-node@v5→v6`, 6 `upload-artifact@v5→v7`, 2 `download-artifact@v5→v8`** across all workflows. **Validation strategy (user-decided "merge on PR + matched-pair analysis"):** PR #57 ran GREEN across ALL PR-testable jobs (rust matrix macos/ubuntu/windows/release, build, clippy, fmt, fuzz, miri, bitcoind-differential, cross-tool, g6, lint) — validating `checkout@v6`/`setup-node@v6`/`upload-artifact@v7` live. The ONE un-PR-testable surface — `manual-gui.yml`'s TAG-gated release job (`download-artifact@v8` by-name + `checkout@v6` gh-pages push) — was **THROWAWAY-TAG-REHEARSED 2026-06-17 and CONFIRMED GREEN**: pushed `manual-gui-v0.0.0-rehearsal`, the full release job passed (Download PDF + Download HTML via `download-artifact@v8` ✓, Upload PDF asset ✓, gh-pages publish via `checkout@v6` ✓ — by-name download + raw gh-pages `git push` both work under v8/v6). Cleaned up: deleted the throwaway release + tag; reverted the gh-pages commit (its only diff was the "Built from commit X on DATE" provenance footer — zero doc-content change). So `download-artifact@v8`/`checkout@v6` are validated on EVERY surface incl. the tag-gated release. **SHA-pinning eval: deferred** (the repo continues floating majors; supply-chain hardening is a separate future call). The D1-A self-trigger sub-item was already shipped (2026-06-07).
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.

### `technical-manual-transcript-lineref-staleness` — the 4 tracked `transcripts/api-harvest-*.md` still carry stale `file.rs:N` line-refs (out of the symbol-pin cycle's `src/`-only scope)
- **Surfaced:** 2026-06-07, technical-manual symbol-pin + lint cycle (R0 I2; scoped out).
- **Where:** `docs/technical-manual/transcripts/api-harvest-{md,mk,ms,mnemonic-toolkit}-codec.md` (~385 `file.rs:N` refs). These are **unrendered authoring scaffolding** (the Makefile globs `src/` only; the harvest snapshots are pinned to old crate versions, e.g. `Version | 0.8.0`); the symbol-pin gate scopes to `src/` and a mechanical migration of the transcripts is provably wrong (e.g. `api-harvest-mnemonic-toolkit.md:466` "BundleJson doc-comment at `format.rs:114`" resolves to `MultisigInfo`). Same class as the deleted cli-help goldens (`project_cli_help_golden_cleanup_shipped`).
- **What:** decide whether to (a) symbol-pin the transcripts too (extend the gate scope + harness plumbing for `TRANSCRIPTS`), or (b) delete them as superseded scaffolding (the rendered Part V chapters are now the authoritative, gated API surface), mirroring the cli-help cleanup. Lean (b).
- **Status:** `resolved` via **option (b) — deleted** (technical-manual CI + transcript-cleanup cycle, 2026-06-07; docs+CI-only, no version bump/tag). `git rm`'d all four `transcripts/api-harvest-{md,mk,ms,mnemonic-toolkit}-codec.md` (1655 lines, ~385 stale `.rs:N` refs). They are unrendered authoring scaffolding (the Makefile globs `src/` only; markdownlint/cspell/lychee never scan `transcripts/`), never `{{#include}}`'d, not consumed by `verify-examples.sh` (which replays the `.cmd`/`.out` pairs — those are KEPT), version-stale (`Version | 0.8.0`), and provably wrong to mechanically migrate; the rendered, symbol-pinned, lint-gated Part V chapters are the authoritative API surface. Link-integrity proven post-delete (`lychee --offline` 547 OK / 0 Errors — no `src/` file linked into them). Same class as the deleted cli-help goldens (`project_cli_help_golden_cleanup_shipped`). Audit trail: `design/SPEC_technical_manual_ci_and_transcript_cleanup.md`.
- **Tier:** `v0.22+-doc-hygiene`
- **Companion:** none.
...
- **Status:** `resolved cefffcc63e97573de78ed3c34d335a0b435cc338` — manual-v0.2.1 cycle landed real `md` binary install step in `manual.yml` mirroring the mk-cli pattern at L72-77; flag-coverage gate now exercises `md <subcommand> --help` against the cargo-installed binary. RE-VERIFIED 2026-06-22 (v0.70.1 cycle, open-followups maturity program): live `.github/workflows/manual.yml` `cargo install … md-cli` + `make audit MD_BIN=md` — already-resolved, status unchanged.
...
- **Status:** `resolved cefffcc63e97573de78ed3c34d335a0b435cc338` — manual-v0.2.1 cycle landed real `ms` binary install step in `manual.yml` mirroring the mk-cli pattern at L72-77; flag-coverage gate now exercises `ms <subcommand> --help` against the cargo-installed binary. RE-VERIFIED 2026-06-22 (v0.70.1 cycle, open-followups maturity program): live `.github/workflows/manual.yml` `cargo install … ms-cli` + `make audit MS_BIN=ms` — already-resolved, status unchanged.
- **Tier:** `v0.28+-ci-hygiene`
...
- **Status:** **resolved** — option (c) [document] shipped v0.34.3; option (b) [per-consumer `--json` wire-shape regression tests on the GUI side] **shipped 2026-06-22 in `mnemonic-gui-v0.49.0`** (Wave-3 W3-1): `mnemonic-gui/tests/wire_shape_snapshot.rs` — 8 live-capture golden cells pinning the wire SHAPE (top-level + nested key sets, the match-vs-no_match conditional-key drift class, stable scalars) for **`xpub-search`** (path/account/passphrase) + **`import-wallet`** (bsms-multisig + coldcard single-sig), captured FROM the `mnemonic-toolkit-v0.70.0` pinned binary; the stale loose `cli_envelope_smoke.rs` + v0.27.0 fixtures were retired. **`export-wallet` DESCOPED** (it has no `--json` envelope — its wire-shape is per-`--format` file output; tracked as a follow-on `export-wallet-format-file-json-wire-shape-regression`). Maintenance contract: future toolkit-pin bumps re-verify the 8 goldens in lockstep (the leading drift gate). Narrowed 2026-05-22 (SHA `9b94a7d`).
- **Tier:** `v0.29+`
- **Tags:** `wallet`
...
- **Status:** `resolved` mnemonic-gui-**v0.28.0** (`c440e91`, tag `mnemonic-gui-v0.28.0`, 2026-06-06). The GUI bumped its toolkit pin v0.46.2 → v0.47.3 and applied the **MINIMAL** fix: `mnemonic-gui/src/schema/mnemonic.rs` `--timestamp` `default_value "now" → "0"` (+ help + `pinned_version` banner + module-doc header). **No `is_at_default` change** — `widget.rs::default_flag_value_for_flag` seeds `FlagKind::Timestamp → FlagValue::Unset`, so the default form emits no `--timestamp` (toolkit applies `0`); an explicit `Now` now emits `--timestamp now`; `Unix(n)` always emits. Regression guards: `tests/argv_assembler.rs::{d33_timestamp_now_is_emitted_when_default_is_zero, cell_3b_export_wallet_timestamp_now_argv}` (inverted RED→GREEN). The cited `45-export-wallet.md:422` manual-gui ref was **wrong** (no such timestamp example at that line; the stale `now`-default PROSE is at `45-export-wallet.md:30,340-343` in THIS toolkit repo) → tracked by `manual-gui-export-wallet-timestamp-default-now-stale`. Audit trail: `mnemonic-gui/design/SPEC_gui_v0_28_0_pin_bump_v0_47_3.md` + `mnemonic-gui/design/agent-reports/gui-v0_28_0-pin-bump-*`.
- **Tier:** `cross-repo`
- **Tags:** `gui`
- **Companion:** `mnemonic-gui/FOLLOWUPS.md::gui-timestamp-default-value-drift-v0.47.3` (resolved mnemonic-gui-v0.28.0; the placeholder branch was superseded + deleted, the resolved entry is on GUI master).

### `manual-gui-export-wallet-timestamp-default-now-stale` — `docs/manual-gui/` export-wallet `--timestamp` prose still says default `now` (toolkit v0.47.3 made it `0`)
- **Surfaced:** 2026-06-06, mnemonic-gui v0.28.0 R0 (the architect's "GUI repo has no `docs/`" observation traced the stale prose to THIS toolkit repo's `docs/manual-gui/`).
- **Where (corrected by cycle-prep recon — THREE sites, not two):** `docs/manual-gui/src/40-mnemonic/45-export-wallet.md:30` (flag-list summary "(default `now`)") + `:342-346` (the `--timestamp` section prose "`now` (the default …)") + **`:422`** (a worked `importdescriptors` JSON example block, `:415-426`, containing `"timestamp": "now"`). **The original filing said the v0.47.3 R0 M2's `:422` ref "was wrong (no such timestamp example)" — that was itself WRONG:** `:422` IS a real stale site; the error came from the GUI v0.28.0 recon grepping the `mnemonic-gui` repo (which has no `docs/`) instead of the toolkit repo where `docs/manual-gui/` lives. The `expected_gui_schema_inventory.json` `--timestamp` entry has NO `default_value` field → NOT stale, NOT gated.
- **What:** toolkit v0.47.3 (`export-wallet-timestamp-default-zero`) flipped `export-wallet --timestamp`'s default `now → 0` (genesis rescan). The GUI user manual still documented the old `now` default.
- **Status:** `resolved` (manual-gui-timestamp-default-prose cycle, 2026-06-06; toolkit `master`, docs-only — NO version bump/tag, cli-help-cleanup `a83dc75` + anchor-dangler `dd7c228` precedent). Reworded all THREE sites to lead with the `0` default (mirroring the toolkit manual's v0.47.3 `41-mnemonic.md:707` + `37-wallet-export.md` wording); `:422` JSON example → `"timestamp": 0` (a JSON number). Impl-phase fold: added `rescan` to `docs/manual-gui/.cspell.json` (the manual-gui cspell flagged it; the toolkit `docs/manual/` cspell accepts it via a different dict). Verified `make -C docs/manual-gui lint` GREEN across all 7 phases with the CI-faithful `MANUAL_GUI_UPSTREAM_ROOT` (GUI v0.3.0 pinned clone — `docs/manual-gui/pinned-upstream.toml`). **LESSON:** running the lint WITHOUT that env var defaults to the live working-copy GUI (now v0.28.0), whose schema is far ahead of the v0.3.0-pinned manual-gui content → spurious 391 gui-schema-coverage / 51 outline-coverage mismatches (a wrong-input artifact, not a defect). Audit trail: `design/SPEC_manual_gui_timestamp_default_prose.md` + `design/agent-reports/manual-gui-timestamp-default-prose-r0-round1-review.md` + `cycle-prep-recon-session-followups-closure-and-manual-gui-timestamp.md`.
- **Tier:** `docs`
- **Tags:** `gui`, `manual-gui`
...
- **Fix:** either (a) a dedicated docs-refresh cycle that regenerates ALL `cli-help/*.txt` goldens against the current binaries (capture with a wide/no-wrap width — clap emits unwrapped at COLUMNS≥~280), or (b) un-exclude `cli-help/` from `verify-examples` and add a regenerate target so the goldens become CI-gated (leading gate, not silent decay), or **(c) DELETE them** (chosen).
...
- **Status:** `resolved` (cli-help-cleanup cycle, 2026-06-06; docs-only, no version bump/tag — anchor-dangler `dd7c228` precedent). **Recon flipped the fix from (b) to (c) DELETE:** the 21 snapshots were authoring scaffolding from the manual-v0.1 build (`db183f3`), pinned to v0.8.0 (~40 versions stale), **NOT `{{#include}}`'d into any rendered chapter**, never gated (no `.cmd`), and SUPERSEDED by the hand-written `40-cli-reference/` chapters whose flag-NAME parity IS gated by the `flag-coverage` lint running live `--help`. Gating them (b) would be redundant + brittle (fires on every help-string reword). Deleted all 21 + reworded the two rendered troubleshooting notes (manual `67` + quickstart `52`, R0 I1 caught the 2nd) + tombstoned the moot `verify-examples` exclusion. Audit trail: `design/SPEC_cli_help_golden_cleanup.md` + `design/agent-reports/cli-help-golden-cleanup-r0-round{1,2}-review.md`.
- **Tier:** `docs`
- **Tags:** none
...
- **Status:** `resolved` toolkit-**v0.47.2** (quick-wins PATCH). The cited `convert.rs:460` had drifted; added a dedicated `refusal_electrum_phrase_to_address()` helper (`convert.rs:570-574`) returning `ConvertRefusal` (same exit 2 as the shared one-way barrier) and a `(ElectrumPhrase, Address)` arm in `classify_edge` (`:703-705`) placed BEFORE the one-way catch-all and AFTER the phrase↔electrum pivot arm. The message now states Electrum uses its own PBKDF2 salt + non-BIP-44 derivation and **redirects to the real path**: `mnemonic addresses --from electrum-phrase=<seed> --address-type <p2pkh|p2wpkh>` (the surface that landed in v0.47.0, `electrum-native-seed-address-derivation`). No other electrum edge changed; `classify_edge` gates before dispatch so the `:1615` `unreachable!` invariant holds. Test `electrum_phrase_to_address_is_refused` strengthened to assert the redirect text. Audit trail: `design/SPEC_quick_wins_v0_47_2.md` + `design/agent-reports/quick-wins-v0_47_2-*`.
- **Tier:** `v0.36+`
- **Tags:** none
...
- **Status:** `resolved` toolkit-**v0.47.0** (`05a1737` + `d31866b`). Surfaced as `mnemonic addresses --from electrum-phrase=` (NOT the convert `(ElectrumPhrase, Address)` edge — the user chose the richer batch surface). `electrum::electrum_seed_to_bip32_seed` (PBKDF2) + `normalize_text_electrum` + the `addresses.rs` `ElectrumPhrase` arm: standard → `m/<chain>/i` P2PKH, segwit → `m/0'/<chain>/i` P2WPKH; `--address-type` must match the seed version, `--account`/`--language` N/A, 2FA refused; watch-only out. Validated against all 3 Electrum `test_wallet_vertical.py` e2e vectors (incl. the `UNICODE_HORROR` passphrase). **Crypto catch:** Electrum's combining-mark strip is the CANONICAL COMBINING CLASS (`combining(c)!=0`), NOT the Mark general category — `is_combining_mark` wrongly strips ccc=0 marks (U+034F/U+0489); the UNICODE_HORROR vector caught it (used `canonical_combining_class`). No GUI schema_mirror (un-refused a runtime `--from` node); manual mirror landed. Audit trail: `design/SPEC_addresses_electrum_native_derivation.md` + `design/agent-reports/addresses-electrum-native-derivation-*`.
- **Tier:** `v0.36+`
- **Tags:** none
...
- **Status:** `resolved` toolkit-**v0.47.2** (quick-wins PATCH). Fix-the-class (R0 I1): the advisory covers BOTH `--ms1 <inline>` AND `--slot @N.phrase=<inline>` (`import_wallet.rs:288-295`), reading the RAW args BEFORE the env-resolution rebind (R0 I2) so an `@env:VAR` value never trips it; the `""` watch-only sentinel is also skipped. Alternative channel is `@env:VAR` (there is no `--ms1-stdin` on `import-wallet`, so the advisory text omits it). Coverage: positive cells `ms1_inline_value_fires_argv_advisory` + `slot_phrase_inline_fires_argv_advisory_with_actual_index`; negative (discriminating) cells `ms1_env_sentinel_no_argv_advisory` + `slot_phrase_env_sentinel_no_argv_advisory`. Additive stderr only. Audit trail: `design/SPEC_quick_wins_v0_47_2.md` + `design/agent-reports/quick-wins-v0_47_2-*`.
- **Tier:** `v0.36+`
- **Tags:** none
...
- **Options:**
  ...
  - Option (c) — CHOSEN: accept convention-only enforcement and keep the honest framing — the byte-parity test **anchors THIS repo's emitted line to this source file** (a within-repo drift guard); cross-repo wording parity is held by the paired-PR discipline, NOT byte-enforced by any automated gate. The tautological test is KEPT (it is a real within-repo guard), not deleted.
...
- **Status:** `resolved-as-documented` (2026-06-23, Wave-3-deferrals L3, NO-BUMP). Disposition: option (c) — the enforcement is **convention + paired-PR discipline only**, and that is now the documented contract. The toolkit's own `cli_output_class.rs` cells already anchor each emitted advisory line to real runtime stderr via `.contains()` (genuine within-repo guards, not tautologies — the tautological `byte_parity_advisory_lines` pattern lives in the sibling CLIs). The honest wording — "the byte-parity test anchors THIS repo's line; cross-repo wording is held by the paired-PR discipline, not byte-enforced" — replaces any "enforces cross-repo parity" claim. **The tautological sibling test is KEPT (do NOT delete);** it is a valid within-repo drift guard. No automated cross-repo gate is added (options a/b rejected as fragile across independent checkouts).
- **Tier:** `cross-repo` / `hardening`
- **Companion:** Affects the advisory-line constants ported into `mnemonic-key` (mk-cli), `descriptor-mnemonic` (md-cli), and `mnemonic-secret` (ms-cli) — each carries the same self-tautological `byte_parity_advisory_lines` pattern. Mirror FOLLOWUP entries are **not yet filed** in those repos (this toolkit entry is the canonical tracker). Cross-repo enforcement is convention + paired-PR discipline only — the documented status quo (resolved-as-documented).
...
- **Status:** `resolved` (2026-06-23, Wave-3-deferrals L4, CI-only NO-BUMP). The live stale instance was already fixed (Wave-3 bumped `docs/manual/src/40-cli-reference/44-mk-cli.md:12`'s prose pin in lockstep; live value is `mk-cli-v0.10.2`). **Resolution (2026-06-23):** extended `sibling-pin-check.yml` to scan `docs/manual/src/**` + `docs/quickstart/src/**` prose (was workflow-YAML-only) and re-keyed the canonical lookup on the `--git` repo-url (was pkg-name), which handles the prose `--tag <tag> --bin <bin>` form without a bin↔pkg map. Trial-runs verified: clean-tree exit 0 (now covers `44-mk-cli.md:12` → `OK … mk-cli-v0.10.2`); synthetic prose-pin drift (`v0.10.2`→`v0.9.0`) → `::error::sibling-pin-check: …44-mk-cli.md:12: pin 'mk-cli-v0.9.0' (url https://github.com/bg002h/mnemonic-key) does not match … 'mk-cli-v0.10.2'` + exit 1; tagless prose NOT flagged (no false positive); actionlint clean. The url-keyed table now also resolves the GUI arm (latent enablement for a future GUI drift-check; neutral today). (The sibling drift-gate's CI-only NO-BUMP landing mirrors the parent `manual-yml-sibling-pin-vs-install-sh-drift-gate`, already `resolved`.)
- **Tier:** `ci-hardening`
...
### `bip388-wallet-policy-to-descriptor-expansion-not-surfaced` — no user-facing inverse of `export-wallet --format bip388`; the policy→descriptor expander exists but is reachable only inside `xpub-search`

- **Surfaced:** 2026-06-08, in-session descriptor-capability exploration (user fed a concrete `wsh(andor(multi(3,…),older,andor(multi(2,…),older,and_v(pk,after))))` tiered-multisig vault and asked whether the toolkit can "accept a full wallet policy and emit a descriptor template/descriptor"). Empirically established against `mnemonic 0.48.0` (master `f84a545`).
- **Where:**
  - **Forward direction (works):** `export-wallet --descriptor <concrete> --format bip388` decomposes a concrete descriptor → BIP-388 wallet policy `{name, description_template (@N/**), keys_info[]}`. Emitter: `crates/mnemonic-toolkit/src/wallet_export/pipeline.rs:178-198` (build `keys_info`, substitute each key-expression with `@N/**`).
  - **The inverse expander ALREADY EXISTS** but is reachable only from `xpub-search`: `crates/mnemonic-toolkit/src/cmd/xpub_search/descriptor_intake.rs:199-222` `parse_bip388_json` reconstructs the concrete descriptor by replacing each `@N/**` with `keys_info[N] + "/<0;1>/*"` (explicitly "exact inverse of `wallet_export/pipeline.rs:192-198`"). Strict schema `BipPolicyJson` (`descriptor_intake.rs:83-90`, `deny_unknown_fields {name, description_template, keys_info}`); shape auto-detect (`detect_shape` `:158-179` — leading `{` → `Bip388Json`); explicit form `--descriptor-from bip388=<value>` (`:118-127`).
  - **The gap:** `xpub-search` *consumes* the expanded descriptor internally (to locate which cosigner role/account a seed plays) and **never echoes the concrete descriptor** — neither the text report nor the `--json` envelope surfaces it. AND `export-wallet --descriptor` / `bundle --descriptor` **refuse** the keyless `@N` template / policy shape: `crates/mnemonic-toolkit/src/cmd/export_wallet.rs:412-417` (`is_at_n_form` → `BadInput`: "accepts only concrete descriptors with inline keys; for keyless @N templates use --template <T> --slot @N.xpub=… or --from-import-json"). The only keyless intakes are the 8 **named** `--template` built-ins (not arbitrary miniscript templates) and `--from-import-json` (`export_wallet.rs:264-277`), which consumes the toolkit's OWN `import-wallet --json` envelope (`bundle.descriptor`/`bundle.mk1`), NOT a BIP-388 policy. `import-wallet` sniffs only vendor blobs (bitcoin-core/bsms/coldcard/coldcard-multisig/electrum/jade/sparrow/specter), not a bip388 policy JSON.
- **What:** A user holding a BIP-388 wallet policy (a vendor export — Sparrow/Nunchuk/Core `listdescriptors`-policy, etc. — or the toolkit's own `--format bip388` output) cannot round-trip it back to a concrete descriptor, nor emit watch-only artifacts / an m-format bundle from it, without manually substituting each `@N` with `keys_info[N]` + the `/<0;1>/*` suffix. The expansion logic is already written and lockstep-locked to the emitter; it is simply not surfaced outside `xpub-search`.
- **Options:**
  - (a) **Reuse `descriptor_intake::parse_bip388_json` in `export-wallet`/`bundle`:** let `--descriptor` auto-detect the leading-`{` BIP-388 policy shape (or add an explicit `--from-policy-json <FILE|->` / `--descriptor-from bip388=`) → expand → feed the normal concrete-descriptor pipeline. Closes the round-trip at the same surface that emits policies; one expander + one emitter, already in lockstep. (DEDUP-friendly; mirrors the resolved forward-direction entry `export-wallet-descriptor-bip388-interop`, FOLLOWUPS ~:1863.)
...
- **Status:** **resolved** toolkit-**v0.53.8** (2026-06-11) — the DIRECT one-step `export-wallet --descriptor <policy> --format bip388` now lifts the policy `name` (new `wallet_import::pipeline::bip388_policy_name`) into `EmitInputs.wallet_name` and the emit reads it (`descriptor_to_bip388_wallet_policy(desc, name)`); precedence `--wallet-name` > policy-name > "imported-descriptor". (Same bug as the audit-backlog slug `bip388-policy-name-lossy-roundtrip`, also resolved in v0.53.8.) The two-step `--format descriptor → bip388` path still drops the name (intermediate concrete step has no policy metadata — expected). Template path carved out as `bip388-template-path-wallet-name`. SPEC `design/SPEC_bip388_policy_name_roundtrip.md`.
- **Tier:** `v0.48+-feature`
- **Companion:** none (toolkit-local).
...
- **Status:** **Release A SHIPPED v0.50.0** (2026-06-09; `mnemonic build-descriptor`). Engine = the versioned JSON `PolicyNode` IR + render + `--spec-schema` (Phase 1) → 4-step validation gate + §3.4 node-addressed localization (Phase 2) → emit descriptor + bip388 + cost preview + the clap surface (Phase 3). SPEC `design/SPEC_descriptor_builder_engine.md` (R0 7 rounds; per-phase reviews persisted). **Two SPEC deviations folded with architect blessing:** the former step-4 per-branch `plan()` satisfiability was CUT (tautological for sane trees — `AnalysisError` defines unspendable-path as exactly resource-limits + timelock-mixing, both `sanity_check` rules); the archetype `degrading-threshold` (same-key) was replaced with `tiered-recovery` (distinct-key — same-key trips `RepeatedPubkeys`). **Release B SHIPPED v0.51.0** (2026-06-09; **resolved**). The 5 archetype presets over the frozen IR as one uniform `--archetype <name>` + generic param-flag surface (`--key`/`--threshold`/`--recovery-key`/`--recovery-threshold`/`--final-key`/`--older`/`--recovery-older`/`--after`/`--hash`, all `requires=archetype`) + `--emit-spec` (print the lowered gate-validated spec JSON) + kind-aware diagnostic provenance (`Diagnostic.flag`, `param` kind, `(from --flag)` human suffix) + the `--spec-schema` `archetypes` field-spec section (registry-generated). The brainstorm's "flag shorthand for the 2 flat archetypes" DISSOLVED into the uniform surface (deliberate over-delivery, recorded in the SPEC §1); `degrading-threshold` remains replaced by `tiered-recovery` (Release A F2). Producers are byte-pinned to the Release-A fixture canon at three layers (IR AST equality vs the fixture JSON, preset CLI goldens vs the same `.descriptor`/`.bip388` files, per-archetype mutated-param non-vacuity). SPEC `design/SPEC_descriptor_builder_presets.md` (R0 2 rounds → GREEN; per-phase reviews persisted to `design/agent-reports/descriptor-builder-presets-*`). Both schema versions stayed 1 (additive). GUI lockstep → `gui-build-descriptor-presets-pending-pin-bump` (below).
- **Status:** **resolved** (Release A `v0.50.0` + Release B `v0.51.0`). Remaining roadmap items live in their own entries: `descriptor-builder-allow-extparams-reviewed-optout` (v0.52+), the GUI wizard (companion line), `miniscript-compiler-optimize-policy` (md-cli track).
- **Tier:** `v0.51-feature` (Release B presets — shipped).
- **Companion:** `mnemonic-gui` — see `gui-build-descriptor-schema-mirror-pending-pin-bump` below (the schema-mirror lockstep). **The GUI wizard is COMPLETE** (`mnemonic-gui-v0.31.0` archetype forms, 2026-06-09; **`mnemonic-gui-v0.32.0` the recursive node-tree builder, 2026-06-10** — full 17-kind grammar authored node-by-node, Validate w/ this engine's node-addressed diagnostics highlighted in place, Edit-as-tree via `--emit-spec`, a live path-grammar parity gate against this gate's `child_paths`; nothing on the wizard roadmap remains deferred).

### `gui-build-descriptor-schema-mirror-pending-pin-bump` — paired mnemonic-gui PR: bump pin → v0.50.0 + add the `build-descriptor` SubcommandSchema

- **Surfaced:** 2026-06-09, descriptor-builder Release A ship (v0.50.0). The new `mnemonic build-descriptor` subcommand is a clap-surface addition → the CLAUDE.md GUI `schema_mirror` lockstep applies, BUT `schema_mirror` runs `<cli> build-descriptor --help` against the GUI's **pinned** toolkit binary — so the GUI schema cannot add `build-descriptor` until the pin is bumped to v0.50.0 (chicken-and-egg: the pin bump needs the toolkit tag, which now exists). Same cross-repo arc as `restore` → GUI v0.25.0.
- **Where:** `mnemonic-gui/src/schema/mnemonic.rs` — add `BUILD_DESCRIPTOR_FLAGS` + `SubcommandSchema { name: "build-descriptor", … }`; bump the toolkit pin v0.49.1 → v0.50.0 (Cargo.toml + lock + README pin markers + module-doc banner). The flag-NAME set the `schema_mirror` set-equality gate expects (from `mnemonic gui-schema`): **`--format`** (Dropdown `["descriptor","bip388"]`), **`--json`** (Boolean), **`--network`** (Dropdown `NETWORKS`), **`--no-auto-repair`** (the shared `NO_AUTO_REPAIR_FLAG`, `global:true`), **`--spec`** (Text), **`--spec-schema`** (Boolean). `positional_args: NO_POSITIONALS`, `conditional: None` (`--network` is optional/default-mainnet — deliberately NO `required_unless_present`, to avoid a conditional-rule projection). Bump the subcommand-floor / `EXPECTED_ARM_COUNT` if the GUI tracks one.
- **Status:** **resolved** `mnemonic-gui-v0.29.0` (2026-06-09, commit `c5ffca4`, tag `mnemonic-gui-v0.29.0`). The paired PR bumped the pin **v0.47.3 → v0.50.0** (this entry mis-said `v0.49.1` — the GUI lagged **5** releases) across **6** lockstep sites (incl. `pinned-upstream.toml:22 [mnemonic].tag`, which this entry omitted — it is the CI `schema-mirror.yml` install source) + added `BUILD_FORMATS` + `BUILD_DESCRIPTOR_FLAGS` + the `build-descriptor` `SubcommandSchema`. **Three citation errors corrected by the GUI SPEC** (`mnemonic-gui/design/SPEC_gui_v0_29_0_build_descriptor.md`): (1) pin was v0.47.3 not v0.49.1; (2) `--spec` is **`FlagKind::Path { stdio_sentinel: true }` NOT Text** — the toolkit `--spec` is a file path/`-`/stdin, never inline JSON, so a Text widget would emit raw JSON → ENOENT (the `--blob` precedent); (3) **no `EXPECTED_ARM_COUNT`** exists GUI-side (toolkit concept) and build-descriptor needs no `SUBCOMMAND_FLOORS` entry (`conditional:None`). Measured: **zero** accumulated flag drift v0.47.3→v0.50.0; `schema_mirror` now validates build-descriptor's 6 flags vs the v0.50.0 binary. R0 GREEN 0C/0I (1 round, empirically validated). Latent manual-gui anchor debt → `mnemonic-gui/FOLLOWUPS.md::manual-gui-build-descriptor-anchors-pending-pin-bump`.
- **Tier:** `gui-sync`.
- **Companion:** toolkit `descriptor-builder-engine` (this file); GUI `mnemonic-gui/FOLLOWUPS.md::manual-gui-build-descriptor-anchors-pending-pin-bump`.

### `gui-build-descriptor-presets-pending-pin-bump` — paired mnemonic-gui PR: bump pin → v0.52.0 + add the 12 build-descriptor flags to the `build-descriptor` SubcommandSchema

- **Surfaced:** 2026-06-09, descriptor-builder Release B ship (toolkit v0.51.0); **extended same day at v0.52.0** (+`--allow` → 12 new flags total; pin target → v0.52.0). `mnemonic build-descriptor` gained 11 clap flags (`--archetype` + 9 value params + `--emit-spec`) → the CLAUDE.md GUI `schema_mirror` flag-NAME lockstep applies, but the GUI schema cannot add them until its toolkit pin is bumped to v0.51.0 (same chicken-and-egg as `gui-build-descriptor-schema-mirror-pending-pin-bump`, resolved GUI v0.29.0 — the pin bump is the forced part; same 6 lockstep sites incl. `pinned-upstream.toml` `[mnemonic].tag` + the 2 ungated banner/module-doc sites).
- **Where:** `mnemonic-gui/src/schema/mnemonic.rs` — extend `BUILD_DESCRIPTOR_FLAGS` with: `--allow` (**Dropdown** `["malleable","mixed-timelock","repeated-keys","resource-limit","sigless-branch"]`, **repeating: true** — NOTE: a repeating Dropdown is a FlagKind combination the GUI schema has not used before; current repeats are Text), `--archetype` (**Dropdown** `["decaying-multisig","hashlock-gated","kofn-recovery","simple-timelocked-inheritance","tiered-recovery"]` — alphabetical, == `CliArchetype` order), `--key` + `--recovery-key` (**Text, repeating: true** — xpub strings, NOT Path; argv order is load-bearing), `--threshold` + `--recovery-threshold` + `--older` + `--recovery-older` + `--after` (**Number**), `--final-key` + `--hash` (**Text**), `--emit-spec` (**Boolean**). `schema_mirror` measured-drift discipline: measure with a local v0.52.0 binary via `MNEMONIC_BIN` BEFORE the add (the gate is flag-NAME set-equality vs the pinned binary's `gui-schema`).
- **Un-gated surfaces the GUI must be TOLD about (not assumed — presets SPEC §3.3/§8 + allow SPEC §3/§6):** (1) `--json` wire-shape additions: `Diagnostic` gains optional `flag` (skip-serialized when absent — spec-mode output byte-unchanged), new diagnostic kind `param`, `node_path: "params"` sentinel for producer param errors; (1b) **v0.52.0:** the success envelope gains `allowed_rules_fired` (only when non-empty) and `cost` is `null` on a sanity-overridden emit; step-3 refusal messages carry a `; rerun with --allow <kebab> after review` suffix; (2) the `--spec-schema` JSON gains an `archetypes` section (per-preset field-specs `{flag, kind, required, repeatable, min}` — the surface a future GUI archetype-FORMS wizard consumes); (3) deliberately UN-projected clap rules (`SubcommandSchema.conditional` stays `None`): the `--archetype`↔`--spec` mutex, the 10 `requires = "archetype"` edges, `--emit-spec` `conflicts_with_all = [--format, --json]` — GUI forms could emit argv clap refuses; acceptable since the CLI is the gate, but it must be a recorded decision in the GUI cycle (compare-cost mutexes ARE hand-projected; precedent either way).
- **Status:** **resolved** `mnemonic-gui-v0.30.0` (2026-06-09, GUI `93902b9` + tag). Pin v0.50.0 → v0.52.0; 18-flag `BUILD_DESCRIPTOR_FLAGS` (incl. `--archetype` w/ the `""` UNSET sentinel + `--allow` repeating Dropdown — NOTE this entry's "repeating Dropdown not used before" claim was WRONG: `--to` was the live precedent); generic repeating-row widget (non-secret) — without it 3 of 5 archetypes (min-2 `--key`) were un-drivable; archetype↔spec conditional mutex. GUI-side spawned `repeating-secret-flags-never-reach-argv` (pre-existing live bug: secret+repeating+TEXT flags emit nothing — `--ms1` ×2 + `--share` ×2; seed-xor `--share` is NodeValueComposite and unaffected). SPEC `mnemonic-gui/design/SPEC_gui_v0_30_0_presets_pin_bump.md` (R0 3 rounds; impl review folded).
- **Tier:** `gui-sync`.
- **Companion:** `mnemonic-gui/FOLLOWUPS.md::gui-build-descriptor-presets-pending-pin-bump` (filed, GUI master 5536933); relates to `descriptor-builder-engine` (resolved) + the eventual GUI archetype-forms wizard.

### `descriptor-builder-allow-extparams-reviewed-optout` — `--allow <variant>` reviewed sanity opt-out via `from_str_ext(ExtParams)`

- **Surfaced:** 2026-06-09, descriptor-builder Release A (SPEC §3.5). Release A ships NO `--allow` flag (the funds-footgun gate is unconditional); a deliberately "insane" policy (a same-key degrading threshold → `RepeatedPubkeys`, or an intentional cross-branch timelock mix) takes the raw `--descriptor` door.
- **Where:** a future `mnemonic build-descriptor --allow <variant>` maps each variant to the matching `ExtParams` toggle (`top_unsafe`/`malleability`/`resource_limitations`/`repeated_pk`/`timelock_mixing`; miniscript `analyzable.rs:28-121`) applied at the **parse stage** via `Miniscript::from_str_ext(&s, ExtParams)` — NOT at `sanity_check` (param-less). New clap flag → GUI `schema_mirror` + manual lockstep.
- **Status:** **resolved** v0.52.0 (2026-06-09). `--allow {malleable, mixed-timelock, repeated-keys, resource-limit, sigless-branch}` (repeatable; kebab names == `DiagnosticKind::as_str`; `raw_pkh` deliberately not exposed — IR-unreachable). CORRECTED MECHANISM (recon overturned this entry's "parse stage — NOT sanity_check" guidance, written pre-Release-A): the shipped gate already parses `from_str_ext(insane)`; the fix was the one-line `sanity_check()` → `ext_check(&AllowSet::to_ext_params())` swap + fired-vs-requested detection via the per-rule predicates. Never silent: unconditional stderr WARNING on fired (all modes), "did not fire" note, refusal hints name the exact token, `--json` `allowed_rules_fired`. Cost preview deterministically SKIPPED on a sanity-overridden emit (R0 C1 — the Tap re-parse re-runs waived rules): `cost: null` / human one-liner. `--emit-spec` records no allowance. Composes with `--spec` AND presets (the cut same-key degrading-threshold is the flagship: `--allow repeated-keys`). SPEC `design/SPEC_descriptor_builder_allow.md` (R0 2 rounds YELLOW→GREEN; impl review GREEN; reviews at `design/agent-reports/descriptor-builder-allow-*`).
- **Tier:** `v0.52+-feature` (shipped v0.52.0).
- **Companion:** none.

### `build-descriptor-spec-mode-silently-ignores-archetype-only-flags` — `--spec … --emit-spec` (and the param flags) accepted-and-ignored

- **Surfaced:** 2026-06-09, `--allow` R0 round 2 (M-r2-1 probe). clap drops a conflicted-out arg's `requires` edges: when `--spec` is present, `--archetype` is excluded, so `--emit-spec`/`--key`/… (`requires = "archetype"`) are ACCEPTED by clap and silently ignored by `run()` (which honors them only inside the archetype branch). `--spec x --emit-spec` runs a normal emit, not a spec print — brushes against the never-silent ethos.
- **Where:** `crates/mnemonic-toolkit/src/cmd/build_descriptor.rs` (`run()` archetype branch; the `requires = "archetype"` family). Fix options: (a) runtime advisory note when archetype-only flags are present without `--archetype`; (b) explicit `conflicts_with = "spec"` on each (heavier clap surface, GUI projection noise); (c) document-only.
- **Status:** open — pre-existing (presets cycle), low-severity UX nit.
- **Tier:** `docs-hygiene` / `v0.53+`.
- **Companion:** none.
...
- **Status:** **resolved** 2026-06-09 — user chose **(a) resurrect**. Backfilled `[0.48.0]`–`[0.51.0]` (5 sections; sources = tag annotations + release commits + the resolved entries in this file; R0 fact-audited every row — notably [0.49.0]'s round-trip is byte-stable-for-`description_template`+`keys_info`/`name`-lossy, NOT byte-perfect). Guard: `.github/workflows/changelog-check.yml` (tag-push check, `install-pin-check.yml` pattern — fails any `mnemonic-toolkit-v*` tag whose version has no CHANGELOG section; first live exercise = the next toolkit tag). Ritual home: `design/RELEASE_CHECKLIST.md` gained a "Toolkit per-release ritual" section (the lapse mechanism was that the checklist never mentioned the changelog). Plan `design/PLAN_changelog_ritual_resurrect.md` (R0 2 rounds YELLOW→GREEN; reviews at `design/agent-reports/changelog-ritual-resurrect-r0-r{1,2}-review.md`).
- **Tier:** `docs-hygiene`.
- **Companion:** none.

### `readme-subcommand-inventory-stale-4-sites` — README prose claims wrong version/subcommand counts at 4 sites

- **Surfaced:** 2026-06-09, changelog-resurrect R0 (r1 I2 — the cycle's proposed 1-site ride-along was under-scoped 4×, so it was cut and filed here). Ground truth at v0.51.0: **24** `mnemonic` subcommands (excl. `help`).
- **Where:** (1) `README.md:14` — "Status: **v0.43.x** — twenty-three `mnemonic` subcommands…"; (2) `README.md:44` — "**Twenty-one** `mnemonic` subcommands, grouped below" + the grouped inventory at `:48-54` omits `ms-shares`, `restore`, `build-descriptor`; (3) `crates/mnemonic-toolkit/README.md:10` — same "v0.43.x — twenty-three" sentence (shorter variant); (4) `crates/mnemonic-toolkit/README.md:32` — same "Twenty-one" + 3 omissions. (Line numbers at `2228ad3` — re-grep at fix time.)
- **What:** de-version + de-count the two prose openers (staleness-proof them — the `<!-- toolkit-version -->` markers are the gated version surface; hardcoded prose versions/counts are not) and complete the two grouped inventories (+`ms-shares`, `restore`, `build-descriptor`). Consider whether any count survives at all (a count is a guaranteed future lie without a gate).
- **Status:** **resolved** 2026-06-10 (docs-only, no tag). De-versioned + de-counted both "Status:" openers (visible CHANGELOG/Subcommands pointers — the markers are invisible HTML comments, R0 C1); completed both inventories (+`ms-shares`/`restore`/`build-descriptor`; restore → Wallet import/export per manual precedent, build-descriptor = its own Descriptor-construction group, manual-last precedent); extended the STALE "single-sig restore" clause in both openers to the v0.44.0/v0.49.1 surface (R0 I2); **also fixed a 5th stale inventory R0 found — the manual chapter intro said "Twenty subcommands" listing 21** (de-counted + 3 names added; full manual lint green). POSTURE: no hardcoded SUBCOMMAND counts ever return (component counts carve-out); manual + `--help` authoritative. Plan `design/PLAN_readme_inventory_refresh.md` (R0 2 rounds YELLOW→GREEN).
- **Tier:** `docs-hygiene`.
- **Companion:** none.
...
- **DIRECTION CHANGE (architect-decided):** the original framing ("toolkit `Cargo.toml` gains `features=["compiler"]`; toolkit builds an optimizer") is **wrong on two counts**, both found by source inspection: (1) **`md` already ships the optimizer** — `md compile <ctx> <expr>` + `md encode --from-policy <expr>` call `miniscript::policy::concrete::Policy::compile()` / `compile_tr()` (`descriptor-mnemonic/crates/md-cli/src/compile.rs::compile_policy_to_template`; behind `md-cli/Cargo.toml` `cli-compiler = ["miniscript/compiler"]`). `md encode --from-policy '<expr>' --context segwitv0 --key @0=<xpub> …` already does **policy → optimal miniscript → engraved md1** in one command. (2) **md-codec's lib-level compiler was deliberately REMOVED** — md-codec v0.7.0 shipped `policy_compiler.rs::policy_to_bytecode`; **v0.12.0 (`5350f8a`) deleted it (220 LOC)** to strip the codec to the pure v11 wire format; v0.17 re-implemented it md-cli-locally (md-codec `FOLLOWUPS:356`). So "promote to md-codec lib" reverses a considered strip, and "enable in the toolkit" co-locates an unpinnable heuristic optimizer with the funds crate whose value is *pinned* canonical forms + bloats its linked attack surface. **The compiler stays in `md`.**
- **Where / the workflow (zero new toolkit code):** `md` emits `wsh(M)` / single-leaf `tr` shapes; the toolkit's `compare-cost --descriptor` (`cmd/compare_cost.rs:22-28`, `cost/strip.rs`) and the v0.19.0 `--descriptor` `bundle`/`export-wallet` ingest already accept exactly those → review (cost) + engrave. **One documented seam:** `md compile` emits `@N` *abstract* templates while `compare-cost --descriptor` wants *concrete* keys → use `md encode --from-policy --key @i=<xpub>` (concrete) or `compare-cost --miniscript` (abstract-label door). This is a manual cross-reference, not code.
- **If seamless in-toolkit UX is ever demanded (deferred, low-priority):** the toolkit **shells out to the pinned `md` binary** (an existing constellation primitive; the manual lint already threads `MD_BIN`) — **never** lib-promotion (dead, per the strip), **never** enable the toolkit's own `miniscript/compiler` (worst: pulls the optimizer into the funds crate's linked graph).
- **Posture (unchanged, still the governing principle):** pinned/curated → toolkit (descriptor-builder hand-builds fragments, `compiler` OFF); unpinned/optimized → `md` (the heuristic compiler). Compiler output is NOT version-stable → this is **user-reviewed optimization** ("optimize a policy, see the cost, engrave"), never a silent pinned emit. (Optimizing an *existing* miniscript string is lossy via `lift` and out of scope; "optimize" = compile-from-policy.)
- **Status:** open — re-scoped to surfacing/docs (the capability exists; see companion).
- **Tier:** `docs` / `v0.52+-feature` (the docs cross-reference is do-now; any toolkit shell-out delegation is the deferred bit).
- **Companion:** `descriptor-mnemonic` `design/FOLLOWUPS.md::md-cli-canonical-policy-compiler-home` (blesses md-cli as the home + the manual recipe). Relates to `descriptor-builder-engine` — its deferred optimizer **points at `md`** (the out-of-envelope escape hatch), NOT a toolkit-local or md-codec-lib compiler.

### `encode-accepts-k-greater-than-n` — md-codec encoder accepts k > n multi/thresh bodies that decode rejects (toolkit-side companion)

- **Surfaced:** 2026-06-11, stress Cycle B (md-codec proptest fragment/domain/nesting expansion, descriptor-mnemonic repo) P8 — `descriptor-mnemonic/design/BRAINSTORM_proptest_fragment_domain_expansion.md` [Q5].
- **Where (primary):** `descriptor-mnemonic` `crates/md-codec/src/tree.rs` `write_node` — the `Body::Variable`/`Body::MultiKeys` arms gate only 1..=32 ranges, never `k ≤ n`; decode rejects with `Error::KGreaterThanN`. So `encode_payload`/`encode_md1_string`/`split` happily emit an md1 card (e.g. k=3-of-n=2) that NO decoder will ever read back — engrave-but-can't-restore, same family as `bundle-accepts-sortedmulti-in-combinator-restore-cannot`.
- **Toolkit exposure (why this companion is informational):** the toolkit cannot emit such a card today — its descriptor intake parses via rust-miniscript, whose `Threshold::new` enforces `k ≤ n` at parse, and the template/slot emit path (which never transits rust-miniscript — `synthesize_unified` → `CliTemplate::wrapper_node` hand-builds the `Body::MultiKeys` and goes straight to `md_codec::chunk::split`) is guarded toolkit-side by `bundle_unified::pre_check_threshold` (`src/bundle_unified.rs:67-90`) + `synthesize_unified`'s own k≤n reject (`src/synthesize.rs:~780` → `ToolkitError::MultisigConfig`) before `wrapper_node` builds the body. NOTE the template-path gate is hand-maintained toolkit validation, not the library — it can drift when a new template arm or flag lands. No toolkit code change is implied; this entry exists to keep the cross-repo lockstep when md-codec adds the encoder-side gate (verify no toolkit golden/vector assumes the ungated encoder; bump the md-codec pin on its release).
- **Pinned by:** `descriptor-mnemonic` `crates/md-codec/tests/proptest_to_miniscript.rs::p8_encode_accepts_k_greater_than_n_decode_rejects` (encode Ok + decode `Err(KGreaterThanN { k: 3, n: 2 })`; flips red when the gate lands).
- **Status:** **resolved** 2026-06-12 — md-codec **0.35.2** added the `k ≤ n` gate to BOTH `write_node` arms (mirror of the decode-side rejects), PUBLISHED to crates.io (tag `md-codec-v0.35.2`). The toolkit was never exposed (both emit paths reject k>n before reaching md-codec — re-verified at the cycle-prep recon), so this is a pure lockfile-bump tail: toolkit `Cargo.lock` md-codec `0.35.1 → 0.35.2` (the `"0.35"` req unchanged), full suite green (zero behavioral delta — no toolkit golden/vector embeds a k>n card). The durable library gate now backstops the hand-maintained toolkit gates (which could drift). NO-BUMP toolkit-side.
- **Tier:** `cross-repo`.
- **Companion:** `descriptor-mnemonic` `design/FOLLOWUPS.md::encode-accepts-k-greater-than-n` (primary; resolved md-codec 0.35.2).
...
- **Cycle-B update (2026-06-23, toolkit v0.72.0):** codex32 is now **vendored inline** in ms-codec (`ms_codec::codex32`, ms-codec 0.7.0); the toolkit **DROPPED its direct `codex32 = "=0.1.0"` dep** and migrated `friendly.rs`'s 15 `codex32::Error`/`Fe` sites to `ms_codec::codex32::`. The redaction now lives entirely in the vendored ms-codec layer (`ms_codec::Error::Codex32` Display/Debug peel the leaky arms; `Codex32String` itself gained `ZeroizeOnDrop` + a redacting `Debug`). A catalog item tracks the remaining belt-and-suspenders option (`mnemonic-secret::codex32-error-enum-redacting-debug-defense-in-depth`). Cross-cites `mnemonic-secret::codex32-upstream-dormant-vendor-vs-accept-decision` (resolved, shape A inline).
- **Tier:** secret-hygiene.
- **Companion:** `mnemonic-secret` `design/FOLLOWUPS.md::ms-codec-error-display-echoes-input` (primary); `mnemonic-secret::codex32-upstream-dormant-vendor-vs-accept-decision` (Cycle-B vendor decision).
...
- **Status:** `resolved` — SHIPPED @ `8eb0f85` ("test(fuzz): toolkit parse_descriptor cargo-fuzz target via cfg(fuzzing) lib.rs mount (NO-BUMP)"). The `descriptor_parse` target + `cfg(fuzzing)` lib.rs mount (closure of ~19 modules incl. `timelock_advisory`, NOT the ~35-module over-estimate — `cmd/` is excluded) + `[lints.rust] check-cfg = ['cfg(fuzzing)']` + own-workspace `fuzz/` (pinned `nightly-2026-04-27`, replicated miniscript `95fdd1c` patch, committed lock + 20-seed corpus + gen_corpus gate) + `fuzz-smoke.yml` (build gate + cron `descriptor_parse` smoke) are all committed and tracked. fuzz-smoke CI GREEN @ a1dcff8. Mini-R0 GREEN (`design/agent-reports/toolkit-descriptor-fuzz-mini-r0-review.md`). Oracle = never-panic (no feasible round-trip inverse — `parse_descriptor` emits an `MdDescriptor` tree, not a re-emittable string). Drifted cites at resolve time: `parse_descriptor.rs` fn `:748`, `Terminal::SortedMultiA` `:592`.
- **Tier:** `next-cycle`.
...
- **Where:** **all FOUR** fuzz dirs pin `channel = "nightly-2026-04-27"` (verified uniform 2026-06-12): `descriptor-mnemonic/fuzz`, `mnemonic-secret/fuzz`, `mnemonic-key/fuzz`, AND `mnemonic-toolkit/fuzz` (the `descriptor_parse` cfg(fuzzing) target, added 2026-06-12) — plus the matching `dtolnay/rust-toolchain` pin in each repo's `fuzz-smoke.yml`. **Also pinned per fuzz dir:** the gnu-target (`--target x86_64-unknown-linux-gnu`, the cargo-fuzz-defaults-to-musl/ASan gotcha) and (toolkit only) the miniscript-git `[patch.crates-io]` rev `95fdd1c` replicated in `fuzz/Cargo.toml`.
- **What:** cargo-fuzz needs nightly; the fuzz dirs pin a DATED nightly for reproducibility (floating `nightly` drifts and breaks builds unpredictably). A dated pin goes stale (sanitizer/cargo-fuzz improvements, eventual yank). Quarterly: bump the date in ALL FOUR `fuzz/rust-toolchain.toml` + the CI pins in lockstep, `cargo +<new-nightly> fuzz build --target x86_64-unknown-linux-gnu` all targets, refresh each `fuzz/Cargo.lock` if needed. Keep all four repos on the SAME nightly date so the constellation fuzz infra is uniform.
...
### `mlock-rs-fmt-exempt` — `mlock.rs` is excluded from the pinned-1.95.0 fmt gate (g6 governs its format)

- **Surfaced:** 2026-06-12, the toolkit rustfmt-clean chore (`1683159`) + its g6 fallout. The chore's `cargo +1.95.0 fmt --all` re-wrapped two spots in `crates/mnemonic-toolkit/src/mlock.rs` (a method chain, `record_failure`'s `total_bytes_unlocked.fetch_add`, live at `:167` post-revert; an `assert_eq!(count, 3, …)` live at `:373`), changing it from 347→351 normalized lines — which broke the toolkit `g6` job (it compares against the FROZEN, unformatted `ms-cli-v0.7.0` pin).
- **The irreducible conflict:** `mlock.rs` is a cross-repo byte-synced file (the "fork-and-document-pattern") under the `g6` invariant — it must stay byte-equal (modulo comment-only lines) to `crates/ms-cli/src/mlock.rs` in mnemonic-secret. The toolkit's g6 job (`.github/workflows/rust.yml`, `g6 invariant` job) pins the sibling to the **`ms-cli-v0.7.0` tag** (resolved from `scripts/install.sh:38`), whose mlock.rs is NOT 1.95.0-formatted. So two gates pull opposite ways: the fmt gate wants `mlock.rs` 1.95.0-clean; g6 wants it byte-equal to the unformatted frozen tag. They cannot both hold. (Architect-reviewed 2026-06-12; Option A chosen over cutting `ms-cli-v0.7.1` — option B would ferry the entire `v0.7.0..master` delta (ms-codec 0.4.1→0.4.4, fuzz infra, ~2781 insertions) through a fresh ms-cli release + fire two lockstep gates for an 8-line formatting delta. NOTE: at filing time ms-codec 0.4.4 was crates.io-PENDING, which made option B even worse; 0.4.4 has since published, but the "heavy release for an 8-line delta" objection stands. Whitespace-normalizing the g6 comparator was rejected: `line.trim()` already absorbs indentation but NOT method-chain line-splits, and going further would weaken g6's logic-drift guarantee.)
- **Resolution (Option A):** `mlock.rs` was reverted to its unformatted (v0.7.0-synced) form in BOTH repos, and the toolkit fmt job EXCLUDES it: it runs `cargo +1.95.0 fmt --all -- --check` (edition-proof; cargo handles per-crate editions) then fails iff a `Diff in <path>:` header names any file OTHER than `mlock.rs`. The exclusion + reason live in a load-bearing comment on the `fmt` step. `mlock.rs`'s format is therefore governed by g6 (byte-sync to the pinned ms-cli tag), not by the formatter — consistent with its already-special hand-maintained status (it is the ONLY file in either repo with a cross-repo byte-sync requirement).
- **When this closes:** the next time the `ms-cli` pin is bumped to a 1.95.0-formatted tag (e.g. a future `ms-cli-v0.7.1+`), reformat `mlock.rs` in BOTH repos in lockstep AND in the new tag, then drop the fmt-gate exclusion + this exemption. Until then, a `cargo fmt --all` author MUST NOT reformat `mlock.rs` (the workflow comment is the guard).
...
- **CHARACTERIZATION SHARPENED (2026-06-12, Cycle-7 recon probes):** the `95fdd1c` bug is **LEFT-branch-specific, not depth-specific** — a `TapTree` as a LEFT child mis-Displays (`{{a,b,c}}`, structurally unparseable: brace-flattening can only produce wrong child-arity, never a valid-but-different tree); a right-spine taptree of any depth Displays fine. The earlier "depth-2+" wording is a conservative superset. The v0.55.1 restore depth gate is DELIBERATELY the conservative superset (structural "no TapTree child of a TapTree", chirality-independent) — an accepted-set keyed to "shapes the bug happens to spare" would be Display-luck and would dissolve confusingly on the #953 release.
- **v0.55.1 UPDATE (T3-partial shipped):** the carve-out predicted above is now LIVE — `restore --md1` reconstructs general single-leaf + depth-1 two-leaf `tr(NUMS,…)` faithfully and refuses depth ≥2 structurally (`ensure_taptree_depth_le_one`, exit 2, citing this slug), plus a §5 parse→print Display-fidelity guard against any future parseable-wrong variant.
- **Actions on close (when the first miniscript release > 13.1.0 with #953 ships and the pin bumps):** LIFT the restore depth gate (`src/cmd/restore.rs::ensure_taptree_depth_le_one` + the `classify_taproot_restore` call) and flip the two depth-2 refusal cells in `tests/cli_restore_taproot.rs` (left-heavy + right-spine) to reconstruction cells.
- **Status:** `open` (the depth≥2 restore refusal is live and slug-citing; blocked on the first miniscript release > 13.1.0 containing #953).
- **Tier:** `upstream`.
...
- **T1 SHIPPED 2026-06-12** (tests/cli_restore_taproot_refusal.rs, NO-BUMP): the 3 bundle-reachable refusal arms are pinned — general tr leaf (restore.rs:710), multi-leaf taptree (same arm) + its `.descriptor` exact round-trip (wire-faithful leg), and the distinct-cosigner-IK non-NUMS arm (restore.rs:689). The verify/address legs were de-scoped (verify-bundle needs cosigner mk1; restore refuses before address derivation).
- **T3-PARTIAL SHIPPED 2026-06-12, toolkit v0.55.1** (Cycle 7; plan `design/PLAN_general_tr_faithful_restore.md`, R0 GREEN ×2 + impl review GREEN): faithful reconstruction of general **single-leaf** + **depth-1 two-leaf** `tr(NUMS,…)` via the v0.54.0 faithful arm — 3-way classifier (`classify_taproot_restore`), strict-NUMS `Single` pass-through in `ReconstructTranslator`, structural depth≥2 + `sortedmulti_a`-under-TapTree refusals (exit 2, slug-citing), §5 Display-fidelity guard, explicit `--format green` P2tr refusal (R0 I1). Test file renamed `tests/cli_restore_taproot.rs` (11 cells; goldens cross-verified against a Bitcoin Core v25 oracle). **T3 remainder:** (i) depth ≥2 / ≥3-leaf trees — blocked upstream, refusal cites `upstream-miniscript-taptree-depth2-display-asymmetry` (lift per its Actions-on-close); (ii) `sortedmulti_a` inside a multi-leaf tree — blocked on `md-codec-sortedmulti-a-to-miniscript-rendering-gap`; (iii) the ONLY fixture-requiring refusal arm — keypath-only `tr(NUMS)` with `tree:None` (bundle's origin-annotation gate blocks it, so it needs a hand-built md_codec wire fixture); (iv) DIAGNOSTICS nit — keypath-only `tr(<xpub>)` cards hit the :689 arm with a "multisig"-worded message that misleads for single-sig; (v) `is_nums:false` (cosigner-internal) reconstruction — needs leaf-membership analysis (`restore-multisig-taproot-reconstruction`).
- **Status:** `open` (T1 + T3-partial SHIPPED; remainder blocked upstream / md-codec / fixture-only).
...
### `restore-non-nums-taproot-internal-key` — ✓ RESOLVED (v0.55.3, 2026-06-13) — `restore --md1` reconstructs a non-NUMS ("real key at the trunk") taproot wallet-policy card

- **Surfaced:** 2026-06-12, m-format miniscript-coverage audit follow-on; the deferred remainder item (v) of `restore-general-and-multi-leaf-taproot-roundtrip` (v0.55.1) — `is_nums:false` reconstruction was carved out as needing leaf-membership analysis. Spec `design/SPEC_restore_non_nums_taproot_internal_key.md` (R0 GREEN ×3); plan `design/PLAN_restore_non_nums_taproot_internal_key.md` (R0 GREEN); branch `restore-non-nums-tr-internal-key`.
- **Where:** `crates/mnemonic-toolkit/src/cmd/restore.rs` — `classify_taproot_restore` (read the trunk key off the wire, `Body::Tr.key_index`, instead of hard-coding NUMS), the `TaprootRestore` enum (carries `TaprootInternalKey` on both arms), the call site (threads it), and `build_multisig_import_payload`'s `None`-branch (explicit bip388 refusal for the route-around arm).
- **What:** `restore --md1` previously REFUSED any `is_nums:false` taproot card (exit 2, "non-NUMS (cosigner) internal key") — a real cosigner key at the tr trunk (a live key-path spend) could be engraved but never mechanically restored. Now: a non-NUMS **general** single-leaf / depth-1 policy (e.g. `tr(D,and_v(v:pk(K),older(N)))`) reconstructs via the faithful route-around (md-codec's `is_nums:false → lookup_key` renders the real trunk key), and a non-NUMS **distinct-trunk** multisig (e.g. `tr(D,multi_a(2,K0,K1))`, trunk `D` not one of the leaf keys) reconstructs via the Template path's `Cosigner(idx)` arm. The funds-unsafe `@-in-both` shape (trunk key is ALSO a leaf key) is refused STRUCTURALLY at classify time (the Template `leaf = all-others` shortcut would emit a silently-different multisig the Display-fidelity guard provably cannot catch) — see the spawned defer below. `--format`: a non-NUMS general-tr emits `descriptor`/`bitcoin-core` only (`bip388`/`green` refused — a tap-script-tree route-around has no named-template form / Green is singlesig-only); a distinct-trunk multisig ALSO emits `bip388` faithfully (`tr(@idx/**,multi_a(k,…))` via the Template path). Depth-≥2 taptrees stay refused (upstream `upstream-miniscript-taptree-depth2-display-asymmetry`).
- **Status:** `resolved` mnemonic-toolkit-**v0.55.3** (tag @ master `1996519`, all CI green; tag force-moved past the merge to include the post-merge CI-hygiene fixes) — impl commits `9c7edeb` (enum + gate-lift + split routing + call-site), `a8fe9bd` (`@-in-both` structural guard, funds-safety), `11cd75e` (explicit bip388 route-around refusal; distinct-trunk multisig bip388 still emits). PATCH (watch-only; lifts a refusal into a faithful reconstruction; zero clap delta → no GUI `schema_mirror`, no paired-PR; no md-codec / sibling companions). Per-phase architect reviews persisted to `design/agent-reports/restore-non-nums-taproot-phase-{1,2,3}-r1-review.md` (each 0C/0I before fold). Tests in `crates/mnemonic-toolkit/tests/cli_restore_taproot.rs` (N1-N3 success goldens, N4 `@-in-both` refusal via direct md_codec construction, N5-N8 `--format` matrix; the prior `cosigner_internal_key_tr_bundles_but_restore_refuses_non_nums` refusal cell was deleted, now redundant with N2).
- **Tier:** `feature`
- **Tags:** `restore` `taproot` `multisig` `non-nums` `funds-safety`
- **Spawned FOLLOWUPs:** `restore-non-nums-tr-internal-key-also-in-leaf` (the `@-in-both` defer, below).

### `restore-non-nums-tr-internal-key-also-in-leaf` — `@-in-both`: a non-NUMS trunk key that is ALSO a leaf key is refused (WONTFIX — degenerate key reuse; the loud refusal is permanent)

- **Surfaced:** 2026-06-13, the `restore-non-nums-taproot-internal-key` cycle (toolkit v0.55.3) — carved out as the one non-NUMS shape that cannot take the Template shortcut. Spec `design/SPEC_restore_non_nums_taproot_internal_key.md` §4; plan `design/PLAN_restore_non_nums_taproot_internal_key.md` Task 2.
- **Where:** `crates/mnemonic-toolkit/src/cmd/restore.rs` `refuse_at_in_both` (called from the `MultiA`/`SortedMultiA` arms of `classify_taproot_restore`); the deferred rebuild would live in the same Template-path family (`wallet_export/pipeline.rs::build_tr_multi_a_descriptor`'s `Cosigner(idx)` arm, which currently hard-codes `leaf = key_segs \ {idx}`).
- **What:** for `tr(@i, multi_a/sortedmulti_a(k, …@i…))` — the non-NUMS trunk index is ALSO one of the leaf key indices — the Template path's `Cosigner(idx)` mode reconstructs the leaf as `{all cosigners EXCEPT idx}`, which DROPS the trunk key from the leaf and emits a DIFFERENT (silently-wrong) multisig. The Display-fidelity guard cannot catch it (the Template output is its own re-print). So this shape is refused STRUCTURALLY at classify time (exit 2, citing this slug) rather than emit a funds-unsafe reconstruction — the engraved card remains a faithful backup. RED-proven in `tests/cli_restore_taproot.rs::at_in_both_tr_refuses_structurally` (a hand-built `n=3` md1 reconstructs WRONG and exits 0 WITHOUT the guard; a `sortedmulti_a` secondary cell confirms the guard fires there too).
- **Resolution — WONTFIX (2026-06-13, user decision):** reconstruction support will NOT be added. Re-using the key-path (internal/trunk) key as a member of a script-path multisig leaf is a **degenerate construction**: the trunk key already spends unilaterally via the BIP-341 key path, so re-listing it inside the leaf multisig adds no distinct capability (every leaf combo that *includes* the trunk is dominated by the cheaper, simpler key-path spend). No sane wallet authors this — and the toolkit's own `bundle` cannot even emit it (the BIP-388 distinct-key gate rejects a reused key at intake), so only an external/legacy tool could ever produce such a card.
- **The structural refusal STAYS — permanently.** WONTFIX means "won't add reconstruction," NOT "remove the guard." `refuse_at_in_both` remains the correct, permanent behavior: a loud exit-2 refusal is funds-safe; silently emitting a different multisig (the only thing the `{all-others}` Template shortcut could offer) is not. The RED-proof tests (`tests/cli_restore_taproot.rs::at_in_both_tr_refuses_structurally` + the `sortedmulti_a` and `2-of-2` cells) stay as the permanent contract. The runtime refusal message was reworded from "not yet supported (FOLLOWUP …)" to a WONTFIX/degenerate framing in the same change.
- **If ever reconsidered (not planned):** the mechanism would be a leaf-membership-aware route-around that re-derives the actual leaf key set from the wire indices (not the `{all-others}` shortcut) — adjacent to `md-codec-sortedmulti-a-to-miniscript-rendering-gap`. Recorded for the audit trail only.
- **Status:** `wontfix`
- **Tier:** `wontfix` (was `feature`/`next-cycle`).
- **Companion:** none (toolkit-local).

### `taproot-coverage-cycle-on-miniscript-gt-13-1-0` — UMBRELLA: combined taproot-coverage cycle, triggered by the first rust-miniscript release > 13.1.0 (PR #953)

- **Surfaced:** 2026-06-13, user-requested after the v0.55.3 non-NUMS taproot cycle — captures the SINGLE upstream dependency that unblocks the remaining substantial taproot gaps, so the cross-repo sequencing lives in one place.
- **What:** the two remaining *substantial* taproot gaps — (1) depth-≥2 / ≥3-leaf tap trees (`{{a,b},c}`) and (2) `sortedmulti_a` as a non-root leaf inside a multi-leaf tree — are BOTH blocked on the same upstream event: the first rust-miniscript crates.io release **> 13.1.0** that ships the two relevant `master` PRs. That one release simultaneously dissolves THREE blockers: (a) the depth-≥2 taptree Display bug (`fmt_helper` comma-before-close-brace → malformed `{{a,b,c}}`); (b) md-codec's inability to render `SortedMultiA` (published rust-miniscript v13 has no `Terminal::SortedMultiA`); (c) the toolkit's `[patch.crates-io]` git-rev pin (`95fdd1c`, carried solely to get `Terminal::SortedMultiA` today). Until then `restore --md1` refuses both shapes loudly (funds-safe).
- **Upstream PR status (VERIFIED 2026-06-13 against github.com/rust-bitcoin/rust-miniscript + crates.io):** both fixes are MERGED to `master` but UNRELEASED. (a) the taptree Display fix = **PR #953** "descriptor: fix Taproot tree descriptor formatting", merged to master **2026-05-25**; (b) the fragment = **PR #910** "Add support for sortedmulti_a" (`Terminal::SortedMultiA`), merged to master **2026-04-03**. Latest crates.io release is **13.1.0 (2026-06-09)** — it does NOT contain either (cut from the 13.x line; CHANGELOG documents only through 13.0.0; the PRs target the next major, presumably 14.0). The toolkit's pinned rev `95fdd1c` (2026-05-02) INCLUDES #910 (→ single-leaf `sortedmulti_a` already works) but PREDATES #953 (→ still mis-Displays depth-≥2, hence the live refusal). So the trigger = the first crates.io release **> 13.1.0 carrying BOTH #910 and #953**.
- **Trigger / sequence (cross-repo, ordered):**
  1. rust-miniscript release > 13.1.0 (with #953) lands on crates.io.
  2. **md-codec sibling cycle** (descriptor-mnemonic): bump the miniscript dep + render `SortedMultiA` as a real tap-leaf fragment (resolves `md-codec-sortedmulti-a-to-miniscript-rendering-gap` option (a)); publish.
  3. **toolkit cycle** (this repo): bump the miniscript dep, **DROP the `[patch.crates-io]` entry** (both `crates/mnemonic-toolkit/Cargo.toml` AND the replicated copy in `fuzz/Cargo.toml`, + realign `Cargo.lock` and `fuzz/Cargo.lock`), then **LIFT** the two restore gates and flip their refusal cells to reconstruction:
     - `src/cmd/restore.rs::ensure_taptree_depth_le_one` (+ its `classify_taproot_restore` call) → flip the depth-2 left-heavy + right-spine refusal cells in `tests/cli_restore_taproot.rs` to reconstruction cells (per `upstream-miniscript-taptree-depth2-display-asymmetry` Actions-on-close);
     - `src/cmd/restore.rs::subtree_contains_sortedmulti_a` gate → reconstruct `sortedmulti_a`-under-a-taptree (per `md-codec-sortedmulti-a-to-miniscript-rendering-gap`).
- **Component entries this umbrella coordinates (each keeps its own record):** `upstream-miniscript-taptree-depth2-display-asymmetry` (depth-≥2 Display bug; Actions-on-close), `md-codec-sortedmulti-a-to-miniscript-rendering-gap` (SortedMultiA rendering; option (a) = the upstream fragment), `restore-general-and-multi-leaf-taproot-roundtrip` remainder items (i)+(ii). The `@-in-both` case is NOT part of this (WONTFIX, `restore-non-nums-tr-internal-key-also-in-leaf`).
- **Note:** dropping the `[patch.crates-io]` rev also touches the fuzz workspace (`fuzz/Cargo.toml` replicates the patch; `fuzz/Cargo.lock` must realign) — see `fuzz-nightly-quarterly-bump` for the fuzz-toolchain coordination.
- **Status:** `open` (blocked on the first rust-miniscript crates.io release > 13.1.0 containing #953; nothing toolkit-side is actionable until then).
- **Tier:** `upstream` / `umbrella`.
- **Companion:** `descriptor-mnemonic` (md-codec is the implementing repo for step 2; the same release dissolves its companion entries). Consider mirroring this umbrella there.

### `export-wallet-green-tr-policy-singlesig-emission` — export-wallet --format green emits a "singlesig"-labeled payload for a general taproot policy descriptor

- **Surfaced:** 2026-06-12, Cycle-7 impl review m1 (`design/agent-reports/general-tr-faithful-restore-impl-round1-review.md`).
- **What:** `export-wallet --descriptor 'tr(<NUMS-hex>,{pk(K0),pk(K1)})' --format green` exits 0 and emits a Green payload headed `# Blockstream Green — Watch-only import (singlesig)` — `script_type_from_descriptor` classifies a general tr WITHOUT a `multi_a(`/`sortedmulti_a(` substring as `P2tr` (taproot singlesig), and `green.rs:36` gates only on `is_multisig()`. The descriptor INSIDE the payload is faithful, so this is a mislabeled-surface bug (Green's singlesig file-import dialog receiving a script-tree policy), not a wrong-descriptor one.
- **restore is FIXED (v0.55.1):** `restore --md1 --format green` refuses the general-tr arm explicitly (`build_multisig_import_payload`'s general arm, BadInput exit 1, "singlesig-only" message; pinned by `tests/cli_restore_taproot.rs::general_tr_format_green_refused`). The export-wallet path was deliberately left out of the Cycle-7 scope (restore-only plan); the manual's restore `--format` ¶ no longer claims export-wallet parity for this case.
- **Fix options:** (a) mirror the restore-side refusal in `green.rs::emit` (refuse `P2tr` whose descriptor carries a tap-script tree — needs a structural probe, the rendered string contains `,{` or a leaf after the internal key); (b) keep emitting but reword the header for tr-policy descriptors. Lean (a) — consistency with restore + the manual's framing that Green's surface is singlesig-only.
- **Fix (SHIPPED, v0.70.1 Wave 1):** option (a), STRUCTURAL. `green.rs::emit`, after the `is_multisig()` guard, parses `inputs.canonical_descriptor` and refuses iff `script_type == P2tr` AND the parsed descriptor is `Descriptor::Tr(tr)` with `tr.tap_tree().is_some()` (`ToolkitError::BadInput`, exit 1, the "singlesig-only" / tap-script-tree message mirroring restore.rs). The `,{` substring draft was DISPROVEN — a single-leaf taptree (`tr(NUMS,pk(A))`) renders WITHOUT `,{` but has `tap_tree().is_some()`; the structural accessor is sound where the substring is not. `P2trMulti` is already caught by `is_multisig()`; only `P2tr` reaches the probe, so the BIP-86 keypath-only case (no tap_tree) is preserved unchanged. Tests: `tests/cli_export_wallet_green.rs` cells 5 (branch `,{`), 6 (single-leaf), 7 (bip86 keypath unchanged — behavior-pinning). PATCH (refuse-loudly behavior change; keypath single-sig byte-unchanged). The open question "does Green's file import actually accept a `tr(KEY)` keypath file?" is UNVERIFIED and tracked separately (companion below), not assumed.
- **Status:** ✓ **RESOLVED 2026-06-22, v0.70.1 (Wave 1).**
- **Tier:** `bug` / `next-cycle`.
- **Companion:** `green-taproot-keypath-file-import-unverified` (the open verify-FOLLOWUP: confirm whether Green imports a `tr(KEY)` keypath file; if not, a future cycle blanket-refuses all P2tr). The direct `--descriptor` typed-upgrade is the separate `export-wallet-direct-descriptor-unsorted-multi-generic-refusal` (also resolved v0.70.1).

### `green-taproot-keypath-file-import-unverified` — does Blockstream Green's file import actually accept a `tr(KEY)` keypath descriptor?

- **Surfaced:** 2026-06-22, v0.70.1 Wave 1 (open-followups maturity program), R0 round-1 finding I-2 of `PLAN_w1_export_refusals_and_reconcile.md`.
- **Where:** `crates/mnemonic-toolkit/src/wallet_export/green.rs::emit` (the `script_type == P2tr` arm — refuses a tap-script-tree policy but PRESERVES the BIP-86 keypath-only `tr(KEY)` emission); `tests/cli_export_wallet_green.rs::cell_7_green_bip86_keypath_emission_unchanged` (behavior-pinning, NOT a correctness assertion).
- **What:** the v0.70.1 fix for `export-wallet-green-tr-policy-singlesig-emission` chose the CONSERVATIVE "discriminate" disposition — refuse a tap-script-tree taproot POLICY, but continue emitting the BIP-86 keypath-only single-sig taproot `tr(KEY/…)` to the Green file (status-quo). Whether Green's "Import from file" dialog ACTUALLY accepts a `tr(KEY)` keypath descriptor file is **UNVERIFIED**: neither the manual, repo tests, nor Blockstream's Help Center confirm it (the "singlesig-only" wording is stated only in contrast to multisig). The v0.70.1 cell 7 test only pins that the fix does not blanket-refuse all P2tr (no regression) — it does NOT prove Green imports the file.
- **Fix (if pursued):** verify against a real Blockstream Green client (or authoritative Blockstream docs) whether a `tr(KEY)` keypath descriptor file imports as a watch-only taproot wallet. If it does NOT, a future cycle escalates `green.rs::emit` to blanket-refuse ALL `P2tr` (keypath included) — mirroring the restore-side `--format green` refusal, which refuses all P2tr. Until verified, the bip86→green emission is preserved as status-quo (do not remove a possibly-working path without positive justification).
- **Status:** `open` (verify).
- **Tier:** `verify` / `next-cycle`.
- **Companion:** `export-wallet-green-tr-policy-singlesig-emission` (RESOLVED v0.70.1 — the discriminate fix this verify-item hangs off).
...
- **Status:** `resolved` — 2026-06-16, toolkit **v0.57.0** (Tier-2 C2). Added the `is_bip388_policy_shape` → `expand_bip388_policy` probe to `verify_bundle.rs::descriptor_mode_verify_run` (immediately after the `descriptor_str` read, before the bare-concrete fork's `classify_descriptor_form`), mirroring `bundle.rs` (read-only verify → no policy-name preservation, unlike `export_wallet.rs`). The GAP-5b pinned-refusal cell `verify_bundle_refuses_bip388_policy_json` was inverted red→green into `verify_bundle_accepts_bip388_policy_json` (2-of-2 sortedmulti policy bundle→verify-bundle round-trip via `--bundle-json`); + a single-sig `wpkh` policy round-trip + a malformed `@N`-beyond-`keys_info` cell still refused loudly (asserts the expander's `"@N beyond keys_info"` message). MINOR (new accepted input FORMAT, v0.49.0 precedent); no schema_mirror surface (no new flag); manual `--descriptor` row updated. Plan + R0 GREEN×2: `design/PLAN_C2_verify_bundle_bip388_intake_2026-06-16.md`, `design/agent-reports/c2-verify-bundle-bip388-plan-r0-round{1,2}-review.md`.
- **Tier:** `feature` / `next-cycle`.
...
- **Status:** `resolved` — SHIPPED (NO-BUMP, test+workflow only). `crates/mnemonic-toolkit/tests/bitcoind_differential.rs` (`#[ignore]`/env-gated, connect-only, offline `-chain=main` v27.0) drives a 9-shape positive corpus end-to-end: `bundle --descriptor` (watch-only) → `restore --md1 --count 5 --json` → assert `first_addresses` == Core `deriveaddresses` on the reconstructed chain-0 descriptor, PLUS the original-descriptor cross-check (catches a different-but-valid reconstruction), the chain-1 (change) descriptor-equivalence, and a per-shape `getdescriptorinfo` checksum round-trip. Anti-vacuity = an INDEPENDENT rust-miniscript golden (`derive_receive`, non-deprecated `derive_at_index`) asserted before any Core compare + `getblockchaininfo`/`chain=="main"` fail-loud. Headline shape `tr(NUMS,sortedmulti_a)` is the toolkit-UNIQUE surface (md-codec's crates.io miniscript 13.0.0 can't render it; the toolkit fork `95fdd1c` can). Shape-6 (n=1 general `wsh(and_v(v:pk,older(144)))`) also got a DEFAULT-suite gate `cli_restore_multisig_general.rs::n1_general_wsh_timelocked_restores_faithfully` (R0-r1 I-1 — the bitcoind job is cron-only). `.github/workflows/bitcoind-differential.yml` (lifted from md-codec's; fork-clone step DROPPED — toolkit pins via `[patch.crates-io]`). plan-R0 GREEN ×3 + impl review GREEN; **empirically validated against a live offline `-chain=main` Core node (9 shapes, 45 receive-address checks + cross-checks, all byte-identical) before commit.** Reviews: `design/agent-reports/toolkit-bitcoind-oracle-{plan-r0-round1,2,3,impl-round1}-review.md`.
- **Tier:** `test-hardening`.
- **Companion:** `descriptor-mnemonic` `design/FOLLOWUPS.md::bitcoind-differential-corpus-breadth` (the codec-side Core oracle corpus-breadth item — STAYS OPEN; separate from this toolkit-side end-to-end oracle).
...
- **Status:** `resolved` — 2026-06-17, toolkit **v0.58.1** (Tier-3 C6). **Product-question OUTCOME: on-card preservation NOT pursued** (R0 proved it requires a breaking mk1 wire-generation bump + mk-codec/mk-cli re-publish; AND it's fundamentally impossible to recover the *exact* input variant non-breakingly — intake normalizes ypub/zpub→xpub, so the card cannot distinguish xpub-at-m/84' from zpub-at-m/84'; a path-derived stdout variant would invert the `xpub→mk1→xpub` byte-identity contract AND Bitcoin Core rejects zpub-in-descriptor — both confirmed empirically by the R0 architect). **Shipped instead (user-decided "stderr hint, keep neutral"):** `convert --from mk1 --to xpub` STDOUT stays the neutral xpub (byte-identity + interop intact); a non-blocking stderr NOTE names the SLIP-0132 variant the card's path *conventionally* implies and points at the existing `--xpub-prefix` (`slip0132::path_implied_xpub_prefix` + `render_path_implied_hint`; emitted via a new 4th `compute_outputs` return slot, gated on `args.xpub_prefix.is_none()`). PATCH (advisory-only, no stdout/flag change). Plan + R0 GREEN×2: `design/PLAN_C6_mk1_slip0132_stderr_hint_2026-06-17.md`, `design/agent-reports/c6-mk1-slip0132-plan-r0-round{1,2}-review.md` (round-1 RED disproved the derive-from-path default-change; round-2 GREEN on the stderr-hint design).
...
### `h13-hardened-multipath-reject` — silent-collapse of a hardened multipath alt at the descriptor lexer → typed REJECT

- **Surfaced:** 2026-06-20, constellation bug-hunt (`design/agent-reports/constellation-bughunt-2026-06-20.md`, finding H13) — empirically proven CRITICAL across BOTH the toolkit (`bundle --descriptor`) and md-cli (`md encode`).
- **Where:** toolkit `crates/mnemonic-toolkit/src/parse_descriptor.rs` (the placeholder lexer regex `@(\d+)…(?:/<([0-9;]+)>)?…`, the group-4 multipath parse, `make_use_site_path`, and the `substitute_synthetic` strip regex). md-cli half: `crates/md-cli/src/parse/template.rs` (companion entry in `descriptor-mnemonic/design/FOLLOWUPS.md`).
- **What:** the multipath capture group used the hardening-blind class `[0-9;]`, which CANNOT match a `'`/`h` hardened marker. A hardened multipath alt (`@0/<0';1'>/*`, `<0h;1h>`) was therefore SILENTLY DROPPED before the parse ran — the descriptor lexed as if the markers were absent, emitting a bare-`/*` `md1` (collapsing to a wrong watch-only address) instead of erroring. md-codec already refuses hardened PUBLIC derivation at derive-time; the defect was that the lexer never WROTE the hardened marker through, so md-codec's guard was never reached. The two lexers (toolkit `parse_descriptor.rs` and md-cli `template.rs`) are INDEPENDENT COPIES of the same defect in two unrelated crates — the toolkit does NOT depend on md-cli (md-cli is a `[[bin]]`, not a library), so the toolkit fix is self-contained against the unchanged `md-codec = "0.37"` pin and there is NO publish-before-pin gate (the spec's "md-cli publish → toolkit pin" framing was WRONG for H13; the coupling is behavioral PARITY — both lexers reject the same input class — NOT a build dependency, so both halves simply ship in the same cycle-1 wave).
- **Fix (SHIPPED, cycle-1):** widen the multipath capture to SEE the marker (strict alternation `((?:\d+(?:'|h)?)(?:;\d+(?:'|h)?)*)`), per-alt detect the hardened marker (mirroring the existing `wildcard_hardened` idiom), and on ANY hardened alt return a typed parse-error REJECT naming the offending alt — toolkit `ToolkitError::DescriptorParse` (**exit 2**), md-cli `CliError::TemplateParse` (**exit 1**). `make_use_site_path` widened to `Result<UseSitePath, ToolkitError>` (toolkit) and propagated at the `resolve_placeholders` call sites; the silent `hardened: false` collapse is gone. NO new error variant (reuses `DescriptorParse`/`TemplateParse`); NO md-codec change. The production path is funds-safe via call-order pre-emption (the lexer/resolve reject fires BEFORE `substitute_synthetic`); both strip regexes additionally widened to `[0-9;'h]` in lockstep as defense-in-depth for direct `pub`/test callers. Clean-negative: non-hardened `<0;1>` still round-trips (receive + change). Q-H13-1 resolved REJECT (no md-codec change). See `design/IMPLEMENTATION_PLAN_cycle1_critical_fixes.md` §1/§4/§6.1 + `design/agent-reports/cycle1-h13-impl-review-round{1,2}.md`.
- **Status:** ✓ **RESOLVED 2026-06-20, cycle-1 (toolkit v0.61.0 + md-cli v0.8.0).**
- **Tier:** `funds-safety` / `cross-repo` (m-format lexer-parity mirror).
- **Companion:** `descriptor-mnemonic` `design/FOLLOWUPS.md::h13-hardened-multipath-reject` (the md-cli half — `md encode` lexer; published as md-cli v0.8.0, MINOR). Cross-cites this toolkit entry; the two are independent lexers shipped in parity in the same cycle-1 wave.

### `h12-descriptor-mode-taproot-default-origin-3prime` — descriptor-mode taproot multisig defaulted its BIP-48 origin to `2'` (segwit) instead of `3'` (P2TR)

- **Surfaced:** 2026-06-20, constellation bug-hunt (`design/agent-reports/constellation-bughunt-2026-06-20.md`, finding H12) — empirically proven CRITICAL (wrong-origin → wrong watched address against a real signer).
- **Where:** `crates/mnemonic-toolkit/src/cmd/bundle.rs::compute_default_origin_path` (the 4th `PathComponent.value` was hardcoded `2`); its mirror caller `crates/mnemonic-toolkit/src/cmd/verify_bundle.rs`; and `crates/mnemonic-toolkit/src/cmd/xpub_search/descriptor_intake.rs` (two `bip48_default_path(network, account, 2)` sites with a literal `2`).
- **What:** when a descriptor-mode multisig supplied cosigner keys with NO explicit origin, the toolkit synthesized the BIP-48 default origin `m/48'/<coin>'/<account>'/2'` — the script-type component hardcoded to `2'` (P2WSH). For a TAPROOT multisig (`tr(NUMS, multi_a/sortedmulti_a)`) the de-facto convention (Sparrow / Coldcard / Jade) is `3'`. Emitting `2'` for a taproot cosigner produces a wrong origin → the engraved card watches a DIFFERENT address set than the user's signer expects. No legitimate use-case wants a taproot cosigner defaulted to `2'`, so there is NO escape-hatch flag (a flag would have flipped H12 to MINOR-with-GUI-schema+manual lockstep).
- **Fix (SHIPPED, cycle-1):** `compute_default_origin_path` gains a `script_type: u32` param (4th component = `script_type`, not `2`); callers compute it from the in-scope md-codec `Tag` via the existing `template.rs::bip48_script_type()` mapping (`Tr → 3`, wsh → `2`, sh-wsh → `1`); the `descriptor_intake.rs` miniscript sites use per-site taproot detection `matches!(parsed, MsDescriptor::Tr(_)) ? 3 : 2` (precedent `bsms.rs::is_taproot`). Info-strings render the actual computed component; `bip48_nonstandard_script_type_warning` preserved for the bip48-family taproot convention. NO new flag (value is tree-deterministic), NO new error variant. Differential-oracle anchored (Core `deriveaddresses` == toolkit at every receive+change index for the `3'` origin). See `design/IMPLEMENTATION_PLAN_cycle1_critical_fixes.md` §1/§6.2 + `design/agent-reports/cycle1-h12-impl-review-round1.md`.
- **Status:** ✓ **RESOLVED 2026-06-20, cycle-1 (toolkit v0.61.0).**
- **Tier:** `funds-safety`.
- **Companion:** none (toolkit-internal).

### `h1-verify-bundle-structural-policy-compare` — `md1_xpub_match` verdict was sorted-pubkey-multiset-only (blind to policy structure + watched-address set)

- **Surfaced:** 2026-06-20, constellation bug-hunt (`design/agent-reports/constellation-bughunt-2026-06-20.md`, finding H1) — empirically proven CRITICAL (false `verify-bundle` GREEN on a structurally-different / different-watched-address md1).
- **Where:** `crates/mnemonic-toolkit/src/cmd/verify_bundle.rs::emit_multisig_checks` (the md1 `md1_xpub_match` block — formerly `exp_sorted == act_sorted` over `Vec<[u8;65]>` as the SOLE determinant).
- **What:** the `md1_xpub_match` check compared only the SORTED cosigner pubkey multiset. That binds the pubkey SET but is blind to: Tag/k/wrapper/nesting drift (`sh(multi)` vs `sh(wsh(multi))` — root `Tag::Sh` covers both; `sortedmulti(2,…)` vs `sortedmulti(1,…)`), slot-order permutation in unsorted `multi`, and — the funds-safety hole — the change-chain / multipath `use_site_path` (`<0;1>` vs `<2;3>`, presence/count) that drives md-codec `derive_address` and therefore FIXES THE WATCHED ADDRESS SET. A `verify-bundle` reporting `passed:true` on a different-watched-address md1 is the worst failure class for a verification tool (false assurance). This is the same hole recorded against (and wrongly closed by) `verify-bundle-multisig-md1-xpub-match-set-equality` — re-opened and folded into this closure.
- **Fix (SHIPPED, cycle-1):** the verdict is now the derived structural compare `expected_md_decoded.tree == desc.tree && expected_md_decoded.use_site_path == desc.use_site_path` (plus the per-`@N` `tlv.use_site_path_overrides` address-fixing map), with the sorted-pubkey-multiset check retained SUBORDINATE. `tree` (the `Node` AST) binds Tag + k + wrapper + index-aware slot binding + nesting; `use_site_path` binds the change-chain/multipath that fixes the watched-address set (`UseSitePath`/`Alternative` derive `Eq` and have NO sort/canonicalize/normalize → `==` is faithful, order-significant, and does NOT false-fail a legitimately-equal wallet). Origins (`path_decl` + origin/fingerprint `tlv`) stay deliberately EXCLUDED — they carry elision/canonicalization (L14 brittleness) and binding them would false-FAIL legitimately origin-elided descriptor-mode bundles (the hole B.3's multiset change existed to avoid). `compute_wallet_policy_id` disqualified (origin-instability). Check NAME `md1_xpub_match` UNCHANGED → zero `--json` wire-shape churn, no GUI/manual leg (`schema_mirror` gates flag-names, not wire-shape; GUI does not consume `checks[]`). NO new error variant. See `design/IMPLEMENTATION_PLAN_cycle1_critical_fixes.md` §1/§5/§6.3 + `design/agent-reports/cycle1-h1-impl-review-round1.md`. (C-PLAN-1 fold: the gate is `tree == && use_site_path ==`, NOT the `.tree`-only draft — the `.tree`-only form would have false-GREENed a `use_site_path`-divergent wallet.)
- **Status:** ✓ **RESOLVED 2026-06-20, cycle-1 (toolkit v0.61.0).**
- **Tier:** `funds-safety`.
- **Companion:** none (toolkit-internal). Supersedes/closes `verify-bundle-multisig-md1-xpub-match-set-equality` (re-opened this cycle).

### `template-form-md1-drops-bip39-wordlist-language` — `--md1-form=template` hardcoded English in its ms1 emit, dropping the run-level wordlist language

- **Surfaced:** 2026-06-21, constellation bug-hunt cycle-2 (`design/agent-reports/constellation-bughunt-2026-06-20.md`, finding H8) — funds-safety.
- **Where:** `crates/mnemonic-toolkit/src/cmd/bundle.rs` — the `synthesize_template_descriptor` per-slot ms1 emit loop (the `is_template()` form path), which built each `Payload::Entr` card with a hardcoded `bip39::Language::English`; the run-level `--language` was never threaded in (the keyed `synthesize_descriptor` twin already forwarded it correctly).
- **What:** when `bundle --md1-form=template` emitted a NON-English seed whose wordlist language is carried only by the run-level `--language` (slot `language == None`), the per-slot ms1 card was re-emitted as ENGLISH. A BIP-39 mnemonic-to-seed is PBKDF2 over the NFKD-normalized wordlist *string*, so an English-defaulted recovery of that same entropy derives a DIFFERENT 512-bit seed → wrong master seed → an unrecoverable card. The shared emit loop serves BOTH the single-sig and the multisig/general template forms, so the defect affected every non-English template bundle.
- **Fix (SHIPPED, cycle-2):** thread `run_language: bip39::Language` into `synthesize_template_descriptor`'s signature, forward it at the sole `md1_form.is_template()` call site (previously dropped on the floor), and replace `c.language.unwrap_or(bip39::Language::English)` in the template ms1 emit loop with `c.language.unwrap_or(run_language)` — byte-identical to the already-correct keyed path. One change covers both single-sig and multisig template forms. Private fn signature → no clap/CLI surface → no GUI schema-mirror / manual / codec leg. See `design/IMPLEMENTATION_PLAN_cycle2_funds_loss_fixes.md` §2 (WS-A / S-TEMPLATE) + `design/agent-reports/cycle2-h8-impl-review-round1.md`.
- **Status:** ✓ **RESOLVED 2026-06-21, cycle-2 (toolkit v0.62.0).**
- **Tier:** `funds-safety`.
- **Companion:** none (toolkit-internal).

### `export-wallet-unsorted-multi-silent-sortedmulti-coercion` — field-less vendor export silently coerced unsorted `multi()` to `sortedmulti` → wrong addresses

- **Surfaced:** 2026-06-21, constellation bug-hunt cycle-2 (`design/agent-reports/constellation-bughunt-2026-06-20.md`, finding H10) — funds-safety; the `wsh-multi-2of3-divergent` differential row in `crates/mnemonic-toolkit/tests/bitcoind_differential.rs` proves the address divergence.
- **Where:** `crates/mnemonic-toolkit/src/cmd/export_wallet.rs` — the shared `emit_payload` dispatch (the `Electrum` / `Coldcard` / `ColdcardMultisig` / `Jade` arms; the coldcard multisig emitter `wallet_export/coldcard.rs::emit_coldcard_multisig_text` writes `Name:/Policy:/Derivation:/Format:/<XFP>:` with NO sorted-vs-unsorted field and lex-sorts only for `*SortedMulti`). `error.rs` for the new variant.
- **What:** the electrum / coldcard / coldcard-multisig / jade multisig file formats are BIP-67 sortedmulti-ONLY — there is no field to express a literal `multi(...)` key order. Exporting an UNSORTED `wsh(multi)` / `sh(wsh(multi))` to them silently re-sorted the cosigner keys (BIP-67 lexicographic), changing the witnessScript and therefore EVERY receive/change address — a silent policy-collapse to a different wallet (oracle-proven).
- **Fix (SHIPPED, cycle-2):** PURE REFUSAL, no flag. New typed `ToolkitError::ExportWalletUnsortedMultisigUnsupported { format: &'static str }` (struct form, exit 2), alphabetically placed in the enum and in the `exit_code` / `kind` / `message` exhaustive match arms (slots AFTER `ExportWalletTaprootMultisigUnsupported`, BEFORE `FutureFormat`). A STRUCTURED guard in the shared `emit_payload` chokepoint (after `collect_missing`, before the per-format emit match) refuses iff the resolved `CliTemplate` is `Some(WshMulti)` / `Some(ShWshMulti)` AND the format is one of {Electrum, Coldcard, ColdcardMultisig, Jade}. The chokepoint also covers the restore-path callers (`build_import_payload` / `build_multisig_import_payload`) — a CONSEQUENCE, funds-safe (more refusals, never fewer). `sortedmulti` / single-sig / faithful (descriptor-preserving) formats are UNAFFECTED. No clap flag, no `--json` wire-shape change, no codec bump. See `design/IMPLEMENTATION_PLAN_cycle2_funds_loss_fixes.md` §2 (WS-B / S-EXPORT-LEX) + `design/agent-reports/cycle2-h10-impl-review-round1.md`.
- **Status:** ✓ **RESOLVED 2026-06-21, cycle-2 (toolkit v0.62.0).**
- **Tier:** `funds-safety`.
- **Companion:** none (toolkit-internal). The direct `--descriptor` typed-upgrade is the separate OPEN follow-up `export-wallet-direct-descriptor-unsorted-multi-generic-refusal`.

### `descriptor-prefix-form-origin-annotation-ignored` — documented BIP-380 prefix-form `[fp/path]@N` key-origin silently ignored → origin dropped + fp cross-check bypassed

- **Surfaced:** 2026-06-21, constellation bug-hunt cycle-2 (`design/agent-reports/constellation-bughunt-2026-06-20.md`, finding H7) — funds-safety.
- **Where:** `crates/mnemonic-toolkit/src/parse_descriptor.rs::lex_placeholders` (the suffix-only placeholder grammar); the `@N` key-origin consumers in `cmd/bundle.rs` (+ a `cmd/verify_bundle.rs` test). Adjacent to — and interacting with — cycle-1's H13 hardened-multipath validator in the same file.
- **What:** the BIP-380-canonical PREFIX origin annotation `[fingerprint/path]@N` was silently IGNORED by `lex_placeholders`, which only recognized the SUFFIX form. A user/tool following the standard had their origin path DROPPED (the slot xpub was built at the default/master path → the backup watched a DIFFERENT address set) AND the per-`@N` master-fingerprint cross-check bypassed (an unverified key silently accepted). Documented-but-unhonored grammar in a funds-bearing path.
- **Fix (SHIPPED, cycle-2):** ACCEPT both positions. `lex_placeholders`'s regex gains an OPTIONAL prefix-origin alternation BEFORE `@(\d+)` and is converted to ALL-NAMED capture groups (`pfx_fp`/`pfx_path`/`idx`/`sfx_fp`/`sfx_path`/`mpath`/`wild`); ALL consumers are rewritten from numeric `caps[N]`/`caps.get(N)` to position-independent `caps.name(...)` — INCLUDING cycle-1's H13 hardened-multipath validator accessor (`.get(4)` → `.name("mpath")`, validator body byte-identical). Named-by-name access is position-independent, so prepending the prefix can NEVER shift the multipath group out from under the H13 reject (the C1 plan fold: the FALSE "groups 6/7, existing 1–5 unchanged" claim was deleted — numeric-index renumbering would have silently broken the H13 reject). The prefix-form origin is preserved and the per-`@N` fingerprint cross-check now runs on it. Guard test: both bare `@0/<0';1'>/*` AND prefix `[fp]@0/<0';1'>/*` STILL reject (proves C1 closed). No new clap flag (lexer-internal), no `--json` wire-shape change, no codec bump. See `design/IMPLEMENTATION_PLAN_cycle2_funds_loss_fixes.md` §3 (WS-C / S-VERIFY-LEX) + `design/agent-reports/cycle2-h7-impl-review-round1.md`.
- **Status:** ✓ **RESOLVED 2026-06-21, cycle-2 (toolkit v0.62.0).**
- **Tier:** `funds-safety`.
- **Companion:** none (toolkit-internal).

### `export-wallet-direct-descriptor-unsorted-multi-generic-refusal` — direct `export-wallet --descriptor 'wsh(multi(…))'` unsorted-multi path refused only by generic `BadInput`, not the typed H10 error

- **Surfaced:** 2026-06-21, constellation bug-hunt cycle-2 — plan round-2 finding I-A, the deliberately-deferred cosmetic leg of the H10 fix (`export-wallet-unsorted-multi-silent-sortedmulti-coercion`).
- **Where:** `crates/mnemonic-toolkit/src/cmd/export_wallet.rs` — the direct `run --descriptor` arm. Unlike the `--template wsh-multi` and `--from-import-json` paths (which set `resolved_template = Some(WshMulti)` and are caught by the new H10 structured guard), the direct `--descriptor` arm builds `canonical` but leaves `resolved_template = None` → `template_opt = None`.
- **What:** the direct `export-wallet --descriptor 'wsh(multi(…))'` path to a field-less vendor format (electrum/coldcard/coldcard-multisig/jade) is ALREADY funds-safe — it is refused by each emitter's generic `inputs.template.ok_or_else(…)` `BadInput` (exit ≠ 0, never silently coerced to `sortedmulti`). It is NOT covered by the new typed `ExportWalletUnsortedMultisigUnsupported` message, so the refusal is less specific than on the `--template` / `--from-import-json` paths. There is NO funds-safety hole on this path (the coercion cannot happen); only the diagnostic message differs.
- **Fix (if pursued):** classify the `template == None` direct `--descriptor` path via the parsed descriptor (`Tag::Multi` / `script_type`) to surface the typed `ExportWalletUnsortedMultisigUnsupported` message there too — purely cosmetic message-quality, no behavioral change (the path is already refused). Deliberately OUT of cycle-2 scope (round-2 I-A pinned option (i), minimal).
- **Fix (SHIPPED, v0.70.1 Wave 1):** a SECOND ARM of the existing H10 guard in `cmd/export_wallet.rs::emit_payload` (after the resolved-template arm). Keys on `inputs.template.is_none()` ∧ `inputs.script_type ∈ {P2wshMulti, P2shP2wshMulti}` ∧ format ∈ {Electrum, Coldcard, ColdcardMultisig, Jade} ∧ `canonical_descriptor.contains("multi(") && !contains("sortedmulti(")` (the `sortedmulti(`-as-substring idiom from `wallet_export/mod.rs::template_from_descriptor`) → returns the typed `ExportWalletUnsortedMultisigUnsupported { format }` (exit 2). The unit test `template_none_falls_through_to_generic_badinput_not_h10` was FLIPPED to `template_none_hits_typed_h10_error` (asserts the typed kind + exit 2 per field-less format); a sorted-multi no-change guard pins the second arm does not over-refuse the BIP-67-faithful sorted case. NO-BUMP — still refuses, same exit code 2; message-quality only (no behavioral / funds change; the path was already refused). See `tests/cli_export_wallet_unsorted_multi_refusal.rs` §3 (typed message + sorted no-change guard).
- **Status:** ✓ **RESOLVED 2026-06-22, v0.70.1 (Wave 1, NO-BUMP within the cycle).**
- **Tier:** `polish` / `next-cycle`.
- **Companion:** none (toolkit-internal). Sibling of the SHIPPED `export-wallet-unsorted-multi-silent-sortedmulti-coercion` (cycle-2, toolkit v0.62.0).

### `phrase-overlay-secretstring` — `import-wallet` phrase-overlay Vec copies the seed phrase into a bare `String` (not scrubbed)

- **Surfaced:** 2026-06-21, constellation bug-hunt cycle-14 (L22 fix; `design/IMPLEMENTATION_PLAN_cycle14_l22_stdin_secret_zeroize.md` §3-D1 sub-decision + §5 `OOS-phrase-overlay-deep-wrap`).
- **Where:** `crates/mnemonic-toolkit/src/cmd/import_wallet.rs:~1229-1234` — `let phrase_overlays: Vec<(u8, String)> = args.slot.iter().filter(|s| s.subkey == SlotSubkey::Phrase).map(|s| (s.index, s.value.to_string())).collect();`, then `apply_seed_overlay(&phrase_overlays, …)` (`src/wallet_import/overlay.rs::apply_seed_overlay`'s `&[(u8, String)]` param → `Source::Phrase(String)` at `overlay.rs:~97`).
- **What:** cycle-14 migrated `SlotInput.value` to `SecretString`, but the `phrase_overlays` collection copies the phrase OUT of the scrubbing newtype into a bare `Vec<(u8, String)>` via `.to_string()` (the minimum edit to restore compile). This is **status-quo-preserving — NO NEW residue** (the overlay phrase was already a bare `String` before cycle-14), and the `import-wallet --slot @N.phrase` path is `@env:`/inline only (no stdin `=-` channel), so it is NOT in L22's named scope. But the phrase still lingers un-scrubbed in 1-2 bare `String` copies downstream until natural drop.
- **Fix (if pursued):** make `phrase_overlays: Vec<(u8, SecretString)>` and flip `apply_seed_overlay`'s signature (`&[(u8, String)]` → `&[(u8, SecretString)]`) + `Source::Phrase(SecretString)`, deref'ing at the bip39 parse consumer. Non-trivial fan-out (the `apply_seed_overlay` signature + `Source` enum + every consumer), which is why cycle-14 deferred it. Defense-in-depth only — no funds-safety / wrong-address concern.
- **Status:** ✓ **RESOLVED (toolkit v0.71.0, wave2 T3 — 2026-06-22).** BOTH the Phrase AND the ms1 arms wrapped (full resolve): `phrase_overlays: Vec<(u8, SecretString)>` (via `.value.clone()`) + `ms1_args: Vec<Option<SecretString>>`; `apply_seed_overlay`'s signature + the fn-local `enum Source { Ms1(SecretString), Phrase(SecretString) }` carry `SecretString` to the decode consumers (`ms_codec::decode` deref-coerces the concrete `&str`; `bip39::Mnemonic::parse_in` is generic → explicit `&**phrase`). No wire/behavior change. PATCH leg folded into the v0.71.0 MINOR.
- **Tier:** `polish` / `next-cycle`.
- **Companion:** none (toolkit-internal). Continuation of the SHIPPED `secret-memory-hygiene-cycle-b` Site-1 scrub leg (cycle-14, toolkit v0.67.0).

### `stdin-reader-transient-buf-zeroizing` — `read_stdin_passphrase` / `read_stdin_to_string` transient `buf` is a bare `String`

- **Surfaced:** 2026-06-21, constellation bug-hunt cycle-14 (L22 fix; spec §5 `OOS-reader-return-type-flip`).
- **Where:** `crates/mnemonic-toolkit/src/cmd/convert.rs:~745-769` — `read_stdin_to_string` / `read_stdin_passphrase` both build `let mut buf = String::new(); stdin.read_to_string(&mut buf)?; …` and return a bare `String`.
- **What:** cycle-14's D1 deliberately kept these readers returning bare `String` — flipping the return type to `Zeroizing<String>` would make the 14 already-wrapping `Zeroizing::new(read_stdin_*(…))` callers illegal `Zeroizing<Zeroizing<String>>` (verified 14 sites). The OWNERS of the lingering secret (the `SlotInput.value` field + the convert/restore/addresses handler locals) are now all wrapped, so the readers' transient stack-rooted `buf` is the last bare copy — short-lived, consumed-by-move at every call site immediately into a `Zeroizing`/`SecretString`. A future focused change could wrap `buf` IN-FN as `Zeroizing<String>` and return `.to_string()` (or `(*buf).clone()`) so the reader's own scratch buffer scrubs without changing the return type.
- **Fix (if pursued):** in-fn `let mut buf = zeroize::Zeroizing::new(String::new());` + return `buf.trim().to_string()` / `(*buf).clone()`; no caller change (return type stays `String`). Defense-in-depth only.
- **Status:** ✓ **RESOLVED (toolkit v0.71.0, wave2 T4 — 2026-06-22).** Both readers wrap the internal `read_to_string` scratch buffer in `zeroize::Zeroizing<String>` (works via DerefMut for `read_to_string` / the in-place CRLF-strip). Return type STAYS `String` (`read_stdin_passphrase`'s `Ok(buf)` → `Ok(buf.to_string())`) so the ~42 wrapping call sites don't double-wrap / type-mismatch. Behavior byte-identical (same trim / single-CRLF-strip). NO-BUMP leg folded into the v0.71.0 MINOR.
- **Tier:** `polish` / `next-cycle`.
- **Companion:** none (toolkit-internal). Continuation of the SHIPPED `secret-memory-hygiene-cycle-b` Site-1 scrub leg (cycle-14, toolkit v0.67.0).

### `bip85-derive-child-output-secretstring` — `derive-child` renders the BIP-85 child secret (WIF / xprv / phrase / password) into a bare `String` instead of `SecretString`

- **Surfaced:** 2026-06-21, the secret-keymat memory-hygiene sweep (`design/agent-reports/sweep-keymat-toolkit.md` finding 1 — the headline). Verified vs `origin/master` `ddabf5e3` (toolkit v0.67.0).
- **Secret type / gap class:** BIP-85 derived child secret — BIP-39 phrase / HD-seed WIF / child xprv / raw hex entropy / derived password (base64/base85) / dice. Gap class 1 (zeroize-on-drop gap) + 4 (`.to_string()`/`to_wif()` escapes into a bare copy).
- **Where:** `crates/mnemonic-toolkit/src/cmd/derive_child.rs:224` (`let output = match … {…}` → bare `String`) + `:304` (`writeln!(stdout, "{output}")`, then dropped un-scrubbed). The seven `format_*` producers in `crates/mnemonic-toolkit/src/bip85.rs` all return a bare `String`: `format_bip39_phrase` (decl `:72`, `Ok(mnemonic.to_string())` `:90`), `format_hd_seed_wif` (`:103`, `Ok(pk.to_wif())` `:121`), `format_xprv_child` (`:131`, `Ok(xprv.to_string())` `:154`), `format_hex_bytes` (`:163`, `Ok(hex::encode(…))` `:170`), `format_password_base64` (`:181`, `:189`), `format_password_base85` (`:196`, `:204`), `format_dice_rolls` (`:222`, `Ok(out.join(","))` `:273`).
...
### `electrum-native-seed-normalize-intermediates-zeroizing` — Electrum seed-derivation normalizes the phrase + passphrase into ~6 bare `String`/`Vec` intermediates and clones secret entropy out of `Zeroizing`

- **Surfaced:** 2026-06-21, the secret-keymat memory-hygiene sweep (`design/agent-reports/sweep-keymat-toolkit.md` finding 2). Verified vs `origin/master` `ddabf5e3`.
- **Secret type / gap class:** BIP-39/Electrum seed phrase + BIP-39 passphrase (normalized copies) + Electrum native-seed entropy. Gap class 6 (key-derivation intermediates not scrubbed) + 1 + 4.
- **Where (`crates/mnemonic-toolkit/src/electrum.rs`):** `normalize_text_electrum` (`:79`) builds bare normalized secret `String`s and returns bare; `electrum_seed_to_bip32_seed` (`:97`) holds `norm_phrase` (`:98`) + `norm_pp` (`:99`) — bare normalized copies of phrase AND passphrase — alive across the PBKDF2 call (the 64-byte output is `Zeroizing`, its two secret inputs are not); `phrase_to_entropy` (`:138`) has `words: Vec<String>` (`:147`) bare and `:175` `Ok((*acc).clone())` copies the secret entropy OUT of `acc: Zeroizing<Vec<u8>>` into a bare returned `Vec<u8>`; `entropy_to_phrase` (`:182`) `:215` `let phrase = words.join(" ")` bare and returned bare; `normalize_phrase_for_hmac` (`:243`) + `strip_cjk_internal_whitespace` (`:249`) build bare copies.
...
### `seedqr-codec-internal-secret-string-zeroizing` — SeedQR codec functions build the phrase / digits / entropy in bare `String`s and return bare (consumer re-wraps fast)

- **Surfaced:** 2026-06-21, the secret-keymat memory-hygiene sweep (`design/agent-reports/sweep-keymat-toolkit.md` finding 3). Verified vs `origin/master` `ddabf5e3`.
- **Secret type / gap class:** BIP-39 phrase + SeedQR digit string + raw entropy hex (all forms of the seed). Gap class 1 (library-internal bare secret) + 6.
- **Where (`crates/mnemonic-toolkit/src/seedqr.rs`):** `decode` (`:96`) — `stripped` raw digits (`:98`), `:131` `let phrase = words.join(" ")`, returns bare; `encode` (`:141`) — `:155` `normalized`/`digits`; `encode_compact` (`:182`) — `:196` `Ok(hex::encode(m.to_entropy()))` (raw entropy hex bare); `decode_compact` (`:205`) — `stripped` (`:206`), `:218` `Ok(m.to_string())`.
...
### `bsms-derive-hmac-key-not-zeroizing` — BIP-129 `HMAC_KEY` returned as a bare `[u8;32]` (deliberate-by-doc; flag for re-decision)

- **Surfaced:** 2026-06-21, the secret-keymat memory-hygiene sweep (`design/agent-reports/sweep-keymat-toolkit.md` finding 5). Verified vs `origin/master` `ddabf5e3`.
- **Secret type / gap class:** BIP-129 `HMAC_KEY` (= `SHA256(ENCRYPTION_KEY)`, derived from secret material). Gap class 6 (derivation intermediate) + 1.
- **Where:** `crates/mnemonic-toolkit/src/bsms_crypto.rs:114` `derive_hmac_key` returns a bare `[u8; 32]` (NOT `Zeroizing`); `compute_mac` (`:136`)'s `out` is also bare.
...
### `bundle-unified-whole-file-allowlist-precision` — zeroize lint whole-file-allowlists `bundle_unified.rs`, which could mask a FUTURE production secret allocation

- **Surfaced:** 2026-06-21, the secret-keymat memory-hygiene sweep (`design/agent-reports/sweep-keymat-toolkit.md` finding 6; matches the prompt's M1 whole-diff finding). Verified vs `origin/master` `ddabf5e3`.
- **Secret type / gap class:** n/a — gate-coverage / lint-precision risk, not a live secret. Gap class meta.
- **Where:** `crates/mnemonic-toolkit/tests/lint_zeroize_discipline.rs:450` whole-file-allowlists `src/bundle_unified.rs` in `NON_ROW_SECRET_FILES`; the file's only `SecretString::new` is `#[cfg(test)]` (`bundle_unified.rs:118` cfg-gate, `:128` `SecretString::new(...)` in the `s()` fixture helper) — no production secret allocation today.
...
- **Status:** `resolved` (v0.69.1, 2026-06-22). The three named scratch locals are now `Zeroizing`: `bip85.rs` `format_password_base64`'s `encoded` → `Zeroizing::new(base64_standard(…))`, `format_password_base85`'s `encoded` → `Zeroizing::new(base85_btc(…))`, and `format_dice_rolls`'s `out` → `Zeroizing<Vec<String>>`. Ripple-free (Deref/DerefMut), BIP-85 KATs byte-identical, no signature/lint/wire change. Out-of-scope (dispositioned): the per-roll `buf` (1–4-byte raw SHAKE scratch overwritten every iteration; wrapping breaks `for &b in &buf` + `reader.read(&mut buf)`) and the encode-helper-internal `out: String` (moved out by return into the caller's now-`Zeroizing` `encoded` — scrubbed on the caller's drop, redundant to wrap inside the helper).
- **Tier:** `polish` / `next-cycle`.
...
- **Surfaced:** 2026-06-21, own-account subset-search re-brainstorm recon (§3.3) + a follow-on architect pass (2 passes) over the 2026-06-21 secret-keymat sweep. The sweep (`design/agent-reports/sweep-keymat-toolkit.md`) and its Lane-T cycle MISSED this site — a previously-uncounted sibling of the same "derived-output secret not zeroized" class. Citations verified vs `origin/master` `1576b2d6` (v0.69.1).
- **Secret type / gap class:** account-level `bitcoin::bip32::Xpriv` (full spending authority over the account branch). Zeroize-on-drop gap — an owned `Xpriv` held in a bare struct field.
- **Where:** `crates/mnemonic-toolkit/src/derive_slot.rs::derive_bip32_from_entropy_at_path` (`:65`) — `account_xpriv = master.derive_priv(&secp, path)` (`:84-85`), `account_xpub = Xpub::from_priv(&secp, &account_xpriv)` (`:87`), then stored into the returned `DerivedAccount`. `crates/mnemonic-toolkit/src/derive.rs::DerivedAccount` (`:23`) — `pub entropy: Zeroizing<Vec<u8>>` (`:24`) but `pub account_xpriv: Xpriv` (`:27`) is a bare, un-scrubbed field that drops without erase. Any caller needing only the public `account_xpub` (the planned own-account subset-search over-supply loop derives K_own such keys) holds the secret `Xpriv` for no reason.
- **What:** the seed buffer is `Zeroizing<[u8;64]>` and the entropy is `Zeroizing<Vec<u8>>`, but the derived account `Xpriv` — equal spending authority — sits in a bare struct field; xpub-only callers hold it un-scrubbed.
- **Fix (architect-validated, 2 passes — PROVISIONAL, re-verify `derive_slot.rs`/`DerivedAccount` at impl time):**
  - `struct ScrubbedXpriv(Xpriv)` — NOT `Copy`/`Clone` (move-only by E0184: `Copy`+`Drop` mutually exclusive); `impl Drop` → `self.0.private_key.non_secure_erase()` + a VOLATILE `chain_code` zero-write; NO escape hatch (only `&self` accessors + `xpub(&self,&secp)->Xpub`; NO `into_inner`/`Deref<Target=Xpriv>`/`Clone`/`Copy`/pub field); compile-time `assert_not_impl_any!(ScrubbedXpriv: Copy, Clone)`.
  - `derive_account_xpub_only(...) -> Result<(Xpub, Fingerprint), ToolkitError>` in `derive_slot.rs` — confines the `Xpriv` inside the fn (wrap → read xpub → scrub on drop) so callers never own an `Xpriv`. Derive the master once when fanning out to multiple accounts (own candidates share the path family).
- **Caveat:** the by-value `derive_priv()->Xpriv` return temp + secp internals stay under `rust-bitcoin-xpriv-zeroize-upstream`; `non_secure_erase` is best-effort. The newtype MINIMIZES (one move-in/derive) but cannot eliminate the residue.
- **Severity:** MED (defense-in-depth; full account spending authority held un-scrubbed in a struct field). Not leak-to-disk/argv/log → not Critical.
- **Status:** ✓ **RESOLVED (toolkit v0.71.0, wave2 T1 — 2026-06-22).** The broader lift SHIPPED: `DerivedAccount.account_xpriv: Xpriv` → `ScrubbedXpriv` (`derive.rs`), the single ctor wraps via `ScrubbedXpriv::new` (`derive_slot.rs`), and `DerivedAccount::into_parts` DROPPED its `Xpriv` tuple element entirely (the field stays in `self` and scrubs in place) — all 8 callers updated (bundle.rs ×5, verify_bundle.rs ×2, synthesize.rs ×1; all discarded `_xpriv`). The only genuine reader (`convert --to xprv`) goes through a new narrow, string-only `ScrubbedXpriv::expose_xprv_string(&self) -> SecretString` (redacting Debug, scrub-on-drop; `--to xprv` byte-identical, golden-pinned). `ScrubbedXpriv` gained a hand-written redacting `Debug` (NOT `#[derive]`). The v0.70.0 narrowing (the `ScrubbedXpriv` newtype + `derive_account_xpub_only`/`derive_accounts_xpub_only` xpub-only helpers + their tests) was the prior P0 leg. MINOR (pub field-type + `into_parts` arity + new pub method) folded into v0.71.0. **Upstream caveat persists:** `rust-bitcoin`'s `Xpriv` has no Drop+Zeroize (`rust-bitcoin-xpriv-zeroize-upstream`); the newtype's scrub is best-effort.
- **Tier:** `polish` / `next-cycle`. **Origin:** P0 of the own-account subset-search cycle (`feature/own-account-subset-search`, v0.70.0) AUTHORED the minimal additive helper (`ScrubbedXpriv` + `derive_account_xpub_only`) that the over-supply loop consumes — decoupled OUT of the broader 7-site struct-field lift on architect advice (the minimal additive helper ships; the field-migration lift is the remaining standalone increment). Natural sibling of the cycle-15/Lane-T derived-output zeroize work (whoever owns that lane may fold the 7-site lift; else a standalone hygiene increment).
- **Companion:** the constellation **derived-output & codec-internal secret-zeroize** program — `design/agent-reports/sweep-keymat-*.md` + `project_secret_keymat_sweep_2026_06_21`. The owned-`Xpriv`-field leg (cycle-14/L22 closed the clap/handler-local/persistent-field leg; the sweep's Lane-T slugs cover the library/output `String` leg). Toolkit-internal (`ScrubbedXpriv` is toolkit-local, no sibling-codec API) → pattern-family cross-cite, not a cross-repo mirror.

### `toolkit-rustfmt-1-95-0-rebaseline-divergence` — the pinned `+1.95.0` fmt CI gate now reports ~29 non-mlock files needing reformat (toolchain rustfmt drift; gate is effectively RED on master)

- **Surfaced:** 2026-06-22, own-account subset-search v0.70.0 ship-prep. The fmt CI gate (`.github/workflows/rust.yml` job `fmt`, `cargo +1.95.0 fmt --all -- --check`, fail on any non-`mlock.rs` `Diff in`) flags **~29 non-mlock files on `origin/master` ITSELF** (`bip85.rs`, `cmd/{addresses,bundle,convert,export_wallet,restore}.rs`, `error.rs`, `lib.rs`, `network.rs`, `parse_descriptor.rs`, `synthesize.rs`, … + ~11 test files). The locally-installed `+1.95.0` toolchain bundles `rustfmt 1.9.0-stable (59807616e1 2026-04-14)`, which reformats DIFFERENTLY than the rustfmt that produced the committed code — i.e. a toolchain-version drift the comment at `rust.yml:30-37` anticipated ("if a future rustfmt diverges, this gate …"). The gate is therefore **effectively RED on master** (pre-existing — the burndown's v0.66–v0.69.1 shipped this way).
- **NOT introduced by v0.70.0.** The subset-search cycle is fmt-NEUTRAL: its 4 changed src files add NO new file to the `bad` list (`restore.rs` was already flagged on master via the cycle-14/L22 passphrase-`Zeroizing` wrap; the cycle's edits even REMOVED `verify_bundle.rs` + `permutation_search.rs` from the list; `derive_slot.rs` is clean). Reformatting the cycle files with the divergent local `1.9.0` would DEVIATE them from the repo's canonical form — so v0.70.0 ships matching the surrounding committed style, NOT the divergent local rustfmt.
- **Fix (deliberate chore, NO-BUMP):** decide the canonical formatter — either (a) pin the gate to the rustfmt build that produced the committed code (not bare `1.95.0` if that now resolves to a drifted patch), or (b) deliberately re-baseline the whole workspace with the NEW `+1.95.0` (`cargo +1.95.0 fmt --all`, then `git checkout -- …/mlock.rs` for the g6 exemption) in ONE chore commit + bump the gate's pinned version comment. Until then the gate masks real fmt drift. Re-grep the file list at fix time.
- **RESOLVED** 2026-06-22 (open-followups maturity program, Wave 1) via **option (b-minimal)** — opus-architect-ruled over (a)/(b-min)/(b-full). Key correction: the gate alias `1.95.0` resolves to `rustfmt 1.9.0-stable (59807616e 2026-04-14)` = **current stable** (`rustc +1.95.0` ≡ `rustc +stable`); the committed code was formatted by an *older* rustfmt, so option (a) would freeze on a *superseded* formatter — wrong direction. Re-baselined the 28 non-mlock source files with `cargo +1.95.0 fmt --all` + `git checkout -- mlock.rs` in one NO-BUMP `chore(fmt)`; gate now GREEN under both 1.95.0 and 1.85.0 (the rust.yml:35-37 re-drift invariant re-verified). **Deferred leg:** the `mlock.rs` re-baseline + g6-exemption drop are NOT in this chore — they ride the next 1.95.0-formatted ms-cli g6-pin tag (the codec32-vendor cycle's forced ms-cli publish is the natural carrier), closing `mlock-rs-fmt-exempt` + `mlock-g4-a-page-count-assert-flake` in lockstep then. `mlock.rs` stays byte-synced to the frozen `ms-cli-v0.7.0` blob (`6cb9667`) meanwhile, so the carve-out remains load-bearing and correct.
- **Status:** `resolved` (b-minimal; mlock leg deferred to the next ms-cli g6-pin tag). **Tier:** `infra`. **Companion:** `mlock-rs-fmt-exempt` (carve-out persists by design until the deferred leg lands) + `mlock-g4-a-page-count-assert-flake` (folds into the same future ms-cli tag) — same gate.
