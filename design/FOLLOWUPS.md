...
- **Status:** open. **Tier:** `cross-repo` (constellation-level: md-codec id-choice + mk-codec stub formula + toolkit bundle/restore/verify + SH device). SeedHammer's T6 flagship is **full-policy-only** for now (user-confirmed); SH revisits template engraving only after the constellation adopts this. Cross-ref: SeedHammer design `seedhammer-T6-recon-bundle-composition-stub.md` + `mnemonic-engrave/design/FOLLOWUPS.md` `constellation-template-only-engraving`.
