...
- **T1 SHIPPED 2026-06-12** (tests/cli_restore_taproot_refusal.rs, NO-BUMP): the 3 bundle-reachable refusal arms are pinned — general tr leaf (restore.rs:710), multi-leaf taptree (same arm) + its `.descriptor` exact round-trip (wire-faithful leg), and the distinct-cosigner-IK non-NUMS arm (restore.rs:689). The verify/address legs were de-scoped (verify-bundle needs cosigner mk1; restore refuses before address derivation). **T3 remainder:** (i) faithful single-leaf + depth-1 two-leaf reconstruction; (ii) the ONLY fixture-requiring refusal arm — keypath-only `tr(NUMS)` with `tree:None` (bundle's origin-annotation gate blocks it, so it needs a hand-built md_codec wire fixture); (iii) DIAGNOSTICS nit — keypath-only `tr(<xpub>)` cards hit the :689 arm with a "multisig"-worded message that misleads for single-sig (reword in T3, not before — T1 PINS that message as the current contract).
- **Status:** `open` (T1 contract SHIPPED; T3 reconstruction = deferred feature, partly blocked on the depth-2 upstream fix).
