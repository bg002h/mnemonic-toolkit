...
### `intake-surfaces-accept-masked-older-no-advisory` — import/round-trip surfaces accept consensus-masked `older()` with no warning

- **Surfaced:** 2026-06-11, v0.53.9 (`older()` mask-gate cycle, R0-r1 M5).
- **Where:** intake/round-trip surfaces (`xpub-search`, `import-wallet`, `export-wallet --descriptor`, `compare-cost`) parse descriptors via miniscript `from_str` (`wallet_import/*`, `parse_descriptor.rs`, `descriptor_intake.rs`), which accepts the masked `older()` values the v0.53.9 *authoring* gate now rejects.
...
- **Fix:** a NON-blocking advisory (warning, not refusal) on verify/import surfaces when a descriptor's `older()` carries garbage bits or a zero 16-bit value — "this descriptor's relative timelock is consensus-masked to <effective>; the literal <N> overstates it."
- **Tier:** deferred (advisory UX; the funds-safety-critical authoring path is fixed in v0.53.9).
