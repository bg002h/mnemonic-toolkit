...
- **Tier:** deferred (low-risk; cosmetic label on a template-derived policy).
  ...
  - **[obs] (OPEN — needs the 19-file audit; split from Cycle B 2026-06-11)** `hand-frozen-lint-canons-no-completeness` — lint_zeroize_discipline relies on a hand-maintained ZEROIZE_ROWS list whose only completeness signal is a 17-wide count range plus per-row file-level source.contains anchors. Unlike the argv lint (whi (`crates/mnemonic-toolkit/tests/lint_zeroize_discipline.rs:251-294 (count check :252-266, evidence loop :268-294); contrast lint_argv_secret_flags.rs:184-223`). **R0 finding (Cycle B r1 I3):** a source→declared file-scan needs a 19-file allowlist, and the classification is NOT trivial — `cmd/verify_bundle.rs` + `cmd/ms_shares.rs` carry OWNED secrets (`Zeroizing::new` on passphrase/entropy) with NO canonical ZEROIZE_ROW; blanket-allowlisting them would write a FALSE "transient" record. **Real work:** audit all 19 unlisted secret-bearing files, PROMOTE genuine owned-secret sites to canonical rows (≥ verify_bundle, ms_shares — a real coverage gain), allowlist only crypto-internal/pass-through, THEN add the scan. Own cycle-prep + SPEC + R0.
