...
### `restore-multisig-taproot-reconstruction` — `mnemonic restore --md1` refuses a taproot multisig md1; reconstruction is blocked only in md-codec's `to_miniscript` classification (the toolkit's own `build_descriptor_string` already builds `tr(sortedmulti_a)`) — route around it
...
- **Why deferred:** descoped to keep the v0.44.0 cycle tight on the proven wsh/sh-wsh path (user decision, recon'd at cycle time). **No longer "material/no-reuse"** — the 2026-06-05 re-check (above) found the reconstruction is reuse-heavy (no new descriptor/checksum code). md-codec pinned `0.35.0` (no bump needed). Option (a) [md-codec/rust-miniscript `Terminal::SortedMultiA`] and the original option (b) [bespoke string emitter] are BOTH unnecessary — superseded by the route-around-md-codec approach.
- **Status:** `open`
- **Tier:** `v0.5`
