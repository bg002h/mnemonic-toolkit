...
### `self-check-ms1-decode-not-zeroizing` — `self_check_bundle` drops a decoded ms1 `Payload` (master-seed entropy) un-scrubbed

- **Surfaced:** 2026-06-20, the SeedHammer fork **Rust-primary cross-check** (new project rule: when a defect is found+fixed in a downstream Go port, ALWAYS check whether the SAME defect exists in the primary Rust implementation, and if so fix Rust first). The fork's Go own-code bug-hunt finding **L1** (scrub the ms1 decode-probe) was cross-checked against this toolkit → the same defect class is present here (the other 7 of 8 findings were Go-only). **[handed off from another Claude instance working the SeedHammer fork — flagged here for pickup, NOT fixed there.]**
- **Defect (LOW):** `self_check_bundle` (`crates/mnemonic-toolkit/src/cmd/bundle.rs:2473`) decodes ms1 via `ms_codec::decode(ms)` into a bare `ms_codec::Payload` carrying master-seed entropy, uses it ONLY as an equality oracle (`:2477` `payload.as_bytes() != expected_bytes`), then drops the husk **un-scrubbed**. `ms_codec::Payload` is `#[non_exhaustive]` and intentionally NOT zeroize-wrapped — its own documented caller-wrap contract (`ms-codec payload.rs:17-44`; widening the public type is a deferred breaking change, OOS-2) puts the scrub on the caller. The **other 4** toolkit ms1-decode sites correctly move bytes into `Zeroizing` (`slot_ms1.rs:50,74`; `silent_payment.rs:150-151`; `overlay.rs:133,148`; `bundle.rs:2029,2038`); this one site is the miss.
- **Fix:** mirror the in-file sibling idiom (`bundle.rs:2028-2039`) — move the entropy OUT of `Payload` into `Zeroizing<Vec<u8>>` (`Entr(bytes)`→`Zeroizing::new(bytes)`, `Mnem{entropy,..}`→`Zeroizing::new(entropy)`, non-exhaustive `_`→`Zeroizing::new(p.as_bytes().to_vec())`) and compare that. Moving (not copying) avoids leaving the husk's `Vec` un-scrubbed. Existing tests `bundle.rs:2687/2694` are the behavior regression guard (pure refactor; the scrub itself is a type-level guarantee — no runtime zeroize test, consistent with the other 4 sites). Add a row to the existing `lint_zeroize_discipline` test. **NO-BUMP candidate** (internal secret-hygiene; no wire/API change).
- **Cross-ref:** verbatim cross-check report `mnemonic-engrave/design/agent-reports/seedhammer-own-code-rust-crosscheck-scrub.md`; the Rust-primary rule (`mnemonic-engrave/CLAUDE.md` Conventions, 2026-06-20); convergence with the already-shipped fork Go L1 fix (`seedhammer` merge `39cb5cf`). **Tier:** toolkit-local (Rust). Once fixed, the constellation-wide convergence for fork-L1 is closed.
...
### `addresses-restore-passphrase-not-zeroizing` — BIP-39 passphrase held as plain `String` (entropy already Zeroizing)
...
- **Tier:** deferred (low-risk; the passphrase is short-lived and the high-value secret — the entropy — is already wrapped). Pairs naturally with any future `addresses`/`restore` touch.
