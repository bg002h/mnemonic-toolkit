...
- **This repo's verified findings (23):**
  - **[IMPORTANT]** `mk1-csi-collision-multisig-grouping` — For n>=2, each cosigner mk1 card's chunk_set_id is derived from the cosigner xpub fingerprint (path-independent), not the policy stub. Two slots sharing the same xpub at different derivation paths pas (`crates/mnemonic-toolkit/src/synthesize.rs:277-278; crates/mnemonic-toolkit/src/cmd/verify_bundle.rs:1568-1602,1625-1682 (NOT src/verify_bundle.rs as the finding cited)`)
