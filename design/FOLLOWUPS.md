...
- **This repo's verified findings (23):**
  ...
  - **[obs]** `addresses-env-sentinel-overapplied` — In addresses::run the --from value is passed through resolve_env_var_sentinel unconditionally (line 162) regardless of from.node, so `addresses --from xpub=@env:VAR` (a non-secret watch-only source) a (`crates/mnemonic-toolkit/src/cmd/addresses.rs:159-163`)
...
- **Tier:** deferred (low-risk; cosmetic label on a template-derived policy).
  ...
  - **[obs]** `gate-after-older-upper-bound-deferred-to-step2` — validate_fields rejects older(N) only when N==0 or N>=2^31 and after(N) only when N==0; other out-of-range/semantically-odd timelocks are left to step-2 from_str. Fail-closed: an invalid timelock is s (`crates/mnemonic-toolkit/src/descriptor_builder/gate.rs:244-256 (Older/After field checks)`)
