...
### `restore-non-nums-tr-internal-key-also-in-leaf` — `@-in-both`: a non-NUMS trunk key that is ALSO a leaf key is refused (needs a leaf-membership-aware rebuild)
...
- **Eventual mechanism:** a leaf-membership-aware route-around that rebuilds the actual leaf key set (including the trunk key) rather than the `{all-others}` shortcut — adjacent to the md-codec `SortedMultiA`-rendering work (`md-codec-sortedmulti-a-to-miniscript-rendering-gap`), since both want a from-scratch tap-leaf rebuild rather than the Template shortcut.
- **Why deferred:** the `{all-others}` Template shortcut is byte-faithful for the common distinct-trunk case; the `@-in-both` case needs a genuinely different rebuild path (re-deriving the leaf from the wire indices, not subtracting the trunk index) — its own R0 + test matrix. Refusing-loudly is funds-safe in the interim.
- **Status:** `open`
- **Tier:** `feature` / `next-cycle`.
- **Companion:** none (toolkit-local; blocked-adjacent to the md-codec SortedMultiA rendering gap).
