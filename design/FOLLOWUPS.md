...
### `miniscript-compiler-optimize-policy` — enable rust-miniscript `compiler` to let a user optimize a policy → optimal miniscript

- **Surfaced:** 2026-06-08, descriptor-builder brainstorm (`design/BRAINSTORM_descriptor_builder.md` §8 "deferred: general `Concrete::Policy` compiler + policy-expression input") — the phase-2 optimizer the builder engine deliberately omits.
- **Where:** `crates/mnemonic-toolkit/Cargo.toml:17` — the `miniscript` git dep (`rev = 95fdd1c`) would gain `features = ["compiler"]` (currently `default-features = false, features = ["std"]` at `:35`; the `compiler` feature is **OFF** today). A new user-facing path: either a `--optimize` mode on the descriptor-builder subcommand, or a standalone `optimize` step, that accepts a high-level spending policy (policy-expression string or the builder's node-tree) and calls `Concrete::Policy::compile::<Segwitv0>()` → optimal miniscript → descriptor, paired with a `compare-cost` before/after preview.
- **What:** The descriptor-builder engine (`descriptor-builder-engine`, v0.50) deliberately **hand-builds** fragments and keeps the `compiler` feature OFF — because for a fixed, curated, funds-holding archetype set we must PIN a stable canonical form, and the compiler *chooses* fragments heuristically (not version-stable → can't pin). This FOLLOWUP is the complementary, **opt-in, user-driven** capability: let a user who supplies an **arbitrary** policy have rust-miniscript compile it to the *optimal* miniscript (smallest witness / best malleability) — exactly the case where hand-tuning per-policy isn't feasible and the compiler's optimization earns its keep. The compiler attaches at the **string boundary** (`Concrete::Policy` → `Display` → the existing `from_str → Display → checksum` pipeline), so it's a separable producer with zero rework to the builder IR.
- **Why deferred / the cost to weigh:** (1) **Audit/attack surface** — the policy compiler is a large optimizer; enabling its feature on a funds crate is real added surface (the reason it's off for the curated builder). (2) **Canonical-form stability** — compiler output is NOT version-stable, so this path is explicitly **user-reviewed optimization** (the user opts to optimize and reviews/cost-compares the result), NOT a pinned-canonical-form emit like the archetypes; the SPEC must frame it as "here is an optimized candidate, verify before funding" with a mandatory `compare-cost` before/after + `sanity_check()` gate, never silent. (3) It only earns its keep once the engine accepts arbitrary policies (the deferred general-policy / policy-expression-string input), so it pairs with that phase, not the curated archetypes.
- **Status:** open
- **Tier:** `v0.52+-feature` (after descriptor-builder Release A `v0.50` + presets Release B `v0.51`; pairs with the deferred general-policy/expression-string input).
- **Companion:** none (toolkit-local). Relates to `descriptor-builder-engine` (the curated builder that deliberately keeps `compiler` OFF) — this is its opt-in optimizer counterpart.
