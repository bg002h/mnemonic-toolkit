...
- **This repo's verified findings (23):**
  ...
  - **[minor]** `g6-invariant-sibling-master-not-pin` — The g6-invariant job asserts byte-equality of toolkit's mlock.rs against mnemonic-secret's mlock.rs but checks out the sibling at ref: master rather than the pinned ms-cli-v0.7.0 tag the constellation (`.github/workflows/rust.yml:215-220`)
...
- **Tier:** deferred (low-risk; T1+T2+T3 already give strong coverage — the JP path works, is words-based, and the shared derivation machinery is externally validated on the English vector).
  - **[obs]** `synthesize-incrate-presence-not-correctness` — The in-crate synthesize sanity tests assert ms1.starts_with("ms1"), mk1/md1 all start_with their HRP, and non-emptiness. A regression emitting a syntactically-valid but WRONG card (wrong key/network/a (`crates/mnemonic-toolkit/src/synthesize.rs:959-992 (full_bundle/watch_only sanity) + :994-1022 (determinism)`)
