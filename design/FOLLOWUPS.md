...
- **This repo's verified findings (23):**
  ...
  - **[minor]** `policynode-grammar-coverage-vacuous-on-joint-omission` — node_kinds_cover_enum only compares all_variant_samples() against NODE_KINDS -- both hand-lists -- so a new PolicyNode variant added to the enum but forgotten in all_variant_samples() passes the test  (`crates/mnemonic-toolkit/src/descriptor_builder/ir.rs:289 (all_variant_samples), :345-354 (node_kinds_cover_enum); schema.rs:104-118 (grammar_matches_node_kinds_hand_list comment); cmd/convert.rs:1767 (declare_node_type_variants! macro precedent)`)
