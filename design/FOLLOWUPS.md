...
- **Status:** **resolved** (2026-06-22, Wave-2 G1-B). The GUI redaction shipped at `mnemonic-gui-v0.39.0` (sibling `gui-run-confirm-modal-secret-redaction` flipped resolved at `mnemonic-gui-v0.48.1`): `SECRET_MASK = "••••"` (`mnemonic-gui/src/form/invocation.rs:137`); the run-confirm modal substitutes the sentinel per masked token at `src/main.rs:1098-1104` (the real argv still spawns on Run). This manual was reverted in lockstep — (i)+(ii)+(iii) all landed: chapter 14 Defense-2 restores the `••••` redaction claim and demotes the cold-node mitigation from `:::danger` to a `:::note` (with a `:::warning` preserving the genuinely-still-true residual: flag NAMES stay visible, only the secret VALUE is masked); chapter 11 feature-2 restores the accurate redaction claim; (iv) `pinned-upstream.toml` `[mnemonic-gui]` bumped `mnemonic-gui-v0.3.0` → `mnemonic-gui-v0.48.1` (implied CLI tags re-pinned for coherence: toolkit-v0.70.0 / md-cli-v0.7.0 / ms-cli-v0.8.0 / mk-cli-v0.9.0). Manual-prose NO-BUMP (no crate version change).
- **Tier:** RESOLVED (was: `v1.1+`).
