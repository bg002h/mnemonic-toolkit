...
- **Companion:** `mnemonic-gui` — see `gui-build-descriptor-schema-mirror-pending-pin-bump` below (the schema-mirror lockstep). **The GUI wizard shipped at the archetype-forms level** (`mnemonic-gui-v0.31.0`, 2026-06-09: data-driven param forms consuming the `--spec-schema` `archetypes` section + an `archetype_schema_mirror` drift gate; the recursive node-tree builder remains deferred — the GUI's own roadmap item).
