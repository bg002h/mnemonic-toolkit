...
- **This repo's verified findings (23):**
  ...
  - **[obs]** `mk1-chunk-set-id-fingerprint-grouping-assumes-distinct-fps` — In multisig emission each cosigner's mk1 chunk_set_id is derived from the 4-byte BIP-32 fingerprint, and verify_bundle re-groups supplied mk1 chunks by chunk_set_id to reassemble per-cosigner. Two cos (`crates/mnemonic-toolkit/src/synthesize.rs:260 (single-sig csi from stub), :277-280 (multisig csi from c.xpub.fingerprint().to_bytes()); cmd/verify_bundle.rs:1571-1576 (group supplied.mk1 by chunk_set_id); mnemonic-key crates/mk-codec/src/string_layer/chunk.rs:131,152,160,189-195 (reassemble integrity guards)`)
  - **[obs]** `n1-vs-nge2-csi-derivation-inconsistency` — n=1 seeds the mk1 csi from the policy stub (derive_mk1_chunk_set_id(&stub)) while n>=2 seeds it from the xpub fingerprint. The engraving-card display helper computes mk1_card_id from the policy stub f (`crates/mnemonic-toolkit/src/synthesize.rs:260 vs 278; cmd/bundle.rs:1087-1109`)
