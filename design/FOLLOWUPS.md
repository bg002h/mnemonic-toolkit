...
- **Status:** `open` — **toolkit half SHIPPED toolkit-v0.46.2** (`restore_conditional_rules()` + `build_subcommand_conditional_rules` `"restore"` arm; `mnemonic gui-schema` now projects `restore`'s `--from required-unless-md1` as `not(flag_present "--md1") → {--from, required}`, mirroring bundle's `--template` precedent; `EXPECTED_ARM_COUNT 6→7`). **GUI consumption half PENDING the GUI's next toolkit-pin bump:** bump pin → v0.46.2 so `gui_schema_conditional_drift` enforces the (already-matching) `conditional::restore` fn + add `("restore", 1)` to `SUBCOMMAND_FLOORS` (`gui_schema_conditional_drift.rs:300`). Flip to `resolved` once the GUI tag ships. Audit trail: `design/SPEC_gui_schema_restore_conditional_projection.md` + `design/agent-reports/gui-schema-restore-conditional-projection-*`.
- **Tier:** `cross-repo`
