...
- **This repo's verified findings (23):**
  ...
  - **[obs]** `bip388-policy-name-lossy-roundtrip` — descriptor_to_bip388_wallet_policy hardcodes the policy name on emit and expand_bip388_policy deserializes the policy name into _name but never threads it through, so a --format bip388 round-trip lose (`crates/mnemonic-toolkit/src/wallet_export/pipeline.rs:207 (emit) + crates/mnemonic-toolkit/src/wallet_import/pipeline.rs:161-207 (expand)`)
