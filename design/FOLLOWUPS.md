...
- **This repo's verified findings (23):**
  ...
  - **[IMPORTANT]** `vacuous-secret-flag-gate` — The test presented as the toolkit-internal half of the secret-flag drift gate cannot fail because both sides of its equality derive from the same function. It provides ZERO protection against a new se (`crates/mnemonic-toolkit/tests/cli_gui_schema_v5_extensions.rs:283-307 (assertion at :298-304); src/cmd/gui_schema.rs:1196; src/secrets.rs:49-64`)
  ...
  - **[obs]** `flag-is-secret-completeness-unguarded-by-design` — The leak-prevention chain anchors on secrets::flag_is_secret(name), a hardcoded matches! allowlist of ~11 flag names. gui_schema.rs:1196 derives the schema's per-flag secret bit from exactly this pred (`crates/mnemonic-toolkit/src/secrets.rs:49-64; crates/mnemonic-toolkit/src/cmd/gui_schema.rs:1196`)
...
- **Status:** open (backlog index; individual items dispositioned in the report). I7/I8 (GUI), I1/I2 (mk-cli v0.8.0), I9 (ms-codec v0.4.1), I10 + n1-vs-nge2 (toolkit v0.53.0) resolved.
- **Tier:** audit-backlog.
