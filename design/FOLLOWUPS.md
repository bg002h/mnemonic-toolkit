...
- **Status:** `open` — **DESIGN COMPLETE + R0-GREEN, implementation NOT started** (halted at plan-green per user directive). Resume at **P1** of the plan (engine: `unrank_kperm` + own-anchored generator + `c_choose` overflow + the `early_exit` knob). Ships toolkit MINOR **0.61.0** (md/mk NO-BUMP; paired GUI schema-mirror for the new flags). #28 phase 2 P3a–P5 already SHIPPED at v0.60.0 *around* this gate (`--own-account-max` wired but **refuses**; the exact-`--account`-list path is live). Flip to RESOLVED in the 0.61.0 shipping commit.
...
- **Status:** `open`.
- **Tier:** `polish` / `next-cycle`. **Origin:** decoupled OUT of the (paused) own-account subset-search cycle (`feature/own-account-subset-search`) on architect advice — subset-search CONSUMES `derive_account_xpub_only` as a 1-line dependency; it does NOT author this. Natural sibling of the cycle-15/Lane-T derived-output zeroize work (whoever owns that lane may fold it; else a standalone hygiene increment).
