...
4. Refusals table has **25 rows** (row 25 added at P3 R1 fold per python `split_ems` rule; `--group N,T` with `T==1 AND N>1`) mirroring SPEC §2.5 byte-faithfully. Lib agreement is via `cli_slip39_refusals.rs::stderr.contains` substring assertions (15 unique substring anchors; not full-stem-by-stem since multiple SPEC rows can share a lib stem when the toolkit reuses an existing `BadGroupSpec` variant for an additional refusal class — row 25 reuses row 4's `1 <= T <= N <= 16` stem, which is doc-only at SPEC layer per §4 G5 note. R0 I2 fold (paired-SPEC-patch mandate): if SPEC and lib emit drift at GREEN-write, GREEN MUST patch SPEC; row 25 was added at P3 R1 fold under exactly this trigger when chapter examples surfaced the previously-undocumented refusal class. R1 I-1 wording tightening: "lib byte-faithful" in this criterion refers to substring-match-faithfulness via the `stderr.contains` assertions, not stem-byte-equality.
