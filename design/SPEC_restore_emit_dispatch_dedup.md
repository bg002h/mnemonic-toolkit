...
- **Phase-1 RED cell:** add ONE cell pinning the NEW single-sig `restore --format coldcard-multisig` refusal (the unified "requires a multisig --template" message, exit 1) — RED against the current binary (old message), GREEN after §3. This makes the §4 reword intentional + locks it.
...
## 7. Phased plan
- **Phase 1 (RED):** the single-sig coldcard-multisig message cell (asserts the new unified message). Verify RED-for-the-right-reason (current binary emits the old message).
...
- **Phase 3 (release):** CHANGELOG `[0.46.1]` (note the reworded single-sig refusal); version v0.46.0 → **v0.46.1** (Cargo.toml/lock + 2 READMEs + install.sh self-pin); FOLLOWUP `restore-emit-dispatch-3way-dedup` → resolved (correcting "3-way" → "4-way" in the close). Per-phase review.
