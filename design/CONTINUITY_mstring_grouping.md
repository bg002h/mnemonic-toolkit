...
1. **P1 DONE.** Next: **P2** (mnemonic-secret/ms-cli — flags, split print-once, `ms combine -`→stdin, remove doubling heuristic, rewrite encode_canonical_12_word.rs), then **P3** (mnemonic-key/mk-cli `read_mk1_strings` intake), **P4** (toolkit pin-bump md-codec 0.36.0 + wire emit/intake + DELETE `chunk_5char`/`chunk_mk1`/`chunk_md1` + golden regen `tests/vectors/v0_1`+`v0_2` + manual + technical-manual remove dead `chunk_*` rows + Examples.pdf regen + **file the toolkit-side `display-grouping-render-strip-v1` companion**), **P5** (mnemonic-gui schema_mirror flags + separator keyword dropdown). Each: own plan → plan-R0 GREEN → TDD execute → pause before release.
2. The toolkit-side P4 must pin-bump the constellation's md-codec dep to `0.36.0` (now on crates.io) when it lands.
