...
| low | toolkit · timelock | decaying-multisig **tier-3 absolute `after(T)` is never validated against the decay invariant** → a past height makes the last-resort key immediately spendable | `descriptor_builder/archetype.rs:305-317`, `gate.rs:306-324` | Distinct facet of the W3 decaying-multisig item (relative `older` there; absolute `after` here). Real funds-relevant; pair the fix. |
