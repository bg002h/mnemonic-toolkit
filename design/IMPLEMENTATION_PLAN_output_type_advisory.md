...
> **PHASE RE-PIN DISCIPLINE (mandatory, every wiring phase P1-P4) [closes the R1/R2/R3 defect-class].** A phase that re-words/removes an OLD literal or addendum, or drops a TTY gate, can break a test assertion in ANY file — including ones not obviously "that command's suite" (e.g. `cli_indel.rs` exercises `repair`). So before committing each phase, do NOT rely on the named cell lists alone: **`grep -rn` across all of `crates/mnemonic-toolkit/tests/` (and ms-cli tests in P4) for every OLD string this phase changed** — the D9 literal `secret material on stdout`, each dropped addendum (`SLIP-39 shares on stdout`, `candidate list is secret material`, `Seed XOR shares on stdout`, `combined phrase is secret material`, `reconstructed secret material on stdout`) — and re-pin/invert EVERY match (positive → new wording; negative `!contains` on a now-unconditional line → invert) so the phase commit is green. The named lists below are the known-at-plan-time matches; the grep is the backstop. Run `cargo test -p mnemonic-toolkit` at each phase commit, not just the touched suites.
...
- [ ] **Step 4: Run — verify PASS + no regression, re-pinning the P1-touched suites IN P1** (so the P1 commit is green; only the cross-cutting/doc remainder defers to P5). Run `cargo test -p mnemonic-toolkit --test cli_output_class` + the touched suites (`ls tests | grep -E 'slip39|seed_xor|final_word|seedqr|electrum|silent|derive_child|addresses|export'`). Re-pin NOW, in P1: (a) positive `.contains("secret material on stdout")` → new P wording; (b) **the 5 TTY-gate NEGATIVE cells [R1-I-new + R2-I1]** — `cli_final_word_advisories.rs:72`, `cli_seed_xor_advisories.rs:96`, `cli_seed_xor_advisories.rs:133`, `cli_slip39_advisories.rs:265` (split), `cli_slip39_advisories.rs:310` (combine) — INVERT to assert the new P-line is PRESENT on piped stdout (find via `grep -rn silent_when_piped tests/`; the `secret material on stdout` grep won't find them). Each P1-command suite green at the P1 commit.
...
- [ ] **Step 4: Run — verify PASS + re-pin (per the PHASE RE-PIN DISCIPLINE grep).** [folds R3-I1] The `repair --ms1 --max-indel` path emits the old literal via `emit_repair_report` (`repair.rs:1333`) — re-routed here — and possibly via the `cmd/repair.rs:216` site (P2). So `cli_indel.rs:223,232` (`ms1_indel_recovery_fires_secret_advisory`, asserts `ADVISORY` = the old D9 literal) breaks at whichever phase first changes its emitting site: **re-pin `cli_indel.rs:223-225,232` to the new P-line in that phase** (P2 if `cmd/repair.rs:216` fires on the indel path, else here in P3). `cargo build -p mnemonic-toolkit` clean. Consolidation guard: `! grep -rq 'secret material on stdout' crates/mnemonic-toolkit/src` (zero orphaned literals) AND `cargo test -p mnemonic-toolkit` GREEN (the catch-all grep covers cli_indel.rs + any straggler).
...
- [ ] **Step 1: Re-pin the REMAINING test assertions** (the P1/P2/P4-touched suites are already re-pinned in their own phase — see those Step-4 notes — so each phase commit stays green). `grep -rl 'secret material on stdout' crates/mnemonic-toolkit/tests` (11 asserting + `cli_secret_in_argv_warning.rs` comment-only [M2]) + `mnemonic-secret/crates/ms-cli/tests`. Three re-pin classes [folds I6]:
  ...
  - **NEGATIVE/absence assertions broken by the TTY-gate drop [folds R1-I-new + R2-I1 — the `secret material on stdout` grep does NOT find these; they assert DIFFERENT literals]. ALL FIVE, enumerated (discover by `grep -rn 'silent_when_piped' tests/` across the 5 TTY-gate suites — NOT the literal):** `cli_slip39_advisories.rs:265-266` (`advisory_k_of_n_stdout_silent_when_piped`, `!contains("SLIP-39 shares on stdout")` — slip39 **split**, gate `slip39.rs:544` + addendum `:548`), `cli_slip39_advisories.rs:310-313` (`advisory_combine_reconstructed_silent_when_piped`, `!contains("reconstructed secret material on stdout")` — slip39 combine), `cli_final_word_advisories.rs:72-75` (`!contains("candidate list is secret material")`), `cli_seed_xor_advisories.rs:96-98` (`!contains("Seed XOR shares on stdout")`), `cli_seed_xor_advisories.rs:133-135` (`!contains("combined phrase is secret material")`). Each command now emits unconditionally → **INVERT** each to assert the new unified P-line IS PRESENT on piped (non-TTY) stdout (do NOT "re-pin to new wording"). **These MUST be folded in P1** (their commands are P1) to keep the P1 commit green — not deferred to P5;
