...
- **`cmd/convert.rs`** — `pub language: Option<CliLanguage>` (`:245`); `language.unwrap_or_default()` (`:1111`) → `Mnemonic::parse_in(language, …)` (`:1130`). `--to` is `pub to: Vec<String>` (`:226`) parsed into `targets: Vec<NodeType>` (`:850`) — **multi-target allowed** (`--to xpub,entropy`); the type is `enum NodeType` (`:31`), NOT `ConvertTarget` (no such type). **`--to seedqr` is REFUSED at parse** (input-only node; `:867-871` comment + `Seedqr => unreachable!` `:1207`). The language-dropping LIVE targets are `Entropy` **and `Ms1`** (`Ms1 => ms_codec::encode(Tag::ENTR, Payload::Entr(entropy))` at `:1192` — an ms1 card carries only the entropy, exactly like raw entropy; it is in fact the canonical engravable language-losing form). `Phrase` keeps the language; `Xpub/Xprv/Wif/Bip38/ElectrumPhrase/Address` are derived keys — the seed's language was already applied, no re-recovery ambiguity. **[end-of-cycle R0 C1 amendment]** the original SPEC draft listed only `Entropy` and silently omitted `Ms1` from this enumeration; the implementation's convert advisory now gates on `Entropy || Ms1`. **[end-of-cycle R0 I1 amendment]** the convert advisory is emitted *after* `compute_outputs` succeeds (not before, at the old `:952` site) — the primary phrase is parsed inside `compute_outputs` (`Mnemonic::parse_in` at `:1147`), so emitting earlier would advise-then-error on a malformed phrase.
