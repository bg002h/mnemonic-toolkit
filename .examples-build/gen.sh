...
# verbatim command input + output captured from the real `mnemonic` v0.104.0
...
[ "$VER" = "mnemonic 0.104.0" ] || { echo "FATAL: expected mnemonic 0.104.0, got '$VER'" >&2; exit 1; }
...
subtitle: "mnemonic-toolkit v0.104.0 -- worked examples (Linux), exact verbatim I/O"
date: "2026-07-05"
...
`mnemonic` **v0.104.0** on Linux and **both the command and its full output are
...
with `mnemonic` v0.104.0 on Linux. See the in-repo manual (`docs/manual/`) for
...
as of v0.104.0** and everything below is live-captured from the shipped
