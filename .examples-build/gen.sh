...
# verbatim command input + output captured from the real `mnemonic` v0.100.1
...
[ "$VER" = "mnemonic 0.100.1" ] || { echo "FATAL: expected mnemonic 0.100.1, got '$VER'" >&2; exit 1; }
...
subtitle: "mnemonic-toolkit v0.100.1 -- worked examples (Linux), exact verbatim I/O"
date: "2026-07-05"
...
`mnemonic` **v0.100.1** on Linux and **both the command and its full output are
...
with `mnemonic` v0.100.1 on Linux. See the in-repo manual (`docs/manual/`) for
...
as of v0.100.1** and everything below is live-captured from the shipped
