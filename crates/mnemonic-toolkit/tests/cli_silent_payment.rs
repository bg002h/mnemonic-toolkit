...
fn json_shape_and_labeled_differs() {
    ...
    assert_ne!(
        v["labeled"][0]["address"], v["address"],
        "labeled must differ from base"
    );
...
fn passphrase_changes_derived_address() {
    ...
    assert_ne!(
        with_pass["address"], no_pass["address"],
        "passphrase must change the SP address"
    );
...
fn xprv_input_with_passphrase_warns_and_ignores() {
    ...
    assert!(
        ...
        "expected ignore-warning; got: {stderr}"
    );
    let with_pass: serde_json::Value = serde_json::from_slice(&out.get_output().stdout).unwrap();
    assert_eq!(
        plain["address"], with_pass["address"],
        "xprv address must be passphrase-independent"
    );
}
...
fn change_address_emits_tagged_m0_distinct_from_base() {
    ...
    assert!(
        s.to_lowercase().contains("change"),
        "change line present + tagged"
    );
...
fn change_address_json_has_value_and_warning() {
    ...
    assert!(v["change_address_warning"]
        .as_str()
        .unwrap()
        .to_lowercase()
