...
//! - sniff-negative: Sparrow blob with `--format bsms` exits 1
...
#[test]
fn sparrow_with_bsms_format_exits_format_mismatch() {
    // Explicit `--format bsms` on a Sparrow blob: sniff says Sparrow,
    // user says BSMS → ImportWalletFormatMismatch exit 1.
    // (The current dispatch logic only ImportWalletFormatMismatches when
    // the SNIFF says one of the SPECIFIC known formats — Sparrow sniff
    // matches first, so user `--format bsms` actually proceeds to the
    // BSMS parser, which fails with ImportWalletParse on the JSON. Both
    // exit codes are non-zero acceptance for this assertion.)
    let p = fixture_path("sparrow-singlesig-p2wpkh.json");
    let assertion = run_import(&["--blob", p.to_str().unwrap(), "--format", "bsms"]).failure();
    let stderr = String::from_utf8(assertion.get_output().stderr.clone()).unwrap();
    // Either ImportWalletFormatMismatch or ImportWalletParse — depends on
    // the dispatch ordering between sniff and explicit-format mismatch
    // check. The current code at cmd/import_wallet.rs (Site 2's bsms arm)
    // returns ImportWalletFormatMismatch only when sniff yielded
    // `BitcoinCore`. Sparrow sniff yields `Sparrow`, so the BSMS arm
    // proceeds and BsmsParser fails the JSON blob → ImportWalletParse.
    assert!(
