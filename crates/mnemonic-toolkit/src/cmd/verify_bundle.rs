...
pub struct VerifyBundleArgs {
    ...
    /// DEPRECATED and IGNORED since v0.99.0 (as on `restore`) — accepted so
    /// existing scripts do not fail on an unknown flag, but it changes nothing.
    /// There is no search-time ceiling any more: the completion declares its
    /// estimate and reports progress instead (Ctrl-C stops it). Still parses a
    /// humantime duration (e.g. `2h`, `90min`) when a search runs.
    ...
    /// `--bundle-json`. `--ms1 -` reads ONE ms1 from stdin (at most one per
    /// invocation; on a terminal it prompts `Enter ms1:` with echo off);
    /// stdin that is empty or only separators is refused, never read as the
    /// watch-only `""`.
...
pub fn run<W: Write, E: Write>(
    ...
    crate::passphrase_input::refuse_second_stdin(&[
        ...
        // F-687b fold 1 (review M3): the path-is-stdin readers the F-687
        // guard above lists.
        (
            args.bundle_json
                .as_deref()
    ...
    let args: &VerifyBundleArgs = if ms1_dash_count == 1 {
        ...
        // Same reader as a stdin passphrase: a terminal gets `Enter ms1: `
        // with echo off (the ms1 is seed material), a pipe is read to EOF.
        let raw_in = crate::passphrase_input::read_stdin_raw(&crate::passphrase_input::MS1, stdin)?;
        let read = zeroize::Zeroizing::new(raw_in.trim().to_string());
        // F-687b fold 1 (review M1): judge emptiness on what the value will
        // BECOME after the display-separator strip below -- stdin of `-` or
        // `---` would otherwise pass here and strip to the watch-only `""`.
        if crate::display_grouping::strip_display_separators(&read).is_empty() {
            return Err(ToolkitError::BadInput(
                "--ms1 -: stdin was empty or only separators (an empty --ms1 would mean a \
                 watch-only cosigner)"
                    .into(),
            ));
        }
    ...
    let stdin_synth;
    let args: &VerifyBundleArgs = if needs_stdin_substitution(args, pp_reads_stdin) {
        stdin_synth = apply_stdin_substitutions(args, stdin, pp_reads_stdin)?;
        &stdin_synth
    } else {
        args
    };
...
fn verify_multisig_template<W: Write, E: Write>(
    ...
    // F-687: `args` here is post-substitution — the passphrase is already
    // resolved (stdin read, `@env:` expanded, argv note emitted). Passing it
    // as written made the helper re-read a drained stdin (the EMPTY
    // passphrase: a false NO MATCH, exit 4) and re-emit the argv note.
    let seed = resolve_template_completion_seed(
        args.from.as_deref(),
        no_from,
        crate::passphrase_input::PassphraseArg::Resolved {
            value: args.passphrase.as_deref(),
            // set by `apply_stdin_substitutions` for `-` as well as the flag
            consumed_stdin: args.passphrase_stdin,
        },
        args.language,
        stdin,
        stderr,
...
fn needs_stdin_substitution(args: &VerifyBundleArgs, pp_reads_stdin: bool) -> bool {
...
fn apply_stdin_substitutions(
    ...
    if pp_reads_stdin {
        ...
        // From here on `passphrase` is RESOLVED and `passphrase_stdin` records
        // that it came from stdin (for `-` too) — see the template path.
        owned.passphrase_stdin = true;
