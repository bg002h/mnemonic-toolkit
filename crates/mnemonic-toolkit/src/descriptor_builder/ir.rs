...
/// Every `PolicyNode` external tag (the single JSON key). Hand-maintained, and
/// **cross-checked** against `kind()`'s sample outputs by `node_kinds_cover_enum`
/// and against the `--spec-schema` grammar by `grammar_matches_node_kinds_hand_list`.
/// Those checks catch a *drift* between these three hand-lists; they cannot by
/// themselves force a brand-new variant into all three (a fully airtight
/// enum→list guard needs a variant-enumerator macro — out of scope). Adding a
/// variant is author discipline; v1 is hand-verified 17 == 17 == 17. Consumed by
/// `super::schema`.
...
impl PolicyNode {
    /// The external tag (single JSON key) for this node — also the diagnostic
    /// fragment label. Must stay in sync with [`NODE_KINDS`].
    pub fn kind(&self) -> &'static str {
        match self {
            PolicyNode::Pk(_) => "pk",
            PolicyNode::Pkh(_) => "pkh",
            PolicyNode::Multi(_) => "multi",
            PolicyNode::Sortedmulti(_) => "sortedmulti",
            PolicyNode::Older(_) => "older",
            PolicyNode::After(_) => "after",
            PolicyNode::Sha256(_) => "sha256",
            PolicyNode::Hash256(_) => "hash256",
            PolicyNode::Hash160(_) => "hash160",
            PolicyNode::Ripemd160(_) => "ripemd160",
            PolicyNode::AndV(_) => "and_v",
            PolicyNode::OrD(_) => "or_d",
            PolicyNode::OrI(_) => "or_i",
            PolicyNode::OrB(_) => "or_b",
            PolicyNode::Andor(_) => "andor",
            PolicyNode::Thresh(_) => "thresh",
            PolicyNode::Wrap(_) => "wrap",
        }
    }
