...
Reported privately by Robin (Liquidium); independently reproduced here before fixing. Upstream published `bip322` 0.0.11 and YANKED 0.0.6–0.0.10.

- **[security · funds-adjacent] witness key is now bound to the challenged address.** `bip322` 0.0.6–0.0.10 lifted the ECDSA public key straight out of attacker-controlled `witness[1]` and never checked it against the address: its one equality check compared that key with *itself* (0.0.10 `verify.rs:134`), and the P2SH arm discarded `script_hash` entirely (`:87`), deriving the sighash script from the attacker's own key (`:168`). `sign_simple_encoded` performed no key↔address check either, so the whole attack was "sign the victim's address with your own key". Affected versions: **0.0.6–0.0.10** (P2WPKH) and **0.0.7–0.0.10** (P2SH-P2WPKH); the P2SH hole accepted *any* P2SH address, including P2SH-multisig. A new fail-closed `bip322_key_binding_holds` pre-gate now runs BEFORE the crate call: exactly two witness items, `witness[1]` a **33-byte compressed** key, and that key must reproduce the address's exact `script_pubkey` (`p2wpkh` / `p2sh(p2wpkh)`). P2TR and P2PKH pass through untouched. The explicit 33-byte check is load-bearing — `CompressedPublicKey` names its *output* serialization, not its accepted input, and `from_slice` accepts 65-byte uncompressed and hybrid keys.
- **[deps] `bip322` `=0.0.10` → `=0.0.11`, re-vendored.** 0.0.11 binds the key itself (`verify.rs:324-326`, compared against the address-derived prevout) and fixes both panics 0.0.10 carried (uncompressed-key `wpubkey_hash().unwrap()`; the P2TR arm's unguarded `witness[0]` on a 0-item witness). **0.0.10 is YANKED**, so the previous exact pin resolved only via the committed `Cargo.lock`. The bump is NOT drop-in: `sign_simple_encoded` is now 4 args taking WIFs as a slice, plus a new proof-of-funds API — test-only impact here, since `verify_simple_encoded` (the sole production call) is unchanged. New transitive deps: `base64` 0.23.0 alongside 0.22.1, `snafu` 0.8.9 → 0.9.2.
- **[expressiveness] P2WSH multisig proofs now verify.** A consequence of the bump, accepted deliberately: 0.0.11 adds `p2wsh`, `p2sh_multisig` and `p2pkh` verify arms where 0.0.10 had only `p2wpkh`/`p2tr`. A genuine P2WSH multisig proof now returns `VALID`/exit 0 where 0.0.10 returned `UnsupportedAddress`. The gate abstains for v0-32 programs (the crate binds them by witness-script hash). The P2SH-multisig and P2PKH arms are entered but can never succeed on this call path — both require a **scriptSig**, and the BIP-322 *simple* encoding carries none (`create_to_sign` hard-codes `script_sig: ScriptBuf::new()`).
- **[fix] surrounding whitespace on the signature is now trimmed.** The address was trimmed but the signature was not, so a genuine proof carrying a trailing newline — `$(cat sig.txt)`, a heredoc, a copy-paste — reported INVALID. Fail-closed and pre-existing; never a correctness or funds risk.

### Migration / dispositions

- **Behaviour change, intended:** a forged proof that previously reported `VALID`/exit 0 now reports `INVALID`/exit 1. P2SH-wrapped scripts that are not P2WPKH previously returned a meaningless `VALID` and now return `INVALID` — a 2-item simple witness cannot prove control of those.
- **Downstream:** `mnemonic-gui` must re-pin to this tag; it was exposed transitively via its `mnemonic-toolkit` pin. No clap flag changed, so the `schema_mirror` gate does NOT catch this — it is a manual paired-PR item.
- **The local gate is RETAINED** now that upstream is fixed, deliberately: it keeps the invariant in code we own and test, so a future bump, a `[patch]`, or a vendor-tree drift cannot silently reintroduce the flaw.
- **Review:** seven independent adversarial reviews with executable proof-of-work — bypass lens (12 address shapes swept, no bypass), over-rejection lens (8 genuine address forms × 4 networks, no false rejection), test-vacuity (every frozen forgery proven to verify under the vulnerable 0.0.10), supply chain (all 5 vendored trees byte-match published crates.io artifacts; 167 lockfile packages scanned for yanks), and three convergence rounds (200k adversarial P2SH witnesses → 0 VALID). Converged 0 Critical / 0 Important. Suite exit 0; clippy clean; pinned-1.95.0 fmt clean (mlock exempt); vendor-freshness GREEN.
...
- **[expressiveness] `mnemonic inspect` partial-decode.** A dead `md1` card now renders the always-available `template:` line, replaces the `origin_fingerprint:`/`origin_path:` lines with an explicit `origin: «unspecified — supply on restore»` marker (never a fabricated `m/` path), prints a `VERIFY-ME` stderr note, and exits **4**. `--json` gains an additive `partial` object (unresolved `@N` indices); `schema_version` unchanged, non-partial JSON byte-identical. The marker + note are byte-identical to `md decode` (cross-binary parity — a human reads the same bytes on `md` and `mnemonic`). `inspect.rs:207` switched from strict `reassemble` to `reassemble_with_opts(.., DecodeOpts::partial())`; chunk-form intake only (non-chunked single-string `md1` remains a pre-existing limitation, tracked).
- **[fail-safe, funds] `verify-bundle` explicit partial gate.** A SUPPLIED dead `md1` that would otherwise verify `ok` now returns `result: "partial"` + exit **4**. Verdict precedence is **mismatch > partial > ok** (`verify_bundle.rs` `run` + `verify_emit_from_expected`): a dead card that ALSO fails any structural check reports `mismatch`, and — funds-load-bearing — a **corrupted** dead card (cross-chunk content-id mismatch) fails `md1_decode` → `mismatch`, NEVER `partial`. The content-id oracle stays enforced UNDER partial (`allow_unresolved_origin` relaxes ONLY the origin gate); the partial verdict downgrades only an otherwise-clean `ok`. `--json` gains an additive `partial` field on the partial verdict. The strict `--template` route and `restore`/`repair` stay fail-closed (unchanged). `repair` does not partial-decode: a dead card stays exit 2.
- **[re-pin] `md-codec` 0.41.0 → 0.42.0 (the partial-decode release).** Adds the 3-layer partial-allowing decode API (`DecodeOpts::partial()`, `decode_md1_string_with_opts`, `reassemble_with_opts`) + `descriptor.unresolved_origin_indices()`; the toolkit consumes them at the render/verify sites. 0.42.0's new `Error::EmptyOriginOverride { idx }` (an empty use-site override — malformed wire) is routed to the decode-reject class (**exit 2**, `kind() == "MdCodec"`) in both hand-exhaustive `md_codec::Error` matches and is deliberately NOT swallowed by partial mode (only `MissingExplicitOrigin` is partial-suppressible). Pinned by `error.rs::md_codec_v0_42_new_variant_routing`. `vendor/md-codec` re-vendored from the crates.io registry (0.42.0; vendor-freshness green).
- **[companion] `md-cli` 0.13.0 (Track A, already on crates.io) shipped the `md decode`/`md encode` side.** `md decode` partial-decodes dead cards (exit 4 + marker); `md encode` emits a loud stderr advisory when a pathless shape is encoded without a resolving origin. Both documented in the manual (`42-md.md`) in lockstep; the `mnemonic inspect`/`verify-bundle` partial behavior documented in `41-mnemonic.md`.

### Migration / dispositions

- **No breaking change.** Every previously-`ok` bundle stays `ok`/exit 0 byte-identical; canonical/explicit-origin cards are unaffected. The only behavior change is that a formerly-rejected (exit 2) dead card now renders partial (exit 4), and a formerly-`ok` dead-card bundle now reports `partial` (exit 4) — both strictly more honest, never a wrong wallet.
- **GUI FOLLOWUP:** `verify-bundle --json` gains a `partial` verdict value; GUI consumers must handle it (`verify-bundle-json-partial-result`). The clap flag surface is unchanged, so the `schema_mirror` gate is not affected — this is a wire-shape (paired-PR) coordination item, not a flag-mirror one.
- **FOLLOWUPs filed:** `repair-corrupted-pathless-card-partial` (partial-repair of a *corrupted* dead card — repair stays strict for now), `toolkit-inspect-nonchunked-md1-intake-gap` (non-chunked single-string `md1` intake), `verify-bundle-json-partial-result` (GUI).
- **R0:** full design pipeline + post-impl whole-diff R0 GREEN (0C/0I); the content-id oracle staying enforced under partial, `EmptyOriginOverride` fatal-in-partial, verdict precedence, and byte-identity on non-partial paths are all mutation-proven. Suite 3743/0.

## mnemonic-toolkit [0.87.0] — 2026-07-11

**SemVer-MINOR, funds-adjacent — re-pins `md-codec` 0.40.0 → 0.41.0, which flips `sh(wpkh(...))` (bip49 nested single-sig) from NON-canonical to CANONICAL in md-codec's `canonical_origin` table. This changes what wallet a no-origin `mnemonic bundle --descriptor "sh(wpkh(...))"` engraves — read the migration callout. Also surfaces md-codec 0.41.0's new `MalformedPayloadPadding` decode-reject class.**

> **⚠️ FUNDS MIGRATION — `bundle --descriptor "sh(wpkh(...))"` WITHOUT an explicit origin now engraves a DIFFERENT wallet.** Pre-0.41.0 the toolkit treated bare `sh(wpkh(@N))` as non-canonical: with no inline `[fp/path]@N` and no `--slot @N.path=`, it inferred the (nonsensical, BIP-48-multisig-derived) default origin `m/48'/0'/0'/1'` and printed a default-path notice. Post-repin `sh(wpkh)` is canonical, so `bind_descriptor_mode_paths` early-returns without inferring anything: the origin is **ELIDED** and the card derives at the **MASTER key (`m/`)**, with **no notice** — exactly like every other canonical no-origin shape (`wpkh(@0)` / `pkh(@0)` / `tr(@0)` already behave this way). **Same command, different wallet: `m/48'/0'/0'/1'`+notice → empty-origin/master.** (The `m/49'/0'/0'` value md-codec's table now assigns to `sh(wpkh)` is used ONLY for policy-id hashing and n≥2 template completion — it is NEVER the toolkit's bundle emit default-origin; do NOT expect a `49'` card.) **To reproduce a pre-repin `sh(wpkh)` wallet, pass the explicit origin** — e.g. `mnemonic bundle --descriptor "sh(wpkh([fp/48'/0'/0'/1']XPUB/...))"`, or `--slot @N.path=m/48'/0'/0'/1'` for an `@N` placeholder. Existing pre-repin bundles that carry inline explicit origins are unaffected and still recover via them; a pre-repin bundle that relied on the old ELIDED default will FAIL verify post-repin (fail-LOUD md1 byte-mismatch, never a silent false-pass). Two adjacent refusals also fire for no-origin `sh(wpkh)`: `--account≠0` now refuses (`ModeViolation`), and a `--slot @N.path=` override on a SECRET-bearing `[Phrase|Seedqr|Ms1, Path]` slot set now refuses (an `[Xpub, Path]` set still succeeds and honors the path).

- **[funds-adjacent] `sh(wpkh(...))` descriptor-mode canonical flip (md-codec F-A1).** `canonical_origin(sh(wpkh(@N)))` flips `None` → `Some(m/49'/0'/0')`; the toolkit's descriptor-mode probes read `is_non_canonical = canonical_origin(tree).is_none()` directly, so four behaviors flip: (1) no-origin bundle default → empty-origin/master (see callout — **NOT `49'`**); (2) `--account≠0` → refuse; (3) `--slot @N.path=` override on a secret-bearing slot → refuse; (4) a pre-repin elided-`sh(wpkh)` bundle → fails verify as an md1 byte-mismatch. Covered by `tests/cli_shwpkh_canonical_flip.rs`. The two funds-load-bearing `canonical_origin` consumers are guard-tested: `restore.rs:1645`'s general/multisig `canonical_fallback` never derives an n==1 sh(wpkh) at 49' (the `cli_template_from_tree` conjunct — which has no `Sh` arm — keeps it out), and `verify_bundle.rs:391`'s single-sig-template verify path never admits an n==1 sh(wpkh) card. `gui-schema --classify-descriptor 'sh(wpkh(...))'` now prints `canonical` (was `non-canonical`).
- **[new decode-reject class] md-codec 0.41.0 F-A8 `MalformedPayloadPadding`.** The shared TLV parser now rejects a non-zero trailing symbol-pad tail (≤7 bits) that pre-0.41.0 silently accepted (`break;`); the reference encoder always zero-pads, so a non-zero pad is non-canonical / adversarial wire. The toolkit routes it to the decode-reject class (**exit 2**, `kind() == "MdCodec"`) in both hand-exhaustive `md_codec::Error` matches (`error.rs::md_codec_exit_code`, `friendly.rs::friendly_md_codec`), and a mid-repair occurrence degrades via `repair_via_md_codec`'s wildcard to `PostCorrectionDecodeFailed`. Pinned by `error.rs::md_codec_v0_41_new_variant_routing` + two `repair.rs` cells (direct decode-path + repair-path).
- **[docs] Manual + 6 in-code comment sites updated to intentional-divergence framing.** `41-mnemonic.md`'s canonical-shape enumeration goes five → **six** (`sh(wpkh(@N))` added). Six code comments that asserted "`canonical_origin(&tree).is_some()` ⟺ template-admissible" (citing bip49 sh(wpkh) as the refused `None` example) now state the divergence is INTENTIONAL: `canonical_origin` is necessary-but-not-sufficient, and `cli_template_from_tree` (no `Sh` arm) is the actual gate — the bip49 template refusal is UNCHANGED, only `canonical_origin`'s classification moved (`synthesize.rs:50-52`/`:349-368`/`:1092-1112`, `restore.rs:303`, `error.rs:343-349`, `tests/cli_bundle_md1_template_form.rs:239`). The mirror was deliberately NOT "fixed".

### Migration / dispositions

- **Sibling-pin semantic-inconsistency window (accepted).** Post-repin `mnemonic gui-schema --classify-descriptor 'sh(wpkh(...))'` says `canonical`, while the `install.sh`-installed `md` sibling binary (FROZEN at `descriptor-mnemonic-md-cli-v0.11.2` = md-codec 0.40.0) still treats sh(wpkh) as needing an explicit origin. This is a known, benign window: the compiled md-codec dep is orthogonal to the installed sibling CLI, and the sibling pin stays frozen (bumping it broke `sibling-pin-check` at v0.75.0). The window closes when a future, separate release bumps the sibling pin to md-cli-v0.12.0+.
- **F-A2** (md-codec `decode_md1_string` chunked auto-dispatch): **no-op for the toolkit.** Production decode uses `chunk::reassemble` directly (`restore.rs` / `verify_bundle.rs`); the three `decode_md1_string` call sites are all non-chunked test fixtures, and md-codec's single-payload versions {4,8,12} are all-even ⇒ never divert. Confirmed by the full suite.
- **F-A9** (md-codec `Error::TooManyErrors` Display message reworded — a comment/message correction): **no-op.** `friendly_md_codec` re-derives its own message from the `chunk_index`/`bound` fields (unchanged); no toolkit test pins md-codec's Display string.
- **F-A3/F-A4** (md-**cli** BIP-alignment changes): **not md-codec** — a separate `md-cli` CHANGELOG entry. The toolkit never links md-cli as a library (frozen sibling binary); zero compiled surface. No action.

Toolkit-only; md/mk/ms NO-BUMP; no crates.io publish (toolkit is not published). Re-vendored `vendor/md-codec` → 0.41.0 from the `md-codec-v0.41.0` tag (vendor-freshness green). No GUI schema-mirror change (no clap flag/subcommand/dropdown delta) — a GUI companion FOLLOWUP (`canonical-origin-sh-wpkh-toolkit-mirror-divergence`) records that mnemonic-gui's `conditional.rs` regex classifier will lag this flip until its next toolkit pin bump (currently frozen at v0.75.0 — no drift yet). Resolves FOLLOWUPs `toolkit-repin-sh-wpkh-canonical-flip` + `canonical-origin-sh-wpkh-toolkit-mirror-divergence`. Full R0 pipeline GREEN (SPEC ×3 rounds + plan R0, per-phase R0, post-impl whole-diff). `cargo test -p mnemonic-toolkit` full suite green; clippy `-D warnings` clean; `.examples-build` corpus regenerated (no sh(wpkh)/bundle example in the corpus — zero content drift beyond the version banner).

### Fixed

- **[funds-adjacent] `sh(wpkh(...))` no-origin descriptor-mode default was a nonsensical `m/48'/0'/0'/1'`** (via `bip48_script_type_for_root_tag(Sh)→1'`, a BIP-48 multisig script-type meaningless for single-key nested segwit). Re-pinning md-codec to 0.41.0 makes `sh(wpkh)` canonical, so the toolkit now elides the origin (master-key card, wpkh-parity) like every other canonical shape, and the `--account`/`--slot` override refusals fire. Resolves `toolkit-repin-sh-wpkh-canonical-flip`.

### Changed

- **`md_codec::Error::MalformedPayloadPadding` routed to exit 2 / `MdCodec`** in both hand-exhaustive `md_codec::Error` matches (compile-blocking at md-codec 0.41.0). Resolves the F-A8 arm of the re-pin.

## mnemonic-toolkit [0.86.0] — 2026-07-11

**SemVer-MINOR, funds-adjacent — demotes a valid-but-wrong single-string md1 "repair" from a silent confident fix to a flagged VERIFY-ME candidate (FOLLOWUP `repair-single-string-fully-valid-alias-second-oracle`, constellation-eval #5). Also bundles 3 documentation-only Minor riders.**

> **BREAKING for `$?`-gated automation:** `mnemonic repair --md1 <string>` now exits **4** instead of **5** when the correction touches a **non-chunked** single-string md1 — the compact form plain `md encode` emits for a small-enough payload. (The `inspect` / `verify-bundle` auto-fire path, which previously short-circuited to exit **5** on such a correction, no longer blesses it either: it falls through to the caller's original decode error plus a `run mnemonic repair --md1 …` advisory. `convert` has no md1 intake.) **Scope: non-chunked only.** A **chunked-of-1** md1 (the form `mnemonic bundle` / `--md1-form=template` always emits) is UNCHANGED and still exits `5`; multi-chunk md1 and mk1 are unaffected. Why: a non-chunked md1 has no cross-chunk/content-id oracle — md-codec 0.35.0's single-string decode path bypasses `reassemble`'s content-id check entirely — so a bounded-distance (≤4-error) BCH correction is provably the original payload, but beyond that bound a "correction" can still succeed while aliasing to a DIFFERENT, valid-but-wrong descriptor, undetectably. The corrected string is still printed on stdout in both cases; only the exit code (and a new stderr `UNVERIFIED` advisory) changed for the non-chunked case. Mirrors the ms1 substitution-correction demotion shipped in Cycle F (v0.81.0).

- **[funds-adjacent] `mnemonic repair --md1` (and its auto-fire callers) demote a touched non-chunked single-string md1 correction to an exit-4 `Unverified` VERIFY-ME candidate.** Predicate reads the corrected string's chunked-flag bit directly (`repair.rs::is_non_chunked_md1`), NOT chunk count — a chunked-of-1 md1 (count `1`, chunked-flag set) retains the content-id oracle via `reassemble` and correctly stays `Blessed`/exit `5`. The `try_repair_and_short_circuit` auto-fire fall-through advisory widened from ms1-only to `(ms1 OR non-chunked-md1)`, so a withheld candidate under `convert`/`inspect`/`verify-bundle` is never silently invisible — it now points at `mnemonic repair --md1`. The defined-but-encoder-unreachable mk1 `SingleString` singleton is demoted symmetrically for defense-in-depth (provably unreachable by any real `KeyCard`, not just untested — see FOLLOWUP `repair-single-string-fully-valid-alias-second-oracle`). New `verdict: "candidate"` JSON output for this case; new `41-mnemonic.md` "md1 non-chunked demotion" manual subsection.
- **[docs] SLIP-39 library-layer error message swept to the v0.85.0 M2 wording.** `src/slip39/error.rs`'s `Slip39Error::DigestVerificationFailed` doc comment + library `Display` still implied `--passphrase` verification after the M2 CLI-facing fix; both now match `cmd/slip39.rs`'s corrected wording (digest check runs pre-decryption; does not verify `--passphrase`). CLI-unreachable (no behavior change) — closes FOLLOWUP `slip39-library-layer-passphrase-message-sweep`.
- **[docs] BSMS Round-1 workflow chapter documents the v0.85.0 lenient-mode exit-4 change.** `docs/manual/src/30-workflows/3A-bsms-round1-verify.md` now states lenient-failure = exit 4 (report still printed) alongside the strict-mode discussion — closes FOLLOWUP `bsms-round1-lenient-exit4-workflow-doc`.
- **[docs] Stderr-template table's encrypted-Round-1-decrypt NOTICE label corrected.** `41-mnemonic.md`'s `NOTICE (exit 0)` label on the Round-1 decrypt-success row is qualified — that NOTICE only reports the encryption layer, and (since v0.85.0) can co-occur with an overall exit 4 if the record's own signature then fails to verify — closes FOLLOWUP `mnemonic-stderr-template-encrypted-notice-label`.

Toolkit-only; md/mk/ms NO-BUMP (the sibling standalone `md repair` binary is UNCHANGED — tracked as cross-repo FOLLOWUP `md-cli-non-chunked-single-string-repair-demote`, mirrored in `descriptor-mnemonic`); no crates.io publish; no re-vendor (no dep change); no GUI schema-mirror change (an exit-code change, not a flag/dropdown delta — tracked informationally as FOLLOWUP `gui-manual-repair-md1-single-string-demote-current-column`). Full R0 pipeline GREEN (SPEC ×2 rounds, TDD with RED-proof-under-reversion for both the positive demote test and the chunked-of-1 boundary test). `cargo test -p mnemonic-toolkit` full suite green (3709 passed); clippy `-D warnings` clean; `.examples-build` corpus regenerated (no repair example exists in the corpus — zero content drift beyond the version banner).

### Fixed

- **[funds-adjacent] `repair.rs::repair_via_md_codec` blessed a touched non-chunked single-string md1 correction with no oracle to back it** (`crates/mnemonic-toolkit/src/repair.rs`) — now demotes to `SetVerify::Unverified` (exit 4) when the corrected single string's chunked-flag bit is clear; a chunked-of-1 or multi-chunk correction is unaffected (still `Blessed`/exit 5). Defensive symmetric demote for the encoder-unreachable mk1 `GroupKey::SingleString` singleton in `verify_mk1_set`. Auto-fire advisory (`try_repair_and_short_circuit`) widened `Ms1`-only → `(Ms1 OR non-chunked Md1)`. Manual mirror (`docs/manual/src/40-cli-reference/41-mnemonic.md`, `42-md.md`) updated in lockstep, incl. a new md1 non-chunked demotion subsection and a qualification of md-cli's unchanged D26 behavior. Closes constellation-eval `repair-single-string-fully-valid-alias-second-oracle`.

## mnemonic-toolkit [0.85.0] — 2026-07-11

**SemVer-MINOR — three Minor-tier bugfixes bundled (constellation-eval §2 Minor tier, M2 + M3 + M4). M4 is a `$?`-breaking exit-code change — read the callout below before scripting against `import-wallet --bsms-round1`.**

> **BREAKING for `$?`-gated automation:** `mnemonic import-wallet --bsms-round1` (in BOTH standalone mode and combined with `--blob`) now exits **4** instead of **0** in the default lenient mode when ANY Round-1 record's signature fails to verify. Scripts that gated on `$? -eq 0` (or just checked for non-crash) to decide whether a BSMS Round-1 signature was trustworthy were previously **silently wrong** — the per-record report/envelope already showed `signature_verified: false`, but the process exit code claimed success. The full report/envelope is still printed in both modes; only the exit code changed. `--bsms-verify-strict` (exit 2, aborts before output) is unaffected. The `mnemonic-gui` run-modal will start presenting these invocations as failed at its next pin bump onto this release (lagging — the toolkit fix itself closes the gap regardless of when the GUI consumes it).

- **M2 — `slip39 combine` digest-verification-failure message no longer implies `--passphrase` verification.** SLIP-0039's digest check runs on the Feistel-encrypted master secret **before** passphrase decryption, so it can never be caused by (or catch) a wrong `--passphrase` — the prior message's "wrong --passphrase OR a share was substituted" wording implied the opposite and could lead a user to retire a correct backup, or worse, trust a wrong-passphrase recovery that silently exits 0 with a different, incorrect master. The message now states the digest check catches only share substitution/corruption/a wrong secret, and explicitly does not verify `--passphrase`.
- **M3 — `derive-child --application bip39 --language portuguese` no longer falsely refused.** BIP-85 §"Application: BIP39" assigns Portuguese path code `9'` (immediately after Czech `8'`); the toolkit's refusal ("not assigned a BIP-85 path code") was a factual error that blocked recovery of a spec-conformant child wallet, even though `bip39::Language::Portuguese` and the ms1 wire-language tables already supported Portuguese elsewhere. `--language portuguese --application bip39` now derives correctly.
- **M4 — `import-wallet --bsms-round1` lenient mode now exits 4 (not 0) when any Round-1 signature fails to verify** (see the BREAKING callout above). Applies identically in standalone mode (no `--blob`) and combined mode (`--blob` + `--bsms-round1` together, where import + card synthesis still proceeds).

Toolkit-only; md/mk/ms NO-BUMP; no crates.io publish (toolkit is not published); no re-vendor (`bip39` was already a runtime dependency — M3 needed no new dep); no GUI schema-mirror change (no flag-name/dropdown delta). Full R0 pipeline GREEN (SPEC ×3 rounds, per-fix TDD with RED-proof-under-mutation for M3 + M4, post-impl review). `cargo test -p mnemonic-toolkit` full suite green; clippy `-D warnings` clean.

### Fixed

- **`slip39 combine` `DigestVerificationFailed` message** (`crates/mnemonic-toolkit/src/cmd/slip39.rs`) — removed the false "wrong --passphrase" implication; states the check runs pre-decryption and does not verify `--passphrase`. Manual mirror (`docs/manual/src/40-cli-reference/41-mnemonic.md`) updated in lockstep. Closes constellation-eval Minor M2.
- **`derive-child --application bip39 --language portuguese` false refusal** (`crates/mnemonic-toolkit/src/cmd/derive_child.rs` `resolve_bip85_language`) — added the BIP-85 code-`9'` arm per bip-0085.mediawiki. Closes constellation-eval Minor M3.
- **[funds-adjacent] `import-wallet --bsms-round1` lenient-mode exit code did not reflect verify failures** (`crates/mnemonic-toolkit/src/cmd/import_wallet.rs`, both the standalone early-return and the combined `--blob` tail) — now exits 4 (VERIFY-ME idiom, consistent with `ImportWalletSeedMismatch`/`BundleMismatch`/`RestoreMismatch`) when any Round-1 record's signature verification failed, instead of exit 0. Report/envelope output unchanged; strict mode unaffected. Manual mirror updated (both-mode exit-4 prose + exit-codes table). Closes constellation-eval Minor M4.

## mnemonic-toolkit [0.84.0] — 2026-07-10

**SemVer-MINOR — funds-safety fix: wc-codec RAID `array_id` same-quorum collision (constellation-eval F2). A mixed-plate RAID reconstruct could silently emit a valid-but-wrong cosigner xpub at exit 0.** The Word-Card RAID `array_id` was derived ONLY from the ordered cosigner master fingerprints, so two arrays for the SAME cosigner set (mainnet vs testnet, different account/script-type, or a re-issue) collided with probability 1; mixing one array's data plate with another's parity plate (1 missing) passed the coherence gate, the MDS solve returned `Ok`, the length prefix XOR-cancelled, and the un-integrity-checked garbage stripe parsed into a wrong cosigner xpub ~50% of the time — a re-engraved/restored watch-only multisig that can't spend. Three-part fix: **(a)** fold a deterministic payload digest into `array_id` at encode (`top22(SHA-256(seed ‖ SHA-256(canonical)))`, `canonical` = u32-BE n ‖ per-index u16-BE payload_bits ‖ minimal bytes; `r` excluded) — a **non-wire-layout** change (decode only equality-compares the engraved id, never re-derives it), so existing plates decode + self-reconstruct unchanged while all FUTURE arrays get distinct ids → the mix now refuses (`RaidArrayMismatch`); **(b)** a spare-parity consistency oracle at reconstruct (when a spare parity equation exists, re-verify it over the reconstructed set) — catches LEGACY r=2 mixes retroactively, never over-rejects a genuine array; **(c)** an always-on `*recovered` advisory (text `! verify:` + stderr WARNING + a `verify_advisory` `--json` field; `WORD_CARD_SCHEMA_VERSION` `"1"`→`"2"`) on every MDS-solved plate — the universal mitigation for legacy r=1. The one honest residual (a legacy 0-missing pure-data chimera decoded with NO parity plate presented) is in-band undetectable and documented ("include a parity plate when decoding a set"). `wc-codec` 0.1.0→0.1.1 (unpublished workspace member). No new CLI flag → no `schema_mirror` change; `mnemonic-gui` = informational `--json` companion (it does not typed-parse word-card `--json`). Full R0 pipeline GREEN (recon + Fable SPEC ×2, plan ×2, post-impl whole-diff with RED-proof against pre-fix code + fixture regeneration). `cargo test --workspace` 3801 passed / 0 failed; clippy `-D warnings` clean. md/mk/ms NO-BUMP; no crates.io publish; no re-vendor.

### Fixed

- **[funds] wc-codec RAID `array_id` same-quorum collision** (`crates/wc-codec/src/raid.rs` `derive_array_id`/`array_id_canonical` + the spare-parity oracle in `raid_reconstruct`; `crates/mnemonic-toolkit/src/cmd/word_card.rs` `*recovered` advisory). The `array_id` now depends on the array's payloads, so two same-cosigner-set arrays no longer collide and a cross-array plate mix refuses (`RaidArrayMismatch`) instead of minting a wrong xpub; legacy r=2 mixes are caught by the spare-parity oracle; every reconstructed plate carries a loud verify-this-xpub advisory. Closes constellation-eval F2.
