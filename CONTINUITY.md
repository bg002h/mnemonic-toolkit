...
## Cycle C IN PROGRESS — cargo-fuzz malformed-input (R0 ×3 GREEN; 3-phase, per-repo fuzz/ workspaces)
- **Phase 1 (md-codec) SHIPPED** @ descriptor-mnemonic `e0d0b12` + CI fix (gnu-target pin), `fuzz-smoke.yml` green. 4 targets, NO-BUMP. Brainstorm + 3 R0 reviews + impl review in `design/BRAINSTORM_stress_cycle_c_fuzzing.md` + `design/agent-reports/cycle-c-fuzzing-*`.
- **NEXT phase 2 (ms-codec)**: 3 targets (ms1_decode + ms1_combine + ms1_no_secret_leak). The no-leak target ships WITH a documented exclusion set (`Codex32(_) | WrongHrp{..}`) per FOLLOWUP `ms-codec-error-display-echoes-input` (ms-codec Error Display echoes the secret share — filed mnemonic-secret `2251741` + toolkit companion). Then **phase 3 (mk-codec)**: 2 targets.
- CI GOTCHA carried forward: each `fuzz-smoke.yml` MUST pin `--target x86_64-unknown-linux-gnu` (cargo-fuzz defaults to musl host on the runner → ASan fails on static libc).
- DESCOPED: toolkit `parse_descriptor` fuzz target → FOLLOWUP `toolkit-descriptor-fuzz-target` (bin-private, needs cfg(fuzzing) lib.rs mount + own mini-R0).
