...
- **FOUND run #1 (ms phase): `ms_codec::decode_with_correction` PANICS on a single 0xaa byte** (char-boundary slice at decode.rs:151; never-panic-charter bug, not a leak). FOLLOWUP `decode-with-correction-panics-on-non-char-boundary-hrp-slice` (mnemonic-secret). ms1_decode HELD OUT of the daily smoke matrix until fixed (compile-gated; re-finds it instantly → would standing-red the cron).
- **Open FOLLOWUPs from this cycle:** `decode-with-correction-panics-on-non-char-boundary-hrp-slice` (ms; trivial char-safe fix, needs its own mini-cycle + eventual user-authorized crates.io publish to reach toolkit); `ms-codec-error-display-echoes-input` (ms Error Display leaks secret share; toolkit already withholds via v0.53.4); `toolkit-descriptor-fuzz-target` (descoped; needs cfg(fuzzing) lib.rs mount); `fuzz-nightly-quarterly-bump` (constellation-wide).
