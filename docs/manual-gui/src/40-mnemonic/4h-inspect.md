...
**At-least-one input (not a conjunction).** The `(required)` markers on `--ms1` / `--mk1` / `--md1` above are conditional-sourced: the form marks all three required only until you fill *any one* of them. Supply at least one card to run; you need not provide all three.
