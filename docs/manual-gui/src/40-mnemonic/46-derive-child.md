...
> **GUI form:** see [GUI Forms › mnemonic › derive-child](#gui-form-mnemonic-derive-child).
...
| Unknown `--from` node token (e.g. `--from foo=bar`) | `unknown --from node "<token>"; expected one of: phrase, seedqr, entropy, xpub, xprv, wif, fingerprint, path, ms1, mk1, bip38, minikey, electrum-phrase, address` (parser-level refusal per `cmd/convert.rs:135-140`, shared with `mnemonic convert`) |
