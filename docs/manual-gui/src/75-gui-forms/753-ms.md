...
## `ms hashlock` {#gui-form-ms-hashlock}

![ms hashlock — GUI form screenshot (dark theme, 2x)](../../figures/gui/ms-hashlock.png)

Structural render of the `ms hashlock` form — every flag and positional with its control kind and on-load default value (secret fields masked).

```{.text include="gui/ms-hashlock.gui"}
(structural form render — generated from the pinned renderer at build time)
```
