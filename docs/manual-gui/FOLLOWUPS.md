...
### `examples-pdf-un-ci-gated` — `docs/Examples.pdf` is generated but has zero CI gating and a hard-stale pin — ✓ RESOLVED (examples-v1.0.0, 2026-07-05)
...
- **What:** `docs/Examples.pdf` was generated by hand-run `.examples-build/gen.sh` with
  **no CI gate at all** and a hard-stale `mnemonic 0.55.3` pin while the tier is v0.75.0
  — so the committed PDF could silently drift from the current CLI.
- **Resolution (cycle `examples-pdf-un-ci-gated`; canonical record in toolkit
  `design/FOLLOWUPS.md::examples-pdf-un-ci-gated`):** `gen.sh` **re-pinned
  0.55.3 → 0.75.0** + determinism-hardened; Appendix B + §6.6 frozen as loudly-labelled
  static captures (no `mnemonic-depth2`/`bitcoind` in CI); new
  `.github/workflows/examples.yml` — **gate A, WIDE + REQUIRED, guard-based** —
  byte-diffs a fresh `gen.sh` regen against the tracked golden
  `.examples-build/Examples.md` (**born green**); `docs/Examples.pdf` regenerated with
  the funds surface **byte-verified stable** vs the old 0.55.3 PDF.
- **Gate-B deferral (Honesty):** gate A catches **regen-drift, NOT narration-truth**;
  full per-command verify-examples parity (`.cmd`/`.out` transcripts +
  `include-transcript.lua`-style inclusion) is **deferred to v1.1+**.
- **Status:** **RESOLVED 2026-07-05** — shipped via the `examples-v1.0.0` tag
  (attach-only; docs+CI-only, NO-BUMP). **Tier:** v1.1+ origin → resolved.
