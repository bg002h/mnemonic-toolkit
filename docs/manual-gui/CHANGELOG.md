...
- **Corpora:** 5 new gallery PNGs and `.gui` renders (61 → 66); the tutorial's J1/J2 private-channel re-drive (9 figures and 5 transcripts moved, 3 empty stderr transcripts dropped) with the prose that described argv warnings updated.

**The installer's newer tags move to toolkit 0.105.1 and ms 0.20.1** (the F-687e macOS stdin fix). `[installer-ahead]` and §82 name them; nothing else in the GUI-facing text changes.
...
- **Fold 1 (review `f679-toolkit-review.md`, I-1/M-1..M-3):** every sentence naming a stale CLI pin now names the v0.62.0 pins. That covers the 12-relation-to-cli table, which now gives the oldest release that works (toolkit 0.104.0, ms 0.19.1) instead of a "re-install at the pinned tag" remedy that pointed at toolkit v0.13.0; the md/ms/mk overview banners; the first-launch banner description (the `Pinned:` label is compiled in, not read from the binary); the ch82 list; the tutorial frontmatter and orientation; and the three repair pages' "pinned to X, exits 5" paragraphs (now exit 4 for `mnemonic repair --ms1` and `ms repair`, mk 0.13.0 behaviour for `mk repair`). New lint phase 13 `cli-pin-consistency` (`tests/check_cli_pins.py`; also run on every push by `sibling-pin-check.yml`) fails when `pinned-upstream.toml` disagrees with `scripts/install.sh`, or when the prose names an off-pin CLI version not listed as history in `tests/cli-version-history.txt`. The 11 measured false claims are corrected: `restore --md1` depth-2 restores; `bundle --descriptor` refuses `--account != 0` only for canonical `@i` shapes; and the exit codes and wording for ms encode/combine/split, md encode/address, bundle's descriptor and passphrase conflicts, and derive-child's node list. The `mk vectors` fixture count is now 41.
