...
| `--bsms-encryption-token <FILE\|->` | (v0.31.0) BIP-129 §Encryption Round-2 decrypt; reads session TOKEN from PATH (or `-` for stdin); applies PBKDF2-SHA512 + AES-256-CTR + HMAC-SHA256 per BIP-129 §Encryption. Combine with `--format bsms` to decrypt encrypted Round-2 wallet shares. Token file contents: lowercase ASCII hex (16 chars for STANDARD, 32 chars for EXTENDED); leading/trailing whitespace + newlines stripped; uppercase is normalized to lowercase. Encrypted blobs lack the `BSMS 1.0` header so `--format bsms` is REQUIRED for the encrypted path. MAC verify failure → exit 2 (typed `BsmsMacMismatch`). |
