...
\usepackage[letterpaper, margin=0.1in, nohead, nofoot]{geometry}
\usepackage{fontspec}
\usepackage{tikz}
...
% \volvellecellinner{<bech32-char>}: ~60% of regular. Used for row 1 (the
% second-innermost ring), where arc-length per cell is small but not yet
% as cramped as row 0.
\newcommand{\volvellecellinner}[1]{\textsf{\fontsize{3.6}{4.2}\selectfont #1}}

% \volvellecellinnermost{<bech32-char>}: kept as alias for \volvellecellinner
% post v0.1.4 grid-shift. Row 0 used to need a tiny font because its arc-
% length per cell was 0.069"; with the grid shifted outward by one ring
% (helper now emits row 0 at radius 0.7" instead of 0.35"), arc-length per
% cell is 0.137" — same as the v0.1.3 row 1 — so the row-1 size suffices.
\let\volvellecellinnermost\volvellecellinner
