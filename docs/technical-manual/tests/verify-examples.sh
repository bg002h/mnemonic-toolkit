...
#
# Transcripts live in docs/technical-manual/transcripts/<stem>.{cmd,out}.
# `<stem>.cmd` is a single shell line invoking the CLI (or a Cargo
# example via `cargo run --quiet --example <name>` — same .cmd/.out
# model); `<stem>.out` is the expected stdout. The script substitutes
# $MNEMONIC_BIN / $MD_BIN / $MS_BIN / $MK_BIN at runtime.
#
# Called from the Makefile as `make verify-examples`. v0.1 of the
# technical manual ships its first transcripts in Phase 1.2-1.4 (wire-
# format chapters) and Phase 4.1+ (Rust API chapters); zero transcripts
# is a vacuous pass.
