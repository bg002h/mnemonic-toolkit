...
Central error enum + `Result` alias. SPEC §6.1–§6.4 exit-code mapping lives in `ToolkitError::exit_code` (`error.rs::ToolkitError::exit_code`); JSON `kind` discriminant in `ToolkitError::kind` (`error.rs::ToolkitError::kind`); friendly-message dispatch in `ToolkitError::message` (`error.rs::ToolkitError::message`); JSON `details` in `ToolkitError::details` (`error.rs::ToolkitError::details`). Variant table in §V.4.4.

| Item | Visibility | Notes |
|---|---|---|
| `ToolkitError`\index{ToolkitError} | `pub` | `#[non_exhaustive]`; 26 variants; `error.rs::ToolkitError` |
| `BitcoinErrorKind`\index{BitcoinErrorKind} | `pub` | bitcoin-crate-sourced error wrapper; `error.rs::BitcoinErrorKind` |
| `Result<T>` | `pub` | `std::result::Result<T, ToolkitError>`; `error.rs::Result` |
...
| Item | Visibility | Notes |
|---|---|---|
| `MsField` (= `Vec<String>`) | `pub` type alias | SPEC §5.8 dense layout; `format.rs::MsField` |
| `MkField` | `pub enum` | `#[serde(untagged)]`; `Single` or `Multi`; `format.rs::MkField` |
| `CosignerEntry` | `pub struct` | `format.rs::CosignerEntry`; `Serialize` |
| `MultisigInfo` | `pub struct` | `format.rs::MultisigInfo`; `Serialize` |
| `BundleJson` | `pub struct` | `format.rs::BundleJson`; `Serialize` |
| `VerifyBundleJson` | `pub struct` | `format.rs::VerifyBundleJson`; `Serialize` |
| `VerifyCheck` | `pub struct` | `format.rs::VerifyCheck`; `Serialize, Clone` |
...
Bundle construction. The current unified entrypoint is `synthesize_unified` (`synthesize.rs::synthesize_unified`), which dispatches across the five `BundleMode` cases (§V.4.3.7). Since v0.47.1 `synthesize_unified` delegates its card-emission half to `synthesize_descriptor` (`synthesize.rs::synthesize_descriptor`), so `synthesize_descriptor` is **live** — the CLI reaches it through that delegation, not directly. The remaining legacy entry points (`synthesize_full`, `synthesize_watch_only`, `synthesize_multisig_full`, `synthesize_multisig_watch_only`) are retained behind `#[allow(dead_code)]` for v0.9+ library extraction; the CLI no longer calls them directly. The `Bundle` struct carries the three sibling outputs (`ms1: MsField`, `mk1: MkField`, `md1: Vec<String>`). `ResolvedSlot` carries the per-slot binding produced by the unified slot pipeline; SPEC §4.11.b distinctness uses its **typed** `bitcoin::bip32::DerivationPath` field (which folds `h ↔ '`), not raw-string equality. `CosignerKeyInfo` is a `pub type` alias for `ResolvedSlot` retained for legacy callers.

| Item | Visibility | Notes |
|---|---|---|
| `Bundle`\index{Bundle (toolkit)} | `pub struct` | three-codec output; `synthesize.rs::Bundle` |
| `Bundle::any_secret_bearing`\index{Bundle::any\_secret\_bearing} | `pub fn` | true iff any slot carries entropy; `synthesize.rs::Bundle::any_secret_bearing` |
| `ResolvedSlot`\index{ResolvedSlot} | `pub struct` | `xpub`, `fingerprint`, typed `path`, `entropy`, `master_xpub`; `synthesize.rs::ResolvedSlot` |
| `ResolvedSlot::is_secret_bearing`\index{ResolvedSlot::is\_secret\_bearing} | `pub fn` | `entropy.is_some()`; `synthesize.rs::ResolvedSlot::is_secret_bearing` |
| `CosignerKeyInfo`\index{CosignerKeyInfo} | `pub type` alias | `= ResolvedSlot` (legacy); `synthesize.rs::CosignerKeyInfo` |
| `xpub_to_65`\index{xpub\_to\_65} | `pub fn` | SPEC §4.6.1 `chain_code \|\| pubkey` form; `synthesize.rs::xpub_to_65` |
| `build_descriptor`\index{build\_descriptor} | `pub fn` | template → `md_codec::Descriptor`; `synthesize.rs::build_descriptor` |
| `synthesize_unified`\index{synthesize\_unified} | `pub fn` | the unified entrypoint; `synthesize.rs::synthesize_unified` |
| `synthesize_descriptor` | `pub fn` | live v0.47.1 delegation target of `synthesize_unified` (`synthesize.rs::synthesize_descriptor`); CLI reaches it via that delegation, not directly |
| `synthesize_full` / `synthesize_watch_only` / `synthesize_multisig_*` | `pub fn` | legacy variants; all `#[allow(dead_code)]`; the CLI no longer calls them directly |

### V.4.3.4 `parse_descriptor`\index{parse\_descriptor (module)} (`src/parse_descriptor.rs`)

The descriptor pipeline: BIP-388 placeholder lexing → key-vector resolution → synthetic-xpub substitution for parser conformance → `miniscript::Descriptor` walk to a `md_codec::Descriptor` AST → distinct-key enforcement. The **sole `pub` function on the BIP-388 distinct-key path** is `check_key_vector_distinctness` (`parse_descriptor.rs::check_key_vector_distinctness`), which compares typed `DerivationPath == DerivationPath` at `parse_descriptor.rs::check_key_vector_distinctness` (folding `h ↔ '` per `bitcoin::bip32`). A second distinct-key check at the CLI layer (`cmd::bundle::check_resolved_slots_distinctness`) is `pub(crate)` and uses typed `DerivationPath ==` equality (converges with the descriptor-mode twin per the v0.5 §4.11.b reversal) — that mirror is **out of scope** for Part V (§V.4.8).

| Item | Visibility | Notes |
|---|---|---|
| `ScriptCtx`\index{ScriptCtx} | `pub enum` | `SingleSig` or `MultiSig`; `parse_descriptor.rs::ScriptCtx` |
| `PlaceholderOccurrence`\index{PlaceholderOccurrence} | `pub struct` | `i`, `fingerprint_anno`, `origin_path_anno`, `multipath_alts`, `wildcard_hardened`; `parse_descriptor.rs::PlaceholderOccurrence` |
| `ResolvedPlaceholders`\index{ResolvedPlaceholders} | `pub struct` | `n`, `path_decl`, `fingerprint_annos`, `use_site_path`, `use_site_path_overrides`; `parse_descriptor.rs::ResolvedPlaceholders` |
| `ParsedKey` / `ParsedFingerprint`\index{ParsedKey}\index{ParsedFingerprint} | `pub struct` | `(i, payload [u8;65])` and `(i, fp [u8;4])`; `parse_descriptor.rs::ParsedKey, parse_descriptor.rs::ParsedFingerprint` |
| `DescriptorMode`\index{DescriptorMode} | `pub enum` | `SingleSig` or `MultiSig`; `parse_descriptor.rs::DescriptorMode` |
| `DescriptorBinding`\index{DescriptorBinding} | `pub struct` | resolved keys + fingerprints + cosigners; `parse_descriptor.rs::DescriptorBinding` |
| `lex_placeholders`\index{lex\_placeholders} | `pub fn` | placeholder enumeration; `parse_descriptor.rs::lex_placeholders` |
| `resolve_placeholders`\index{resolve\_placeholders} | `pub fn` | reconcile annotations across occurrences; `parse_descriptor.rs::resolve_placeholders` |
| `substitute_synthetic`\index{substitute\_synthetic} | `pub fn` | replace `@i` with deterministic synthetic xpubs for parser; `parse_descriptor.rs::substitute_synthetic` |
| `walk_root`\index{walk\_root} | `pub fn` | `miniscript::Descriptor` → `md_codec::tree::Node`; `parse_descriptor.rs::walk_root` |
| `parse_descriptor`\index{parse\_descriptor (function)} | `pub fn` | top-level pipeline driver; `parse_descriptor.rs::parse_descriptor` |
| `synthetic_xpub_for`\index{synthetic\_xpub\_for} | `pub fn` | deterministic synthetic xpub for `@i`; seed prefix `b"toolkit-v0.3"`; `parse_descriptor.rs::synthetic_xpub_for` |
| `bind_descriptor_keys`\index{bind\_descriptor\_keys} | `pub fn` | bind resolved placeholders to entropy + cosigners; `parse_descriptor.rs::bind_descriptor_keys` |
| `check_key_vector_distinctness`\index{check\_key\_vector\_distinctness} | `pub fn` | SPEC §4.11.b typed-`DerivationPath` distinctness; `parse_descriptor.rs::check_key_vector_distinctness` |

### V.4.3.5 `derive`\index{derive (module)} (`src/derive.rs`)

BIP-32 derivation from a BIP-39 mnemonic to a template-relative account xpub + xpriv + fingerprint. The `DerivedAccount`\index{DerivedAccount} struct collects all five outputs (`entropy: Vec<u8>`, `master_fingerprint: Fingerprint`, `account_xpub: Xpub`, `account_xpriv: Xpriv`, `account_path: DerivationPath`); `derive_full`\index{derive\_full} (`derive.rs::derive_full`) is the single entry point and is parameterised by `(phrase, passphrase, CliLanguage, CliNetwork, CliTemplate, account)`.
...
| Item | Visibility | Notes |
|---|---|---|
| `read_phrase_input`\index{read\_phrase\_input} | `pub fn` | argv-or-stdin read; `parse.rs::read_phrase_input` |
| `parse_master_fingerprint`\index{parse\_master\_fingerprint} | `pub fn` | 4-byte hex parse to `Fingerprint`; `parse.rs::parse_master_fingerprint` |
| `parse_cosigner_spec`\index{parse\_cosigner\_spec} | `pub fn` | legacy `xpub:fp:path` parse; `parse.rs::parse_cosigner_spec` |
| `parse_cosigners_file`\index{parse\_cosigners\_file} | `pub fn` | legacy `--cosigners-file` JSON parse; `parse.rs::parse_cosigners_file` |
| `check_no_concurrent_stdin`\index{check\_no\_concurrent\_stdin} | `pub fn` | reject simultaneous phrase + passphrase stdin reads; `parse.rs::check_no_concurrent_stdin` |
| `CosignerSpec`\index{CosignerSpec} | `pub struct` | `xpub`, `master_fingerprint`, `path: Option<DerivationPath>`; `parse.rs::CosignerSpec` |
| `MultisigPathFamily`\index{MultisigPathFamily} | `pub enum` | `Bip48` or `Bip87` (Default); `parse.rs::MultisigPathFamily` |
...
|---|---|---|
| `is_multisig`\index{CliTemplate::is\_multisig} | `fn (&self) -> bool` | matches `WshMulti`, `WshSortedMulti`, `ShWshMulti`, `ShWshSortedMulti`, `TrMultiA`, `TrSortedMultiA`; `template.rs::CliTemplate::is_multisig` |
| `origin_path_str`\index{CliTemplate::origin\_path\_str} | `fn (&self, CliNetwork, u32) -> String` | BIP-44 `m/{purpose}'/{coin}'/{account}'` (single-sig) or BIP-48 `m/48'/.../{script_type}'`; `template.rs::CliTemplate::origin_path_str` |
| `derivation_path`\index{CliTemplate::derivation\_path} | `fn (&self, CliNetwork, u32) -> DerivationPath` | typed form of `origin_path_str`; `template.rs::CliTemplate::derivation_path` |
| `md_origin_path`\index{CliTemplate::md\_origin\_path} | `fn (&self, CliNetwork, u32) -> md_codec::origin_path::OriginPath` | md1 wire-form path; `template.rs::CliTemplate::md_origin_path` |
| `wrapper_node`\index{CliTemplate::wrapper\_node} | `fn (&self, u8, usize) -> md_codec::tree::Node` | md1 wrapper for `(K, N)`; `template.rs::CliTemplate::wrapper_node` |
| `bip48_script_type`\index{CliTemplate::bip48\_script\_type} | `fn (&self) -> Option<u32>` | `Some(1)` for `WshMulti` / `WshSortedMulti`, `Some(2)` for `TrMultiA` / `TrSortedMultiA`, `None` otherwise; `template.rs::CliTemplate::bip48_script_type` |
| `human_name`\index{CliTemplate::human\_name} | `fn (&self) -> &'static str` | human-readable; `template.rs::CliTemplate::human_name` |
...
`ToolkitError`\index{ToolkitError} is `#[non_exhaustive]` (`error.rs::ToolkitError`); the 26-row table below covers every variant at HEAD. The v0.8.1 vendor-emitter expansion (8 `WalletFormatEmitter` impls — §V.4.5.9) wired `ExportWalletMissingFields` to live construction sites: `Sparrow` returns it when `--threshold` is absent on a multisig template (`wallet_export/sparrow.rs::SparrowEmitter::collect_missing`); `Specter` returns it when `--wallet-name` is absent (`wallet_export/specter.rs::SpecterEmitter::collect_missing`); the dispatcher routes the variant through `build_missing_fields_refusal` (`wallet_export/mod.rs::build_missing_fields_refusal`) for byte-exact stderr. The variant is no longer reserved at HEAD. The `Exit` column maps to `ToolkitError::exit_code` (`error.rs::ToolkitError::exit_code`) per SPEC §6.1; the `kind()` column is the stable JSON discriminant emitted into `details` blocks (SPEC §6.4); the `Emitted by` column lists representative call sites (not exhaustive).
...
| `ExportWalletMissingFields { format, missing }` | 2 | `ExportWalletMissingFields` | SPEC\_export\_wallet §4 missing-fields refusal | `SparrowEmitter::collect_missing`, `SpecterEmitter::collect_missing` (re-emitted by `cmd::export_wallet::run` at `cmd/export_wallet.rs::emit_payload`) |
...
**Schema version.** `BundleJson.schema_version` and `VerifyBundleJson.schema_version` are both `&'static str` literals fixed at `"4"` at every construction site at HEAD — 7 sites total: `synthesize.rs::tests::bundle_json_schema_version_pinned_to_4`, `cmd/bundle.rs::emit_unified`, `cmd/import_wallet.rs::emit_json_envelope`, `cmd/verify_bundle.rs::run`, `cmd/verify_bundle.rs::verify_emit_from_expected`, `wallet_import/json_envelope.rs::tests::bundle_json_view_round_trips_every_field_of_bundle_json`, `wallet_import/json_envelope.rs::tests::empty_envelope_for_name_lift_test`. The `format.rs` module-level doc-comment still reads `v0.2: schema_version "2"`; that doc-comment is **stale** and persists at HEAD (§V.4.8). External consumers MUST pin against the constructor-site literal `"4"`, not the module-level doc-comment.
...
(Source: `format.rs::BundleJson`; `#[derive(Serialize)]`. Field order is part of the schema.)
...
(Source: `format.rs::VerifyBundleJson`.)
...
(Source: `format.rs::VerifyCheck`. All four forensic fields use `#[serde(skip_serializing_if = "Option::is_none")]`.)
...
(Source: `format.rs::CosignerEntry`.)

### V.4.5.6 `MkField`\index{MkField}

`MkField` is `#[serde(untagged)]` (`format.rs::MkField`), so the JSON wire is either a bare array (single-sig) or an array of arrays (multi-source multisig) — there is **no** discriminator key:
...
`MsField` is a `pub type` alias for `Vec<String>` (`format.rs::MsField`) and serialises as a bare JSON array. SPEC §5.8 dense-vec invariant: length `N` (one entry per slot), with the empty-string sentinel `""` marking watch-only slots. A single-sig watch-only bundle emits `["", "", ""]` for a triple-cosigner pure-watch-only setup; a full single-sig emits `["ms10entrsqqq..."]`.
...
`mnemonic export-wallet` emits a vendor-specific watch-only wallet-import artifact selected by `--format <NAME>`. The shape is fully orchestrated through the `WalletFormatEmitter`\index{WalletFormatEmitter} trait (`wallet_export/mod.rs::WalletFormatEmitter`) — one impl per vendor, all called from `cmd::export_wallet::run` (`cmd/export_wallet.rs::run`). The trait carries three methods: `collect_missing(&EmitInputs) -> Vec<MissingField>`\index{MissingField} (SPEC §4 deterministic-refusal predicate; non-empty result short-circuits to `ToolkitError::ExportWalletMissingFields` via `build_missing_fields_refusal` at `wallet_export/mod.rs::build_missing_fields_refusal`), `emit(&EmitInputs) -> Result<String, ToolkitError>` (final byte-exact output string), and `extension() -> &'static str` (file extension hint for `--output`). Every emitter borrows the same `EmitInputs`\index{EmitInputs} struct (`wallet_export/mod.rs::EmitInputs`); construction at `cmd/export_wallet.rs::run` runs **after** watch-only validation (`validate_watch_only` pre-resolve at `wallet_export/mod.rs::validate_watch_only` and `validate_watch_only_resolved` post-resolve at `wallet_export/mod.rs::validate_watch_only_resolved`), so by the time an emitter sees `EmitInputs` no slot carries `Phrase` / `Entropy` / `Xprv` / `Wif` material. The `--output <path>` flag writes the emitted string with a single trailing newline (`cmd/export_wallet.rs::run`); `-` (default) writes to stdout. `extension()` is not enforced against the `--output` path at HEAD — it is documented as a suggestion.

Eight vendor formats are wired at HEAD; their `--format` selector values come from the `CliExportFormat` enum at `cmd/export_wallet.rs::CliExportFormat`. The eight sub-sub-sections below cover each format in selector-enum-declaration order. The matrix in §V.4.5.10 cross-tabulates accept / refuse by descriptor shape.

`EmitInputs` field reference (per `wallet_export/mod.rs::EmitInputs`): `canonical_descriptor: &str` (BIP-380 form with `#checksum`); `resolved_slots: &[ResolvedSlot]` (slot-index order; empty when `--descriptor` was supplied without `--template`); `template: Option<CliTemplate>`; `script_type: WalletScriptType`\index{WalletScriptType} (`wallet_export/mod.rs::WalletScriptType`; eight variants — `P2pkh`, `P2shP2wpkh`, `P2wpkh`, `P2tr`, `P2shMulti`, `P2shP2wshMulti`, `P2wshMulti`, `P2trMulti`); `network: CliNetwork`; `account: u32`; `threshold: Option<u8>`; `threshold_user_supplied: bool` (Sparrow consults this); `master_xpub_at_0: Option<bitcoin::bip32::Xpub>` (`@0.master_xpub=<base58>` plumbing — Coldcard only); `wallet_name: &str`; `wallet_name_was_user_supplied: bool` (Specter consults this); `taproot_internal_key: Option<TaprootInternalKey>`\index{TaprootInternalKey} (`Nums` or `Cosigner(u8)`; `wallet_export/mod.rs::TaprootInternalKey`); `range: (u32, u32)`; `timestamp: TimestampArg`\index{TimestampArg} (`wallet_export/mod.rs::TimestampArg`; `Now` or `Unix(i64)`); `bitcoin_core_version: u8`.

#### V.4.5.9.1 `--format bitcoin-core`

| Property | Value |
|---|---|
| Selector | `CliExportFormat::BitcoinCore` (`cmd/export_wallet.rs::CliExportFormat::BitcoinCore`) |
| Emitter | `BitcoinCoreEmitter` (`wallet_export/bitcoin_core.rs::BitcoinCoreEmitter`) |
...
| `collect_missing` | always empty (`wallet_export/bitcoin_core.rs::BitcoinCoreEmitter::collect_missing`) — Bitcoin Core's `importdescriptors` consumes the canonical descriptor as-is |
| Vendor-specific flags | `--range <start>,<end>` (default `0,999`; `cmd/export_wallet.rs::ExportWalletArgs::range`), `--timestamp <now\|unix>` (default `now`; `cmd/export_wallet.rs::ExportWalletArgs::timestamp`), `--bitcoin-core-version <24\|25>` (default `25`; `cmd/export_wallet.rs::ExportWalletArgs::bitcoin_core_version`) |
...
(Source: `format_bitcoin_core_importdescriptors` at `wallet_export/bitcoin_core.rs::format_bitcoin_core_importdescriptors`. Multipath form `<0;1>` is split via `parsed.into_single_descriptors()` and asserted to produce exactly 2 entries; non-multipath descriptors emit a single entry with `internal: false`.) `--bitcoin-core-version` is currently consumed but unused (parameter `_bitcoin_core_version` at `wallet_export/bitcoin_core.rs::format_bitcoin_core_importdescriptors`; reserved for future shape divergence between Core 24 vs 25).

#### V.4.5.9.2 `--format bip388`

| Property | Value |
|---|---|
| Selector | `CliExportFormat::Bip388` (`cmd/export_wallet.rs::CliExportFormat::Bip388`) |
| Emitter | `Bip388Emitter` (`wallet_export/bip388.rs::Bip388Emitter`) |
| Output | JSON object with `name`, `description_template`, `keys_info` |
| `extension()` | `"json"` |
| Accepted shapes | every template + every BIP-388-parseable `--descriptor` with multipath `/<0;1>/*` on every key; non-multipath suffixes refused as `BadInput` from `descriptor_to_bip388_wallet_policy` (`wallet_export/pipeline.rs::descriptor_to_bip388_wallet_policy`) |
| `collect_missing` | always empty (`wallet_export/bip388.rs::Bip388Emitter::collect_missing`) |
...
(Source: `format_bip388_wallet_policy` at `wallet_export/bip388.rs::format_bip388_wallet_policy`. The template path renders `@N/**` placeholders by direct enumeration over `CliTemplate`; the descriptor-passthrough path re-extracts the keys from a canonical descriptor via `descriptor_to_bip388_wallet_policy` and string-substitutes each key-expression with `@N/**` longest-first to avoid prefix collisions — `wallet_export/pipeline.rs::descriptor_to_bip388_wallet_policy`. For `tr-multi-a` / `tr-sortedmulti-a` with `--taproot-internal-key nums`, the literal `NUMS_XONLY_HEX` (`50929b74...803ac0`) is embedded as the internal key; with `--taproot-internal-key @N`, cosigner `N`'s placeholder is the internal key and the remaining cosigners form the `multi_a` / `sortedmulti_a` leaf.) The `name` field is `template.human_name()` for the template path or `"imported-descriptor"` for the descriptor-passthrough path.

#### V.4.5.9.3 `--format coldcard`

| Property | Value |
|---|---|
| Selector | `CliExportFormat::Coldcard` (`cmd/export_wallet.rs::CliExportFormat::Coldcard`) |
| Emitter | `ColdcardEmitter` (`wallet_export/coldcard.rs::ColdcardEmitter`) |
...
| Refused shapes | `bip86` → `BadInput` (`wallet_export/coldcard.rs::emit_coldcard_generic_json`); `tr-multi-a` / `tr-sortedmulti-a` → `BadInput` (`wallet_export/coldcard.rs::emit_coldcard_multisig_text`); descriptor passthrough without `--template` → `BadInput` (`wallet_export/coldcard.rs::emit_coldcard_generic_json`) |
| `collect_missing` | always empty (`wallet_export/coldcard.rs::ColdcardEmitter::collect_missing`) — refusals surface as `BadInput` with byte-exact pointer text (more helpful than the generic §4 bullet) |
| Vendor-specific flags | `--slot @0.master_xpub=<base58>` (singlesig only; top-level `xpub` field) |

Representative singlesig BIP-84 output (`wallet_export/coldcard.rs::emit_coldcard_generic_json`):
...
When `--slot @0.master_xpub=<base58>` is supplied (`cmd/export_wallet.rs::run`), a top-level `xpub` field is emitted between `xfp` and `account` carrying the depth-0 master xpub; the field is otherwise omitted entirely (`wallet_export/coldcard.rs::ColdcardGenericJson (wallet_export/coldcard.rs::emit_coldcard_generic_json)`). Top-level `xfp` is the **master** fingerprint of slot `@0` in uppercase 8-hex; the `bipNN.xfp` field is the **parent** fingerprint of the account xpub (BIP-32 serialization bytes 5–8). `bipNN.first` is the address at `/0/0` relative to the account xpub. `bipNN._pub` carries the SLIP-132 prefix variant (`ypub` for `bip49`, `zpub` for `bip84`; absent for `bip44`). `chain` is `"BTC"` mainnet, `"XTN"` testnet+signet, `"XRT"` regtest (`wallet_export/coldcard.rs::chain_string`).

Representative multisig output (`wallet_export/coldcard.rs::emit_coldcard_multisig_text`) is a 4-header-plus-N-cosigner text block:
...
Notes: `Format: P2WSH` for `wsh-multi` / `wsh-sortedmulti`; `Format: P2SH-P2WSH` for `sh-wsh-multi` / `sh-wsh-sortedmulti`. Cosigners are emitted in slot-index order for `multi`; lexicographically by xpub for `sortedmulti` (`wallet_export/coldcard.rs::emit_coldcard_multisig_text`). `Name` truncates by Unicode scalar value via `chars().take(20)` so multi-byte UTF-8 input cannot slice mid-character (`wallet_export/coldcard.rs::emit_coldcard_multisig_text`). `Derivation` is the shared per-slot origin path if all cosigners agree; otherwise the `m/0'/0'` Coldcard placeholder. Cosigner xpubs are emitted in BIP-32 base58 form, **not** SLIP-132 (per SPEC §5.2). Multisig text is the byte-identical input format accepted by Blockstream Jade (§V.4.5.9.4).

#### V.4.5.9.4 `--format jade`

| Property | Value |
|---|---|
| Selector | `CliExportFormat::Jade` (`cmd/export_wallet.rs::CliExportFormat::Jade`) |
| Emitter | `JadeEmitter` (`wallet_export/jade.rs::JadeEmitter`) |
| Output | text (delegates to `emit_coldcard_multisig_text`) |
| `extension()` | `"txt"` |
| Accepted shapes | `wsh-multi` / `wsh-sortedmulti` / `sh-wsh-multi` / `sh-wsh-sortedmulti` (delegates byte-identical to Coldcard's §5.2 multisig text — `wallet_export/jade.rs::JadeEmitter::emit`) |
| Refused shapes | `bip44` / `bip49` / `bip84` / `bip86` → `BadInput` ("multisig wallet config only; for singlesig setups Jade reads the seed on-device"; `wallet_export/jade.rs::JadeEmitter::emit`); `tr-multi-a` / `tr-sortedmulti-a` → `BadInput` (pending firmware support, FOLLOWUPS `jade-tr-multi-a-pending-firmware`; `wallet_export/jade.rs::JadeEmitter::emit`); descriptor passthrough without `--template` → `BadInput` (`wallet_export/jade.rs::JadeEmitter::emit`) |
| `collect_missing` | always empty (`wallet_export/jade.rs::JadeEmitter::collect_missing`) — refusals route through `BadInput` |
...
| Property | Value |
|---|---|
| Selector | `CliExportFormat::Sparrow` (`cmd/export_wallet.rs::CliExportFormat::Sparrow`) |
| Emitter | `SparrowEmitter` (`wallet_export/sparrow.rs::SparrowEmitter`) |
...
| Refused shapes | descriptor passthrough without `--template` → `BadInput` (`wallet_export/sparrow.rs::emit_sparrow_wallet_json`); multisig templates without `--threshold` → `ExportWalletMissingFields { format: "sparrow", missing: [Threshold] }` (`wallet_export/sparrow.rs::SparrowEmitter::collect_missing`) |
| `collect_missing` | returns `[Threshold]` when `template.is_multisig() && !inputs.threshold_user_supplied` (`wallet_export/sparrow.rs::SparrowEmitter::collect_missing`) |
...
Representative singlesig output (`emit_sparrow_wallet_json` at `wallet_export/sparrow.rs::emit_sparrow_wallet_json`):
...
`policyType` is `"SINGLE"` / `"MULTI"`. `scriptType` is one of `P2PKH` / `P2SH_P2WPKH` / `P2WPKH` / `P2TR` / `P2SH` / `P2SH_P2WSH` / `P2WSH` (`sparrow_script_type` at `wallet_export/sparrow.rs::sparrow_script_type`); taproot multisig keeps `P2TR` because Sparrow conveys the script-path `multi_a` / `sortedmulti_a` via the `defaultPolicy.miniscript.script` field, not via a separate discriminant. `masterFingerprint` is lowercase 8-hex; `extendedPublicKey` is BIP-32 base58 form (never SLIP-132). The `defaultPolicy.miniscript.script` field carries `@N/**` placeholder expressions (`pkh(@0/**)` / `sh(wpkh(@0/**))` / `wpkh(@0/**)` / `tr(@0/**)` / `wsh(multi(K,@0/**,...))` / etc.) for non-taproot-multisig templates, and the canonical descriptor (with `#checksum` stripped — see Note below) for `tr-multi-a` / `tr-sortedmulti-a`.

> **Note (Sparrow taproot-multisig checksum stripping).** Sparrow's `defaultPolicy.miniscript.script` is a bare miniscript policy expression, **not** a BIP-380 descriptor with checksum. For `tr-multi-a` / `tr-sortedmulti-a`, `build_miniscript_script` strips the trailing `#<8-char>` suffix from the canonical descriptor before emitting (`wallet_export/sparrow.rs::build_miniscript_script`) — keeping the checksum breaks Sparrow's policy parser, which substring-matches on `script` for policy detection. (Folded in commit `a890881` per v0.8.1 Phase 2 R1 review.)

#### V.4.5.9.6 `--format specter`

| Property | Value |
|---|---|
| Selector | `CliExportFormat::Specter` (`cmd/export_wallet.rs::CliExportFormat`) |
| Emitter | `SpecterEmitter` (`wallet_export/specter.rs::SpecterEmitter`) |
| Output | JSON object (`label`, `blockheight`, `descriptor`, `devices`) |
| `extension()` | `"json"` |
| Accepted shapes | every template + every BIP-388-parseable `--descriptor` |
| Refused shapes | missing `--wallet-name` → `ExportWalletMissingFields { format: "specter", missing: [WalletName] }` (`wallet_export/specter.rs::SpecterEmitter::collect_missing`) |
| `collect_missing` | returns `[WalletName]` when `!inputs.wallet_name_was_user_supplied` (`wallet_export/specter.rs::SpecterEmitter::collect_missing`) |
| Vendor-specific flags | `--wallet-name <STRING>` (REQUIRED; Specter's UX requires a wallet label) |

Representative 2-of-3 `wsh-sortedmulti` output (`emit_specter_wallet_json` at `wallet_export/specter.rs::emit_specter_wallet_json`):
...
| Property | Value |
|---|---|
| Selector | `CliExportFormat::Electrum` (`cmd/export_wallet.rs::CliExportFormat::Electrum`) |
| Emitter | `ElectrumEmitter` (`wallet_export/electrum.rs::ElectrumEmitter`) |
...
| Refused shapes | `tr-multi-a` / `tr-sortedmulti-a` → `BadInput` (pending Electrum libsecp-taproot support, FOLLOWUPS `electrum-tr-multi-a-pending-libsecp-taproot`; `wallet_export/electrum.rs::ElectrumEmitter::emit`); descriptor passthrough without `--template` → `BadInput` (`wallet_export/electrum.rs::ElectrumEmitter::emit`) |
| `collect_missing` | always empty (`wallet_export/electrum.rs::ElectrumEmitter::collect_missing`) — `keystore.label` defaults to `""` when `--wallet-name` is absent |
| Vendor-specific flags | `--wallet-name <STRING>` (label; defaults to `<template>-<account>`) |
| Schema-version pin | `ELECTRUM_SEED_VERSION_PIN: u32 = 17`\index{ELECTRUM\_SEED\_VERSION\_PIN} (`wallet_export/electrum.rs::ELECTRUM_SEED_VERSION_PIN`) — empirically validated 2026-05-12 against Electrum 4.5.5; the loader walks `_convert_version_<N>` migrations forward to FINAL\_SEED\_VERSION on first save |

Representative singlesig BIP-84 output (`emit_electrum_standard_json` at `wallet_export/electrum.rs::emit_electrum_standard_json`):
...
`keystore.xpub` is rendered through `render_slip132_xpub` (`wallet_export/electrum.rs::render_slip132_xpub`): singlesig uses lowercase SLIP-132 variants (`ypub` / `zpub`); multisig uses uppercase (`Ypub` / `Zpub`); `P2pkh`, `P2tr`, `P2shMulti` have no SLIP-132 variant and emit the neutral xpub form. Taproot multisig is refused upstream by `emit()`.

Representative 2-of-4 `wsh-sortedmulti` output (`emit_electrum_multisig_json` at `wallet_export/electrum.rs::emit_electrum_multisig_json`):
...
| Refused shapes | every multisig template (`is_multisig()`) → `BadInput` ("Green's multisig setup is server-mediated (Green Multisig Shield) and not a file-import surface"; `wallet_export/green.rs::GreenEmitter::emit`) |
| `collect_missing` | always empty (`wallet_export/green.rs::GreenEmitter::collect_missing`) |
| Vendor-specific flags | none beyond shared |

Representative output (`wallet_export/green.rs::GreenEmitter::emit`):
...
- **[a]** Coldcard generic-wallet-export schema documents only `bip44`/`bip49`/`bip84`; BIP-86 refuses with byte-exact `BadInput` (`wallet_export/coldcard.rs::emit_coldcard_generic_json`).
- **[b]** Sparrow refuses multisig templates without explicit `--threshold` — `collect_missing` returns `[MissingField::Threshold]` (`wallet_export/sparrow.rs::SparrowEmitter::collect_missing`) → `ExportWalletMissingFields { format: "sparrow", missing: [Threshold] }` (exit 2).
...
- **[d]** Specter requires explicit `--wallet-name` — `collect_missing` returns `[MissingField::WalletName]` (`wallet_export/specter.rs::SpecterEmitter::collect_missing`) → `ExportWalletMissingFields { format: "specter", missing: [WalletName] }` (exit 2).
- **[e]** Coldcard firmware does not ingest taproot multisig text exports (FOLLOWUPS `coldcard-tr-multi-a-pending-firmware`; `wallet_export/coldcard.rs::emit_coldcard_multisig_text`).
- **[f]** Blockstream Jade firmware does not ingest taproot multisig wallet config (FOLLOWUPS `jade-tr-multi-a-pending-firmware`; `wallet_export/jade.rs::JadeEmitter::emit`).
- **[g]** Electrum's wallet-db does not ingest taproot multisig (FOLLOWUPS `electrum-tr-multi-a-pending-libsecp-taproot`; `wallet_export/electrum.rs::ElectrumEmitter::emit`).
- Jade refuses every singlesig template ("multisig wallet config only; for singlesig setups Jade reads the seed on-device"; `wallet_export/jade.rs::JadeEmitter::emit`).
- Green refuses every multisig template ("Green's multisig setup is server-mediated"; `wallet_export/green.rs::GreenEmitter::emit`).
- For `tr-multi-a` / `tr-sortedmulti-a`, `cmd::export_wallet::run` requires `--taproot-internal-key <nums|@N>` (`cmd/export_wallet.rs::run`) before any emitter is reached; the matrix rows are conditioned on that flag being present.

All refusals route to exit code 2 (per `ToolkitError::exit_code` in §V.4.4). Refusals routed through `BadInput` produce a single-line `error: ...` stderr; refusals routed through `ExportWalletMissingFields` produce the multi-line `build_missing_fields_refusal` shape (`wallet_export/mod.rs::build_missing_fields_refusal`).

## V.4.6 Engraving-card layout

The `mnemonic bundle` subcommand emits a unified engraving card on **stderr** (the `--quiet` flag suppresses it). The card is a three-card mental model — ms1, mk1, md1 cards laid out alongside each other physically — rendered as a single human-readable text block. The sole producing function is `format::engraving_card_unified` (`format.rs::engraving_card_unified`); `BundleJson.engraving_card` was removed in v0.5.0 Phase A.3.
...
- **The eight `WalletFormatEmitter` impls are a shipped CLI-level surface.** v0.8.1 promoted Sparrow + Specter from format stubs to real emitters, added Coldcard, Jade, Electrum, and Green, and wired `--slot @0.master_xpub=<base58>` into Coldcard. At HEAD the `wallet_export` module exposes eight `pub(crate)` emitter structs implementing the `WalletFormatEmitter` trait (§V.4.5.9) plus a `pub(crate) build_missing_fields_refusal` function that is the sole construction site of the SPEC §4 byte-exact missing-info refusal shape. The `ExportWalletFormatStub` variant of `ToolkitError` (`error.rs::ToolkitError::ExportWalletFormatStub`) is `#[allow(dead_code)]` at HEAD — no construction site remains in the codebase; the variant is retained for future per-vendor stub introductions (removing it from a `#[non_exhaustive]` enum would otherwise be a breaking change). `ExportWalletTaprootMultisigUnsupported` (`error.rs::ToolkitError::ExportWalletTaprootMultisigUnsupported`) is similarly retained for back-compat but is unreachable post-v0.8 NUMS lift; the per-vendor taproot-multisig refusals all surface as `BadInput` with byte-exact pointer text (per-vendor footnotes in §V.4.5.10).
