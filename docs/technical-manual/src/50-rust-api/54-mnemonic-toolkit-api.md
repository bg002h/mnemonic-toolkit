...
|---|---|---|
| `ToolkitError` | `pub` | `#[non_exhaustive]`; 26 variants; `error.rs:10` |
| `BitcoinErrorKind` | `pub` | bitcoin-crate-sourced error wrapper; `error.rs:119` |
...
| `BundleInputForCard` | `pub struct` | engraving-card input (not serde); `format.rs:223` |
| `TemplateOrDescriptor` | `pub enum` | `format.rs:236` |
| `SlotCardBlock` | `pub struct` | per-slot card block; `format.rs:242` |
| `chunk_5char` | `pub fn` | 5-char groups, 10/line; `format.rs:10` |
| `chunk_md1` | `pub fn` | delegates to `md_codec::encode::render_codex32_grouped(s, 5)`; `format.rs:38` |
| `engraving_card_unified` | `pub fn` | SPEC §5.5 sole card surface; `format.rs:259` |
...
| Item | Visibility | Notes |
|---|---|---|
| `Bundle` | `pub struct` | three-codec output; `synthesize.rs:20` |
| `Bundle::any_secret_bearing` | `pub fn` | true iff any slot carries entropy; `synthesize.rs:33` |
| `ResolvedSlot` | `pub struct` | `xpub`, `fingerprint`, typed `path`, `path_raw`, `entropy`; `synthesize.rs:569` |
| `ResolvedSlot::is_secret_bearing` | `pub fn` | `entropy.is_some()`; `synthesize.rs:579` |
| `CosignerKeyInfo` | `pub type` alias | `= ResolvedSlot` (legacy); `synthesize.rs:190` |
| `xpub_to_65` | `pub fn` | SPEC §4.6.1 `chain_code \|\| pubkey` form; `synthesize.rs:69` |
| `build_descriptor` | `pub fn` | template → `md_codec::Descriptor`; `synthesize.rs:80` |
| `synthesize_unified` | `pub fn` | the unified entrypoint; `synthesize.rs:593` |
...
| Item | Visibility | Notes |
|---|---|---|
| `ScriptCtx` | `pub enum` | `SingleSig` or `MultiSig`; `parse_descriptor.rs:32` |
| `PlaceholderOccurrence` | `pub struct` | `i`, `fingerprint_anno`, `origin_path_anno`, `multipath_alts`, `wildcard_hardened`; `parse_descriptor.rs:50` |
| `ResolvedPlaceholders` | `pub struct` | `n`, `path_decl`, `fingerprint_annos`, `use_site_path`, `use_site_path_overrides`; `parse_descriptor.rs:145` |
| `ParsedKey` / `ParsedFingerprint` | `pub struct` | `(i, payload [u8;65])` and `(i, fp [u8;4])`; `parse_descriptor.rs:673, 679` |
| `DescriptorMode` | `pub enum` | `SingleSig` or `MultiSig`; `parse_descriptor.rs:751` |
| `DescriptorBinding` | `pub struct` | resolved keys + fingerprints + cosigners; `parse_descriptor.rs:790` |
| `lex_placeholders` | `pub fn` | placeholder enumeration; `parse_descriptor.rs:60` |
| `resolve_placeholders` | `pub fn` | reconcile annotations across occurrences; `parse_descriptor.rs:156` |
| `substitute_synthetic` | `pub fn` | replace `@i` with deterministic synthetic xpubs for parser; `parse_descriptor.rs:263` |
| `walk_root` | `pub fn` | `miniscript::Descriptor` → `md_codec::tree::Node`; `parse_descriptor.rs:353` |
| `parse_descriptor` | `pub fn` | top-level pipeline driver; `parse_descriptor.rs:687` |
| `synthetic_xpub_for` | `pub fn` | deterministic synthetic xpub for `@i`; seed prefix `b"toolkit-v0.3"`; `parse_descriptor.rs:769` |
| `bind_descriptor_keys` | `pub fn` | bind resolved placeholders to entropy + cosigners; `parse_descriptor.rs:815` |
| `check_key_vector_distinctness` | `pub fn` | SPEC §4.11.b typed-`DerivationPath` distinctness; `parse_descriptor.rs:1104` |

### V.4.3.5 `derive`\index{derive (module)} (`src/derive.rs`)

BIP-32 derivation from a BIP-39 mnemonic to a template-relative account xpub + xpriv + fingerprint. The `DerivedAccount` struct collects all five outputs (`entropy: Vec<u8>`, `master_fingerprint: Fingerprint`, `account_xpub: Xpub`, `account_xpriv: Xpriv`, `account_path: DerivationPath`); `derive_full` (`derive.rs:22`) is the single entry point and is parameterised by `(phrase, passphrase, CliLanguage, CliNetwork, CliTemplate, account)`.
...
| Item | Visibility | Notes |
|---|---|---|
| `read_phrase_input` | `pub fn` | argv-or-stdin read; `parse.rs:17` |
| `parse_master_fingerprint` | `pub fn` | 4-byte hex parse to `Fingerprint`; `parse.rs:38` |
| `parse_cosigner_spec` | `pub fn` | legacy `xpub:fp:path` parse; `parse.rs:111` |
| `parse_cosigners_file` | `pub fn` | legacy `--cosigners-file` JSON parse; `parse.rs:173` |
| `check_no_concurrent_stdin` | `pub fn` | reject simultaneous phrase + passphrase stdin reads; `parse.rs:224` |
| `CosignerSpec` | `pub struct` | `xpub`, `master_fingerprint`, `path: Option<DerivationPath>`; `parse.rs:53` |
| `MultisigPathFamily` | `pub enum` | `Bip48` or `Bip87` (Default); `parse.rs:63` |
...
|---|---|---|
| `is_multisig` | `fn (&self) -> bool` | matches `WshMulti`, `WshSortedMulti`, `ShWshMulti`, `ShWshSortedMulti`, `TrMultiA`, `TrSortedMultiA`; `template.rs:46` |
| `origin_path_str` | `fn (&self, CliNetwork, u32) -> String` | BIP-44 `m/{purpose}'/{coin}'/{account}'` (single-sig) or BIP-48 `m/48'/.../{script_type}'`; `template.rs:61` |
| `derivation_path` | `fn (&self, CliNetwork, u32) -> DerivationPath` | typed form of `origin_path_str`; `template.rs:75` |
| `md_origin_path` | `fn (&self, CliNetwork, u32) -> md_codec::origin_path::OriginPath` | md1 wire-form path; `template.rs:82` |
| `wrapper_node` | `fn (&self, u8, usize) -> md_codec::tree::Node` | md1 wrapper for `(K, N)`; `template.rs:111` |
| `bip48_script_type` | `fn (&self) -> Option<u32>` | `Some(1)` for `WshMulti` / `WshSortedMulti`, `Some(2)` for `TrMultiA` / `TrSortedMultiA`, `None` otherwise; `template.rs:219` |
| `human_name` | `fn (&self) -> &'static str` | human-readable; `template.rs:228` |

### V.4.3.8 Miscellaneous support modules

| Module | Items | Role |
|---|---|---|
| `language` (`src/language.rs`) | `CliLanguage` (enum) | 10 BIP-39 wordlists; `human_name`; `From<CliLanguage> for bip39::Language` |
| `network` (`src/network.rs`) | `CliNetwork` (enum) | mainnet / testnet / signet / regtest; `coin_type`, `network_kind`, `known_hrp`, `human_name` |
| `slip0132` (`src/slip0132.rs`) | `XpubPrefix` (enum), `parse_xpub_prefix_arg` | xpub / ypub / Ypub / zpub / Zpub prefix selector |
| `slot_input` (`src/slot_input.rs`) | `SlotSubkey`, `SlotInput`, `ParseError`, `parse_slot_input`, `validate_slot_set` | unified `@N.subkey=value` parser (SPEC §6.6) |
| `bundle_unified` (`src/bundle_unified.rs`) | `BundleMode` (enum), `detect_bundle_mode`, `pre_check_threshold`, `pre_check_template_n` | five-way mode dispatch over `&[SlotInput]` |
...
- **`ToolkitError` is `#[non_exhaustive]`.** Any future library extraction (v0.9+) will require external matchers to include `_ => ...` arms. The annotation is at `error.rs:10`; the forward-compatibility implication is identical to `ms_codec::Error`, `mk_codec::Error`, and `md_codec::Error` (each likewise `#[non_exhaustive]`).
