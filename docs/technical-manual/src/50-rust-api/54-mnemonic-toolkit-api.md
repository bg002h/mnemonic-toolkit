...
Bundle construction. The current unified entrypoint is `synthesize_unified` (`synthesize.rs:745`), which dispatches across the five `BundleMode` cases (§V.4.3.7). Since v0.47.1 `synthesize_unified` delegates its card-emission half to `synthesize_descriptor` (`synthesize.rs:826`), so `synthesize_descriptor` is **live** — the CLI reaches it through that delegation, not directly. The remaining legacy entry points (`synthesize_full`, `synthesize_watch_only`, `synthesize_multisig_full`, `synthesize_multisig_watch_only`) are retained behind `#[allow(dead_code)]` for v0.9+ library extraction; the CLI no longer calls them directly. The `Bundle` struct carries the three sibling outputs (`ms1: MsField`, `mk1: MkField`, `md1: Vec<String>`). `ResolvedSlot` carries the per-slot binding produced by the unified slot pipeline; SPEC §4.11.b distinctness uses its **typed** `bitcoin::bip32::DerivationPath` field (which folds `h ↔ '`), not raw-string equality. `CosignerKeyInfo` is a `pub type` alias for `ResolvedSlot` retained for legacy callers.

| Item | Visibility | Notes |
|---|---|---|
| `Bundle`\index{Bundle (toolkit)} | `pub struct` | three-codec output; `synthesize.rs:22` |
| `Bundle::any_secret_bearing`\index{Bundle::any\_secret\_bearing} | `pub fn` | true iff any slot carries entropy; `synthesize.rs:35` |
| `ResolvedSlot`\index{ResolvedSlot} | `pub struct` | `xpub`, `fingerprint`, typed `path`, `entropy`, `master_xpub`; `synthesize.rs:642` |
| `ResolvedSlot::is_secret_bearing`\index{ResolvedSlot::is\_secret\_bearing} | `pub fn` | `entropy.is_some()`; `synthesize.rs:690` |
| `CosignerKeyInfo`\index{CosignerKeyInfo} | `pub type` alias | `= ResolvedSlot` (legacy); `synthesize.rs:219` |
| `xpub_to_65`\index{xpub\_to\_65} | `pub fn` | SPEC §4.6.1 `chain_code \|\| pubkey` form; `synthesize.rs:98` |
| `build_descriptor`\index{build\_descriptor} | `pub fn` | template → `md_codec::Descriptor`; `synthesize.rs:109` |
| `synthesize_unified`\index{synthesize\_unified} | `pub fn` | the unified entrypoint; `synthesize.rs:745` |
| `synthesize_descriptor` | `pub fn` | live v0.47.1 delegation target of `synthesize_unified` (`synthesize.rs:826`); CLI reaches it via that delegation, not directly |
...
The descriptor pipeline: BIP-388 placeholder lexing → key-vector resolution → synthetic-xpub substitution for parser conformance → `miniscript::Descriptor` walk to a `md_codec::Descriptor` AST → distinct-key enforcement. The **sole `pub` function on the BIP-388 distinct-key path** is `check_key_vector_distinctness` (`parse_descriptor.rs:1208`), which compares typed `DerivationPath == DerivationPath` at `parse_descriptor.rs:1212` (folding `h ↔ '` per `bitcoin::bip32`). A second distinct-key check at the CLI layer (`cmd::bundle::check_resolved_slots_distinctness`) is `pub(crate)` and uses typed `DerivationPath ==` equality (converges with the descriptor-mode twin per the v0.5 §4.11.b reversal) — that mirror is **out of scope** for Part V (§V.4.8).
...
| `check_key_vector_distinctness`\index{check\_key\_vector\_distinctness} | `pub fn` | SPEC §4.11.b typed-`DerivationPath` distinctness; `parse_descriptor.rs:1208` |
...
- **`schema_version = "4"` is HEAD; the `format.rs:114` doc-comment is stale.** Every constructor site of `BundleJson` and `VerifyBundleJson` literal-encodes `schema_version: "4"` — 7 sites total: `synthesize.rs:1732`, `cmd/bundle.rs:906`, `cmd/import_wallet.rs:1499`, `cmd/verify_bundle.rs:329`, `cmd/verify_bundle.rs:1017`, `wallet_import/json_envelope.rs:595`, `wallet_import/json_envelope.rs:761`. The module-level doc-comment at `format.rs:114` still reads `v0.2: schema_version "2"`. The doc-comment drift was first reported in v0.3 Phase 3.1 and persists at HEAD; chapter authors and external consumers should treat the constructor-site literal as authoritative and ignore the doc-comment until it is reconciled.
- **`check_key_vector_distinctness` is the sole `pub` BIP-388 distinct-key entry point.** It lives at `parse_descriptor.rs:1208` and compares **typed** `bitcoin::bip32::DerivationPath` equality at `parse_descriptor.rs:1212` (which folds `h` and `'`). The CLI-layer mirror `cmd::bundle::check_resolved_slots_distinctness` is `pub(crate)` and now uses the same **typed** `DerivationPath ==` equality on `(xpub.to_string(), path)` (the former raw-string `path_raw` field was deleted in v0.37.9) — that mirror is **out of Part V scope**, but it no longer diverges from the descriptor layer. Its `bundle.rs` doc-comment has been updated to the typed framing; the lone residual lag is the `error.rs` (`:13-16`) `Bip388Distinctness` doc, which still describes the equality semantic as raw-string and is tracked as a known drift item.
