...
\index{cross-card binding (bundle)}**The binding rule:** cards from the same bundle share at minimum their leading 16 `chunk_set_id` bits. A reader who finds an mk1 card with `chunk_set_id` `1c017` and an md1 card with `chunk_set_id` `1c01` can reasonably conjecture they belong together; the full verification then runs `verify-bundle`. Cards whose leading 16 bits disagree are definitely from different bundles. (False positives at the 16-bit prefix happen with probability ≈ 2⁻¹⁶ ≈ 1 in 65,536 under random `policy_id`s; the substantive verification — the next four invariants — is what *certifies* bundle membership.) In a **multisig** bundle the per-cosigner mk1 cards all share that leading-16-bit prefix (and the md1 prefix) but differ in the fifth hex char by their slot index — so `1c010`, `1c011`, … denote cosigners 0, 1, … of the same wallet. (This concerns the **engraved** display identifiers. The md1 *engraved* identifier — the 4 hex chars `policy_id[0..2]` printed on the card by `bundle.rs::build_unified_card` — shares those leading 16 bits, which is what makes the cross-card binding work. Do not confuse it with the md1 **wire** `chunk_set_id`: that field, carried inside the md1 chunked-string headers, is a *different* value — 20 bits of `Md1EncodingId` = `SHA-256` of the canonical bytecode (§II.1) — and does **not** necessarily agree with mk1's wire `chunk_set_id`. That is expected: the md1 wire field serves chunk-grouping at md1 reassembly, not cross-card binding. Cross-card binding uses only the engraved display identifiers, all of which derive from the same `policy_id`.)

The wire `chunk_set_id` (a 20-bit identifier in the chunked-string headers, used to detect mixed chunks of one card) is per-format: the mk1/ms1 wire value is `policy_id`-derived (`derive_mk1_chunk_set_id`), so it equals that card's engraved identifier; the md1 wire value is `Md1EncodingId`-derived (§II.1), so it differs from md1's engraved `policy_id[0..2]` identifier. This chapter is about the cross-card binding *use* of the **engraved** identifiers — all `policy_id`-derived — not the per-format wire fields.
...
```{.text include="mnemonic-bundle-bip388-collision.out" lines="1-1"}
