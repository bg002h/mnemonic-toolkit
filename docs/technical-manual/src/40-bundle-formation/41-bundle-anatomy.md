...
- The engraving-card identifiers print as `# ms1: 1c017 / # mk1: 1c017 / # md1: 1c01`. The two formats use different bit-lengths of the same `policy_id` stub (md1 emits 16 bits = 4 hex; mk1/ms1 emit 20 bits = 5 hex per `bundle.rs::build_unified_card` and `bundle.rs::build_unified_card` respectively), but the leading 16 bits `1c01` are shared across all three. That common prefix is the bundle-level binding — cards from a different bundle would not all share it.
