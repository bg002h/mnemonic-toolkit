...
- **md1 + mk1 → forked BCH.** The `md-codec` and `mk-codec` crates use a Bitcoin-tuned BCH polynomial *forked* from BIP-93's. Same algebraic machinery (GF(32), polynomial syndromes, BCH distance properties), but each format mixes its own HRP (`md` or `mk`) into the polynomial generator constants. Two consequences:
  1. A decoder applying the wrong format's BCH (e.g., applying mk1's polynomial to an md1 string) fails the checksum check, so cards cannot be confused across formats even if a transcription error happens to land on a sibling format's character set.
  2. The polynomial constants for md1 and mk1 are *distinct* even though the algebra is identical. Phrases that round-trip under `md` will not round-trip if fed to `mk`, even ignoring payload content.
