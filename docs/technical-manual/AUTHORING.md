# Authoring conventions — technical manual
...
**Audience:** this is the technical-manual companion to the end-user
manual's `AUTHORING.md`. The conventions below are mostly shared, but
the *content* tone differs: the technical manual targets
implementer/auditor and Rust integrator audiences in one volume.
Bit-level wire detail, BCH math, AST walkthroughs, and Rust API
surface live here; end-user workflow prose stays in the end-user
manual. The two-track `:::primer` convention is preserved but used
sparingly — readers here are expected to know BIP-32 / BIP-39 /
multisig basics. Use `:::primer` only when a chapter genuinely needs
to bridge to an under-discussed concept (e.g., the BCH polynomial
math in Foundations §I.3).
